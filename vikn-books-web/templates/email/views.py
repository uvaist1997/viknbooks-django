...
from django.contrib.auth.models import User, Group
from api.v6.users.functions import get_auto_id
from brands import models
from api.v6.general.functions import get_current_role
...
from api.v6.users.functions import send_email, get_master_auto_id
...
from api.v6.users.serializers import CustomTokenSerializer, SoftwareVersionSerializer
...
def create_general_settings(request):
    ...
    # ======Uvais=====
    print(data['loyalty_PointValue'], 'loyalty_PointValueloyalty_PointValue')
    try:
        loyalty_point_value = data['loyalty_PointValue']
    except:
        loyalty_point_value = ""

    # for i in data['loyalty_taskList']:
    #     print(i["LoyaltyPoint"], 'OPOPOPO')
    #     if i["LoyaltyPoint"] and i["LoyaltyValue"]:
    #         loyalty_point_value += str(i["LoyaltyPoint"]) + \
    #             str('-')+str(i["LoyaltyValue"]+str(','))
    print(loyalty_point_value)
    # ===========
    today = datetime.datetime.now()
    ...
    if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="Loyalty_Point_Expire").exists():
            Loyalty_Point_Expire = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="Loyalty_Point_Expire").SettingsValue
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="is_Loyalty_SalesReturn_MinimumSalePrice").exists():
            is_Loyalty_SalesReturn_MinimumSalePrice = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="is_Loyalty_SalesReturn_MinimumSalePrice").SettingsValue
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="EnableShippingCharge").exists():
            EnableShippingCharge = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="EnableShippingCharge").SettingsValue
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ShowDueBalanceInSales").exists():
            ShowDueBalanceInSales = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="ShowDueBalanceInSales").SettingsValue
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ShowDueBalanceInPurchase").exists():
            ShowDueBalanceInPurchase = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="ShowDueBalanceInPurchase").SettingsValue
            if ShowDueBalanceInPurchase == "True":
                ShowDueBalanceInPurchase = True
            else:
                ShowDueBalanceInPurchase = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ShowEmployeesInSales").exists():
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="EnableSerialNoInSales").exists():
            EnableSerialNoInSales = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="EnableSerialNoInSales").SettingsValue
            if EnableSerialNoInSales == "True":
                EnableSerialNoInSales = True
            else:
                EnableSerialNoInSales = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="EnableItemCodeNoInSales").exists():
            EnableItemCodeNoInSales = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="EnableItemCodeNoInSales").SettingsValue
            if EnableItemCodeNoInSales == "True":
                EnableItemCodeNoInSales = True
            else:
                EnableItemCodeNoInSales = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="EnableSalesManInSales").exists():
            EnableSalesManInSales = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="EnableSalesManInSales").SettingsValue
            if EnableSalesManInSales == "True":
                EnableSalesManInSales = True
            else:
                EnableSalesManInSales = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ShowWarrantyPeriodInProduct").exists():
            ShowWarrantyPeriodInProduct = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="ShowWarrantyPeriodInProduct").SettingsValue
            if ShowWarrantyPeriodInProduct == "True":
                ShowWarrantyPeriodInProduct = True
            else:
                ShowWarrantyPeriodInProduct = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ShowDescriptionInSales").exists():
            ShowDescriptionInSales = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="ShowDescriptionInSales").SettingsValue
            if ShowDescriptionInSales == "True":
                ShowDescriptionInSales = True
            else:
                ShowDescriptionInSales = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="AllowExtraSerielNos").exists():
            AllowExtraSerielNos = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="AllowExtraSerielNos").SettingsValue
            if AllowExtraSerielNos == "True":
                AllowExtraSerielNos = True
            else:
                AllowExtraSerielNos = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="Free_Quantity_In_Sales").exists():
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="Free_Quantity_In_Purchase").exists():
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ShowCustomerInPurchase").exists():
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ShowDiscountInPayments").exists():
            ShowDiscountInPayments = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="ShowDiscountInPayments").SettingsValue
            if ShowDiscountInPayments == "True":
                ShowDiscountInPayments = True
            else:
                ShowDiscountInPayments = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ShowDiscountInReceipts").exists():
            ShowDiscountInReceipts = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="ShowDiscountInReceipts").SettingsValue
            if ShowDiscountInReceipts == "True":
                ShowDiscountInReceipts = True
            else:
                ShowDiscountInReceipts = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="EnableLoyaltySettings").exists():
            EnableLoyaltySettings = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="EnableLoyaltySettings").SettingsValue
            if EnableLoyaltySettings == "True":
                EnableLoyaltySettings = True
            else:
                EnableLoyaltySettings = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="EnableCardNetWork").exists():
            EnableCardNetWork = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="EnableCardNetWork").SettingsValue
            if EnableCardNetWork == "True":
                EnableCardNetWork = True
            else:
                EnableCardNetWork = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="BlockTransactionsByDate").exists():
            BlockTransactionsByDate = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="BlockTransactionsByDate").SettingsValue
            if BlockTransactionsByDate == "True":
                BlockTransactionsByDate = True
            else:
                BlockTransactionsByDate = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="EnableCardDetails").exists():
            EnableCardDetails = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="EnableCardDetails").SettingsValue
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="AllowCashReceiptMorePurchaseAmt").exists():
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="AllowAdvanceReceiptinPurchase").exists():
            AllowAdvanceReceiptinPurchase = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="AllowAdvanceReceiptinPurchase").SettingsValue
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="VoucherNoAutoGenerate").exists():
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="EnableVoucherNoUserWise").exists():
            EnableVoucherNoUserWise = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="EnableVoucherNoUserWise").SettingsValue
            if EnableVoucherNoUserWise == "True":
                EnableVoucherNoUserWise = True
            else:
                EnableVoucherNoUserWise = False

        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ShowSalesPriceInPurchase").exists():
            ShowSalesPriceInPurchase = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="ShowSalesPriceInPurchase").SettingsValue
        ...
        if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ShowSettingsinPurchase").exists():
            ShowSettingsinPurchase = administrations_models.GeneralSettings.objects.get(
                CompanyID=CompanyID, SettingsType="ShowSettingsinPurchase").SettingsValue
...
def get_defaults(request):
    ...
    if GeneralSettings.objects.filter(CompanyID=CompanyID,SettingsType="VAT").exists():
        is_vat = GeneralSettings.objects.get(CompanyID=CompanyID,SettingsType="VAT").SettingsValue
    if GeneralSettings.objects.filter(CompanyID=CompanyID,SettingsType="GST").exists():
        is_gst = GeneralSettings.objects.get(CompanyID=CompanyID,SettingsType="GST").SettingsValue
    comapny_instance = get_company(CompanyID)
    comapny_instance.is_gst = vat
    comapny_instance.is_vat = gst
    comapny_instance.save()
    print(is_vat,is_gst,"OOOOOOOOOOOOOOOOOOOOOOOOO############################")
    ...
    if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        ...
        if administrations_models.PrintSettings.objects.filter(CompanyID=CompanyID).exists():
            ...
            IsProductDescription = administrations_models.PrintSettings.objects.get(
                CompanyID=CompanyID).IsProductDescription
