from brands.models import Employee, Employee_Log, AccountLedger, Country, AccountLedger_Log, LedgerPosting
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from api.v1.employees.serializers import EmployeeSerializer, EmployeeRestSerializer, CountrySerializer
from api.v1.brands.serializers import ListSerializer
from api.v1.employees.functions import generate_serializer_errors, get_auto_id
from rest_framework import status
from api.v1.employees.functions import get_auto_id, get_EmployeeCode
...
def edit_employee(request,pk):
    ...
    ledgerInstance = AccountLedger.objects.get(BranchID=BranchID,LedgerID=LedgerID,CompanyID=CompanyID)
    ...
    if serialized.is_valid():
        ...
        if  employee_ok:
            ...
            if AccountLedger.objects.filter(BranchID=BranchID,LedgerID=LedgerID,CompanyID=CompanyID).exists():
                accountLedgerInstance = AccountLedger.objects.get(BranchID=BranchID,LedgerID=LedgerID,CompanyID=CompanyID)
...
def employees(request):
    ...
    if serialized1.is_valid():
        ...
        if Employee.objects.filter(BranchID=BranchID,CompanyID=CompanyID).exists():
            instances = Employee.objects.filter(BranchID=BranchID,CompanyID=CompanyID)
            serialized = EmployeeRestSerializer(instances,many=True,context = {"CompanyID": CompanyID, "PriceRounding": PriceRounding })
            response_data = {
                "StatusCode" : 6000,
                "data" : serialized.data
            }
...
def employee(request,pk):
    ...
    if Employee.objects.filter(pk=pk,CompanyID=CompanyID).exists():
        instance = Employee.objects.get(pk=pk,CompanyID=CompanyID)
        serialized = EmployeeRestSerializer(instance,context = {"CompanyID": CompanyID, "PriceRounding" :int(PriceRounding) })
        response_data = {
            "StatusCode" : 6000,
            "data" : serialized.data
        }
