from brands.models import Color, Color_Log, TestImage,ProductDemo
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from api.v1.colors.serializers import ProductDemoRestSerializer,ProductDemoSerializer,ColorSerializer, ColorRestSerializer, TestImageSerializer, TaskFormReactSerializer
from api.v1.colors.functions import generate_serializer_errors
from rest_framework import status
from api.v1.colors.functions import get_auto_id
import datetime
import base64
...
def testImage_list(request):
    ...
    instances = TestImage.objects.all()
    serialized = TestImageSerializer(instances,many=True,context={"request":request})
    response_data = {
        "StatusCode" : 6000,
        "data" : serialized.data
    }
...
def taskReacts(request):
    ...
    serialized = TaskFormReactSerializer(instances,many=True,context = {"CompanyID": CompanyID })
    response_data = {
        "StatusCode" : 6000,
        "data" : serialized.data
    }
...
def taskReact(request,pk):
    ...
    if TaskFormReact.objects.filter(pk=pk,CompanyID=CompanyID).exists():
        ...
        serialized = TaskFormReactSerializer(instance,context = {"CompanyID": CompanyID })
        response_data = {
            "StatusCode" : 6000,
            "data" : serialized.data
        }
...
def edit_taskReact(request,pk):
    ...
    for tsk in tasks:
        ...
        if detailID == 0:
            detailInstance = TaskFormSetReact.objects.get(pk=ID,CompanyID=CompanyID)
...
def create_product_demo(request):
    data = request.data
    today = datetime.datetime.now()
    serialized = ProductDemoSerializer(data=request.data)
    # image = data['image']
    try:
...
@permission_classes((AllowAny,))
@renderer_classes((JSONRenderer,))
def edit_product_demo(request,pk):
    data = request.data

    today = datetime.datetime.now()
    serialized = ProductDemoSerializer(data=request.data)
    instance = None
    if ProductDemo.objects.filter(pk=pk).exists():
        instance = ProductDemo.objects.get(pk=pk)
    if instance:
        if serialized.is_valid():
            ...
            name = serialized.data['name']
            description = serialized.data['description']
            print(image,"edithhhhhhhhhhhhhhhhhhhhhh",instance.image)
        ...
        response_data = {
            "StatusCode" : 6001,
            "message" : "Product Demo Not Found!"
        }
...
@permission_classes((AllowAny,))
@renderer_classes((JSONRenderer,))
def product_demos(request):
    data = request.data

    instances = ProductDemo.objects.filter()
    serialized = ProductDemoRestSerializer(instances,many=True)
    response_data = {
        "StatusCode" : 6000,
        "data" : serialized.data
    }
...
def product_demo(request,pk):
    ...
    if ProductDemo.objects.filter(pk=pk).exists():
        instance = ProductDemo.objects.get(pk=pk)
    if instance:
        serialized = ProductDemoRestSerializer(instance)
        response_data = {
            "StatusCode" : 6000,
            "data" : serialized.data
        }
...
@permission_classes((AllowAny,))
@renderer_classes((JSONRenderer,))
def delete_product_demo(request,pk):
    data = request.data
    ...
    if ProductDemo.objects.filter(pk=pk).exists():
        ...
        response_data = {
            "StatusCode" : 6000,
            "message" : "ProductDemo Deleted Successfully!"
        }
    else:
        response_data = {
            "StatusCode" : 6001,
