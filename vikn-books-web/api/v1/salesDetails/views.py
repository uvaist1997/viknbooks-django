...
from api.v1.salesDetails.serializers import SalesDetailsSerializer, SalesDetailsRestSerializer, SalesDetailsDummySerializer, SalesDetailsDeletedDummySerializer
from api.v1.brands.serializers import ListSerializer
from api.v1.salesDetails.functions import generate_serializer_errors
from rest_framework import status
from api.v1.salesDetails.functions import get_auto_id
import datetime
...
def list_salesDetailsDummy(request):
    ...
    if serialized1.is_valid():
        ...
        if SalesDetailsDummy.objects.filter(BranchID=BranchID).exists():
            ...
            instances = SalesDetailsDummy.objects.filter(BranchID=BranchID,IsDeleted=False)
            deleted_instances = SalesDetailsDummy.objects.filter(BranchID=BranchID,IsDeleted=True)
            ...
            serialized = SalesDetailsDummySerializer(instances,many=True,context = {"DataBase": DataBase })
            serializedDeleted = SalesDetailsDeletedDummySerializer(deleted_instances,many=True)
