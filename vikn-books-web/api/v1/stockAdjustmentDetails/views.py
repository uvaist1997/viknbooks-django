...
from api.v1.stockAdjustmentDetails.serializers import StockAdjustmentDetailsSerializer, StockAdjustmentDetailsRestSerializer, StockAdjustmentDetailsDummySerializer
from api.v1.brands.serializers import ListSerializer
from api.v1.stockAdjustmentDetails.functions import generate_serializer_errors
from rest_framework import status
from api.v1.stockAdjustmentDetails.functions import get_auto_id
import datetime
...
def create_stockAdjustmentDetails(request):
    ...
    if serialized.is_valid():
        ...
        StockAdjustmentDetailsID = get_auto_id(StockAdjustmentDetails,BranchID)
...
def list_stockAdjustmentDetails(request):
    ...
    if serialized1.is_valid():
        ...
        if StockAdjustmentDetails.objects.filter(BranchID=BranchID).exists():
            ...
            instances = StockAdjustmentDetails.objects.filter(BranchID=BranchID)

            serialized = StockAdjustmentDetailsRestSerializer(instances,many=True)
...
def list_stockAdjustmentDetailsDummy(request):
    ...
    if serialized1.is_valid():
        ...
        if StockAdjustmentDetailsDummy.objects.filter(BranchID=BranchID).exists():
            ...
            instances = StockAdjustmentDetailsDummy.objects.filter(BranchID=BranchID)

            serialized = StockAdjustmentDetailsDummySerializer(instances,many=True)
