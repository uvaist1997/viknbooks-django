...
from brands.models import SalesMaster, SalesDetails, Product, AccountLedger, Warehouse, PriceList, TransactionTypes,Unit, TaxCategory, StockPosting, StockRate, LedgerPosting, Parties
...
class SalesMasterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id','BranchID','Action','VoucherNo','Date',
            ...
            'NetTotal','AdditionalCost','GrandTotal','RoundOff','CashReceived','BankAmount','WarehouseID','TableID','SeatNumber','NoOfGuests'
            ,'INOUT','TokenNumber','CardTypeID','CardNumber','IsActive','IsPosted','SalesType','CreatedUserID',
...
class SalesMasterRestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id','SalesMasterID','BranchID','Action','VoucherNo','Date',
            ...
            'Notes','FinacialYearID','TotalGrossAmt','AddlDiscPercent','AddlDiscAmt','TotalDiscount','TotalTax',
            'NetTotal','AdditionalCost','GrandTotal','RoundOff','CashReceived','CashAmount','BankAmount','WarehouseID','WareHouseName','TableID','SeatNumber','NoOfGuests'
            ,'INOUT','TokenNumber','CardTypeID','CardTypeName','CardNumber','IsActive','IsPosted','SalesType','CreatedDate','UpdatedDate','CreatedUserID',
            'TaxID','TaxType','BillDiscPercent','BillDiscAmt','VATAmount','SGSTAmount','CGSTAmount','IGSTAmount',
            'TAX1Amount','TAX2Amount','TAX3Amount','Balance','TransactionTypeID','DetailID','is_customer','SalesDetails')
    ...
    def get_CardTypeName(self, instances):
        ...
        if TransactionTypes.objects.filter(CompanyID=CompanyID,TransactionTypesID=TransactionTypesID,BranchID=BranchID).exists():
            ...
            transactionType = TransactionTypes.objects.get(CompanyID=CompanyID,TransactionTypesID=TransactionTypesID,BranchID=BranchID)
...
class SalesDetailsRestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id','unq_id','SalesDetailsID','BranchID','Action','SalesMasterID','ProductID','ProductName',
            'Qty','FreeQty','UnitPrice','ReturnQty','InclusivePrice',
            'RateWithTax','CostPerPrice','PriceListID','UnitName','DiscountPerc','DiscountAmount','AddlDiscPerc','AddlDiscAmt',
            'GrossAmount','TaxableAmount','VATPerc','VATAmount','SGSTPerc','SGSTAmount','CGSTPerc',
            'CGSTAmount','IGSTPerc','IGSTAmount','NetAmount','CreatedDate','CreatedUserID','Flavour',
            'TAX1Perc','TAX1Amount','TAX2Perc','TAX2Amount','TAX3Perc','TAX3Amount','TotalTax'
            ,'DeliveryDetailsID','OrderDetailsID','detailID','ExistingQty',
    ...
    def get_UnitName(self, purchase_details):
        ...
        if PriceList.objects.filter(CompanyID=CompanyID,PriceListID=PriceListID,BranchID=BranchID).exists():
            ...
            UnitName = Unit.objects.get(BranchID=BranchID,UnitID=UnitID,CompanyID=CompanyID).UnitName
        else:
            UnitName = ""
...
class SalesMasterReportSerializer(serializers.ModelSerializer):
    ...
    def get_AccountGroupUnder(self, instances):
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID=LedgerID,BranchID=BranchID).exists():
            AccountGroupUnder = AccountLedger.objects.get(CompanyID=CompanyID,LedgerID=LedgerID,BranchID=BranchID).AccountGroupUnder
    ...
    def get_CashSales(self, instances):
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID=LedgerID,BranchID=BranchID,AccountGroupUnder=9).exists():
            ...
            if LedgerPosting.objects.filter(CompanyID=CompanyID,LedgerID=LedgerID,Date__gte=FromDate,Date__lte=ToDate,VoucherType="SI").exists():
                ...
                ledger_ins = LedgerPosting.objects.filter(CompanyID=CompanyID,LedgerID=LedgerID,Date__gte=FromDate,Date__lte=ToDate,VoucherType="SI")
                for i in ledger_ins:
...
class SalesMasterForReturnSerializer(serializers.ModelSerializer):
    ...
    def get_SalesAccountName(self, instances):
        ...
        SalesAccountName = ""
        if AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID=SalesAccount,BranchID=BranchID).exists():
            SalesAccountName = AccountLedger.objects.get(CompanyID=CompanyID,LedgerID=SalesAccount,BranchID=BranchID).LedgerName
    ...
    def get_SalesDetails(self, instances):
        ...
        SalesMasterID = instances.SalesMasterID
        BranchID = instances.BranchID
        sales_details = SalesDetails.objects.filter(CompanyID=CompanyID,SalesMasterID=SalesMasterID,BranchID=BranchID,ReturnQty__gt=0)
...
class SalesDetailsForReturnsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SalesDetails
        fields = ('id','unq_id','SalesDetailsID','BranchID','Action','SalesMasterID','ProductID','ProductName',
            'Qty','FreeQty','UnitPrice','InclusivePrice','ReturnQty',
            'RateWithTax','CostPerPrice','PriceListID','UnitName','DiscountPerc','DiscountAmount','AddlDiscPerc','AddlDiscAmt',
            'GrossAmount','TaxableAmount','VATPerc','VATAmount','SGSTPerc','SGSTAmount','CGSTPerc',
            'CGSTAmount','IGSTPerc','IGSTAmount','NetAmount','CreatedDate','CreatedUserID','Flavour',
            'TAX1Perc','TAX1Amount','TAX2Perc','TAX2Amount','TAX3Perc','TAX3Amount','TotalTax'
            ,'DeliveryDetailsID','OrderDetailsID','detailID','ExistingQty','UnitList')
    ...
    def get_UnitList(self, purchase_details):
        ...
        UnitList = PriceList.objects.filter(CompanyID=CompanyID,ProductID=ProductID,BranchID=BranchID)

        serialized = PriceListRestSerializer(UnitList,many=True,context = {"CompanyID": CompanyID,"PriceRounding": PriceRounding })
...
class StockSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ('id','AutoBarcode','ProductID','ProductName','WareHouseName','PurchasePrice','SalesPrice','UnitName','BaseUnitName','is_BasicUnit','MultiFactor','Qty','Cost')
    ...
    def get_Cost(self, instance):
        ...
        if StockPosting.objects.filter(ProductID=ProductID,CompanyID=CompanyID,WareHouseID=WareHouseID,QtyIn__gt=0).exists():
            stock_ins = StockPosting.objects.filter(ProductID=ProductID,CompanyID=CompanyID,WareHouseID=WareHouseID,QtyIn__gt=0)
            TotalQtyInRate = 0
            TotalQtyIn = 0
            for s in stock_ins:
    ...
    def get_Qty(self, instance):
        ...
        if StockPosting.objects.filter(ProductID=ProductID,CompanyID=CompanyID,WareHouseID=WareHouseID).exists():
            stock_ins = StockPosting.objects.filter(ProductID=ProductID,CompanyID=CompanyID,WareHouseID=WareHouseID)
            TotalQtyIn = 0
            TotalQtyOut = 0
            for s in stock_ins:
                TotalQtyIn += s.QtyIn
    ...
    def get_SalesPrice(self, final_instance):
        ...
        ProductID = final_instance.ProductID
        BranchID = final_instance.BranchID
        SalesPrice = PriceList.objects.get(CompanyID=CompanyID,ProductID=ProductID,BranchID=BranchID,DefaultUnit=True).SalesPrice
    ...
    def get_UnitName(self, final_instance):
        ...
        if PriceList.objects.filter(CompanyID=CompanyID,ProductID=ProductID,BranchID=BranchID,UnitInReports=True).exists():
            UnitID = PriceList.objects.get(CompanyID=CompanyID,ProductID=ProductID,BranchID=BranchID,UnitInReports=True).UnitID
    ...
    def get_MultiFactor(self, final_instance):
        ...
        if PriceList.objects.filter(CompanyID=CompanyID,ProductID=ProductID,BranchID=BranchID,UnitInReports=True,DefaultUnit=False).exists():
            MultiFactor = PriceList.objects.get(CompanyID=CompanyID,ProductID=ProductID,BranchID=BranchID,UnitInReports=True,DefaultUnit=False).MultiFactor
...
class StockRateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = StockRate
        fields = ('id','WareHouseID','WareHouseName','ProductName','UnitName','Date','PurchasePrice','SalesPrice','Qty','Cost','TotalCost')
    ...
    def get_ProductName(self, instance):
        ...
        ProductID = instance.ProductID
        BranchID = instance.BranchID
        ProductName = Product.objects.get(CompanyID=CompanyID,ProductID=ProductID,BranchID=BranchID).ProductName
    ...
    def get_UnitName(self, instance):
        ...
        UnitID = PriceList.objects.get(CompanyID=CompanyID,PriceListID=PriceListID,BranchID=BranchID,DefaultUnit=True).UnitID
        UnitName = Unit.objects.get(CompanyID=CompanyID,UnitID=UnitID).UnitName
...
class SalesPrintSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('MasterID','BranchID','Action','VoucherNo','Date',
            ...
            ,'INOUT','TokenNumber','CardTypeID','CardNumber','IsActive','IsPosted','Types','CreatedDate','UpdatedDate','CreatedUserID',
            'TaxID','TaxType','BillDiscPercent','BillDiscAmt','VATAmount','SGSTAmount','CGSTAmount','IGSTAmount',
    ...
    def get_PartyName(self, instances):
        ...
        if AccountLedger.objects.filter(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID,AccountGroupUnder__in=[10,29]).exists():
            PartyName = Parties.objects.get(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID).PartyName
        return PartyName
    ...
    def get_DisplayName(self, instances):
        ...
        DisplayName = ""
        if AccountLedger.objects.filter(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID,AccountGroupUnder__in=[10,29]).exists():
            DisplayName = Parties.objects.get(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID).DisplayName
        return DisplayName
    ...
    def get_FirstName(self, instances):
        ...
        FirstName = ""
        if AccountLedger.objects.filter(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID,AccountGroupUnder__in=[10,29]).exists():
            FirstName = Parties.objects.get(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID).FirstName
        return FirstName
    ...
    def get_LastName(self, instances):
        ...
        LastName = ""
        if AccountLedger.objects.filter(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID,AccountGroupUnder__in=[10,29]).exists():
            LastName = Parties.objects.get(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID).LastName
        return LastName
    ...
    def get_Address1(self, instances):
        ...
        if AccountLedger.objects.filter(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID,AccountGroupUnder__in=[10,29]).exists():
            if Parties.objects.get(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID).Address1:
                Address1 = Parties.objects.get(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID).Address1
        return Address1
    ...
    def get_Address2(self, instances):
        ...
        if AccountLedger.objects.filter(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID,AccountGroupUnder__in=[10,29]).exists():
            if Parties.objects.get(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID).Address2:
                Address2 = Parties.objects.get(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID).Address2
        return Address2
    ...
    def get_City(self, instances):
        ...
        if AccountLedger.objects.filter(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID,AccountGroupUnder__in=[10,29]).exists():
            if Parties.objects.get(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID).City:
                City = Parties.objects.get(BranchID=BranchID,CompanyID=CompanyID,LedgerID=LedgerID).City
        return City
    ...
    def get_Details(self, instances):
        ...
        BranchID = instances.BranchID
        sales_details = SalesDetails.objects.filter(CompanyID=CompanyID,SalesMasterID=SalesMasterID,BranchID=BranchID)
        serialized = SalesDetailsPrintSerializer(sales_details,many=True,context = {"CompanyID": CompanyID, "PriceRounding" : PriceRounding })
...
class SalesIntegratedSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LedgerPosting
        fields = ('CashSale','CreditSale','BankSale','CashSaleReturn','CreditSaleReturn','BankSaleReturn')
