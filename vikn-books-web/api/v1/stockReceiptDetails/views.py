...
from api.v1.stockReceiptDetails.serializers import StockReceiptDetailsSerializer, StockReceiptDetailsRestSerializer, StockReceiptDetailsDummySerializer
from api.v1.brands.serializers import ListSerializer
from api.v1.stockReceiptDetails.functions import generate_serializer_errors
from rest_framework import status
from api.v1.stockReceiptDetails.functions import get_auto_id
import datetime
...
def list_stockReceiptDetails(request):
    ...
    if serialized1.is_valid():
        ...
        if StockReceiptDetails.objects.filter(BranchID=BranchID).exists():
            ...
            serialized = StockReceiptDetailsRestSerializer(instances,many=True)
...
def list_stockReceiptDetailsDummy(request):
    ...
    if serialized1.is_valid():
        ...
        if StockReceiptDetailsDummy.objects.filter(BranchID=BranchID).exists():
            ...
            instances = StockReceiptDetailsDummy.objects.filter(BranchID=BranchID)

            serialized = StockReceiptDetailsDummySerializer(instances,many=True)
