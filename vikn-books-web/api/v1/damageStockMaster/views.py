...
from api.v1.damageStockDetails.serializers import DamageStockDetailsSerializer, DamageStockDetailsRestSerializer
from api.v1.brands.serializers import ListSerializer
from api.v1.damageStockMaster.functions import generate_serializer_errors
from rest_framework import status
from api.v1.damageStockMaster.functions import get_auto_id
import datetime
...
def create_damageStockMaster(request):
    ...
    if serialized.is_valid():
        ...
        Action = 'A'
        DamageStockMasterID = get_auto_id(DamageStockMaster,BranchID,DataBase)
...
def list_damageStockMaster(request):
    ...
    if serialized1.is_valid():
        ...
        if DamageStockMaster.objects.filter(BranchID=BranchID).exists():
            ...
            serialized = DamageStockMasterRestSerializer(instances,many=True,context = {"DataBase": DataBase })
            response_data = {
                "StatusCode" : 6000,
...
def damageStockMaster(request,pk):
    ...
    if DamageStockMaster.objects.filter(DamageStockMasterID=pk).exists():
        ...
        serialized = DamageStockMasterRestSerializer(instances,context = {"DataBase": DataBase })
        response_data = {
            "StatusCode" : 6000,
            "data" : serialized.data,
        }
