...
from api.v1.flavours.serializers import FlavoursSerializer, FlavoursRestSerializer
from api.v1.brands.serializers import ListSerializer
from api.v1.flavours.functions import generate_serializer_errors
from rest_framework import status
from api.v1.flavours.functions import get_auto_id
...
def create_flavour(request):
    ...
    if serialized.is_valid():
        ...
        flavours = Flavours.objects.filter(BranchID=BranchID,CompanyID=CompanyID)
...
def flavours(request):
    ...
    if serialized1.is_valid():
        ...
        if Flavours.objects.filter(BranchID=BranchID,CompanyID=CompanyID).exists():
            instances = Flavours.objects.filter(BranchID=BranchID,CompanyID=CompanyID)
