...
from api.v1.banks.serializers import BankSerializer, BankRestSerializer
from api.v1.brands.serializers import ListSerializer
from api.v1.banks.functions import generate_serializer_errors
from rest_framework import status
from api.v1.banks.functions import get_auto_Bankid, get_BankCode
from api.v1.accountLedgers.functions import get_auto_id, get_auto_LedgerPostid
import datetime
...
def create_bank(request):
    ...
    if serialized.is_valid():
        ...
        if not is_nameExist:
            ...
            activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Bank', 'Create', 'Bank created successfully.', 'Bank saved successfully.')
            ...
            #request , company, log_type, user, source, action, message, description
            activity_log(request._request, CompanyID, 'Warning', CreatedUserID, 'Bank', 'Create', 'Bank created failed.', 'Ledger Name Already Exist.')
            response_data = {
            "StatusCode" : 6001,
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'Bank', 'Create', 'Bank created failed.', generate_serializer_errors(serialized._errors))
        response_data = {
            "StatusCode" : 6001,
    ...
    if serialized.is_valid():
        ...
        if  accountLedger_ok:
            ...
            if AccountLedger.objects.filter(LedgerName=instanceLedgerName,BranchID=BranchID,CompanyID=CompanyID).exists():
                account_ledger = AccountLedger.objects.get(LedgerName=instanceLedgerName,BranchID=BranchID,CompanyID=CompanyID)
            ...
            #request , company, log_type, user, source, action, message, description
            activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Bank', 'Edit', 'Bank Updated Successfully.', "Bank Updated Successfully.")
            response_data = {
                "StatusCode" : 6000,
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'Bank', 'Edit', 'Bank Updated Failed.',generate_serializer_errors(serialized._errors))
        response_data = {
            "StatusCode" : 6001,
            "message" : generate_serializer_errors(serialized._errors)
        }
...
def banks(request):
    ...
    if serialized1.is_valid():
        ...
        if Bank.objects.filter(BranchID=BranchID,CompanyID=CompanyID).exists():
            ...
            serialized = BankRestSerializer(instances,many=True,context={"PriceRounding":PriceRounding})

            #request , company, log_type, user, source, action, message, description
            activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Bank', 'View', 'Bank List View successfully.',"Bank List View successfully.")
            response_data = {
                "StatusCode" : 6000,
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'Bank', 'View', 'Bank List View Failed.',"Invalid BranchID Given!")
        response_data = {
            "StatusCode" : 6001,
...
def bank(request,pk):
    ...
    if Bank.objects.filter(pk=pk,CompanyID=CompanyID).exists():
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Bank', 'View', 'Bank single page Viewed.', 'Bank single page Viewed.')
        response_data = {
            "StatusCode" : 6000,
            "data" : serialized.data
        }
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'Bank', 'View', 'Bank single page View failed.', 'Bank Not Found')
        response_data = {
            "StatusCode" : 6001,
...
def delete_bank(request,pk):
    ...
    if Bank.objects.filter(pk=pk,CompanyID=CompanyID).exists():
        ...
        if AccountLedger.objects.filter(BranchID=BranchID,LedgerName=LedgerName,CompanyID=CompanyID).exists():
            account_ledger = AccountLedger.objects.get(BranchID=BranchID,LedgerName=LedgerName,CompanyID=CompanyID)
            ...
            # if not LPInstances:
                ...
                #request , company, log_type, user, source, action, message, description
                activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Bank', 'Deleted', 'Bank Deleted Successfully.', 'Bank Deleted Successfully')
                response_data = {
                    "StatusCode" : 6000,
                    "title" : "Success",
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'Bank', 'Deleted', 'Bank Deleted Failed.', 'Bank Not Found')
        response_data = {
            "StatusCode" : 6001,
            "title" : "Failed",
            "message" : "Bank Not Found!"
