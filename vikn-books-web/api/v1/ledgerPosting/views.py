from brands.models import LedgerPosting, LedgerPosting_Log, AccountLedger, AccountGroup, StockRate, StockPosting, Product, FinancialYear, PriceList, CompanySettings
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from api.v1.ledgerPosting.serializers import LedgerPostingSerializer, LedgerPostingRestSerializer, ListSerializerforTrialBalance,\
    ...
    BalanceSheetSerializer, LedgerReportLedgerWiseSerializer, StockReportSerializer
from api.v1.brands.serializers import ListSerializer
from api.v1.stockPostings.serializers import StockPostingRestSerializer, StockValueAllSerializer, StockValueSingleSerializer
from api.v1.ledgerPosting.functions import generate_serializer_errors
from rest_framework import status
from api.v1.sales.serializers import ListSerializerforReport, ListSerializerforStockReport, ListSerializerforStockValueReport, ListSerializerforStockValueReportSingle
from api.v1.ledgerPosting.functions import get_auto_id, convertOrderdDict, get_VoucherName
import datetime
        ...
        if LedgerPosting.objects.filter(BranchID=BranchID, CompanyID=CompanyID).exists():
            ...
            if instances:
                ...
                if Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InventoryType="StockItem").exists():
                    product_instances_CS = Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InventoryType="StockItem")
                    ...
                    for pi in product_instances_CS:
                        ...
                        if StockPosting.objects.filter(ProductID=ProductID,Date__lte=ToDate,CompanyID=CompanyID).exists():
                            stock_instance_CS = StockPosting.objects.filter(ProductID=ProductID,Date__lte=ToDate,CompanyID=CompanyID)
                            QtyInRate = 0
                            QtyOutTot = 0
                            QtyInTot = 0
                            for si in stock_instance_CS:
...
def ledgerReport(request):
    ...
    if serialized1.is_valid():
        ...
        if LedgerPosting.objects.filter(BranchID=BranchID, CompanyID=CompanyID, Date__gte=FromDate, Date__lte=ToDate).exists():
            instances = LedgerPosting.objects.filter(
                BranchID=BranchID, CompanyID=CompanyID, Date__gte=FromDate, Date__lte=ToDate).order_by('Date')
            ...
            if ID == 0:
                ...
                instances = LedgerPosting.objects.filter(
                BranchID=BranchID, CompanyID=CompanyID).order_by('Date')
                virtual_array = []
                last_data = []
                ob_array = []
                Balance = 0
                ...
                if instances.filter(CompanyID=CompanyID, LedgerID=value).exists():
                    ...
                    instances = instances.filter(
                        CompanyID=CompanyID, LedgerID=value).order_by('Date')
                ...
                for i in jsnDatas:
                    ...
                    if GroupUnder == True:
                        ...
                        filtered_dictionary = {"LedgerID": LedgerID, "LedgerName": LedgerName, "Debit": round(Debit, PriceRounding), "Credit": round(Credit, PriceRounding),
                                               "Balance": round(Balance, PriceRounding), "GroupUnder": GroupUnder, }
        ...
        response_data = {
            "StatusCode": 6001,
            "message": "please Select Dates and Groups!!!"
        }
        return Response(response_data, status=status.HTTP_200_OK)
...
def profitAndLoss_web(request):
    ...
    if serialized1.is_valid():
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate).exists():
            ...
            TotalAvgValueOpening = 0
            if Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InventoryType="StockItem").exists():
                product_instances_OS = Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InventoryType="StockItem")
                ...
                for pi in product_instances_OS:
                    ...
                    if StockPosting.objects.filter(ProductID=ProductID,Date__lt=FromDate,CompanyID=CompanyID).exists():
                        stock_instance_OS = StockPosting.objects.filter(ProductID=ProductID,Date__lt=FromDate,CompanyID=CompanyID)
                        QtyInRate = 0
                        QtyOutTot = 0
                        QtyInTot = 0
                    ...
                    if StockPosting.objects.filter(ProductID=ProductID,Date__gte=FromDate, Date__lte=ToDate,CompanyID=CompanyID,VoucherType="OS").exists():
                        stock_instance_OS = StockPosting.objects.filter(ProductID=ProductID,Date__gte=FromDate, Date__lte=ToDate,CompanyID=CompanyID,VoucherType="OS")
                        QtyInRate = 0
                        QtyOutTot = 0
                        QtyInTot = 0
                        for si in stock_instance_OS:
