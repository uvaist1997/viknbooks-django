...
from api.v1.parties.serializers import PartiesSerializer, PartiesRestSerializer, PartyListSerializer
# from api.v1.accountLedgers.serializers import AccountLedgerSerializer
# from api.v1.brands.serializers import ListSerializer
from api.v1.parties.functions import generate_serializer_errors
from rest_framework import status
from api.v1.parties.functions import get_auto_id, get_auto_idLedger, get_PartyCode
...
def create_party(request):
    ...
    if party_serialized.is_valid():
        ...
        parties = Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
        ...
        if not is_nameExist:
            ...
            accountLedgers = AccountLedger.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
            ...
            activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Party', 'Create', 'Party created successfully.', 'Party saved successfully.')
            ...
            #request , company, log_type, user, source, action, message, description
            activity_log(request._request, CompanyID, 'Warning', CreatedUserID, 'Party', 'Create', 'Party created Failed.', "Party Name Already exists")
            response_data = {
            "StatusCode" : 6001,
            "message" : "Party Name Already Exist!!!"
            }
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'Party', 'Create', 'Party created Failed.', generate_serializer_errors(party_serialized._errors))
        response_data = {
            "StatusCode" : 6001,
...
def edit_party(request,pk):
    ...
    accountLedgerInstance = AccountLedger.objects.get(CompanyID=CompanyID,LedgerID=LedgerID,BranchID=BranchID)
    ...
    if LedgerPosting.objects.filter(CompanyID=CompanyID,LedgerID=LedgerID,BranchID=BranchID,VoucherType="LOB").exists():
        ledgerPostInstance = LedgerPosting.objects.get(CompanyID=CompanyID,LedgerID=LedgerID,BranchID=BranchID,VoucherType="LOB")
    ...
    if party_serialized.is_valid():
        ...
        if  party_ok:
            ...
            #request , company, log_type, user, source, action, message, description
            activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Party', 'Edit', 'Party Updated successfully.', 'Party Updated successfully.')
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'Party', 'Edit', 'Party Updated Failed.', generate_serializer_errors(party_serialized._errors))
        response_data = {
            "StatusCode" : 6001,
            "message" : generate_serializer_errors(party_serialized._errors)
        }
    ...
    if serialized1.is_valid():
        ...
        if PartyType == "0":
            ...
            if Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
                instances = Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
                serialized = PartiesRestSerializer(instances,many=True,context = {"CompanyID": CompanyID,"PriceRounding" : PriceRounding  })

                #request , company, log_type, user, source, action, message, description
                activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Party', 'List Viewed', 'Party List Viewed Successfully.', 'Party List Viewed Successfully.')
                response_data = {
                    "StatusCode" : 6000,
                ...
                #request , company, log_type, user, source, action, message, description
                activity_log(request._request, CompanyID, 'Warning', CreatedUserID, 'Party', 'List Viewed', 'Party List Viewed Failed.', "Parties not found in this branch.")
                response_data = {
                "StatusCode" : 6001,
            ...
            instances = Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PartyType=PartyType)
...
def party(request,pk):
    ...
    if Parties.objects.filter(CompanyID=CompanyID,pk=pk).exists():
        ...
        serialized = PartiesRestSerializer(instance,context = {"CompanyID": CompanyID,"PriceRounding" : PriceRounding  })
        
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Party', 'Viewe', 'Party Single Page Viewed Successfully.', 'Party Single Page Viewed Successfully.')
        response_data = {
            "StatusCode" : 6000,
            "data" : serialized.data
        }
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Warning', CreatedUserID, 'Party', 'Single Page Viewed', 'Party Single Page Viewed Failed.', "Party not found in this branch.")
        response_data = {
            "StatusCode" : 6001,
...
def delete_party(request,pk):
    ...
    if Parties.objects.filter(CompanyID=CompanyID,pk=pk).exists():
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID,LedgerID=LedgerID,BranchID=BranchID).exists():
            ledgerPostInstance = LedgerPosting.objects.get(CompanyID=CompanyID,LedgerID=LedgerID,BranchID=BranchID)
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Party', 'Delete', 'Party Deleted Successfully.', 'Party Deleted Successfully.')
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Warning', CreatedUserID, 'Party', 'Delete', 'Party Deleted Failed.', 'Party Not Found!')
        response_data = {
            "StatusCode" : 6001,
            "message" : "Party Not Found!"
        }
