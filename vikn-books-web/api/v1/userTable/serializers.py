...
from brands.models import UserTable, Customer, UserType, AccountLedger
...
class UserTableSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserTable
        fields = ('UserType','DefaultAccountForUser','customer','ExpiryDate',)
...
class UserTableSerializerRestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserTable
        fields = ('id','CompanyID','UserType','user_type_name','Action','DefaultAccountForUser','CreatedUserID','customer','customer_name','CreatedDate','UpdatedDate','Cash_Account','Bank_Account','Sales_Account','Sales_Return_Account','Purchase_Account','Purchase_Return_Account','JoinedDate','ExpiryDate','LeaveDate','cash_account_name', 'bank_account_name', 'sales_account_name', 'sales_return_account_name', 'purchase_account_name', 'purchase_return_account_name')

    def get_customer_name(self, instance):
        CompanyID = self.context.get("CompanyID")
        customer_name = Customer.objects.get(pk=instance.customer.pk).user.username
    ...
    def get_user_type_name(self, instance):
        ...
        if instance.UserType:
            user_type_name = UserType.objects.get(pk=instance.UserType.pk).UserTypeName
    ...
    def get_cash_account_name(self, instance):
        ...
        if AccountLedger.objects.filter(LedgerID=instance.Cash_Account,CompanyID=CompanyID).exists() :
            cash_account_name = AccountLedger.objects.get(LedgerID=instance.Cash_Account,CompanyID=CompanyID).LedgerName
    ...
    def get_bank_account_name(self, instance):
        ...
        if AccountLedger.objects.filter(LedgerID=instance.Bank_Account,CompanyID=CompanyID).exists() :
            bank_account_name = AccountLedger.objects.get(LedgerID=instance.Bank_Account,CompanyID=CompanyID).LedgerName
    ...
    def get_sales_account_name(self, instance):
        ...
        if AccountLedger.objects.filter(LedgerID=instance.Sales_Account,CompanyID=CompanyID).exists() :
            sales_account_name = AccountLedger.objects.get(LedgerID=instance.Sales_Account,CompanyID=CompanyID).LedgerName
    ...
    def get_sales_return_account_name(self, instance):
        ...
        if AccountLedger.objects.filter(LedgerID=instance.Sales_Return_Account,CompanyID=CompanyID).exists() :
            sales_return_account_name = AccountLedger.objects.get(LedgerID=instance.Sales_Return_Account,CompanyID=CompanyID).LedgerName
    ...
    def get_purchase_account_name(self, instance):
        ...
        if AccountLedger.objects.filter(LedgerID=instance.Purchase_Account,CompanyID=CompanyID).exists() :
            purchase_account_name = AccountLedger.objects.get(LedgerID=instance.Purchase_Account,CompanyID=CompanyID).LedgerName
    ...
    def get_purchase_return_account_name(self, instance):
        ...
        if AccountLedger.objects.filter(LedgerID=instance.Purchase_Return_Account,CompanyID=CompanyID).exists() :
            purchase_return_account_name = AccountLedger.objects.get(LedgerID=instance.Purchase_Return_Account,CompanyID=CompanyID).LedgerName
