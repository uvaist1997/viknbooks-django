...
def get_auto_id(model,BranchID,DataBase):
    ...
    latest_auto_id =  model.objects.all().aggregate(Max('DamageStockDetailsID'))
    

    if model.objects.filter(BranchID=BranchID).exists():
        max_value =  model.objects.filter(BranchID=BranchID).aggregate(Max('DamageStockDetailsID'))
