...
from api.v9.warehouses.functions import generate_serializer_errors, get_auto_id
from api.v9.warehouses.serializers import WarehouseRestSerializer, WarehouseSerializer
...
from main.functions import activity_log, get_ProductStock, get_ProductWareHouseStock, get_company
...
def create_warehouse(request):
    ...
    if serialized.is_valid():
        ...
        if not Warehouse.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID, WarehouseName__iexact=WarehouseName
        ):
...
def edit_warehouse(request, pk):
    ...
    if serialized.is_valid():
        ...
        if not Warehouse.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID, WarehouseName__iexact=WarehouseName
        ).exclude(WarehouseName=instanceWarehouseName):
            ...
            activity_log(
                ...
                "Warehouse Name Already exist with this Branch",
            )
            if Warehouse.objects.filter(
                CompanyID=CompanyID,
                BranchID=BranchID,
                WarehouseName__iexact=WarehouseName,
            ):
                response_data = {
                    "StatusCode": 6001,
...
def warehouses(request):
    ...
    if UserTable.objects.filter(
        ...
        DefaultWarehouse = UserTable.objects.get(
            CompanyID=CompanyID, customer__user__pk=CreatedUserID
        ).DefaultWarehouse
    ...
    if serialized1.is_valid():
        ...
        if POS_Settings.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            Pos_ins = POS_Settings.objects.filter(
        ...
        if Warehouse.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            ...
            if GeneralSettings.objects.filter(
                CompanyID=CompanyID, SettingsType="ExpiryDays"
            ).exists():
            ...
            if GeneralSettings.objects.filter(
                CompanyID=CompanyID, SettingsType="ShowTotalTaxInPurchase"
            ).exists():
                general_ins = GeneralSettings.objects.filter(
                    CompanyID=CompanyID, SettingsType="ShowTotalTaxInPurchase"
                ).first()
...
def warehouse(request, pk):
    ...
    if Warehouse.objects.filter(CompanyID=CompanyID, pk=pk).exists():
        ...
        activity_log(
            ...
            "Warehouse",
            "View",
            "Warehouse Single Viewed Failed",
            "Warehouse Not Found in this Branch",
        )
        response_data = {"StatusCode": 6001, "message": "Ware House Not Found!"}
...
def delete_warehouse(request, pk):
    ...
    if pk == "1":
        ...
        activity_log(
            ...
            "Warehouse",
            "Delete",
            "Warehouse Deleted Failed",
            "User Tried to Delete Default Warehouse!",
        )
        response_data = {
            "StatusCode": 6001,
        ...
        if instances:
            for instance in instances:
                ...
                if (
                    not purchaseMaster_exist
                    and not salesMaster_exist
                    ...
                    activity_log(
                        ...
                        "Delete",
                        "Warehouse Deleted Failed",
                        "Cant Delete this Warehouse,this WarehouseID is using somewhere",
                    )
                    response_data = {
                        "StatusCode": 6001,
                        "title": "Failed",
            ...
            activity_log(
                ...
                "Delete",
                "Warehouse Deleted Failed",
                "WareHouse Not Found under this Branch",
            )
            response_data = {
                "StatusCode": 6001,
                "title": "Failed",
...
def list_warehouses(request):
    ...
    if Warehouse.objects.filter(CompanyID=CompanyID).exists():
        instances = Warehouse.objects.filter(CompanyID=CompanyID)
        serialized = WarehouseRestSerializer(instances, many=True)

        response_data = {"StatusCode": 6000, "data": serialized.data}
    else:
        response_data = {"StatusCode": 6001, "message": "Warehouses Not Found!"}
...
def get_warehouse_stock(request):
    ...
    print(WarehouseID)
    print(type(ProductList),'TYPE')
    if WarehouseID and ProductList:
        for i in ProductList:
            if i['ProductID'] > 0:
                i['Stock'] = get_ProductWareHouseStock(
                    CompanyID, BranchID, i['ProductID'], WarehouseID, i["BatchCode"])
        print(ProductList)
        response_data = {"StatusCode": 6000, "data": ProductList}
    else:        
        response_data = {"StatusCode": 6001, "message": "WareHouse Not Found!"}
