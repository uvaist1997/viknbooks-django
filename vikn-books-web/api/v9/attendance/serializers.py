...
from brands.models import AttendanceDetail, AttendanceMaster
...
class AttendanceSingleSerializer(serializers.ModelSerializer):
    ...
    def get_AttendanceDetails(self, instances):
        ...
        pk = instances.pk
        sales_details = AttendanceDetail.objects.filter(AttendanceId=pk)
        print(sales_details, "AttendanceId??????????????????????????????", pk)
        serialized = AttendanceDetailSerializer(
            sales_details,
            many=True,
            context={
                "CompanyID": CompanyID,
