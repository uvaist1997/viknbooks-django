...
from api.v9.ledgerPosting.functions import DayBook_excel_data
...
def download_report(request):
   ...
   # Purchase Both Report
    ...
    if invoice_type == "OutStanding":
        ...
        df.columns = [str(_("LedgerName")), str(_("Debit")), str(_("Credit"))]
        df.loc[str(_("Total"))] = (
    ...
    # Trail Balance Report
        ...
        df, details = query_trialBalance_report_data(
            filterValue,CompanyID, UserID, PriceRounding, BranchID, ToDate, FromDate
        )
    # Day Book
    ...
    # Sales Integrated Report
        ...
        if UserTable.objects.filter(CompanyID=CompanyID_ins).exists():
            user_list = UserTable.objects.filter(CompanyID=CompanyID_ins).values_list(
    ...
    if download_type == "PDF":
        ...
        print(invoice_type,"<<<<<<<<<invoice_type>>>>>>>>,",report_title)
        response["Content-Disposition"] = 'attachment; filename="{}.xlsx"'.format(
            report_name
        )
        # writer = pd.ExcelWriter('uvais.xlsx', engine='xlsxwriter')
