...
def create_stockTransfer(request):
    try:
        with transaction.atomic():
            ...
            transfer_ok = True
            if StockTransferMaster_ID.objects.filter(CompanyID=CompanyID, BranchID=BranchID, VoucherNo__iexact=VoucherNo).exists():
                VoucherNoAutoGenerate = get_GeneralSettings(
                    CompanyID, BranchID, "VoucherNoAutoGenerate")
                if VoucherNoAutoGenerate == False:
                    transfer_ok = False
