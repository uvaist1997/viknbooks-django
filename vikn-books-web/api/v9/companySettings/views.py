...
from api.v9.branchs.functions import get_ledger_instance
from api.v9.companySettings.functions import generate_serializer_errors
...
from api.v9.settings.functions import (
    create_or_update_user_type_settings,
    get_user_type_settings,
)
from api.v9.users.serializers import ImageSerializer
from api.v9.users.views import send_mail_invitaion, send_mail_renew_request
...
from main.functions import activity_log, get_company, get_no_of_days_bwn_dates
...
def check_company_leave(request):
    ...
    if user is not None:
        ...
        if UserTable.objects.filter(
            CompanyID=company, customer__user__username=username, is_owner=False
        ).exists():
...
def check_company_status(request):
    ...
    if CompanyID:
        ...
        company_details = []
        user_pic = ""
        if UserTable.objects.filter(CompanyID=company, customer__user__id=user_id).exists():
            user_table_ins = UserTable.objects.filter(CompanyID=company, customer__user__id=user_id).first()
            owner_name = UserTable.objects.filter(
                CompanyID=company, is_owner=True).first().customer.user.first_name
            user_type = user_table_ins.UserType.ID
            UserTypeName = user_table_ins.UserType.UserTypeName
            if user_type == 1 and ExpiryDays < 6:
