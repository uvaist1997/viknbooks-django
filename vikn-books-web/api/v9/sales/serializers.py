...
from api.v9.workOrder.serializers import Batch_ListSalesSerializer, Batch_ListSerializer
...
from main.functions import converted_float, get_BalanceFromLedgerPost, get_GeneralSettings, get_ProductStock

# from api.v9.sales.functions import GST_finalList_fun
...
class SalesMasterRestSerializer(serializers.ModelSerializer):
    ...
    def get_LoyaltyCustomer_AccountLedgerID(self, instances):
        ...
        if instances.LoyaltyCustomerID:
            if instances.LoyaltyCustomerID.AccountLedgerID:
                AccountLedgerID = instances.LoyaltyCustomerID.AccountLedgerID.pk

        return str(AccountLedgerID)

    def get_LoyaltyCustomerValue(self, instance):
        LoyaltyCustomerValue = 0
        CompanyID = self.context.get("CompanyID")
        if LoyaltyPoint.objects.filter(CompanyID=instance.CompanyID,VoucherType="SI",VoucherMasterID=instance.SalesMasterID):
            LoyaltyCustomerValue = LoyaltyPoint.objects.get(CompanyID=instance.CompanyID,VoucherType="SI",VoucherMasterID=instance.SalesMasterID).Value
    ...
    def get_LoyaltyCustomerPoint(self, instance):
        ...
        CompanyID = self.context.get("CompanyID")
        if LoyaltyPoint.objects.filter(CompanyID=instance.CompanyID,VoucherType="SI",VoucherMasterID=instance.SalesMasterID):
            LoyaltyCustomerPoint = LoyaltyPoint.objects.get(CompanyID=instance.CompanyID,VoucherType="SI",VoucherMasterID=instance.SalesMasterID).Point
...
class SalesDetailsRestSerializer(serializers.ModelSerializer):
    ...
    def get_current_stock(self, purchase_details):
        current_stock = ""
        return current_stock

    def get_stock_status(self, purchase_details):
        stock_status = ""
        return stock_status
