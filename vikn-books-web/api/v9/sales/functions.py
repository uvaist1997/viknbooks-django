...
from main.functions import converted_float, get_BranchLedgerId_for_LedgerPosting, get_BranchList, get_GeneralSettings, get_company
import math
...
import re
from api.v9.sales.serializers import BillWiseSerializer, SalesMasterReportSerializer, StockSerializer, StockValueInventoryFlowSerializer
from brands.models import BillWiseDetails, BillWiseMaster, ExpenseMaster, PaymentMaster_Log, PurchaseMaster_Log, ReceiptMaster_Log, State, StockPosting, Product
...
from api.v9.sales.serializers import SalesGSTReportSerializer, GST_CDNR_Serializer, GST_B2B_Serializer
...
def get_Genrate_VoucherNo(model, BranchID, CompanyID, VoucherType):
    ...
    if model.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        ...
        if not OldVoucherNo.isdecimal():
            ...
            print("VoucherNoffffffffffffffffff")
            print(new_num)
...
def set_LoyaltyCalculation(instance, loyalty_customer, details, Loyalty_Point_Expire, Action, RadeemPoint):
    ...
    print(RadeemPoint,loyalty_point_value, "*********************************************************************************************************", loyalty_point_value)
...
def gstr1_data(CompanyID, BranchID, FromDate, ToDate, PriceRounding):
                ...
                for s in sales_gsts:
                    if not s in unq_sales_gsts and converted_float(GrandTotal) < 250000:
                        ...
                        if s > 0:
                            ...
                            print(Total_taxable_value_b2cs,
                                  "1Total_taxable_value_b2csYYYYYYYY@@@@@@")
    ...
    if SalesReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, VoucherDate__gte=FromDate, VoucherDate__lte=ToDate, TaxType__in=tax_types).exists():
        ...
        for i in jsnDatas:
            ...
            print(SalesMasterID, "2##########YYYYYYYYYYY", i['VoucherNo'])
...
def billWise_report_data(CompanyID,PriceRounding,WareHouseID,RouteID,UserID,CustomerID,BranchID,FromDate,ToDate):
...
def sales_list_data(CompanyID,BranchID,PriceRounding):
    cursor = connection.cursor()
    dic = {'BranchID': BranchID, 'CompanyID':CompanyID,'PriceRounding':PriceRounding}
    ...
    cursor.execute('''
            ...
            AND bw."VoucherType" = 'SI' AND bw."Payments" > 0) > 0 THEN true else false END AS is_billwised
            
            FROM public."salesMasters_salesMaster" AS SM  
            WHERE SM."BranchID" = %(BranchID)s AND SM."CompanyID_id" = %(CompanyID)s  ORDER BY "SalesMasterID" DESC
            ''',dic)
    ...
    heads = [                
        ...
        str(_('GrandTotal_Rounded')), str(_('CustomerName')), str(_('TotalTax')), str(_('Status')), str(_('GrandTotal')), str(_('is_billwised'))]
    df.columns = heads
...
def get_CashAmount(account_group,AllowCashReceiptMoreSaleAmt,CashReceived,BankAmount,GrandTotal):
    ...
    if AllowCashReceiptMoreSaleAmt == True and account_group != 8 and account_group != 9:
        ...
        CashAmount = 0
    elif BankAmount == 0 and CashReceived <= GrandTotal:
        CashAmount = CashReceived

    elif (CashReceived + BankAmount) == GrandTotal:
        CashAmount = CashReceived
    elif (CashReceived + BankAmount) >= GrandTotal:
        CashAmount = GrandTotal-BankAmount

    elif BankAmount < GrandTotal and CashReceived == 0:
        PostingCashAmount = GrandTotal-BankAmount
