...
from django.db.models import Max, Prefetch, Q, Sum,F
...
from api.v9.accountLedgers.functions import get_auto_id as generated_ledgerID
from api.v9.accountLedgers.functions import get_auto_LedgerPostid, get_LedgerCode
from api.v9.accountLedgers.serializers import AccountLedgerListSerializer
from api.v9.brands.serializers import ListSerializer
from api.v9.companySettings.serializers import (
    CompanySettingsPrintRestSerializer,
    CompanySettingsRestSerializer,
    QRCompanySettingsRestSerializer,
)
from api.v9.ledgerPosting.functions import convertOrderdDict
from api.v9.loyaltyProgram.functions import get_point_auto_id
from api.v9.parties.serializers import PartyImageSerializer
from api.v9.products.functions import get_auto_AutoBatchCode, update_stock
from api.v9.purchaseOrders.serializers import PurchaseOrderPrintSerializer
from api.v9.purchaseReturns.serializers import (
    PurchaseReturnMasterRestSerializer,
    PurchaseReturnPrintSerializer,
)
from api.v9.purchases.functions import get_auto_StockRateID, get_auto_StockTransID
from api.v9.purchases.serializers import (
    PurchaseMasterRestSerializer,
    PurchasePrintSerializer,
    PurchaseHistorySerializer
)
from api.v9.reportQuerys.functions import (
    billwise_profit_report_data,
    query_Expense_summary,
    query_export_to_excel_sales,
...
from api.v9.sales.functions import (
    SetBatchInSales,
    at_excel,
    ata_excel,
    atadj_excel,
    ...
    export_to_excel_dailyreport,
    export_to_excel_sales,
    export_to_excel_sales_gst,
    export_to_excel_sales_taxgroup,
    export_to_excel_salesReturn,
    export_to_excel_taxSummary,
    ...
    hsn_excel,
    master_excel,
    sales_dailyReport_excel_data,
    sales_dailyReport_excel_data_top_items,
    sales_excel_data,
    sales_gst_excel_data,
    sales_list_data,
    sales_taxgroup_excel_data,
    salesReturn_excel_data,
    set_LoyaltyCalculation,
    taxSummary_excel_data,
    taxSummary_InwardSupplies,
    taxSummary_OutwardSupplies,
    vat_taxSummary_InwardSupplies,
    vat_taxSummary_OutwardSupplies,
    vat_zeroRated_taxSummary_InwardSupplies,
    vat_zeroRated_taxSummary_OutwardSupplies,
    zeroRated_taxSummary_InwardSupplies,
    zeroRated_taxSummary_OutwardSupplies,
)
from api.v9.sales.serializers import (
    BatchSerializer,
    BillWiseSerializer,
    FilterEstimateSerializer,
    FilterOrderSerializer,
    GST_B2B_Serializer,
    GST_CDNR_Serializer,
    ListSerializerforReport,
    ListSerializerforSummaryReport,
    ProductVsSuppliersReportSerializer,
    SalesDetailsRestSerializer,
    SalesDetailsSerializer,
    SalesEstimateForOrderSerializer,
    SalesGSTReportSerializer,
    SalesHistorySerializer,
    SalesIntegratedSerializer,
    SalesMasterForReturnSerializer,
    SalesMasterReportSerializer,
    SalesMasterRest1Serializer,
    SalesMasterRestSerializer,
    SalesMasterSerializer,
    SalesMobileReportSerializer,
    SalesReportNewSerializer,
    SalesSingleSerializer,
    SalesSummaryReportSerializer,
    StockRateSerializer,
    StockSerializer,
    SupplierVsProductReportSerializer,
)
from api.v9.sales.tasks import taskAPIfor_gstr1_report
from api.v9.salesApp.serializers import ExpenseListSerializer, PaymentListSerializer, ReceiptListSerializer
from api.v9.salesEstimates.serializers import (
    SalesEstimateDetailsRestSerializer,
    SalesEstimateMasterRestSerializer,
)
from api.v9.salesOrders.serializers import (
    SalesOrderDetailsRestSerializer,
    SalesOrderMasterRestSerializer,
    SalesOrderPrintSerializer,
)
from api.v9.salesReturns.serializers import (
    SalesReturnMasterReportSerializer,
    SalesReturnMasterRestSerializer,
)
from api.v9.settings.serializers import QRPrintSettingsSerializer
from api.v9.stockPostings.serializers import (
    DamageStockMasterSerializer,
    ExcessStockMasterSerializer,
    ShortageStockMasterSerializer,
    StockPostingRestSerializer,
    StockPostingSerializer,
    UsedStockDetailSerializer,
    UsedStockMasterSerializer,
)
from api.v9.stockTransfers.serializers import StockTransferMaster_IDRestSerializer
...
from main.functions import (
    activity_log,
    converted_float,
    extract_alphabet_from_string,
    generateVoucherNoFunction,
    get_BalanceFromLedgerPost,
    ...
    try:
        with transaction.atomic():
            ...
            try:
                ...
                if table.VoucherNoGenerateTable.objects.filter(
                    CompanyID=CompanyID,
                    VoucherType=VoucherType,
                    LastInvoiceNo__isnull=False,
                ).exists():
                    PreFix = (
                        table.VoucherNoGenerateTable.objects.filter(
                            CompanyID=CompanyID, VoucherType=VoucherType
                        )
                        .exclude(LastInvoiceNo__isnull=True)
                        .first()
                        .PreFix
                    )
                else:
            ...
            if ShippingAddress:
                if UserAdrress.objects.filter(id=ShippingAddress).exists():
                    ShippingAddress = UserAdrress.objects.get(
                        id=ShippingAddress)
            if BillingAddress:
            ...
            if is_except == False:
                if not GeneralSettings.objects.filter(
                    CompanyID=CompanyID, SettingsType="ShowTotalTax"
                ).exists():
            ...
            if (
                is_voucherExist == True
                and check_VoucherNoAutoGenerate == "True"
                or check_VoucherNoAutoGenerate == True
            ):
            ...
            if is_SaleOK:
                ...
                #     or AllowCashReceiptMoreSaleAmt == "true"
                # ):
                ...
                CashAmount,PostingCashAmount = get_CashAmount(account_group,AllowCashReceiptMoreSaleAmt,CashReceived,BankAmount,GrandTotal)
                ...
                # ShippingAddress = None
                # if UserAdrress.objects.filter(CompanyID=CompanyID,id=ShippingAddress).exists():
                #     ShippingAddress = UserAdrress.objects.get(CompanyID=CompanyID,id=ShippingAddress)
                ...
                try:
                    url = (
                        ...
                        + "/invoice/"
                        + str(sales_instance.pk)
                        + str("/")
                        + str("SI")
                    )
                except:
                    url = str("https://www.api.vikncodes.in/invoice/") + str(sales_instance.pk)+str('/')+ str('SI')
                ...
                print(AccountLedger.objects.filter(CompanyID=CompanyID,
                      LedgerID=LedgerID, AccountGroupUnder__in=[10, 29]))
                ...
                if is_LoyaltyCustomer:
                    ...
                    try:
                        ...
                        RadeemPoint = None
                    print(is_LoyaltyCustomer,"<<<<<<<<<<<<<<<<<<<is_LoyaltyCustomer>>>>>>>>>>>>>>>>>>>>>")
                    print(Loyalty_Point_Expire,"<<<<<<<<<<<<<<<<<<<is_LoyaltyCustomer>>>>>>>>>>>>>>>>>>>>>")
                    print(RadeemPoint,"<<<<<<<<<<<<<<<<<<<is_LoyaltyCustomer>>>>>>>>>>>>>>>>>>>>>")
                ...
                if SalesDetails:
                    for salesdetail in salesdetails:
                        ...
                        if ProductID:
                            ...
                            check_SalesPriceUpdate = get_GeneralSettings(CompanyID, BranchID, "SalesPriceUpdate")
                            if check_SalesPriceUpdate:
                                is_pricecategory = get_general_settings(
                                        BranchID, CompanyID, "PriceCategory"
                                    )
                                if is_pricecategory == True:
                                    price_list_ins = PriceList.objects.get(
                                        CompanyID=CompanyID, PriceListID=PriceListID
                                    )
                                    if PriceCategoryID == 2:
                                        price_list_ins.SalesPrice1 = Batch_salesPrice
                                    elif PriceCategoryID == 3:
                                        price_list_ins.SalesPrice2 = Batch_salesPrice
                                    elif PriceCategoryID == 4:
                                        price_list_ins.SalesPrice3 = Batch_salesPrice
                                    else:
                                        price_list_ins.SalesPrice=Batch_salesPrice
                                    price_list_ins.save()
...
def edit_sales(request, pk):
    try:
        with transaction.atomic():
            ...
            loyalty_customer = None
            if LoyaltyCustomerID:
                if LoyaltyCustomer.objects.filter(CompanyID=CompanyID,LoyaltyCustomerID=LoyaltyCustomerID).exists():
                    loyalty_customer = LoyaltyCustomer.objects.get(
                        CompanyID=CompanyID, LoyaltyCustomerID=LoyaltyCustomerID)
                    is_LoyaltyCustomer = True
            ...
            if enable_billwise and converted_float(total_amount_received) <= converted_float(GrandTotal) and AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, AccountGroupUnder__in=[10, 29]).exists():
                if BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo, VoucherType=VoucherType,CustomerID=LedgerID).exists():
                    billwise_ins = BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo, VoucherType=VoucherType,CustomerID=LedgerID).first()
                    ...
                    billwise_ins = BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo, VoucherType=VoucherType).first()
                    billwise_ins.VoucherDate = Date
                    billwise_ins.InvoiceAmount = GrandTotal
                    billwise_ins.Payments = total_amount_received
            ...
            #             Batch_ins.StockOut = converted_float(Batch_ins.StockOut) - converted_float(Qty)
            #             Batch_ins.save()
            ...
            if is_LoyaltyCustomer:
                ...
                try:
                    ...
                    RadeemPoint = None
                print(RadeemPoint,"URAPPAA KAYARUNNUND-=============")
                edit_LoyaltyCalculation(
                    ...
                    if PriceList.objects.filter(
                        ...
                        if not deleted_pk == "" or not deleted_pk == 0:
                            if SalesDetails.objects.filter(
                                ...
                                deleted_detail = SalesDetails.objects.get(
                                    CompanyID=CompanyID, pk=deleted_pk
                                )
...
def salesInvoice_for_SalesReturn(request):
    ...
    if SalesMaster.objects.filter(
        ...
        serialized = SalesMasterForReturnSerializer(
...
def salesInvoice_for_SalesEstimate(request):
    ...
    if SalesEstimateMaster.objects.filter(
        ...
        serialized = SalesEstimateForOrderSerializer(
            instance, context={"CompanyID": CompanyID,
                               "PriceRounding": PriceRounding}
        )
...
def delete_salesMaster(request, pk):
    ...
    instances = None
    if selecte_ids:
        if SalesMaster.objects.filter(CompanyID=CompanyID, pk__in=selecte_ids).exists():
            instances = SalesMaster.objects.filter(pk__in=selecte_ids)
    else:
        if SalesMaster.objects.filter(CompanyID=CompanyID, pk=pk).exists():
            instances = SalesMaster.objects.filter(pk=pk)
    ...
    if instances:
        for instance in instances:
            ...
            Action = "D"
            if BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo,TransactionID=SalesMasterID,VoucherType="SI",Payments__gt=0).exists():
                response_data = {
                    "StatusCode": 6000,
                    "title": "Failed",
                ...
                if BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo,TransactionID=SalesMasterID,VoucherType="SI").exists():
                    BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo,TransactionID=SalesMasterID,VoucherType="SI").delete()
                SalesMaster_Log.objects.create(
                    TransactionID=SalesMasterID,
                    LoyaltyCustomerID=LoyaltyCustomerID,
                    BranchID=BranchID,
    ...
    if serialized1.is_valid():
        ...
        if UserID:
            ...
            if SalesMaster.objects.filter(
                ...
                serialized_sales = SalesGSTReportSerializer(
                    instances,
                    many=True,
                    context={
                        "CompanyID": CompanyID,
                ...
                serialized_return = SalesReturnMasterRestSerializer(
                    return_instances,
                    many=True,
                    context={"CompanyID": CompanyID,
                             "PriceRounding": PriceRounding},
                ...
                for i in return_jsnDatas:
                    ...
                    TotalTaxableAmount = converted_float(
                ...
                for i in final_array:
                    Total_TotalTaxableAmount += converted_float(
                        i["TotalTaxableAmount"])
            ...
            return Response(response_data, status=status.HTTP_200_OK)
        elif SalesMaster.objects.filter(
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def sales_mobile_report(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    UserID = data["UserID"]
    ...
    if serialized1.is_valid():
        ...
        if SalesMaster.objects.filter(
            ...
            serialized_sales = SalesMobileReportSerializer(
                sale_sort_pagination,
                many=True,
                context={
                    "CompanyID": CompanyID,
        ...
        if SalesReturnMaster.objects.filter(
            ...
            count_sales_return = instances_salesReturn.count()
            serialized_salesReturn = SalesReturnMasterReportSerializer(
                instances_salesReturn,
                many=True,
                context={
                    "CompanyID": CompanyID,
                    "PriceRounding": PriceRounding,
        ...
        if sales_data or salesReturn_data:
            count_divided_sales = math.ceil(converted_float(count_sales) / 10)
            count_divided_sales_return = math.ceil(
                converted_float(count_sales_return) / 10)
            length = 0
            if instances:
            ...
            response_data = {
                ...
                "Total_billDiscount_sale": round(
                    Total_billDiscount_sale, PriceRounding
                ),
                "Total_billDiscount_saleRetn": round(
                    Total_billDiscount_saleRetn, PriceRounding
                ),
                "Total_grandTotal_sale": round(Total_grandTotal_sale, PriceRounding),
                "Total_grandTotal_saleRetn": round(
                    Total_grandTotal_saleRetn, PriceRounding
                ),
                ...
                "Total_creditSalesReturn": round(
                    Total_creditSalesReturn, PriceRounding
                ),
            }
            return Response(response_data, status=status.HTTP_200_OK)
        else:
...
def old_sales_report(request):
    ...
    if serialized1.is_valid():
        ...
        if SalesMaster.objects.filter(
            ...
            serialized_sales = SalesReportNewSerializer(
                sale_sort_pagination,
                many=True,
                context={
                    "CompanyID": CompanyID,
            ...
            sales_data = serialized_sales.data
            tot_json = []
            count_divided_sales = math.ceil(
                converted_float(count_sales) / items_per_page)
            if count_divided_sales == page_number:
                # Taiking Query Total
                data = query_sales_report_data(
                    data["CompanyID"],
                    BranchID,
                    FromDate,
                    ToDate,
                ...
                sales_total = {
                    "LedgerName": "Total",
                    "CashSales": str(tot_json[9]["0"]),
                    "BankSales": str(tot_json[10]["0"]),
                    "CreditSales": str(tot_json[11]["0"]),
                    "TotalGrossAmt": str(tot_json[5]["0"]),
                    "TotalTax": str(tot_json[6]["0"]),
                    "BillDiscAmt": str(tot_json[7]["0"]),
                    "GrandTotal": str(tot_json[8]["0"]),
                }
    ...
    if serialized1.is_valid():
        ...
        if filter_type == "sales":
            df,details = query_sales_report_data(
                data["CompanyID"], BranchID, FromDate, ToDate, PriceRounding, UserID, EmployeeID,CustomerID,filterValue
            )
...
def sales_return_report(request):
    ...
    if serialized1.is_valid():
        ...
        if SalesReturnMaster.objects.filter(
            ...
            df,details = query_salesReturn_report_data(
                data["CompanyID"], BranchID, FromDate, ToDate, PriceRounding, UserID, EmployeeID,CustomerID
            )
            response_data = {
                "StatusCode": 6000,
                "new_salesReturn_data": details,
            }
            return Response(response_data, status=status.HTTP_200_OK)
        else:
            response_data = {
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def old_sales_return_report(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    UserID = data["UserID"]
    ...
    if serialized1.is_valid():
        ...
        if SalesReturnMaster.objects.filter(
            ...
            sale_return_sort_pagination = list_pagination(
                instances_salesReturn, items_per_page, page_number
            )
            serialized_salesReturn = SalesReturnMasterReportSerializer(
        ...
        if salesReturn_data:
            ...
            if salesReturn_data:
                count_divided_sales_return = math.ceil(
                    converted_float(count_sales_return) / items_per_page
                )
            ...
            if count_divided_sales_return == page_number:
    ...
    if serialized1.is_valid():
        ...
        if LedgerID:
            ...
            if SalesMaster.objects.filter(
                ...
                serialized_sales = SalesMasterReportSerializer(
                    instances,
                    many=True,
                    context={
                        "CompanyID": CompanyID,
...
def sales_integrated_report(request):
    ...
    if serialized1.is_valid():
        ...
        df, details = query_sales_integrated_report_data(data["CompanyID"], BranchID, FromDate, ToDate, UserID)
        response_data = {
            "StatusCode": 6000,
            "new_data": details,
    ...
    if VoucherType == "SI":
        ...
        if SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            ...
            if not OldVoucherNo.isdecimal():
                ...
                # new_VoucherNo = converted_float(OldVoucherNo) + 1
        ...
        if PurchaseOrderMaster.objects.filter(
            ...
            response_data = {"StatusCode": 6000, "VoucherNo": "PO1"}
        ...
        if OpeningStockMaster.objects.filter(
            ...
            response_data = {"StatusCode": 6000,
                             "VoucherNo": "OS1", "new_num": 1}

            return Response(response_data, status=status.HTTP_200_OK)

    elif VoucherType == "JL":
        if JournalMaster.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID
        ).exists():
            ...
            response_data = {"StatusCode": 6000,
                             "VoucherNo": "JL1", "new_num": 1}

            return Response(response_data, status=status.HTTP_200_OK)

    elif VoucherType == "CP":
        if PaymentMaster.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID, VoucherType="CP"
        ).exists():
            ...
            response_data = {"StatusCode": 6000,
                             "VoucherNo": "CP1", "new_num": 1}

            return Response(response_data, status=status.HTTP_200_OK)

    elif VoucherType == "BP":
        if PaymentMaster.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID, VoucherType="BP"
        ).exists():
            instance = PaymentMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, VoucherType="BP"
            ).first()
            ...
            response_data = {"StatusCode": 6000,
                             "VoucherNo": "BP1", "new_num": 1}

            return Response(response_data, status=status.HTTP_200_OK)

    elif VoucherType == "CR":
        if ReceiptMaster.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID, VoucherType="CR"
        ).exists():
            ...
            response_data = {"StatusCode": 6000,
                             "VoucherNo": "CR1", "new_num": 1}

            return Response(response_data, status=status.HTTP_200_OK)

    elif VoucherType == "BR":
        if ReceiptMaster.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID, VoucherType="BR"
        ).exists():
            instance = ReceiptMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, VoucherType="BR"
            ).first()
        ...
        if AccountGroup.objects.filter(CompanyID=CompanyID).exists():
            ...
            if not OldGroupCode.isdecimal():
                ...
                prefix = extract_alphabet_from_string(OldGroupCode)
                num = res.group(0)
                new_num = int(num) + 1
                new_GroupCode = prefix + str(new_num)
            else:
                # new_GroupCode = converted_float(OldGroupCode) + 1
                new_num = int(OldGroupCode) + 1
            ...
            response_data = {"StatusCode": 6000,
                             "GroupCode": "GP1", "new_num": 1}

            return Response(response_data, status=status.HTTP_200_OK)

    elif VoucherType == "ES":
        if ExcessStockMaster.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID
        ).exists():
            ...
            response_data = {"StatusCode": 6000,
                             "VoucherNo": "ES1", "new_num": 1}

            return Response(response_data, status=status.HTTP_200_OK)
    elif VoucherType == "SS":
        if ShortageStockMaster.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID
        ).exists():
            ...
            response_data = {"StatusCode": 6000,
                             "VoucherNo": "SS1", "new_num": 1}

            return Response(response_data, status=status.HTTP_200_OK)

    elif VoucherType == "DS":
        if DamageStockMaster.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID
        ).exists():
            ...
            response_data = {"StatusCode": 6000,
                             "VoucherNo": "DS1", "new_num": 1}

            return Response(response_data, status=status.HTTP_200_OK)
    elif VoucherType == "US":
        if UsedStockMaster.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID
        ).exists():
        ...
        response_data = {"StatusCode": 6000, "ProductCode": ProductCode}
...
def generate_printReport(request):
    from api.v9.sales.serializers import SalesPrintSerializer
    from api.v9.salesReturns.serializers import SalesReturnMasterPrintSerializer
    ...
    company_serialized = CompanySettingsRestSerializer(
        company_instance, context={"request": request,"CreatedUserID":CreatedUserID}
    )
    if VoucherType == "SI":
        if SalesMaster.objects.filter(CompanyID=CompanyID, id=unq_id).exists():
            ...
            serialized = SalesPrintSerializer(
                instance,
                context={
                    "CompanyID": CompanyID,
                    "request": request,
        ...
        if PurchaseMaster.objects.filter(CompanyID=CompanyID, id=unq_id).exists():
            ...
            serialized = PurchasePrintSerializer(
                instance,
                context={"CompanyID": CompanyID,
                         "PriceRounding": PriceRounding},
            )
        ...
        if SalesReturnMaster.objects.filter(CompanyID=CompanyID, id=unq_id).exists():
            ...
            serialized = SalesReturnMasterPrintSerializer(
                instance,
                context={"CompanyID": CompanyID,
                         "PriceRounding": PriceRounding},
            )
        ...
        if PurchaseReturnMaster.objects.filter(CompanyID=CompanyID, id=unq_id).exists():
            ...
            serialized = PurchaseReturnPrintSerializer(
                instance,
                context={"CompanyID": CompanyID,
                         "PriceRounding": PriceRounding},
            )
        ...
        if PurchaseOrderMaster.objects.filter(CompanyID=CompanyID, id=unq_id).exists():
            ...
            serialized = PurchaseOrderPrintSerializer(
                instance,
                context={"CompanyID": CompanyID,
                         "PriceRounding": PriceRounding},
            )
        ...
        if SalesOrderMaster.objects.filter(CompanyID=CompanyID, id=unq_id).exists():
            instance = SalesOrderMaster.objects.get(
                CompanyID=CompanyID, id=unq_id)

            serialized = SalesOrderPrintSerializer(
                instance,
                context={"CompanyID": CompanyID,
                         "PriceRounding": PriceRounding},
            )
        ...
        if PaymentDetails.objects.filter(CompanyID=CompanyID, id=unq_id).exists():
            instance = PaymentDetails.objects.get(
                CompanyID=CompanyID, id=unq_id)

            serialized = PaymentListSerializer(
                instance,
        ...
        if ReceiptDetails.objects.filter(CompanyID=CompanyID, id=unq_id).exists():
            instance = ReceiptDetails.objects.get(
                CompanyID=CompanyID, id=unq_id)

            serialized = ReceiptListSerializer(
                instance,
                context={"request": request, "CompanyID": CompanyID,
                         "PriceRounding": PriceRounding},
            )
            ...
            return Response(response_data, status=status.HTTP_200_OK)
    elif VoucherType == "EXP":
        if ExpenseDetails.objects.filter(CompanyID=CompanyID, id=unq_id).exists():
            instance = ExpenseDetails.objects.get(
                CompanyID=CompanyID, id=unq_id)
        ...
        if StockTransferMaster_ID.objects.filter(CompanyID=CompanyID, id=unq_id).exists():
...
def vat_report(request):
    ...
    if serialized1.is_valid():
        ...
        if SalesMaster.objects.filter(
            ...
            for i_sale in sales_instances:
                sale_details = SalesDetails.objects.filter(
                    SalesMasterID=i_sale.SalesMasterID,
                    CompanyID=CompanyID,
                    BranchID=BranchID,
                )
                ...
                LedgerName_sale = AccountLedger.objects.get(
                    CompanyID=CompanyID, BranchID=BranchID, LedgerID=i_sale.LedgerID
                ).LedgerName
        ...
        if SalesReturnMaster.objects.filter(
            ...
            for i_saleRetn in salesRetrn_instances:
                saleRetn_details = SalesReturnDetails.objects.filter(
                    SalesReturnMasterID=i_saleRetn.SalesReturnMasterID,
                    CompanyID=CompanyID,
                    BranchID=BranchID,
                )
                ...
                LedgerName_sale = AccountLedger.objects.get(
                    CompanyID=CompanyID, BranchID=BranchID, LedgerID=i_saleRetn.LedgerID
                ).LedgerName
        ...
        if PurchaseMaster.objects.filter(
            ...
            for i_purchase in purchase_instances:
                purchase_details = PurchaseDetails.objects.filter(
                    PurchaseMasterID=i_purchase.PurchaseMasterID,
                    CompanyID=CompanyID,
                    BranchID=BranchID,
                )
                ...
                LedgerName_purchase = AccountLedger.objects.get(
                    CompanyID=CompanyID, BranchID=BranchID, LedgerID=i_purchase.LedgerID
                ).LedgerName
        ...
        if PurchaseReturnMaster.objects.filter(
            ...
            for i_purchaseRetn in purchaseRetn_instances:
                purchase_details = PurchaseReturnDetails.objects.filter(
                    PurchaseReturnMasterID=i_purchaseRetn.PurchaseReturnMasterID,
                    CompanyID=CompanyID,
                    BranchID=BranchID,
                )
                total_taxable_purchase = 0
                ...
                LedgerName_purchaseRetn = AccountLedger.objects.get(
                    CompanyID=CompanyID,
                    BranchID=BranchID,
                    LedgerID=i_purchaseRetn.LedgerID,
                ).LedgerName
        ...
        total_received_taxbleAmt = converted_float(Total_taxbleAmt_sale) + converted_float(
            Total_taxbleAmt_purchaseRetrn
        )
        total_received_taxAmt = converted_float(Total_taxAmt_sale) + converted_float(
            Total_taxAmt_purchaseRetrn
        )
        total_received_netAmt = converted_float(Total_netAmt_sale) + converted_float(
            Total_netAmt_purchaseRetrn
        )

        total_paid_taxbleAmt = converted_float(Total_taxbleAmt_saleRetrn) + converted_float(
            Total_taxbleAmt_purchase
        )
        total_paid_taxAmt = converted_float(
            Total_taxAmt_saleRetrn) + converted_float(Total_taxAmt_purchase)
        total_paid_netAmt = converted_float(
            Total_netAmt_saleRetrn) + converted_float(Total_netAmt_purchase)

        grandTotal_received = total_received_taxAmt
        grandTotal_paid = total_paid_taxAmt

        total_payable = converted_float(
            grandTotal_received) - converted_float(total_paid_taxAmt)
        ...
        if (
            sale_data_cons
            or saleRetrn_data_cons
            or purchase_data_cons
            or purchaseRetrn_data_cons
        ):
            response_data = {
                "StatusCode": 6000,
            ...
            response_data = {"StatusCode": 6001, "message": "Data Not Found!"}
...
def stock_report(request):
    ...
    if serialized1.is_valid():
        ...
        if stockPosting_instances:
            df, data = query_stock_report_data(
                data["CompanyID"],
                BranchID,
                FromDate,
                ToDate,
    ...
    if serialized1.is_valid():
        ...
        if stockPosting_instances:
            ...
            qurried_instances = (
                ...
                .annotate(in_stock_quantity=Sum("QtyIn") - Sum("QtyOut"))
                .order_by("ProductID")
            )
            for q in qurried_instances:
                ...
                check_stock_dic = {"ProductID": pro, "TotalStock": TotalStock}
                ...
                if StockFilter == 1:
                    ...
                    StockReOrder = Product.objects.get(
                        ProductID=pro, CompanyID=CompanyID
                    ).StockReOrder
            ...
            if page_number and items_per_page:
                product_instances = Product.objects.filter(
                    CompanyID=CompanyID, ProductID__in=final_pro_arry
                )
            ...
            # tot_SalesPrice = product_instances.aggregate(Sum('SalesPrice'))[
            #     'SalesPrice__sum']
            if jsnDatas:
                counted_datas = product_instances.count()
                ...
                if count_divided_datas == page_number:
                    ...
                    json_records = total.reset_index().to_json(orient="records")
                    tot_json = json.loads(json_records)
                    print(str(tot_json[3]["0"]))

                    tot_CurrentStock = tot_json[3]["0"]
                    tot_PurchasePrice = tot_json[5]["0"]
                    tot_SalesPrice = tot_json[6]["0"]
                    total_dic = {
                        "ProductName": "Total",
...
def stock_ledger_report(request):
    ...
    if serialized1.is_valid():
        ...
        if ProductID > 0 and WareHouseID > 0:
            ...
            stockPosting_instances = stockPosting_instances.filter(
                WareHouseID=WareHouseID)
        if stockPosting_instances.exists():
            # stockPosting_instances = StockPosting.objects.filter(
            #     CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID
            # ).order_by("Date", "StockPostingID")
            # if WareHouseID > 0:
            #     stockPosting_instances = stockPosting_instances.filter(
            #         WareHouseID=WareHouseID
            #     ).order_by("Date", "StockPostingID")
            ...
            # stockSerializer = StockPostingRestSerializer(
            #     stockPosting_instances,
            #     many=True,
            ...
            #     Balance = (converted_float(Balance) + converted_float(QtyIn)) - converted_float(QtyOut)

            #     i["Balance"] = round(Balance, PriceRounding)
            #     QtyIn = converted_float(QtyIn)
            #     QtyOut = converted_float(QtyOut)
            ...
            #         "UnitName": UnitName,
            #         "UserName": UserName,
            #         "QtyIn": round(QtyIn, PriceRounding),
            #         "QtyOut": round(QtyOut, PriceRounding),
            #         "Balance": round(Balance, PriceRounding),
            ...
            # Query Data Start heare
            df, details = query_stockLedger_report_data(
                data["CompanyID"], BranchID, FromDate, ToDate, PriceRounding, int(ProductID), int(WareHouseID))
            ...
            response_data = {"StatusCode": 6001, "message": msg}
...
def stock_report_print(request):
    ...
    if FromVal and ToVal:
        FromVal = converted_float(FromVal)
        ToVal = converted_float(ToVal) + 1
    ...
    if serialized1.is_valid():
        ...
        if stockPosting_instances:
            ...
            for pro in product_arry:
                ...
                TotalStock = converted_float(
                    tot_QtyIn) - converted_float(tot_QtyOut)
                ...
                if StockFilter == 1:
                    ...
                    StockReOrder = Product.objects.get(
                        ProductID=pro, BranchID=BranchID, CompanyID=CompanyID
                    ).StockReOrder
                    if TotalStock < StockReOrder:
                        final_pro_arry.append(pro)
                elif StockFilter == 6:
...
def batchwise_report(request):
    ...
    if Batch.objects.filter(
        ...
        response_data = {"StatusCode": 6000, "data": batchSerializer.data}
        return Response(response_data, status=status.HTTP_200_OK)
    else:
        # request , company, log_type, user, source, action, message, description
...
def stockValue_report(request):
    ...
    if StockPosting.objects.filter(
        ...
        # if page_number and items_per_page:
        #     product_instances = Product.objects.filter(
        #         CompanyID=CompanyID, ProductID__in=product_arry
        #     )
        #     tot_counted_data = product_instances.count()
        #     product_sort_pagination = list_pagination(
        #         product_instances, items_per_page, page_number
        #     )
        ...
        #         new_final_array.append(items)

        #         # TotalQty += float(i['Qty'])
        #         # GrandTotalCost += TotalCost

        # count_divided_datas = math.ceil(
        #     float(tot_counted_data) / items_per_page
        # )
        # TotalQty = 0
        # GrandTotalCost = 0
        # TotalCost = 0
        ...
        #     TotalQty += int(i["Qty"])
        #     GrandTotalCost += int(i["TotalCost"])
        #     TotalCost += int(i["Cost"])
        # # TotalQty = product_instances.aggregate(Sum('Qty'))['Qty__sum']
        # # GrandTotalCost = product_instances.aggregate(Sum('TotalCost'))['TotalCost__sum']
        # # TotalCost = product_instances.aggregate(Sum('Cost'))['Cost__sum']
        ...
        #             }
        #             count = count + 1
        #             final_array_tot.append(items)
        #             new_final_array_tot.append(items)
        ...
        #         "SalesPrice": "Total",
        #     }
        #     new_final_array.append(tot_items)
        df, details = query_stockValue_report_data(
            data["CompanyID"], BranchID, "", ToDate, PriceRounding, ProductID, WareHouseID)
        ...
        if SalesMaster.objects.filter(
            ...
            if wareHouseVal > 0:
                if saleMaster_instances.filter(WarehouseID=wareHouseVal).exists():
                    ...
                    activity_log(
                        ...
                        "Report",
                        "sales Register Report Viewed Failed.",
                        "Sales is not Found in this Warehouse",
                    )
                    response_data = {
                        "StatusCode": 6001,
            ...
            if userVal > "0":
                ...
                if saleMaster_instances.filter(CreatedUserID=UserID).exists():
                    ...
                    activity_log(
                        ...
                        "Report",
                        "sales Register Report Viewed Failed.",
                        "Sales is not Found by this user",
                    )
                    response_data = {
                        "StatusCode": 6001,
            ...
            if branchVal > 0:
                if saleMaster_instances.filter(BranchID=branchVal).exists():
                    ...
                    activity_log(
                        ...
                        "Report",
                        "sales Register Report Viewed Failed.",
                        "Sales is not Found under this Branch",
                    )
                    response_data = {
                        "StatusCode": 6001,
            ...
            if ledgerVal > 0:
                if saleMaster_instances.filter(LedgerID=ledgerVal).exists():
                    ...
                    activity_log(
                        ...
                        "Report",
                        "sales Register Report Viewed Failed.",
                        "Sales is not Found under this Ledger",
                    )
                    response_data = {
                        "StatusCode": 6001,
            ...
            for i in saleMaster_instances:
                ...
                if produtVal > 0:
                    ...
                    if ProductGroup.objects.filter(
                        ...
                        activity_log(
                            ...
                            "Report",
                            "sales Register Report Viewed Failed.",
                            "Sales is not Found under this Category",
                        )
                        response_data = {
                            "StatusCode": 6001,
                    ...
                    if Product.objects.filter(
                        ...
                        activity_log(
                            ...
                            "Report",
                            "sales Register Report Viewed Failed.",
                            "Sales is not Found under this Group",
                        )
                        response_data = {
                            "StatusCode": 6001,
                    ...
                    if Product.objects.filter(
                        ...
                        activity_log(
                            ...
                            "Report",
                            "sales Register Report Viewed Failed.",
                            "Sales is not Found under this ProductCode",
                        )
                        response_data = {
                            "StatusCode": 6001,
                    ...
                    if PriceList.objects.filter(
                        ...
                        activity_log(
                            ...
                            "Report",
                            "sales Register Report Viewed Failed.",
                            "Sales is not Found under this Bracode",
                        )
                        response_data = {
                            "StatusCode": 6001,
                ...
                for s in salesDetail_instances:
                    ...
                    Cost = s.CostPerPrice
                    Profit = converted_float(
                        NetAmount) - (converted_float(Qty) * converted_float(Cost))
    ...
    if serialized1.is_valid():
        ...
        if SalesMaster.objects.filter(
            ...
            df, details = billwise_profit_report_data(
                CompanyIDUid, BranchID, FromDate, ToDate, UserID, WareHouseID, CustomerID, RouteID)
            response_data = {
                "StatusCode": 6000,
                "data": details
            }
        ...
        #             return Response(response_data, status=status.HTTP_200_OK)
        #     if customerVal > 0:
        #         LedgerID = Parties.objects.get(
        #             CompanyID=CompanyID, BranchID=BranchID, PartyID=customerVal
        #         ).LedgerID
        ...
        #             CompanyID=CompanyID, BranchID=BranchID, RouteID=routeVal
        #         )
        ...
        #             saleMaster_instances, items_per_page, page_number
        #         )

        #         billWiseSerializer = BillWiseSerializer(
        #             billWise_sort_pagination,
        #             many=True,
        #             context={"CompanyID": CompanyID},
        #         )
        ...
        #     count_divided = math.ceil(converted_float(count) / items_per_page)
        #     print(count_divided, page_number)
        #     if count_divided == page_number:
        #         print("***********&(Uvais)&&^^^^")
...
def excessStock_report(request):
    ...
    if serialized1.is_valid():
        ...
        if ExcessStockMaster.objects.filter(
            ...
            if WareHouseID > 0:
                if excess_instances.filter(WarehouseID=WareHouseID).exists():
                    ...
                    activity_log(
                        ...
                        "Excess Stock Report",
                        "Report",
                        "Excess Stock Report Viewed Successfully.",
                        "Excess Stock Report Viewed Successfully",
                    )
                    response_data = {
                        "StatusCode": 6001,
            ...
            serialized = ExcessStockMasterSerializer(
                excess_instances,
                many=True,
                context={"CompanyID": CompanyID,
                         "PriceRounding": PriceRounding},
            ...
            activity_log(
                ...
                "Excess Stock Report",
                "Report",
                "Excess Stock Report Viewed Failed.",
                "Excess Stock is not Found in this Warehouse",
            )
            ...
            activity_log(
                ...
                "Report",
                "Excess Stock Report Viewed Failed.",
                "No data During This Time Periods",
            )
            response_data = {
                "StatusCode": 6001,
                "message": "No data During This Time Periods!!!",
...
def shortageStock_report(request):
    ...
    if serialized1.is_valid():
        ...
        if ShortageStockMaster.objects.filter(
            ...
            if WareHouseID > 0:
                if shortage_instances.filter(WarehouseID=WareHouseID).exists():
                    ...
                    activity_log(
                        ...
                        "Report",
                        "Shortage Stock Report Viewed Successfully.",
                        "Shortage Stock Report Viewed Successfully",
                    )
                    response_data = {
                        "StatusCode": 6001,
            ...
            serialized = ShortageStockMasterSerializer(
                shortage_instances,
                many=True,
                context={"CompanyID": CompanyID,
                         "PriceRounding": PriceRounding},
            ...
            activity_log(
                ...
                "Shortage Stock Report",
                "Report",
                "Shortage Stock Report Viewed Failed.",
                "Shortage Stock is not Found in this Warehouse",
            )
...
def damageStock_report(request):
    ...
    if serialized1.is_valid():
        ...
        if DamageStockMaster.objects.filter(
            ...
            if WareHouseID > 0:
                if damage_instances.filter(WarehouseID=WareHouseID).exists():
                    ...
                    activity_log(
                        ...
                        "Damage Stock Report",
                        "Report",
                        "Damage Stock Report Viewed Successfully.",
                        "Damage Stock Report Viewed Successfully",
                    )
                    response_data = {
                        "StatusCode": 6001,
            ...
            serialized = DamageStockMasterSerializer(
                damage_instances,
                many=True,
                context={"CompanyID": CompanyID,
                         "PriceRounding": PriceRounding},
            ...
            activity_log(
                ...
                "Damage Stock Report",
                "Report",
                "Damage Stock Report Viewed Failed.",
                "Damage Stock is not Found in this Warehouse",
            )
...
def usedStock_report(request):
    ...
    if serialized1.is_valid():
        ...
        if UsedStockMaster.objects.filter(
            ...
            if WareHouseID > 0:
                if used_instances.filter(WarehouseID=WareHouseID).exists():
                    ...
                    activity_log(
                        ...
                        "Used Stock Report",
                        "Report",
                        "Used Stock Report Viewed Successfully.",
                        "Used Stock Report Viewed Successfully",
                    )
                    response_data = {
                        "StatusCode": 6001,
            ...
            serialized = UsedStockMasterSerializer(
                used_instances,
                many=True,
                context={"CompanyID": CompanyID,
                         "PriceRounding": PriceRounding},
            ...
            activity_log(
                ...
                "Used Stock Report",
                "Report",
                "Used Stock Report Viewed Failed.",
                "Used Stock is not Found in this Warehouse",
            )
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def salesEstimate_for_salesInvoice(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    CreatedUserID = data["CreatedUserID"]
    ...
    if serialized1.is_valid():
        ...
        if stockPosting_instances:
            ...
            if report_type == 11:
                ...
                for d in dates_array:
                    ...
                    for p in product_arry:
                        product_name = Product.objects.get(
                            CompanyID=CompanyID, BranchID=BranchID, ProductID=p
                        ).ProductName
                        ...
                        if with_voucher:
                            ...
                            for std in stockPost_arry:
                                ...
                                avg_out_price = 0
                                for dw in datas:
                                    totalIn += converted_float(dw.QtyIn)
                                    totalOut += converted_float(dw.QtyOut)
                                    total_rate += converted_float(dw.Rate)
                                # last_datass.append(dw)

                                if not converted_float(totalIn) == 0:
                                    avg_in_price = converted_float(
                                        total_rate) / converted_float(totalIn)
                                in_total = converted_float(
                                    avg_in_price) * converted_float(totalIn)

                                if not converted_float(totalOut) == 0:
                                    avg_out_price = converted_float(
                                        total_rate) / converted_float(totalOut)
                                out_total = converted_float(
                                    avg_out_price) * converted_float(totalOut)
                            ...
                            for c in last_datass:
                                totalIn += converted_float(c.QtyIn)
                                totalOut += converted_float(c.QtyOut)
                                total_rate += converted_float(c.Rate)
                ...
                df = (
                    df.groupby([pd.Grouper(key="Date", freq="W-SAT")])
                    .sum()
                    .reset_index()
                    .sort_values("Date")
                )
                ...
                N = 7
                date_week_group = [
                    date_weeks[n: n + N] for n in range(0, len(date_weeks), N)
                ]
...
def sales_summary_report(request):
    ...
    if serialized1.is_valid():
        ...
        if SalesMaster.objects.filter(
            ...
            #         return Response(response_data, status=status.HTTP_200_OK)
            # count = len(saleMaster_instances)
            # sale_sort_pagination = list_pagination(
            #     saleMaster_instances, items_per_page, page_number
            # )
            # serialized = SalesSummaryReportSerializer(
            #     sale_sort_pagination,
            #     many=True,
            #     context={
            #         "CompanyID": CompanyID,
...
#         if filterValue == 1:
#             if Parties.objects.filter(
#                 CompanyID=CompanyID, BranchID=BranchID, PartyID=PartyID
#             ).exists():
...
#                             "ProductID", flat=True
#                         )
#                         sales_diff_ids = list(set(sales_productids) - set(productids))
...
#                             SalesReturnMasterID__in=salesReturn_master_ids,
#                         )
#                         salesReturn_productids = (
#                             salesReturnDetail_instances.values_list(
#                                 "ProductID", flat=True
#                             )
#                         )
...
#                         LedgerID=party_LedgerID,
#                     )
#                     purchaseReturn_master_ids = (
#                         purchaseReturnMaster_instance.values_list(
...
#                     ).exists():
#                         purchaseReturnDetail_instances = (
#                             PurchaseReturnDetails.objects.filter(
#                                 CompanyID=CompanyID,
#                                 BranchID=BranchID,
#                                 PurchaseReturnMasterID__in=purchaseReturn_master_ids,
#                             )
#                         )
#                         purchaseReturn_productids = (
#                             purchaseReturnDetail_instances.values_list(
#                                 "ProductID", flat=True
#                             )
#                         )
...
#                         prodct_ins, items_per_page, page_number
#                     )
#                 serialized = SupplierVsProductReportSerializer(
#                     party_sort_pagination,
#                     many=True,
#                     context={
#                         "CompanyID": CompanyID,
#                         "PriceRounding": PriceRounding,
...
#                 count_divided = math.ceil(converted_float(count) / items_per_page)
#                 if count_divided == page_number:
#                     tot_serialized = SupplierVsProductReportSerializer(
#                         prodct_ins,
#                         many=True,
#                         context={
#                             "CompanyID": CompanyID,
...
#                         },
#                     )
#                     tot_data = tot_serialized.data
#                     tot_new_data = convertOrderdDict(tot_data)
#                     TotalLastPurchasePrice = 0
#                     TotalLastPurchaseQty = 0
#                     GrandTotalSales = 0
#                     GrandTotalSalesAmt = 0
#                     TotalAvgSalesRate = 0
#                     GrandTotalPurchase = 0
#                     GrandTotalPurchaseAmt = 0
#                     TotalAvgPurchaseRate = 0
#                     GrandTotalSalesReturn = 0
#                     GrandTotalSalesReturnAmt = 0
#                     TotalAvgSalesReturnRate = 0
#                     GrandTotalPurchaseReturn = 0
#                     GrandTotalPurchaseReturnAmt = 0
#                     TotalAvgPurchaseReturnRate = 0
#                     TotalCurrentStock = 0
#                     for i in tot_new_data:
#                         TotalLastPurchasePrice += converted_float(i["LastPurchasePrice"])
#                         TotalLastPurchaseQty += converted_float(i["LastPurchaseQty"])
#                         GrandTotalSales += converted_float(i["TotalSales"])
#                         GrandTotalSalesAmt += converted_float(i["TotalSalesAmount"])
#                         TotalAvgSalesRate += converted_float(i["AvgSalesRate"])
#                         GrandTotalPurchase += converted_float(i["TotalPurchase"])
#                         GrandTotalPurchaseAmt += converted_float(i["TotalPurchaseAmount"])
#                         TotalAvgPurchaseRate += converted_float(i["AvgPurchaseRate"])
#                         GrandTotalSalesReturn += converted_float(i["TotalSalesReturn"])
#                         GrandTotalSalesReturnAmt += converted_float(i["TotalSalesReturnAmount"])
#                         TotalAvgSalesReturnRate += converted_float(i["AvgSalesReturnRate"])
#                         GrandTotalPurchaseReturn += converted_float(i["TotalPurchaseReturn"])
#                         GrandTotalPurchaseReturnAmt += converted_float(
#                             i["TotalPurchaseReturnAmount"]
#                         )
#                         TotalAvgPurchaseReturnRate += converted_float(i["AvgPurchaseReturnRate"])
#                         TotalCurrentStock += converted_float(i["CurrentStock"])
#                     dic = {
#                         "ProductName": "Total",
#                         "LastPurchasePrice": TotalLastPurchasePrice,
#                         "LastPurchaseQty": TotalLastPurchaseQty,
#                         "TotalSales": GrandTotalSales,
#                         "TotalSalesAmount": GrandTotalSalesAmt,
#                         "AvgSalesRate": TotalAvgSalesRate,
#                         "TotalPurchase": GrandTotalPurchase,
#                         "TotalPurchaseAmount": GrandTotalPurchaseAmt,
#                         "AvgPurchaseRate": TotalAvgPurchaseRate,
#                         "TotalSalesReturn": GrandTotalSalesReturn,
#                         "TotalSalesReturnAmount": GrandTotalSalesReturnAmt,
#                         "AvgSalesReturnRate": TotalAvgSalesReturnRate,
#                         "TotalPurchaseReturn": GrandTotalPurchaseReturn,
#                         "TotalPurchaseReturnAmount": GrandTotalPurchaseReturnAmt,
#                         "AvgPurchaseReturnRate": TotalAvgPurchaseReturnRate,
...
#                     SalesMasterID__in=sales_master_ids,
#                 )
#                 sales_partyIds = sales_masterIns.values_list("LedgerID", flat=True)
...
#                     "LedgerID", flat=True
#                 )
#                 purchase_diff_ids = list(set(purchase_partyIds) - set(ledger_ids))
...
#                     "LedgerID", flat=True
#                 )
#                 salesReturn_diff_ids = list(set(salesReturn_partyIds) - set(ledger_ids))
...
#             if AccountLedger.objects.filter(
#                 CompanyID=CompanyID, LedgerID__in=ledger_ids, AccountGroupUnder=29
#             ).exists():
...
#                     )
#                     party_sort_pagination = list_pagination(
#                         party_ins, items_per_page, page_number
#                     )
#                 serialized = ProductVsSuppliersReportSerializer(
#                     party_sort_pagination,
#                     many=True,
#                     context={
#                         "CompanyID": CompanyID,
...
def supplier_vs_product_report(request):
    ...
    if serialized1.is_valid():
        ...
        df, details = supplierVsproduct_report_data(
            CompanyIDUid, BranchID, FromDate, ToDate, PartyID, ProductID, filterValue)
...
def inventory_flow_report(request):
    ...
    if serialized1.is_valid():
        ...
        if StockPosting.objects.filter(
            ...
            opening_stock = converted_float(opening_stock_qtyinSum) - converted_float(
                opening_stock_qtyoutSum
            )
            my_dict = {
        ...
        if StockPosting.objects.filter(
            ...
            stock_transfr_sum = converted_float(stock_transfer_qtyinSum) - converted_float(
                stock_transfer_qtyoutSum
            )
        ...
        if StockPosting.objects.filter(
            ...
            stock_transfer_qtyoutSum = stock_transfer_qtyoutSum["QtyOut__sum"]
            stock_adjust_sum = converted_float(stock_adjust_qtyinSum) - converted_float(
                stock_transfer_qtyoutSum
            )
        ...
        if LedgerPosting.objects.filter(
            ...
            supp_balance = converted_float(
                ledger_supp_Debit_sum) - converted_float(ledger_supp_Credit_sum)
        ...
        if LedgerPosting.objects.filter(
            ...
            cust_balance = converted_float(
                ledger_cust_Debit_sum) - converted_float(ledger_cust_Credit_sum)
        ...
        if LedgerPosting.objects.filter(
            ...
            Date__lte=ToDate,
            LedgerID__in=supplier_ids,
            VoucherType__in=Pay_voucherTypes,
        ).exists():
...
def get_order_details(request):
    ...
    if SalesOrderMaster.objects.filter(
        ...
        # LedgerID = sales_order_instances.LedgerID
        # LedgerName = ""
        # if AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID=LedgerID).exists():
        #     LedgerName = AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID=LedgerID).first().LedgerName
        # LegerList = [{
        #     "LedgerID": LedgerID,
        #     "LedgerName": LedgerName,
        # }]
        ledger_ids = sales_order_instances.values_list(
            "LedgerID", flat=True)
        LedgerList = []
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID__in=ledger_ids).exists():
            ledger_ins = AccountLedger.objects.filter(
                CompanyID=CompanyID, LedgerID__in=ledger_ids)
            for l in ledger_ins:
                LedgerID = l.LedgerID
                LedgerName = l.LedgerName
                Balance = get_BalanceFromLedgerPost(
                    CompanyID, LedgerID, BranchID)
                GST_Treatment = "2"
                VAT_Treatment = "1"
                if Parties.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID).exists():
                    party_ins = Parties.objects.filter(
                        CompanyID=CompanyID, LedgerID=LedgerID).first()
                    GST_Treatment = party_ins.GST_Treatment
                    VAT_Treatment = party_ins.VAT_Treatment
                LedgerList.append({
                    "LedgerID": LedgerID,
                    "LedgerName": LedgerName,
                    "Balance": Balance,
        ...
        for i in jsnDatas:
            ...
            if not ProductID in pro_ids:
                ...
                pro_ids.append(ProductID)
                pro_dict.append(
                    {"ProductID": ProductID, "ProductName": i["ProductName"]}
                )
            else:
                for f in final_data:
        ...
        response_data = {"StatusCode": 6000,
                         "data": final_data, "pro_dict": pro_dict, "LedgerList": LedgerList}
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def get_estimate_details(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    CreatedUserID = data["CreatedUserID"]
    ...
    if SalesEstimateMaster.objects.filter(
        ...
        sales_estimate_instances = SalesEstimateMaster.objects.filter(
            CompanyID=CompanyID,
            BranchID=BranchID,
            VoucherNo__in=estimate_vouchers,
            Status="N",
        )
        ledger_ids = sales_estimate_instances.values_list("LedgerID",flat=True)
        LedgerList = []
        if AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID__in=ledger_ids).exists():
            ledger_ins = AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID__in=ledger_ids)
            for l in ledger_ins:
                LedgerID = l.LedgerID
                LedgerName = l.LedgerName
        ...
        master_ids = sales_estimate_instances.values_list(
            "SalesEstimateMasterID", flat=True
        )
        sales_estimate_details = SalesEstimateDetails.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID, SalesEstimateMasterID__in=master_ids
        )

        serialized = SalesEstimateDetailsRestSerializer(
            sales_estimate_details,
            many=True,
            context={"CompanyID": CompanyID, "PriceRounding": PriceRounding},
        )
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def sales_filterd_sales_estimates(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    CreatedUserID = data["CreatedUserID"]
    PriceRounding = data["PriceRounding"]
    BranchID = data["BranchID"]
    EstimateCustomerID = data["EstimateCustomerID"]
    EstimateFromDate = data["EstimateFromDate"]
    EstimateToDate = data["EstimateToDate"]
    if SalesEstimateMaster.objects.filter(
        CompanyID=CompanyID, BranchID=BranchID, LedgerID=EstimateCustomerID, Status="N"
    ).exists():
        sales_estimate_instances = SalesEstimateMaster.objects.filter(
            CompanyID=CompanyID,
            BranchID=BranchID,
            LedgerID=EstimateCustomerID,
            Status="N",
        )
        if EstimateFromDate and EstimateToDate:
            sales_estimate_instances = sales_estimate_instances.filter(
                Date__gte=EstimateFromDate, Date__lte=EstimateToDate
            )
        if sales_estimate_instances:
            serialized = FilterEstimateSerializer(
                sales_estimate_instances, many=True)
...
def gstR_report(request):
    ...
    if serialized1.is_valid():
        ...
        if SalesMaster.objects.filter(
            ...
            serialized_sales = GST_B2B_Serializer(
                sales_instances,
                many=True,
                context={
                    "CompanyID": CompanyID,
                ...
                if not SalesMasterID in unq_master_ids:
                    ...
                    for s in sales_gsts:
                        if not s in unq_sales_gsts:
                            ...
                            Total_invoice_value_b2b_suppliers += converted_float(
                                GrandTotal)
                            Total_taxable_value_b2b_suppliers += converted_float(
                                taxable_value)
                            Total_cess_value_b2b_suppliers += converted_float(
                                CESS_amount)

                            if not Customer_GST_No in unq_recipient_b2b_supply:
                                unq_recipient_b2b_supply.append(
                                    Customer_GST_No)
        ...
        if SalesMaster.objects.filter(
            ...
            for i in jsnDatas:
                ...
                if not SalesMasterID in unq_master_ids:
                    ...
                    total_value = converted_float(
                        s.shipping_tax_amount) + converted_float(s.ShippingCharge)
                ...
                if not SalesMasterID in unq_master_ids:
                    ...
                    for s in sales_gsts:
                        if not s in unq_sales_gsts and converted_float(GrandTotal) < 250000:
                            ...
                            Total_invoice_value_b2cs += converted_float(
                                GrandTotal)
                            Total_taxable_value_b2cs += converted_float(
                                ShippingCharge)
                            Total_cess_value_b2cs += converted_float(
                                CESS_amount)
        ...
        if SalesReturnMaster.objects.filter(
            ...
            serialized_sales_return = GST_CDNR_Serializer(
                sales_return_instances,
                many=True,
                context={
                    "CompanyID": CompanyID,
                    "PriceRounding": PriceRounding,
                ...
                if not SalesMasterID in unq_master_ids:
                    ...
                    for s in sales_gsts:
                        if not s in unq_sales_gsts and converted_float(GrandTotal) >= 250000:
                            ...
                            Total_invoice_value_b2cl += converted_float(
                                GrandTotal)
                            Total_taxable_value_b2cl += converted_float(
                                taxable_value)
                            Total_cess_value_b2cl += converted_float(
                                CESS_amount)
                ...
                if not SalesReturnMasterID in unq_master_ids:
                    ...
                    for s in sales_gsts:
                        if not s in unq_sales_gsts:
                            ...
                            Total_invoice_value_cdnr += converted_float(
                                GrandTotal)
                            Total_taxable_value_cdnr += converted_float(
                                taxable_value)
                            Total_cess_value_cdnr += converted_float(
                                CESS_amount)
                ...
                if not SalesReturnMasterID in unq_master_ids:
                    ...
                    for s in sales_gsts:
                        if not s in unq_sales_gsts and converted_float(GrandTotal) < 250000:
                            ...
                            Total_invoice_value_cdnur += converted_float(
                                GrandTotal)
                            Total_taxable_value_cdnur += converted_float(
                                taxable_value)
                            if CESS_amount == "-":
                                CESS_amount = 0
                            Total_cess_value_cdnur += converted_float(
                                CESS_amount)
        ...
        HSN_values = (
            Product.objects.filter(CompanyID=CompanyID, BranchID=BranchID)
            .exclude(HSNCode="null")
            .values_list("HSNCode", flat=True)
        )
        ...
        for h in HSN_values:
            ...
            product_ins_ids = Product.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, HSNCode=h
            ).values_list("ProductID", flat=True)
            if SalesDetails.objects.filter(
                CompanyID=CompanyID,
                BranchID=BranchID,
                ProductID__in=product_ins_ids,
                SalesMasterID__in=master_ids,
            ).exists():
                    ...
                    if Unit.objects.filter(
                        ...
                        if not TotalQuantity == 0 and not TotalNetAmount == 0:
                            ...
                            total_taxable_value_hsn += converted_float(
                                TotalTaxableValue)
                            total_igst_tax_hsn += converted_float(
                                TotalIGSTAmount)
                            total_cgst_tax_hsn += converted_float(
                                TotalCGSTAmount)
                            total_sgst_tax_hsn += converted_float(
                                TotalSGSTAmount)
                            is_ok = True

            if SalesReturnDetails.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, ProductID__in=product_ins_ids
            ).exists():
                ...
                salesReturnDetails_ins = SalesReturnDetails.objects.filter(
                    ...
                    BranchID=BranchID,
                    ProductID__in=product_ins_ids,
                    SalesReturnMasterID__in=master_ids,
                )
                ...
                for uq in uqc_ins:
                    if Unit.objects.filter(
                        ...
                        if not TotalQuantity == 0 and not TotalNetAmount == 0:
                            final_data_HSN_summary.append(
                                {
                                    ...
                                    "TotalNetAmount": converted_float(TotalNetAmount) * -1,
                                    "TotalTaxableValue": converted_float(TotalTaxableValue) * -1,
                                    "TotalIGSTAmount": converted_float(TotalIGSTAmount) * -1,
                                    "TotalCGSTAmount": converted_float(TotalCGSTAmount) * -1,
                                    "TotalSGSTAmount": converted_float(TotalSGSTAmount) * -1,
                                    "CessAmount": 0,
                                }
                            )
                            ...
                            if not h in unq_hsn:
                                unq_hsn.append(h)
                            total_value_hsn += converted_float(
                                TotalNetAmount) * -1
                            total_taxable_value_hsn += converted_float(
                                TotalTaxableValue) * -1
                            total_igst_tax_hsn += converted_float(
...
def scanned_invoice(request):
    ...
    if VoucherType == "SI":
        ...
        if SalesReturnMaster.objects.filter(id=Unq_id).exists():
            ...
            sales_return_details = SalesReturnDetails.objects.filter(
                CompanyID=CompanyID,
                SalesReturnMasterID=SalesReturnMasterID,
                BranchID=BranchID,
            )
        ...
        if PurchaseReturnMaster.objects.filter(id=Unq_id).exists():
            instance = PurchaseReturnMaster.objects.get(id=Unq_id)
            VATNumber = instance.CompanyID.VATNumber
            GSTNumber = instance.CompanyID.GSTNumber
            CompanyCity = instance.CompanyID.City
            if instance.CompanyID.CompanyLogo:
            ...
            purchase_return_details = PurchaseReturnDetails.objects.filter(
                CompanyID=CompanyID,
                PurchaseReturnMasterID=PurchaseReturnMasterID,
                BranchID=BranchID,
            )
            for s in purchase_return_details:
                ProductID = s.ProductID
                ProductName = (
                    Product.objects.filter(
                        CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID
            ...
            VATAmount = instance.VATAmount
            # =========== Qr Print Page SaleMaster
            print_serialized = PurchaseReturnMasterRestSerializer(
                instance,
                context={
                    "CompanyID": CompanyID,
                    "PriceRounding": 2,
...
def search_sales_list(request):
    ...
    if serialized1.is_valid():
        ...
        if SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            ...
            serialized = SalesMasterRest1Serializer(
                instances,
                many=True,
                context={
                    "request": request,
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def tax_summary_gst_report(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    PriceRounding = data["PriceRounding"]
    ...
    if serialized1.is_valid():
        ...
        # Add TotalTax Due
        due_TaxableAmount = (
            outward_supplies[3]["TaxableAmount"] -
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def tax_summary_vat_report(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    PriceRounding = data["PriceRounding"]
...
def sales_report_excel(request):
    ...
    print(Type, "CompanyID,BranchID,FromDate,ToDate")
    response = HttpResponse(content_type="application/ms-excel")
    ...
    if Type == "1":
        ...
        # export_to_excel_sales(wb, data, FromDate, ToDate, title)
        # query_export_to_excel_sales(wb, data, FromDate, ToDate, title)
        ...
        response[
            "Content-Disposition"
        ] = 'attachment; filename="Sales Return Report.xlsx"'
        # data = salesReturn_excel_data(
        #     CompanyID, BranchID, FromDate, ToDate, PriceRounding, UserID, EmployeeID)
        # export_to_excel_salesReturn(wb, data, FromDate, ToDate, title)
        data = query_salesReturn_report_data(
            CompanyID, BranchID, FromDate, ToDate, PriceRounding, UserID, EmployeeID
        )
        df = pd.DataFrame(data)
        df.columns = [
...
@permission_classes((AllowAny,))
@renderer_classes((JSONRenderer,))
def taxSummary_report_excel(request):
    # content-type of response
    CompanyID = request.GET.get("CompanyID")
    BranchID = request.GET.get("BranchID")
    FromDate = request.GET.get("FromDate")
    ...
    # decide file name
    response["Content-Disposition"] = 'attachment; filename="Tax Summary.xls"'
    ...
    export_to_excel_taxSummary(wb, data, Type, title)
...
@permission_classes((AllowAny,))
@renderer_classes((JSONRenderer,))
def sales_taxgroup_report_excel(request):
    # content-type of response
    CompanyID = request.GET.get("CompanyID")
    BranchID = request.GET.get("BranchID")
    FromDate = request.GET.get("FromDate")
    ...
    # decide file name
    response["Content-Disposition"] = 'attachment; filename="Sales TaxGroup List.xls"'

    # creating workbook
    wb = xlwt.Workbook(encoding="utf-8")
    data = sales_taxgroup_excel_data(
    ...
    export_to_excel_sales_taxgroup(wb, data, FromDate, ToDate)
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def daily_report_cards(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    CreatedUserID = data["CreatedUserID"]
    ...
    if SalesMaster.objects.filter(
        ...
        if AccountLedger.objects.filter(
            ...
            if LedgerPosting.objects.filter(
                ...
                CashSales = converted_float(
                    sales_debit_sum) - converted_float(sales_credit_sum)
    ...
    net_sales = converted_float(
        CashSales) + converted_float(BankSales) + converted_float(CreditSales)
    if net_sales > 0 and month_sales_grand_total > 0:
        sales_perc = (converted_float(net_sales) /
                      converted_float(month_sales_grand_total)) * 100
    TotalSales = {
        "GrandTotal": net_sales,
        "CashSales": CashSales,
        "BankSales": BankSales,
    ...
    if PurchaseMaster.objects.filter(
        ...
        if AccountLedger.objects.filter(
            ...
            if LedgerPosting.objects.filter(
                ...
                CashPurchase = converted_float(
                    purchase_debit_sum) - converted_float(purchase_credit_sum)
        ...
        if AccountLedger.objects.filter(
            ...
            if LedgerPosting.objects.filter(
                ...
                purchase_credit_sum = ledger_ins.aggregate(Sum("Credit"))
                purchase_credit_sum = purchase_credit_sum["Credit__sum"]
                CreditPurchase = converted_float(
                    purchase_debit_sum) - converted_float(purchase_credit_sum)
    if PurchaseMaster.objects.filter(
        CompanyID=CompanyID, BranchID=BranchID, Date__year=year, Date__month=month
    ).exists():
        ...
        month_purchase_grand_total = month_purchase.aggregate(
            Sum("GrandTotal"))
        month_purchase_grand_total = month_purchase_grand_total["GrandTotal__sum"]

    net_purchase = converted_float(
        CashPurchase) + converted_float(BankPurchase) + converted_float(CreditPurchase)
    if abs(net_purchase) > 0 and month_purchase_grand_total > 0:
        purchase_perc = (
            converted_float(abs(net_purchase)) /
            converted_float(month_purchase_grand_total)
        ) * 100
    TotalPurchase = {
        "GrandTotal": abs(net_purchase),
        "CashPurchase": abs(CashPurchase),
        "BankPurchase": abs(BankPurchase),
        "CreditPurchase": abs(CreditPurchase),
        "purchase_perc": purchase_perc,
    }
    ...
    if SalesReturnMaster.objects.filter(
        ...
        if AccountLedger.objects.filter(
            ...
            if LedgerPosting.objects.filter(
                ...
                CashSalesReturn = converted_float(
                    salesReturn_debit_sum) - converted_float(salesReturn_credit_sum)
        ...
        if AccountLedger.objects.filter(
            ...
            if LedgerPosting.objects.filter(
                ...
                salesReturn_credit_sum = ledger_ins.aggregate(Sum("Credit"))
                salesReturn_credit_sum = salesReturn_credit_sum["Credit__sum"]
                BankSalesReturn = converted_float(
                    salesReturn_debit_sum) - converted_float(salesReturn_credit_sum)
        ...
        if AccountLedger.objects.filter(
            ...
            if LedgerPosting.objects.filter(
                ...
                salesReturn_credit_sum = ledger_ins.aggregate(Sum("Credit"))
                salesReturn_credit_sum = salesReturn_credit_sum["Credit__sum"]
                CreditSalesReturn = converted_float(
                    salesReturn_debit_sum) - converted_float(salesReturn_credit_sum)
    if SalesReturnMaster.objects.filter(
        ...
        month_salesReturn_grand_total = month_sales.aggregate(
            Sum("GrandTotal"))
        month_salesReturn_grand_total = month_salesReturn_grand_total["GrandTotal__sum"]

    net_salesReturn = converted_float(
        CashSalesReturn) + converted_float(BankSalesReturn) + converted_float(CreditSalesReturn)
    if net_salesReturn > 0 and month_salesReturn_grand_total > 0:
        salesReturn_perc = (converted_float(net_salesReturn) /
                            converted_float(month_salesReturn_grand_total)) * 100
    TotalSalesReturn = {
        "GrandTotalReturn": abs(net_salesReturn),
        "CashSalesReturn": CashSalesReturn,
        "BankSalesReturn": BankSalesReturn,
        "CreditSalesReturn": abs(CreditSalesReturn),
        "salesReturn_perc": salesReturn_perc,
    }
    ...
    if converted_float(expense_grand_total) > 0 and month_expense_grand_total > 0:
        expense_perc = (
            converted_float(expense_grand_total) /
            converted_float(month_expense_grand_total)
        ) * 100
    TotalExpense = {
        "GrandTotal": abs(expense_grand_total),
        "expense_perc": expense_perc,
    }
    ...
    if converted_float(payment_grand_total) > 0 and month_payment_grand_total > 0:
        payment_perc = (
            converted_float(payment_grand_total) /
            converted_float(month_payment_grand_total)
        ) * 100
    TotalPayment = {
        "GrandTotal": abs(payment_grand_total),
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def daily_report_statistic(request):
    from datetime import datetime
    ...
    Date = data["Date"]
    sales_hourly_total = []
    purchase_hourly_total = []
    payment_hourly_total = []

    sales_instances = None
    purchase_instances = None
    payment_instances = None
    if SalesMaster.objects.filter(
        CompanyID=CompanyID, BranchID=BranchID, Date=Date
    ).exists():
        sales_instances = SalesMaster.objects.filter(
    ...
    while i < 24:
        ...
        newfromdatetime = dt.replace(hour=i, minute=0, second=0)
        newtodatetime = dt.replace(hour=i, minute=59, second=59)
        sales_hourly_grand_total = None
        purchase_hourly_grand_total = None
        payment_hourly_grand_total = None
        if sales_instances:
            sales_filter_instances = sales_instances.filter(
                CreatedDate__gte=newfromdatetime, CreatedDate__lt=newtodatetime
            )
            sales_hourly_grand_total = sales_filter_instances.aggregate(
                Sum("GrandTotal")
            )
            sales_hourly_grand_total = sales_hourly_grand_total["GrandTotal__sum"]
        if purchase_instances:
            purchase_filter_instances = purchase_instances.filter(
                CreatedDate__gte=newfromdatetime, CreatedDate__lt=newtodatetime
            )
            purchase_hourly_grand_total = purchase_filter_instances.aggregate(
                Sum("GrandTotal")
            )
            purchase_hourly_grand_total = purchase_hourly_grand_total["GrandTotal__sum"]
        if payment_instances:
            payment_filter_instances = payment_instances.filter(
                CreatedDate__gte=newfromdatetime, CreatedDate__lt=newtodatetime
            )
            payment_hourly_grand_total = payment_filter_instances.aggregate(
                Sum("TotalAmount")
            )
            payment_hourly_grand_total = payment_hourly_grand_total["TotalAmount__sum"]
        if not sales_hourly_grand_total:
            sales_hourly_grand_total = 0
        if not purchase_hourly_grand_total:
            purchase_hourly_grand_total = 0
        if not payment_hourly_grand_total:
            payment_hourly_grand_total = 0

        sales_hourly_total.append(sales_hourly_grand_total)
        purchase_hourly_total.append(purchase_hourly_grand_total)
        payment_hourly_total.append(payment_hourly_grand_total)
        i += 1

    response_data = {
        "StatusCode": 6000,
        "sales_hourly_total": sales_hourly_total,
        "purchase_hourly_total": purchase_hourly_total,
        "payment_hourly_total": payment_hourly_total,
    }
    return Response(response_data, status=status.HTTP_200_OK)
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def daily_report_topItems(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    CreatedUserID = data["CreatedUserID"]
    ...
    if LedgerPosting.objects.filter(
        ...
        account_item_ins = AccountLedger.objects.filter(
            CompanyID=CompanyID, LedgerID__in=ledger_ids, AccountGroupUnder__in=[
                8, 9]
        )
        ...
        account_ledger_instances = ledger_instances.filter(
            LedgerID__in=account_itemids)
        ...
        for k in queried_instances:
            ...
            if party.PartyImage:
                image_serialized = PartyImageSerializer(
                    party, context={"CompanyID": CompanyID, "request": request}
                )
        ...
        for q in account_queried_instances:
            ...
            accountsItems.append(
                {"account_name": LedgerName, "account_amount_sum": balance}
            )
...
@permission_classes((AllowAny,))
@renderer_classes((JSONRenderer,))
def billWise_report_excel(request):
    # content-type of response
    CompanyID = request.GET.get("CompanyID")
    ...
    ToDate = request.GET.get("ToDate")
    PriceRounding = int(request.GET.get("PriceRounding"))
    WareHouseID = int(request.GET.get("WareHouseID"))
    RouteID = int(request.GET.get("RouteID"))
    ...
    export_to_excel_billWise(wb, data, FromDate, ToDate)
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def customer_product_sales_history(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    BranchID = data["BranchID"]
    ...
    if Type == "Sales":
        if SalesMaster.objects.filter(
            ...
            sales_ledger_ins = SalesMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID
            )
            sales_master_ids = sales_ledger_ins.values_list(
                "SalesMasterID", flat=True)
            if SalesDetails.objects.filter(
                CompanyID=CompanyID,
                SalesMasterID__in=sales_master_ids,
                ProductID__in=ProductIDs,
            ).exists():
                sales_product_details = SalesDetails.objects.filter(
                    CompanyID=CompanyID,
                    SalesMasterID__in=sales_master_ids,
                    ProductID__in=ProductIDs,
                ).order_by("-SalesDetailsID")[:5]
                history_serialized = SalesHistorySerializer(
                ...
                StatusCode = 6000
                history_datas = history_serialized.data
    elif Type == "Purchase":
        if PurchaseMaster.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID
        ).exists():
            ...
            if PurchaseDetails.objects.filter(
                ...
                sales_product_details = PurchaseDetails.objects.filter(
                    ...
                    PurchaseMasterID__in=purchase_master_ids,
                    ProductID__in=ProductIDs,
                ).order_by("-PurchaseDetailsID")[:5]
                history_serialized = PurchaseHistorySerializer(
                    sales_product_details,
                    many=True,
                    context={
                        "request": request,
                ...
                StatusCode = 6000
                history_datas = history_serialized.data
    response_data = {"StatusCode": StatusCode, "history_datas": history_datas}
    return Response(response_data, status=status.HTTP_200_OK)
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def salesRegister_report_qry(request):
    data = request.data
    CompanyIDuid = data["CompanyID"]
    CompanyID = get_company(CompanyIDuid)
    PriceRounding = data["PriceRounding"]
        ...
        if SalesMaster.objects.filter(
            ...
            if AccountLedger.objects.filter(CompanyID=CompanyID).exists():
                ledger_list = AccountLedger.objects.filter(
                    CompanyID=CompanyID).values_list('LedgerID', flat=True)
            if table.Warehouse.objects.filter(CompanyID=CompanyID).exists():
                warehouse_list = table.Warehouse.objects.filter(
                    CompanyID=CompanyID).values_list('WarehouseID', flat=True)
            if UserTable.objects.filter(CompanyID=CompanyID).exists():
                user_list = UserTable.objects.filter(
                    CompanyID=CompanyID).values_list('customer_id', flat=True)
                user_list = table.Customer.objects.filter(
                    id__in=user_list).values_list('user_id', flat=True)
            ...
            df, details = sales_register_report_data(
                CompanyIDuid, BranchList, FromDate, ToDate, warehouse_list, user_list, ledger_list, ProductID, GroupID, CategoryID, ProductCode, Barcode)
            response_data = {
                "StatusCode": 6000,
                "data": details
            }
...
def daily_report_excel(request):
    ...
    # decide file name
    response["Content-Disposition"] = 'attachment; filename="Daily Report.xls"'

    # creating workbook
    wb = xlwt.Workbook(encoding="utf-8")
    data_cards = sales_dailyReport_excel_data(
        CompanyID, BranchID, date
    )
    data_top_items = sales_dailyReport_excel_data_top_items(
        CompanyID, BranchID, date
    )
    ...
    if type == "Excel":
        export_to_excel_dailyreport(
            wb, data_cards, data_top_items, "Daily Report", date)
        wb.save(response)
    else:
        report_title = str(_("DAILY REPORT"))
        template = get_template('reports/daily_report.html')
        ...
        for c in customerItems:
            cusomer_account_datas.append(c)
        accountsItems = data_top_items[0]['accountsItems']
        for d in accountsItems:
            cusomer_account_datas.append(d)
        ...
        context = {
            ...
            'productItems': productItems,
            'cusomer_account_datas': cusomer_account_datas,
            'TransactionsDatas': TransactionsDatas,
            'PriceRounding': int(2),
            'Date': date,
            "print_template": True,
...
def sale_list_page(request):
    ...
    PriceRounding = data["PriceRounding"]
    instance = None
    if SalesMaster.objects.filter(CompanyID=CompanyID).exists():
        df, details = sales_list_data(data["CompanyID"], BranchID,PriceRounding)
        # request , company, log_type, user, source, action, message, description
        # activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Sales Invoice', 'View',
        #              'Sales Invoice Single Viewed successfully.', 'Sales Invoice Single Viewed successfully.')
...
@permission_classes((IsAuthenticated,))
@renderer_classes((JSONRenderer,))
def single_salesMaster(request, pk):
    import time
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    ...
    if SalesMaster.objects.filter(CompanyID=CompanyID, pk=pk).exists():
        instance = SalesMaster.objects.get(CompanyID=CompanyID, pk=pk)
        start = time.time()
        serialized = SalesSingleSerializer(
            instance,
            context={
                "CompanyID": CompanyID,
                "PriceRounding": PriceRounding,
...
def expense_summary_report(request):
    ...
    # CompanyID = get_company(CompanyID)
    PriceRounding = data["PriceRounding"]
    serialized1 = ListSerializerforSummaryReport(data=request.data)
    if serialized1.is_valid():
        ...
        arr = np.array(BranchIDs)
        is_BranchIDs = np.all((arr == 0))
        if is_BranchIDs:
            BranchIDs = 0
        else:
            BranchIDs = tuple(BranchIDs)
        df,data = query_Expense_summary(CompanyID,BranchIDs,FromDate,ToDate,PriceRounding)
        response_data = {
            "StatusCode": 6000,
            "data":data,
        }
