...
from api.v9.workOrder.serializers import Batch_ListSerializer
...
class SalesMasterRestSerializer(serializers.ModelSerializer):
    ...
    def get_SalesAccountName(self, instances):
        ...
        SalesAccountName = ""
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=SalesAccount).exists():
            ledger = AccountLedger.objects.get(
...
class SalesDetailsRestSerializer(serializers.ModelSerializer):
    ...
    def get_actualSalesPrice(self, sales_details):
        ...
        ProductID = sales_details.ProductID
        SalesPrice = 0
        if PriceList.objects.filter(ProductID=ProductID, CompanyID=CompanyID, PriceListID=PriceListID).exists():
            SalesPrice = PriceList.objects.get(
                ProductID=ProductID, CompanyID=CompanyID, PriceListID=PriceListID).SalesPrice
    ...
    def get_is_VAT_inclusive(self, sales_details):
        ...
        VatID = Product.objects.get(
            CompanyID=CompanyID, ProductID=ProductID).VatID
    ...
    def get_is_GST_inclusive(self, sales_details):
        ...
        GST = Product.objects.get(
            CompanyID=CompanyID, ProductID=ProductID).GST
        Inclusive = False
        if GST:
            Inclusive = TaxCategory.objects.get(
    ...
    def get_is_TAX1_inclusive(self, sales_details):
        ...
        Tax1 = Product.objects.get(
            CompanyID=CompanyID, ProductID=ProductID).Tax1
        Inclusive = False
        if Tax1:
            Inclusive = TaxCategory.objects.get(
    ...
    def get_is_TAX2_inclusive(self, sales_details):
        ...
        Tax2 = Product.objects.get(
            CompanyID=CompanyID, ProductID=ProductID).Tax2
        Inclusive = False
        if Tax2:
            Inclusive = TaxCategory.objects.get(
    ...
    def get_is_TAX3_inclusive(self, sales_details):
        ...
        Tax3 = Product.objects.get(
            CompanyID=CompanyID, ProductID=ProductID).Tax3
        Inclusive = False
        if Tax3:
            Inclusive = TaxCategory.objects.get(
    ...
    def get_SerialNos(self, sales_details):
        ...
        if SerialNumbers.objects.filter(CompanyID=CompanyID, BranchID=BranchID, SalesMasterID=SalesMasterID, SalesDetailsID=SalesDetailsID).exists():
            Serial_details = SerialNumbers.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, SalesMasterID=SalesMasterID, SalesDetailsID=SalesDetailsID)
...
class SalesGSTReportSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SalesMaster
        fields = ('id', 'SalesMasterID', 'BranchID', 'Action', 'VoucherNo', 'TotalQty', 'Date', 'CreditPeriod',
                  'LedgerID', 'PriceCategoryID', 'EmployeeID', 'SalesAccount', 'CustomerName', 'TaxableValue')
    ...
    def get_TaxableValue(self, instances):
        ...
        return converted_float(TaxableValue)
...
class SalesMasterReportSerializer(serializers.ModelSerializer):
    ...
    def get_CashSales(self, instances):
        ...
        TotalDebit = 0
        TotalCredit = 0
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, AccountGroupUnder=9).exists():
            print("--------------->", LedgerID)
    ...
    def get_BankSales(self, instances):
        ...
        TotalDebit = 0
        TotalCredit = 0
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, AccountGroupUnder=8).exists():
            if LedgerPosting.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, Date__gte=FromDate, Date__lte=ToDate, VoucherType="SI").exists():
                ledger_ins = LedgerPosting.objects.filter(
                    CompanyID=CompanyID, LedgerID=LedgerID, Date__gte=FromDate, Date__lte=ToDate, VoucherType="SI")
                for i in ledger_ins:
    ...
    def get_SalesAccountName(self, instances):
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=SalesAccount).exists():
            SalesAccountName = AccountLedger.objects.get(
                CompanyID=CompanyID, LedgerID=SalesAccount).LedgerName
...
class StockRateSerializer(serializers.ModelSerializer):
    ...
    def get_UnitName(self, instance):
        ...
        UnitID = PriceList.objects.get(
            CompanyID=CompanyID, PriceListID=PriceListID, DefaultUnit=True).UnitID
        UnitName = Unit.objects.get(
            CompanyID=CompanyID, UnitID=UnitID).UnitName
    ...
    def get_WareHouseName(self, instance):
        ...
        BranchID = instance.BranchID
        WareHouseName = Warehouse.objects.get(
            CompanyID=CompanyID, WarehouseID=WareHouseID).WarehouseName
    ...
    def get_PartyName(self, instances):
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, AccountGroupUnder__in=[10, 29]).exists():
            PartyName = Parties.objects.get(
                CompanyID=CompanyID, LedgerID=LedgerID).PartyName
        return PartyName
    ...
    def get_FirstName(self, instances):
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, AccountGroupUnder__in=[10, 29]).exists():
            FirstName = Parties.objects.get(
                CompanyID=CompanyID, LedgerID=LedgerID).FirstName
        return FirstName
    ...
    def get_LastName(self, instances):
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, AccountGroupUnder__in=[10, 29]).exists():
            LastName = Parties.objects.get(
                CompanyID=CompanyID, LedgerID=LedgerID).LastName
        return LastName
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, AccountGroupUnder__in=[10, 29]).exists():
            if Parties.objects.get(CompanyID=CompanyID, LedgerID=LedgerID).Address1:
                Address1 = Parties.objects.get(
                    CompanyID=CompanyID, LedgerID=LedgerID).Address1
        return Address1
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, AccountGroupUnder__in=[10, 29]).exists():
            if Parties.objects.get(CompanyID=CompanyID, LedgerID=LedgerID).Address2:
                Address2 = Parties.objects.get(
                    CompanyID=CompanyID, LedgerID=LedgerID).Address2
        return Address2
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, AccountGroupUnder__in=[10, 29]).exists():
            if Parties.objects.get(CompanyID=CompanyID, LedgerID=LedgerID).City:
                City = Parties.objects.get(
                    CompanyID=CompanyID, LedgerID=LedgerID).City
        return City
    ...
    def get_AvgSalesRate(self, instances):
        ...
        if SalesDetails.objects.filter(CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID).exists():
            ...
            for s in salesDetails_ins:
                oneAvg = (converted_float(s.Qty) * converted_float(MultiFactor) *
                          converted_float(s.UnitPrice)) / (converted_float(s.Qty) * converted_float(MultiFactor))
                AvgSalesRate += oneAvg
        return AvgSalesRate
