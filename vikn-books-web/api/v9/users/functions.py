...
def create_or_update_general_settings(
    BranchID, CompanyID, SettingsType, SettingsValue, GroupName, User, Action
):
    today = datetime.datetime.now()
    if not administrations_models.GeneralSettings.objects.filter(
        CompanyID=CompanyID, BranchID=BranchID, SettingsType=SettingsType
    ).exists():
        ...
        administrations_models.GeneralSettings.objects.filter(
            CompanyID=CompanyID, SettingsType=SettingsType, BranchID=BranchID
        ).update(
            SettingsValue=SettingsValue,
            UpdatedDate=today,
            CreatedUserID=User,
...
def update_general_settings(CompanyID, SettingsType, SettingsValue, User, Action):
    today = datetime.datetime.now()
    administrations_models.GeneralSettings.objects.filter(
        CompanyID=CompanyID, SettingsType=SettingsType
    ).update(
        SettingsValue=SettingsValue,
        UpdatedDate=today,
        CreatedUserID=User,
...
def get_general_settings(BranchID, CompanyID, SettingsType):
