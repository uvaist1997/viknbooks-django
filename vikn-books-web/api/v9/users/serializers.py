...
class UserTableRestSerializer(serializers.ModelSerializer):
    ...
    def get_Role(self, instance):
        ...
        if instance.UserType:
            Role = instance.UserType.UserTypeName
        return Role
...
class CompaniesSerializer(serializers.ModelSerializer):
    ...
    def get_IsPosUser(self, instances):
        if UserTable.objects.get(pk=instances.pk).CompanyID.IsPosUser:
            IsPosUser = UserTable.objects.get(pk=instances.pk).CompanyID.IsPosUser
            return IsPosUser
        else:
            return False
    ...
    def get_Permission(self, instances):
        if UserTable.objects.get(pk=instances.pk).CompanyID.Permission:
            Permission = UserTable.objects.get(pk=instances.pk).CompanyID.Permission
            return Permission
        else:
            return ""
    ...
    def get_CompanyLogo(self, instances):
        ...
        if UserTable.objects.get(pk=instances.pk).CompanyID.CompanyLogo:
            CompanyLogo = UserTable.objects.get(pk=instances.pk).CompanyID.CompanyLogo
            logo_url = CompanyLogo.url
            if web_host:
                logo_url = str(protocol) + str(web_host) + str(logo_url)
            return logo_url
        else:
            return ""
    ...
    def get_StateName(self, instances):
        if UserTable.objects.get(pk=instances.pk).CompanyID.State.Name:
            StateName = UserTable.objects.get(pk=instances.pk).CompanyID.State.Name
            return StateName
        else:
            return ""
