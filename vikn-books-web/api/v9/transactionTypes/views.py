...
from api.v9.transactionTypes.functions import generate_serializer_errors
from rest_framework import status
from api.v9.transactionTypes.functions import get_auto_id
import datetime
...
def create_transactionType(request):
    ...
    if serialized.is_valid():
        ...
        if not is_nameExist:
            ...
            trans_ins = TransactionTypes.objects.create(
            ...
            data = {"TransactionTypesID": TransactionTypesID,"id": trans_ins.id}
...
def transactionTypesByMasterName(request):
    ...
    if serialized1.is_valid():
        ...
        if MasterType.objects.filter(CompanyID=CompanyID, Name=MasterName).exists():

            MasterTypeinstance = MasterType.objects.get(
                CompanyID=CompanyID, Name=MasterName)
            MasterTypeID = MasterTypeinstance.MasterTypeID

            if MasterTypeID == 2:
                instances = TransactionTypes.objects.filter(
                    CompanyID=CompanyID, MasterTypeID=MasterTypeID)
            else:
                instances = TransactionTypes.objects.filter(
                    CompanyID=CompanyID, MasterTypeID=MasterTypeID)
