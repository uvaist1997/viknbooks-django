...
from api.v9.purchaseOrders.serializers import PurchaseOrderDetailsRestSerializer, PurchaseOrderMasterRestSerializer
    ...
    if serialized1.is_valid():
        ...
        if PurchaseMaster.objects.filter(
            ...
            if UserTable.objects.filter(CompanyID=CompanyID).exists():
                user_list = UserTable.objects.filter(CompanyID=CompanyID).values_list(
...
def get_order_details(request):
    ...
    if PurchaseOrderMaster.objects.filter(
        ...
        response_data = {"StatusCode": 6000,
                         "data": final_data, "pro_dict": pro_dict, "GST_Treatment": GST_Treatment, "VAT_Treatment": VAT_Treatment, "State_of_Supply": State_of_Supply}
