...
from api.v9.sales.functions import SetBatchInSales, edit_LoyaltyCalculation, get_BillwiseMasterID, get_CashAmount, get_Genrate_VoucherNo, get_actual_point, get_auto_id, get_auto_idMaster, get_auto_stockPostid, set_LoyaltyCalculation
from api.v9.sales.serializers import SalesMasterRestSerializer
from api.v9.sales.views import salesMaster
from api.v9.salesApp.serializers import ExpenseListSerializer, POS_ProductList_Serializer, PaymentListSerializer, ReceiptListSerializer, SalesFaeraSerializer, SalesListSerializer, SalesOrderListSerializer, SalesReturnListSerializer, SalesReturnMasterSerializer, StockOrderListSerializer
...
from main.functions import converted_float, generateVoucherNoFunction, get_BranchLedgerId_for_LedgerPosting, get_GeneralSettings, get_LedgerBalance, get_ModelInstance, get_ProductStock, get_company, activity_log, get_financialYearDates, get_nth_day_date
...
from api.v9.payments.functions import get_auto_id as payment_detailid, get_auto_idMaster as payment_masterid
from api.v9.receipts.functions import get_auto_id as receipt_detailid, get_auto_idMaster as receipt_masterid
from api.v9.users.functions import get_general_settings
from api.v9.salesReturns.functions import (
...
from api.v9.expense.functions import get_masterID as expenseMasterID, get_detailID as expenseDetailID
from api.v9.expense.serializers import ExpenseMasterSerializer
from api.v9.stockTransfers.functions import (
    get_auto_id as stockdetailsid,
    get_auto_idMaster as stockmasterid,
)
from api.v9.salesOrders.functions import get_auto_idMaster as OrderMasterID,get_auto_id as OrderDetailID
...
def pos_product_search(request):
    ...
    if table.Product.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        ...
        final_data = []
        if table.POS_Devices.objects.filter(DeviceCode=DeviceCode).exists():
            device = table.POS_Devices.objects.filter(DeviceCode=DeviceCode).first()
            StockAlert = device.StockAlert
            WarehouseID = device.WareHouseID.WarehouseID
...
def list_sales(request):
    ...
    instances = None
    count = 0
    if table.SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        instances = table.SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID)
...
def list_sales_return(request):
    ...
    instances = None
    count = 0
    if table.SalesReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
...
def list_sales_order(request):
    ...
    instances = None
    count = 0
    if table.SalesOrderMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
...
def list_payment(request):
    ...
    if table.PaymentMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, VoucherType=VoucherType).exists():
...
def salesApp_delete_payment(request, pk):
    ...
    if detail_ins:
        ...
        if table.PaymentMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID,PaymentMasterID=PaymentMasterID).exists():
            instance = table.PaymentMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID,PaymentMasterID=PaymentMasterID).first()
...
def list_receipt(request):
    ...
    count = 0
    voucher_search = False
    if table.ReceiptMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, VoucherType=VoucherType).exists():
...
def salesApp_edit_receipt(request, pk):
    try:
        with transaction.atomic():
            ...
            instance = table.ReceiptMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ReceiptMasterID=ReceiptMasterID).first()
            CompanyID = get_company(CompanyID)
            VoucherNo = instance.VoucherNo
            Date = data['Date']
            MasterLedgerID = data['MasterLedgerID']
...
def salesApp_edit_salesReturn(request, pk):
    try:
        with transaction.atomic():
            ...
            if enable_billwise and converted_float(total_amount_received) < converted_float(GrandTotal) and AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, AccountGroupUnder__in=[10, 29]).exists():
                if table.BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo, VoucherType=VoucherType,CustomerID=LedgerID).exists():
                    billwise_ins = table.BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo, VoucherType=VoucherType,CustomerID=LedgerID).first()
                    billwise_ins.VoucherDate = VoucherDate
                    billwise_ins.InvoiceAmount = GrandTotal
                    billwise_ins.Payments = total_amount_received
                    billwise_ins.save()
                elif table.BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo, VoucherType=VoucherType).exists():
                    billwise_ins = table.BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo, VoucherType=VoucherType).first()
                    billwise_ins.VoucherDate = VoucherDate
                    billwise_ins.InvoiceAmount = GrandTotal
                    billwise_ins.Payments = total_amount_received
                    billwise_ins.CustomerID = LedgerID
            ...
            LedgerPostingID = get_auto_LedgerPostid(table.LedgerPosting, BranchID, CompanyID)
...
def salesApp_salesReturnMaster(request, pk):
    ...
    if table.SalesReturnMaster.objects.filter(CompanyID=CompanyID, pk=pk).exists():
        instance = table.SalesReturnMaster.objects.get(CompanyID=CompanyID, pk=pk)
...
def salesApp_delete_salesReturnMaster(request, pk):
    ...
    if selecte_ids:
        if table.SalesReturnMaster.objects.filter(
            ...
            instances = table.SalesReturnMaster.objects.filter(pk__in=selecte_ids)
    else:
        if table.SalesReturnMaster.objects.filter(CompanyID=CompanyID, pk=pk).exists():
...
def salesApp_salesOrderMaster(request, pk):
    ...
    if table.SalesOrderMaster.objects.filter(pk=pk).exists():
        instance = table.SalesOrderMaster.objects.get(CompanyID=CompanyID, pk=pk)
...
def salesApp_delete_salesOrderMaster(request, pk):
    ...
    if selecte_ids:
        if table.SalesOrderMaster.objects.filter(
            ...
            instances = table.SalesOrderMaster.objects.filter(pk__in=selecte_ids)
    else:
...
def list_salesApp_expense(request):
    ...
    count = 0
    voucher_search = False
    if table.ExpenseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
...
def salesApp_edit_expense(request, pk):
    try:
        with transaction.atomic():
            ...
            BranchID = detail_instance.BranchID
            expense_instance = table.ExpenseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ExpenseMasterID=ExpenseMasterID).first()
...
def list_stock_order(request):
    ...
    instances = None
    count = 0
    if table.StockTransferMaster_ID.objects.filter(CompanyID=CompanyID, BranchID=BranchID,VoucherType="STO").exists():
        instances = table.StockTransferMaster_ID.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID,VoucherType="STO")
    ...
    if length > 0:
        instances = instances.filter(CompanyID=CompanyID, BranchID=BranchID,VoucherNo__icontains=search_val)
        if length < 3:
            instances = instances.filter()[:10]
    else:
        count = len(instances)
...
def salesApp_stockOrderMasterID(request, pk):
    ...
    if table.StockTransferMaster_ID.objects.filter(CompanyID=CompanyID, pk=pk).exists():
        instance = table.StockTransferMaster_ID.objects.get(CompanyID=CompanyID, pk=pk)
...
def list_stock_transfer(request):
    ...
    instances = None
    count = 0
    if table.StockTransferMaster_ID.objects.filter(CompanyID=CompanyID, BranchID=BranchID,VoucherType="ST").exists():
        instances = table.StockTransferMaster_ID.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID,VoucherType="ST")
...
def salesApp_edit_stockTransfer(request, pk):
    try:
        ...
        body_params = str(request.data)
        table.Activity_Log.objects.filter(id=activity_id).update(body_params=body_params)
        return Response(response_data, status=status.HTTP_200_OK)
...
def salesApp_summary(request):
    ...
    if table.SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate).exists():
        ...
        ledgerids = sales_ins.values_list("LedgerID",flat=True)
        if table.AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID__in=ledgerids,AccountGroupUnder=9).exists():
            ledgerids = table.AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID__in=ledgerids,AccountGroupUnder=9).values_list("LedgerID",flat=True)
            cash_sales_ins = sales_ins.filter(LedgerID__in=ledgerids)
            CashSale = cash_sales_ins.aggregate(Sum("GrandTotal"))
            CashSale = CashSale["GrandTotal__sum"]
        if table.AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID__in=ledgerids,AccountGroupUnder=8).exists():
            ledgerids = table.AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID__in=ledgerids,AccountGroupUnder=8).values_list("LedgerID",flat=True)
            bank_sales_ins = sales_ins.filter(LedgerID__in=ledgerids)
            BankSale = bank_sales_ins.aggregate(Sum("GrandTotal"))
            BankSale = BankSale["GrandTotal__sum"]
        if table.AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID__in=ledgerids,AccountGroupUnder__in=[10,29]).exists():
            ledgerids = table.AccountLedger.objects.filter(CompanyID=CompanyID,LedgerID__in=ledgerids,AccountGroupUnder__in=[10,29]).values_list("LedgerID",flat=True)
            credit_sales_ins = sales_ins.filter(LedgerID__in=ledgerids)
            CreditSale = credit_sales_ins.aggregate(Sum("GrandTotal"))
            CreditSale = CreditSale["GrandTotal__sum"]
            
    if table.SalesReturnMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherDate__gte=FromDate,VoucherDate__lte=ToDate).exists():
    ...
    if table.ReceiptMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherType="CR").exists():
    ...
    if table.ReceiptMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherType="BR").exists():
    ...
    if table.PaymentMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherType="CP").exists():
    ...
    if table.PaymentMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherType="BP").exists():
    ...
    if table.ExpenseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        expense_ins = table.ExpenseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
...
def salesApp_dashboard(request):
    ...
    BranchID = data["BranchID"]
    UserID = data["UserID"]
    financial_years = get_financialYearDates(CompanyID)
    FromDate = financial_years["financial_FromDate"]
    ToDate = financial_years["financial_ToDate"]
