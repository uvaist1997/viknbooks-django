...
def get_voucherNo(CompanyID, BranchID, VoucherType, VoucherMasterID):
    ...
    if VoucherType == 'JL':
        if tables.JournalMaster.objects.filter(CompanyID=CompanyID, JournalMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.JournalMaster.objects.get(
                CompanyID=CompanyID, JournalMasterID=VoucherMasterID, BranchID=BranchID)
        ...
        if tables.SalesMaster.objects.filter(CompanyID=CompanyID, SalesMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.SalesMaster.objects.get(
                CompanyID=CompanyID, SalesMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            LedgerID = instances.LedgerID
            Unq_id = instances.id
            if tables.AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID).exists():
                LedgerName = tables.AccountLedger.objects.get(
        ...
        if tables.PurchaseMaster.objects.filter(CompanyID=CompanyID, PurchaseMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.PurchaseMaster.objects.get(
                CompanyID=CompanyID, PurchaseMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
            LedgerID = instances.LedgerID
            if tables.AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID).exists():
        ...
        if tables.SalesReturnMaster.objects.filter(CompanyID=CompanyID, SalesReturnMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.SalesReturnMaster.objects.get(
                CompanyID=CompanyID, SalesReturnMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
            LedgerID = instances.LedgerID
            if tables.AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID).exists():
        ...
        if tables.PurchaseReturnMaster.objects.filter(CompanyID=CompanyID, PurchaseReturnMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.PurchaseReturnMaster.objects.get(
                CompanyID=CompanyID, PurchaseReturnMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
            LedgerID = instances.LedgerID
            if tables.AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID).exists():
        ...
        if tables.SalesOrderMaster.objects.filter(CompanyID=CompanyID, SalesOrderMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.SalesOrderMaster.objects.get(
                CompanyID=CompanyID, SalesOrderMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
            LedgerID = instances.LedgerID
            if tables.AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID).exists():
        ...
        if tables.PurchaseOrderMaster.objects.filter(CompanyID=CompanyID, PurchaseOrderMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.PurchaseOrderMaster.objects.get(
                CompanyID=CompanyID, PurchaseOrderMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
            LedgerID = instances.LedgerID
            if tables.AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID).exists():
        ...
        if tables.SalesEstimateMaster.objects.filter(CompanyID=CompanyID, SalesEstimateMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.SalesEstimateMaster.objects.get(
                CompanyID=CompanyID, SalesEstimateMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
            LedgerID = instances.LedgerID
            if tables.AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID).exists():
        ...
        if tables.PaymentMaster.objects.filter(CompanyID=CompanyID, PaymentMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.PaymentMaster.objects.get(
                CompanyID=CompanyID, PaymentMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
        ...
        if tables.ReceiptMaster.objects.filter(CompanyID=CompanyID, ReceiptMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.ReceiptMaster.objects.get(
                CompanyID=CompanyID, ReceiptMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
    elif VoucherType == 'ST':
        if tables.StockTransferMaster_ID.objects.filter(CompanyID=CompanyID, StockTransferMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.StockTransferMaster_ID.objects.get(
                CompanyID=CompanyID, StockTransferMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
    elif VoucherType == 'ES':
        if tables.ExcessStockMaster.objects.filter(CompanyID=CompanyID, ExcessStockMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.ExcessStockMaster.objects.get(
                CompanyID=CompanyID, ExcessStockMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
    elif VoucherType == 'SS':
        if tables.ShortageStockMaster.objects.filter(CompanyID=CompanyID, ShortageStockMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.ShortageStockMaster.objects.get(
                CompanyID=CompanyID, ShortageStockMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
    elif VoucherType == 'DS':
        if tables.DamageStockMaster.objects.filter(CompanyID=CompanyID, DamageStockMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.DamageStockMaster.objects.get(
                CompanyID=CompanyID, DamageStockMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
    elif VoucherType == 'US':
        if tables.UsedStockMaster.objects.filter(CompanyID=CompanyID, UsedStockMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.UsedStockMaster.objects.get(
                CompanyID=CompanyID, UsedStockMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
    elif VoucherType == 'OS':
        if tables.OpeningStockMaster.objects.filter(CompanyID=CompanyID, OpeningStockMasterID=VoucherMasterID, BranchID=BranchID).exists():
            instances = tables.OpeningStockMaster.objects.get(
                CompanyID=CompanyID, OpeningStockMasterID=VoucherMasterID, BranchID=BranchID)
            VoucherNo = instances.VoucherNo
            Unq_id = instances.id
...
class StockPostingRestSerializer(serializers.ModelSerializer):
    ...
    def get_LedgerName(self, instance):
        ...
        VoucherNo, Unq_id, LedgerName = get_voucherNo(
            CompanyID, BranchID, VoucherType, VoucherMasterID)
        return LedgerName
    ...
    def get_WareHouseName(self, instance):
        ...
        if Warehouse.objects.filter(WarehouseID=WarehouseID, BranchID=BranchID, CompanyID=CompanyID).exists():
            WareHouseName = Warehouse.objects.get(
                WarehouseID=WarehouseID, BranchID=BranchID, CompanyID=CompanyID).WarehouseName
    ...
    def get_UnitName(self, instance):
        ...
        if PriceList.objects.filter(PriceListID=PriceListID, BranchID=BranchID, CompanyID=CompanyID).exists():
            UnitID = PriceList.objects.get(
                PriceListID=PriceListID, BranchID=BranchID, CompanyID=CompanyID).UnitID
            UnitName = Unit.objects.get(
                UnitID=UnitID, BranchID=BranchID, CompanyID=CompanyID).UnitName
        return UnitName
...
class StockValueAllSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ('ProductID', 'ProductName', 'QtyIn',
                  'QtyOut', 'VoucherType', 'Date')
    ...
    def get_QtyIn(self, instance):
        ...
        if StockPosting.objects.filter(ProductID=ProductID, BranchID=BranchID).exists():
            stockInstances = StockPosting.objects.filter(
...
class StockValueSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = StockPosting
        fields = ('ProductID', 'ProductName', 'WareHouseID', 'WareHouseName',
    ...
    def get_ProductName(self, instance):
        ...
        if Product.objects.filter(ProductID=ProductID, BranchID=BranchID, CompanyID=CompanyID).exists():
            ProductName = Product.objects.get(
                ProductID=ProductID, BranchID=BranchID, CompanyID=CompanyID).ProductName
        return ProductName
    ...
    def get_PurchasePrice(self, instance):
        ...
        if PriceList.objects.filter(PriceListID=PriceListID, BranchID=BranchID, CompanyID=CompanyID).exists():
            PurchasePrice = PriceList.objects.get(
                PriceListID=PriceListID, BranchID=BranchID, CompanyID=CompanyID).PurchasePrice
    ...
    def get_SalesPrice(self, instance):
        ...
        if PriceList.objects.filter(PriceListID=PriceListID, BranchID=BranchID, CompanyID=CompanyID).exists():
            SalesPrice = PriceList.objects.get(
                PriceListID=PriceListID, BranchID=BranchID, CompanyID=CompanyID).SalesPrice
        return SalesPrice
...
class StockValueSingleSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = StockPosting
        fields = ('id', 'StockPostingID', 'BranchID', 'Date', 'VoucherType',
                  'ProductID', 'WareHouseID', 'QtyIn', 'QtyOut', 'ProductName')
...
class ExcessStockMasterSerializer(serializers.ModelSerializer):
    ...
    def get_Details(self, instances):
        ...
        BranchID = instances.BranchID
        details = ExcessStockDetails.objects.filter(
            CompanyID=CompanyID, ExcessStockMasterID=ExcessStockMasterID, BranchID=BranchID)
        serialized = ExcessStockDetailSerializer(details, many=True, context={
...
class ExcessStockDetailSerializer(serializers.ModelSerializer):
    ...
    def get_ExcessStock(self, details):
        ...
        return converted_float(ExcessStock)
...
class ShortageStockMasterSerializer(serializers.ModelSerializer):
    ...
    def get_Details(self, instances):
        ...
        BranchID = instances.BranchID
        details = ShortageStockDetails.objects.filter(
            CompanyID=CompanyID, ShortageStockMasterID=ShortageStockMasterID, BranchID=BranchID)
        serialized = ShortageStockDetailSerializer(details, many=True, context={
                                                   "CompanyID": CompanyID, "PriceRounding": PriceRounding})
...
class ShortageStockDetailSerializer(serializers.ModelSerializer):
    ...
    def get_ShortageStock(self, details):
        ...
        return converted_float(ShortageStock)
...
class DamageStockMasterSerializer(serializers.ModelSerializer):
    ...
    def get_Details(self, instances):
        ...
        BranchID = instances.BranchID
        details = DamageStockDetails.objects.filter(
            CompanyID=CompanyID, DamageStockMasterID=DamageStockMasterID, BranchID=BranchID)
        serialized = DamageStockDetailSerializer(details, many=True, context={
                                                 "CompanyID": CompanyID, "PriceRounding": PriceRounding})
...
class DamageStockDetailSerializer(serializers.ModelSerializer):
    ...
    def get_DamageStock(self, details):
        ...
        return converted_float(DamageStock)
...
class UsedStockMasterSerializer(serializers.ModelSerializer):
    ...
    def get_Details(self, instances):
        ...
        BranchID = instances.BranchID
        details = UsedStockDetails.objects.filter(
            CompanyID=CompanyID, UsedStockMasterID=UsedStockMasterID, BranchID=BranchID)
        serialized = UsedStockDetailSerializer(details, many=True, context={
                                               "CompanyID": CompanyID, "PriceRounding": PriceRounding})
...
class UsedStockDetailSerializer(serializers.ModelSerializer):
    ...
    def get_UsedStock(self, details):
        ...
        return converted_float(UsedStock)
