...
from brands.models import ExpenseMaster,ExpenseDetails,TaxCategory
...
def expence_excel_data(CompanyID,BranchID,FromDate,ToDate,PriceRounding,CashLedgers,):
    ...
    if ExpenseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate).exists():
        ...
        tot_dictionary = {"VoucherNo": "Total", "Date": "", "LedgerName": "",
                            "TaxType": "", "IGSTAmount": tot_IGSTAmount,"SGSTAmount": tot_SGSTAmount,"CGSTAmount": tot_CGSTAmount,"Amount": tot_Amount, "Discount": tot_Discount, "NetAmount": tot_NetAmount, "Narration": ""}
...
def export_to_excel_expence(wb, data, FromDate, ToDate, title,columns):
    ...
    for j in data_list:
        ...
        ws.write(data_row, 4, converted_float(VATAmount), value_decimal_style)
        ws.write(data_row, 5, converted_float(IGSTAmount), value_decimal_style)
        ws.write(data_row, 6, converted_float(SGSTAmount), value_decimal_style)
        ws.write(data_row, 7, converted_float(CGSTAmount), value_decimal_style)
        ws.write(data_row, 8, converted_float(Amount), value_decimal_style)
