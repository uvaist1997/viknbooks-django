...
from brands.models import AccountLedger, Batch,SalesMaster,SalesDetails,PurchaseMaster,PurchaseDetails
...
def get_LastSalesPrice_PurchasePrice(Type,Price,CompanyID,BranchID,LedgerID,ProductID):
        ...
        if Type == "Sales":
            if ShowCustomerLastSalesPriceinSales == True and check_EnableProductBatchWise == True:
                if SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
                    ...
                    if SalesDetails.objects.filter(CompanyID=CompanyID,SalesMasterID__in=sales_master_ids,ProductID=ProductID,).exists():
                ...
                if SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
                    ...
                    if SalesDetails.objects.filter(CompanyID=CompanyID,SalesMasterID__in=sales_master_ids,ProductID=ProductID,).exists():
                        ...
                        print(sales_product_details.UnitPrice,"HIIIIIIII")
                        if sales_product_details.UnitPrice > 0:
                            Price = sales_product_details.UnitPrice
