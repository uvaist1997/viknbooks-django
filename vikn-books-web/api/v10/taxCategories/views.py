...
from api.v10.taxCategories.functions import generate_serializer_errors, get_auto_id
from api.v10.taxCategories.serializers import (
    ListSerializerTaxType,
    ListTaxTypeSerializer,
    TaxCategoryRestSerializer,
    ...
    if serialized.is_valid():
        ...
        if not is_nameExist:
            ...
            activity_log(
                ...
                "TaxCategory",
                "Create",
                "TaxCategory created successfully.",
                "TaxCategory saved successfully.",
            )
            ...
            activity_log(
                ...
                "TaxCategory",
                "Create",
                "TaxCategory created Failed.",
                "TaxCategory Name Already Exist.",
            )
            response_data = {
                "StatusCode": 6001,
    ...
    if serialized.is_valid():
        ...
        if tax_ok:
            ...
            activity_log(
                ...
                "TaxCategory",
                "Edit",
                "TaxCategory updated Successfully.",
                "TaxCategory updated Successfully.",
            )
            ...
            if is_nameExist:
                ...
                activity_log(
                    ...
                    "TaxCategory",
                    "Edit",
                    "TaxCategory updated Failed.",
                    "TaxCategory Name Already exist with this Branch",
                )
                response_data = {
                    "StatusCode": 6001,
    ...
    if serialized1.is_valid():
        ...
        if TaxCategory.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            ...
            check_TAX1 = GeneralSettings.objects.get(
                BranchID=BranchID, CompanyID=CompanyID, SettingsType="TAX1"
            ).SettingsValue
            check_TAX2 = GeneralSettings.objects.get(
                BranchID=BranchID, CompanyID=CompanyID, SettingsType="TAX2"
            ).SettingsValue
            check_TAX3 = GeneralSettings.objects.get(
                BranchID=BranchID, CompanyID=CompanyID, SettingsType="TAX3"
            ).SettingsValue
            ...
            activity_log(
                ...
                "TaxCategory",
                "List",
                "TaxCategory List Viewed Successfully.",
                "TaxCategory List Viewed Successfully",
            )
            response_data = {"StatusCode": 6000, "data": serialized.data}
            ...
            activity_log(
                ...
                "TaxCategory",
                "List",
                "TaxCategory List Viewed Failed.",
                "Tax Category Not Found in this Branch",
            )
...
def taxCategory(request, pk):
    ...
    if TaxCategory.objects.filter(pk=pk, CompanyID=CompanyID).exists():
        ...
        activity_log(
            ...
            "TaxCategory",
            "View",
            "TaxCategory Single Viewed Successfully.",
            "TaxCategory Single Viewed Successfully.",
        )
        ...
        activity_log(
            ...
            "TaxCategory",
            "View",
            "TaxCategory Single Viewed Failed.",
            "TaxCategory Not Found",
        )
        response_data = {"StatusCode": 6001, "message": "TaxCategory Not Found"}
    return Response(response_data, status=status.HTTP_200_OK)
...
def delete_taxCategory(request, pk):
    ...
    if pk == "1":
        ...
        activity_log(
            ...
            "TaxCategory",
            "Delete",
            "TaxCategory Deleted Failed.",
            "User Tried to Delete Default TaxCategory",
        )
        response_data = {
            "StatusCode": 6001,
        ...
        if TaxCategory.objects.filter(pk=pk, CompanyID=CompanyID).exists():
            ...
            if Product.objects.filter(
                ...
                CompanyID=CompanyID, BranchID=BranchID, Tax1=TaxID
            ).exists():
                is_productExist = True
            elif Product.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, Tax2=TaxID
            ).exists():
                is_productExist = True
            elif Product.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, Tax3=TaxID
            ).exists():
                is_productExist = True

            posholdDetails_exist = POSHoldDetails.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, TaxID=TaxID
            ).exists()
            ...
            if (
                not posholdDetails_exist
                and not purchaseDetails_exist
                and not purchaseOrderDetails_exist
                and not purchaseReturnDetails_exist
                and not salesDetails_exist
                and not salesOrderDetails_exist
                and not salesReturnDetails_exist
                and is_productExist == False
            ):
                instance.delete()
                ...
                activity_log(
                    ...
                    "TaxCategory",
                    "Delete",
                    "TaxCategory Deleted Successfully.",
                    "TaxCategory Deleted Successfully",
                )
                response_data = {
                    "StatusCode": 6000,
                ...
                activity_log(
                    ...
                    "Delete",
                    "TaxCategory Deleted Failed.",
                    "can't Delete this TaxCategory,this TaxID is using somewhere",
                )
                response_data = {
                    "StatusCode": 6001,
                    "title": "Failed",
...
def taxListByType(request):
    ...
    if serialized1.is_valid():
        ...
        if (
            ...
            tax_category_list = TaxCategory.objects.filter(
                CompanyID=CompanyID, TaxType__in=taxArr
            )
        else:
            tax_category_list = TaxCategory.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, TaxType__in=taxArr
            )
        if tax_category_list.exists():
            TaxCategoryInstance = tax_category_list
            print(TaxCategoryInstance)
...
def listTaxType(request):
    ...
    if serialized1.is_valid():
        ...
        if TaxType.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            ...
            print(">>>>>>>>>>>>>>>>>>>>>>>>>")
            print(check_GST)
            if check_VAT == "false" or check_VAT == "False":
                checkTax.append("VAT")
            if check_GST == "false" or check_GST == "False":
                checkTax.append("GST")
            if check_TAX1 == "false":
            ...
            instances = TaxType.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID
            ).exclude(TaxTypeName__in=checkTax)
...
@authentication_classes((JWTTokenUserAuthentication,))
@renderer_classes((JSONRenderer,))
def setDefault_taxCategory(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    BranchID = data["BranchID"]
    ...
    value = data["value"]
    StatusCode = 6001
    if TaxCategory.objects.filter(id=id).exists():
        TaxCategory.objects.filter(CompanyID=CompanyID).update(IsDefault=False)
        if value == True:
            TaxCategory.objects.filter(id=id).update(IsDefault=True)
        else:
            TaxCategory.objects.filter(CompanyID=CompanyID, TaxID=1).update(
                IsDefault=True
            )
        StatusCode = 6000
...
@authentication_classes((JWTTokenUserAuthentication,))
@renderer_classes((JSONRenderer,))
def tax_conversion(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    BranchID = data["BranchID"]
    TaxTypeFilter = data["TaxTypeFilter"]
    TaxID = data["TaxID"]
    ProductGroupID = data["ProductGroupID"]
    ProductID = data["ProductID"]
    all_product = data["all_product"]
    product_wise = data["product_wise"]
    group_wise = data["group_wise"]
    dropDwn_type = data["dropDwn_type"]
    FromTaxID = data["FromTaxID"]
    ToTaxID = data["ToTaxID"]

    is_inclusive = False
    if TaxTypeFilter == "inclusive":
        is_inclusive = True

    check_VAT = get_GeneralSettings(CompanyID, BranchID, "VAT")
    check_GST = get_GeneralSettings(CompanyID, BranchID, "GST")
    ...
    if Product.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        ...
        if dropDwn_type == "0":
            if product_wise:
                product_instances = product_instances.filter(ProductID=ProductID)
            elif group_wise:
            ...
            if product_instances:
                if check_VAT:
                    product_instances.update(VatID=TaxID)
            elif check_GST:
                product_instances.update(GST=TaxID)

        else:
            if FromTaxID and ToTaxID:
                if check_VAT:
                    product_instances.filter(VatID=FromTaxID).update(VatID=ToTaxID)
                elif check_GST:
                    product_instances.filter(GST=FromTaxID).update(GST=ToTaxID)

        if not TaxTypeFilter == "no_change":
            product_instances.update(is_inclusive=is_inclusive)
