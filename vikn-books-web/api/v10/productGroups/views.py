from brands.models import BranchSettings, Kitchen, ProductGroup, ProductGroup_Log, ProductCategory, Product, StockAdjustmentMaster
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes,authentication_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from rest_framework_simplejwt.authentication import JWTTokenUserAuthentication
from api.v10.productGroups.serializers import ProductGroupSerializer, ProductGroupRestSerializer
from api.v10.brands.serializers import ListSerializer
from api.v10.productGroups.functions import generate_serializer_errors
...
def productGroups(request):
    ...
    if serialized1.is_valid():
        ...
        if productsGroups.exists():
            if page_number and items_per_page:
                instances = productsGroups
                if CategoryID > 0:
                ...
                ledger_sort_pagination = productsGroups
                if CategoryID > 0:
                    ledger_sort_pagination = ledger_sort_pagination.filter(
                        CategoryID=CategoryID)
...
def delete_productGroup(request, pk):
    ...
    if selecte_ids:
        if ProductGroup.objects.filter(CompanyID=CompanyID, pk__in=selecte_ids).exists():
            instances = ProductGroup.objects.filter(pk__in=selecte_ids)
    else:
        if ProductGroup.objects.filter(CompanyID=CompanyID, pk=pk).exists():
            instances = ProductGroup.objects.filter(pk=pk)
        ...
        if instances:
            for instance in instances:
                ...
                if check_productsForAllBranches == True or check_productsForAllBranches == "True":
                    ...
                    stock_adjustment_list = StockAdjustmentMaster.objects.filter(
                        CompanyID=CompanyID, ProductGroupID=ProductGroupID)
                else:
                    products_list = Product.objects.filter(
                ...
                product_exist = products_list.exists()
                StockAdjustmentMaster_exist = stock_adjustment_list.exists()
...
@authentication_classes((JWTTokenUserAuthentication,))
@renderer_classes((JSONRenderer,))
def setDefault_productGroups(request):
    data = request.data
    CompanyID = data['CompanyID']
    CompanyID = get_company(CompanyID)
    ...
    value = data['value']
    StatusCode = 6001
    if ProductGroup.objects.filter(id=id).exists():
        ProductGroup.objects.filter(CompanyID=CompanyID).update(IsDefault=False)
        if value == True:
            ProductGroup.objects.filter(id=id).update(IsDefault=True)
        else:
            ProductGroup.objects.filter(CompanyID=CompanyID,ProductGroupID=1).update(IsDefault=True)
        StatusCode = 6000
...
@authentication_classes((JWTTokenUserAuthentication,))
@renderer_classes((JSONRenderer,))
def search_productGroup_list(request):
    data = request.data
    CompanyID = data['CompanyID']
    CompanyID = get_company(CompanyID)
    CreatedUserID = data['CreatedUserID']
    ...
    if serialized1.is_valid():
        ...
        if ProductGroup.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            cat_ids = []
            if ProductCategory.objects.filter(CompanyID=CompanyID, BranchID=BranchID, CategoryName__icontains=product_name).exists():
                cat_ins = ProductCategory.objects.filter(
                    CompanyID=CompanyID, BranchID=BranchID, CategoryName__icontains=product_name)
                cat_ids = cat_ins.values_list("ProductCategoryID",flat=True)
            if length < 3:
