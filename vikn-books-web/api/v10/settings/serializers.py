...
class PrintSettingsSerializer(serializers.ModelSerializer):
    ...
    IBANCode = serializers.SerializerMethodField()
    SwiftCode = serializers.SerializerMethodField()
    Notes = serializers.SerializerMethodField()
    SignatureName = serializers.SerializerMethodField()

    TransactionName = serializers.SerializerMethodField()
    TermsAndConditions = serializers.SerializerMethodField()
    UploadDigitalSignFile = serializers.SerializerMethodField()
    UploadSealFile = serializers.SerializerMethodField()

    class Meta:
        model = PrintSettingsNew
        fields = "__all__"

    def get_UploadDigitalSignFile(self, instance):
        DigitalSignFile = ""
        if instance.DigitalSignFile:
            DigitalSignFile = instance.DigitalSignFile.url
        return DigitalSignFile

    def get_UploadSealFile(self, instance):
        SealFile = ""
        if instance.SealFile:
            SealFile = instance.SealFile.url
        return SealFile
    ...
    def get_AccountNumber(self, instance):
        AccountNumber = ""
        if instance.AccountNumber:
            AccountNumber = instance.AccountNumber
        return AccountNumber

    def get_IBANCode(self, instance):
        IBANCode = ""
        if instance.IBANCode:
            IBANCode = instance.IBANCode
        return IBANCode

    def get_SwiftCode(self, instance):
        SwiftCode = ""
        if instance.SwiftCode:
    ...
    def get_SignatureName(self, instance):
        SignatureName = ""
        if instance.SignatureName:
...
class UserTypeSettingsSerializer(serializers.Serializer):
    ...
    PurchaseIntegratedReportPrint = serializers.BooleanField()
    PurchaseIntegratedReportView = serializers.BooleanField()
    PurchaseIntegratedReportExport = serializers.BooleanField()
