...
from api.v10.openingStockDetails.serializers import OpeningStockDetailsSerializer, OpeningStockDetailsRestSerializer, OpeningStockDetailsDummySerializer, OpeningStockDetailsDeletedDummySerializer
from api.v10.brands.serializers import ListSerializer
from api.v10.openingStockDetails.functions import generate_serializer_errors
from rest_framework import status
from api.v10.openingStockDetails.functions import get_auto_id
import datetime
...
def create_openingStockDetails(request):
    ...
    if serialized.is_valid():
        ...
        OpeningStockDetailsID = get_auto_id(
            OpeningStockDetails, BranchID, DataBase)
...
def list_openingStockDetailss(request):
    ...
    if serialized1.is_valid():
        ...
        if OpeningStockDetails.objects.filter(BranchID=BranchID).exists():
            ...
            serialized = OpeningStockDetailsRestSerializer(
                instances, many=True)
...
def list_openingStockDetailsDummy(request):
    ...
    if serialized1.is_valid():
        ...
        if OpeningStockDetailsDummy.objects.filter(BranchID=BranchID).exists():
            TotalAmount = 0
            TotalQty = 0
            instances = OpeningStockDetailsDummy.objects.filter(
                BranchID=BranchID, IsDeleted=False)
            for i in instances:
                TotalAmount += i.Amount
                TotalQty += i.Qty
            deleted_instances = OpeningStockDetailsDummy.objects.filter(
                BranchID=BranchID, IsDeleted=True)

            serialized = OpeningStockDetailsDummySerializer(
                instances, many=True, context={"DataBase": DataBase})
            serializedDeleted = OpeningStockDetailsDeletedDummySerializer(
