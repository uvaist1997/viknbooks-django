...
from api.v10.salesReturnDetails.serializers import SalesReturnDetailsSerializer, SalesReturnDetailsRestSerializer, SalesReturnDetailsDummySerializer, SalesReturnDetailsDeletedDummySerializer
from api.v10.brands.serializers import ListSerializer
from api.v10.salesReturnDetails.functions import generate_serializer_errors
from rest_framework import status
from api.v10.salesReturnDetails.functions import get_auto_id
import datetime
...
def list_salesReturnDetailsDummy(request):
    ...
    if serializedList.is_valid():
        ...
        if SalesReturnDetailsDummy.objects.filter(BranchID=BranchID).exists():
            ...
            instances = SalesReturnDetailsDummy.objects.filter(
                BranchID=BranchID, IsDeleted=False)
            deleted_instances = SalesReturnDetailsDummy.objects.filter(
            ...
            serialized = SalesReturnDetailsDummySerializer(
                instances, many=True, context={"DataBase": DataBase})
            serializedDeleted = SalesReturnDetailsDeletedDummySerializer(
