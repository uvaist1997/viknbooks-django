...
from api.v10.purchaseOrders.functions import (
    generate_serializer_errors,
    get_auto_id,
    get_auto_idMaster,
)
from api.v10.purchaseOrders.serializers import (
    PurchaseOrderDetailsRestSerializer,
    PurchaseOrderDetailsSerializer,
    PurchaseOrderMaster1RestSerializer,
    PurchaseOrderMasterRestSerializer,
...
from brands.models import (
    ...
    PurchaseOrderDetails_Log,
    PurchaseOrderDetailsDummy,
    PurchaseOrderMaster,
    PurchaseOrderMaster_Log,
    VoucherNoTable,
)
from main.functions import activity_log, converted_float, get_company, update_voucher_table
...
def create_purchaseOrder(request):
    try:
        with transaction.atomic():
            ...
            insts = PurchaseOrderMaster.objects.filter(
            ...
            if is_PurchaseOrderOK:
                ...
                instance = PurchaseOrderMaster.objects.create(
                    CompanyID=CompanyID,
                ...
                for purchaseOrderDetail in purchaseOrderDetails:
                    ...
                    if ProductID:
                        ...
                        try:
                            ProductTaxID = purchaseOrderDetail["ProductTaxID"]
                        except:
                            ProductTaxID = ""
...
def edit_purchaseOrder(request, pk):
    try:
        with transaction.atomic():
            ...
            purchaseOrderMaster_instance.BillDiscPercent = BillDiscPercent
            purchaseOrderMaster_instance.BillDiscAmt = BillDiscAmt
            purchaseOrderMaster_instance.ShippingCharge = ShippingCharge
            purchaseOrderMaster_instance.shipping_tax_amount = shipping_tax_amount
            purchaseOrderMaster_instance.TaxTypeID = TaxTypeID
            purchaseOrderMaster_instance.SAC = SAC
            purchaseOrderMaster_instance.PurchaseTax = PurchaseTax
            purchaseOrderMaster_instance.Country_of_Supply = Country_of_Supply
            purchaseOrderMaster_instance.State_of_Supply = State_of_Supply
            purchaseOrderMaster_instance.GST_Treatment = GST_Treatment
            purchaseOrderMaster_instance.VAT_Treatment = VAT_Treatment
            purchaseOrderMaster_instance.UpdatedUserID = CreatedUserID
            purchaseOrderMaster_instance.TaxTaxableAmount = TaxTaxableAmount
            purchaseOrderMaster_instance.NonTaxTaxableAmount = NonTaxTaxableAmount
            purchaseOrderMaster_instance.TotalDiscount = TotalDiscount
            purchaseOrderMaster_instance.TotalGrossAmt = TotalGrossAmt
            ...
            if deleted_datas:
                for deleted_Data in deleted_datas:
                    ...
                    if not deleted_pk == "" or not deleted_pk == 0:
                        if PurchaseOrderDetails.objects.filter(
                ...
                if ProductID:
                    ...
                    if detailID == 0:
                        ...
                        PurchaseOrderDetailsID = (
                            purchaseDetail_instance.PurchaseOrderDetailsID
                        )
                        ...
                        purchaseDetail_instance.Description = Description
...
def delete_purchaseOrderMaster(request, pk):
    ...
    if selecte_ids:
        if PurchaseOrderMaster.objects.filter(
            ...
            instances = PurchaseOrderMaster.objects.filter(pk__in=selecte_ids)
    else:
        if PurchaseOrderMaster.objects.filter(CompanyID=CompanyID, pk=pk).exists():
            instances = PurchaseOrderMaster.objects.filter(pk=pk)
    ...
    if serialized1.is_valid():
        ...
        if page_number and items_per_page:
            ...
            if is_filter == True:
                ...
                purchase_object = purchase_object.filter(IsInvoiced__in=is_invoicedList)
                if purchase_object:
                    is_data_exist = True
            else:
                if filterID == "Pending":
            ...
            sale_serializer = PurchaseOrderMaster1RestSerializer(
...
def report_purchasesOrder(request):
    ...
    if PurchaseOrderMaster.objects.filter(
        ...
        serialized = PurchaseOrderReportSerializer(
            instance,
            many=True,
            context={
                "CompanyID": CompanyID,
...
@authentication_classes((JWTTokenUserAuthentication,))
@renderer_classes((JSONRenderer,))
def search_purchaseOrder_list(request):
    data = request.data
    CompanyID = data["CompanyID"]
    CompanyID = get_company(CompanyID)
    PriceRounding = data["PriceRounding"]
    ...
    if serialized1.is_valid():
        ...
        if PurchaseOrderMaster.objects.filter(
            ...
            if length < 3:
                ...
                instances = PurchaseOrderMaster.objects.filter(
                    CompanyID=CompanyID, BranchID=BranchID
                )
            ...
            serialized = PurchaseOrderMaster1RestSerializer(
                instances,
                many=True,
                context={
                    "request": request,
