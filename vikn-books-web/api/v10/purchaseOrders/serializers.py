...
from main.functions import converted_float, get_GeneralSettings, get_LedgerBalance, get_ProductStock
...
class PurchaseOrderMasterRestSerializer(serializers.ModelSerializer):
    ...
    def get_PurchaseOrderDetails(self, instances):
        ...
        purchaseOrder_details = PurchaseOrderDetails.objects.filter(
            CompanyID=CompanyID,
            PurchaseOrderMasterID=instances.PurchaseOrderMasterID,
            BranchID=instances.BranchID,
        ).order_by("PurchaseOrderDetailsID")
    ...
    def get_Balance(self, instances):
        ...
        PriceRounding = int(self.context.get("PriceRounding"))
        LedgerID = instances.LedgerID
        Balance = get_LedgerBalance(CompanyID, LedgerID)
    ...
    def get_ProductList(self, instances):
        ...
        WarehouseID = 1
        PurchaseOrderMasterID = instances.PurchaseOrderMasterID
        purchase_details = PurchaseOrderDetails.objects.filter(
            CompanyID=CompanyID,
...
class PurchaseOrderDetailsRestSerializer(serializers.ModelSerializer):
    ...
    def get_DiscountAmount_print(self, purchase_details):
        PriceRounding = self.context.get("PriceRounding")
    ...
    def get_ExpiryDate(self, purchase_details):
        ...
        one_year_delta = dt.timedelta(
            days=366
            if (
                (day.month >= 3 and calendar.isleap(day.year + 1))
                or (day.month < 3 and calendar.isleap(day.year))
            )
            else 365
        )
