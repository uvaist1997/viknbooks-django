...
from api.v10.accountLedgers.functions import (
...
from api.v10.banks.functions import get_auto_Bankid
...
from main.functions import activity_log, converted_float, get_BalanceFromLedgerPost, get_company, get_today
    ...
    if serialized1.is_valid():
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID).exists():
            ...
            if UserTable.objects.filter(
                ...
                user_table_ins = UserTable.objects.get(CompanyID=CompanyID, customer__user__pk=CreatedUserID, Active=True)
                show_all_accounts = user_table_ins.show_all_accounts
                Cash_Account = user_table_ins.Cash_Account
                Bank_Account = user_table_ins.Bank_Account
    ...
    if serialized1.is_valid():
        ...
        if AccountLedger.objects.filter(
            ...
            if Parties.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID).exists():
                ...
                RouteID = pary_ins.RouteID
                if RouteID and Route.objects.filter(CompanyID=CompanyID, RouteID=RouteID).exists():
                    RouteName = Route.objects.filter(
                        CompanyID=CompanyID, RouteID=RouteID).first().RouteName
            ...
            #     DueDate = DueDate + datetime.timedelta(days=CreditPeriod)
            
            DueDate = date.today()
            if converted_float(CreditPeriod) > 0:
                DueDate = date.today() + timedelta(days=CreditPeriod)
            final_dict = {
                "id": id,
                "BranchID": BranchID,
                "LedgerName": LedgerName,
