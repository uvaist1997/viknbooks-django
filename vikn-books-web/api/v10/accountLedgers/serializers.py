...
class AccountLedgerRestSerializer(serializers.ModelSerializer):
    ...
    def get_as_on_date(self, instances):
        as_on_date = instances.as_on_date
        OpeningBalance = instances.OpeningBalance
        if not as_on_date or OpeningBalance == 0:
            as_on_date = datetime.datetime.now().date()
...
class AccountLedgerListSerializer(serializers.ModelSerializer):
    ...
    def get_LoyaltyCustomerID(self, instances):
        ...
        LoyaltyCustomerID = ""
        if LoyaltyCustomer.objects.filter(AccountLedgerID=instances,CompanyID=CompanyID).exists():
            LoyaltyCustomerID = LoyaltyCustomer.objects.get(AccountLedgerID=instances,CompanyID=CompanyID).LoyaltyCustomerID
        print(LoyaltyCustomerID,'LoyaltyCustomerID',instances.LedgerName,CompanyID,BranchID,"*******(((((((()))))))))))")
    ...
    def get_name(self, instances):
        CompanyID = self.context.get("CompanyID")
        is_phone_search = self.context.get("is_phone_search")
        name = instances.LedgerName
        LedgerID = instances.LedgerID
        if is_phone_search == True:
            if Parties.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID).exists():
                phone = (
                    Parties.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID)
                    .first()
                    .WorkPhone
                )
                name = str(name) + str("-") + str(phone)
        return str(name)

    def get_State_id(self, instances):
        CompanyID = self.context.get("CompanyID")
        AccountGroupUnder = instances.AccountGroupUnder
        State_id = ""
    ...
    def get_GST_Treatment(self, instances):
        ...
        if Parties.objects.filter(
            ...
            GST_Treatment = Parties.objects.get(
                LedgerID=instances.LedgerID, CompanyID=CompanyID
            ).GST_Treatment
    ...
    def get_CustomerBalance(self, instances):
        ...
        CrOrDr = instances.CrOrDr
        if CrOrDr == "Cr":
            OpeningBalance = converted_float(OpeningBalance) * -1
    ...
    def get_CustomerLedgerBalance(self, instance):
        CompanyID = self.context.get("CompanyID")
        PriceRounding = self.context.get("PriceRounding")
        ...
        if LedgerPosting.objects.filter(
            ...
            Balance = converted_float(debit_sum) - converted_float(credit_sum)
        return Balance
...
class ListSerializerforPaymentAll(serializers.Serializer):
    BranchID = serializers.IntegerField()
