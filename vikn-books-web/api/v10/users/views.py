...
from api.v10.users.functions import (
...
from api.v10.users.serializers import (
...
from main.functions import (
    CheckTokenExpired,
    activity_log,
    company_insert_query_to_accounts,
    generateTokenNo,
    get_GeneralSettings,
    get_company,
...
def create_user(request):
    ...
    if serialized.is_valid():
        ...
        users = UserTable.objects.filter(CompanyID=CompanyID, Active=True)
        for user in users:
...
def edit_user(request, pk):
    ...
    today = datetime.datetime.now()
    serialized = UserTableSerializer(data=request.data)
    instance = UserTable.objects.get(pk=pk, CompanyID=CompanyID, Active=True)
...
def users(request):
    ...
    if serialized1.is_valid():
        ...
        ShowInactiveUsers = get_GeneralSettings(
            CompanyID, BranchID, "ShowInactiveUsers")
        active = [True]
        if ShowInactiveUsers:
            active.append(False)
        if UserTable.objects.filter(CompanyID=CompanyID, Active__in=active).exists():
            instances = UserTable.objects.filter(
                CompanyID=CompanyID, Active__in=active)
...
def user(request, pk):
    ...
    if UserTable.objects.filter(CompanyID=CompanyID, pk=pk, Active=True).exists():
        instance = UserTable.objects.get(
            CompanyID=CompanyID, pk=pk, Active=True)
    if instance:
...
def delete_user(request, pk):
    ...
    instance = None
    if UserTable.objects.filter(CompanyID=CompanyID, pk=pk,Active=True).exists():
        instance = UserTable.objects.get(CompanyID=CompanyID, pk=pk,Active=True)
    if instance:
        BranchID = instance.BranchID
        ID = instance.ID
            ...
            if InviteUsers.objects.filter(Email=email).exists():
                ...
                if not Customer.objects.filter(user=user).exists():
                    ...
                    user_table = UserTable.objects.create(
                        ...
                        show_all_accounts=invited_user.show_all_accounts,
                    )
...
def companies(request):
    ...
    if accesstoken and userId:
        Customer.objects.filter(user__id=userId).update(LastLoginToken=accesstoken)
    print(userId)
    ...
    if is_mobile == True:
        member_company_instances = models.UserTable.objects.filter(
            customer__user=userId, is_mobile=True, CompanyID__is_deleted=False,Active=True
        )
    else:
        member_company_instances = models.UserTable.objects.filter(
            customer__user=userId, is_web=True, CompanyID__is_deleted=False, Active=True
        )
...
def create_company_user(request, pk):
    ...
    if serialized.is_valid():
        ...
        if password1 == password2:
            if UserTable.objects.filter(
                CompanyID__Email=email, CompanyID=CompanyID, Active=True
            ).exists():
                error = True
                message += "This email already exists."
            elif UserTable.objects.filter(
                UserName=username, CompanyID=CompanyID, Active=True
            ).exists():
                error = True
                message += "This username already exists."
...
def create_general_settings(request):
    ...
    if serialized.is_valid():
        ...
        try:
            ShowInactiveUsers = data["ShowInactiveUsers"]
        except:
        ...
        try:
            EnableBillwise = data["EnableBillwise"]
        except:
        ...
        try:
            loyalty_PointValue = data["loyalty_PointValue"]
        except:
            loyalty_PointValue = ""
        # inventory => general
        CreateBatchForWorkOrder = serialized.data["CreateBatchForWorkOrder"]
        try:
            ShowAllProductsInWorkOrder = data["ShowAllProductsInWorkOrder"]
        except:
            ShowAllProductsInWorkOrder = False
            
        EnableProductBatchWise = serialized.data["EnableProductBatchWise"]
        try:
            EnableDuplicateProductName = data["EnableDuplicateProductName"]
        except:
            EnableDuplicateProductName = False
        try:
            EnableWarehouse = data["EnableWarehouse"]
        except:
            EnableWarehouse = False
            
        try:
            EnableCreditPeriod = data["EnableCreditPeriod"]
        except:
            EnableCreditPeriod = False
            
        try:
            show_productcode_InsearchSale = data["show_productcode_InsearchSale"]
        except:
            show_productcode_InsearchSale = False
        try:
            show_productcode_InsearchPurchase = data[
        ...
        try:
            show_stock_InsearchSale = data["show_stock_InsearchSale"]
        except:
            show_stock_InsearchSale = False
            
        try:
            show_stock_InsearchPurchase = data["show_stock_InsearchPurchase"]
        except:
            show_stock_InsearchPurchase = False
            
        try:
            show_purchasePrice_InsearchSale = data[
        ...
        try:
            show_salesPrice_InsearchPurchase = data[
        ...
        try:
            AllowNegativeStockInStockTransfer = data[
        ...
        try:
            BlockSalesBelowPurchase = data["BlockSalesBelowPurchase"]
        except:
            BlockSalesBelowPurchase = False
        try:
            ShowProfitinSalesRegisterReport = data[
                "ShowProfitinSalesRegisterReport"
            ]
        except:
            ShowProfitinSalesRegisterReport = False
        try:
            ShowCustomerLastSalesPriceinSales = data[
                "ShowCustomerLastSalesPriceinSales"
            ]
        except:
            ShowCustomerLastSalesPriceinSales = False
        try:
            ShowSupplierLastPurchasePriceinPurchase = data[
        ...
        try:
            EnableCreditLimit = data["EnableCreditLimit"]
        except:
            EnableCreditLimit = False
        try:
            EnableFaeraSettings = data["EnableFaeraSettings"]
        except:
        ...
        try:
            ShowMaximumStockAlert = data["ShowMaximumStockAlert"]
        except:
        ...
        try:
            CustomerBasedPrint = data["CustomerBasedPrint"]
        except:
...
def general_settings_list(request):
    ...
    if administrations_models.GeneralSettings.objects.filter(
        ...
        ShowInactiveUsers = get_general_settings(
            BranchID, CompanyID, "ShowInactiveUsers"
        )
...
def get_company_details(request):
    ...
    if Customer.objects.filter(user__pk=userId).exists() and (
        ...
        or UserTable.objects.filter(customer__user=userId, Active=True).exists()
    ):
        ...
        elif UserTable.objects.filter(customer__user=userId, Active=True).exists():
            LastLoginCompanyID = (
                UserTable.objects.filter(customer__user=userId,Active=True).last().CompanyID
            )
...
def get_user_table(request):
    ...
    if UserTable.objects.filter(
        CompanyID__pk=CompanyID, customer__user=userId, Active=True
    ).exists():
        user_table_instance = UserTable.objects.get(
            CompanyID__pk=CompanyID, customer__user=userId,Active=True
        )
        DefaultAccountForUser = UserTable.objects.get(
            CompanyID=CompanyID, customer__user__pk=userId, Active=True
...
def get_defaults(request):
    ...
    IsTrialVersion = company.IsTrialVersion
    Edition = company.Edition
    # usertable exceed delete
    NoOfUsers = int(company.NoOfUsers)
    VATNumber = company.VATNumber
    GSTNumber = company.GSTNumber
    Phone = company.Phone
    EnableZatca = company.EnableZatca
    usertable_count = UserTable.objects.filter(
        CompanyID=CompanyID, Active=True).count()
    if usertable_count > NoOfUsers:
        corrent_user = usertable_count - NoOfUsers
        last_user_table_instance = UserTable.objects.filter(CompanyID=CompanyID, Active=True)[
            corrent_user:
        ]
        UserTable.objects.exclude(
            CompanyID=CompanyID, pk__in=last_user_table_instance, Active=True
        ).delete()
    ...
    if UserTable.objects.filter(CompanyID=CompanyID, customer__user__pk=userId,Active=True).exists():
        user_table_ins = UserTable.objects.get(CompanyID=CompanyID, customer__user__pk=userId,Active=True)
        WarehouseID = user_table_ins.DefaultWarehouse
        show_all_warehouse = user_table_ins.show_all_warehouse
        show_all_accounts = user_table_ins.show_all_accounts
    # =============Uvais================
    # ===========VAT======
    vat = False
    gst = False
...
def password_reset_token_created(
    ...
    context = {
        ...
        "email": reset_password_token.user.email,
        'reset_password_url': getUrl("password_confirm"),
...
def user_invite(request):
    ...
    if serialized.is_valid():
        ...
        try:
            show_all_employees = data["show_all_employees"]
        except:
        ...
        try:
            show_all_accounts = data["show_all_accounts"]
        except:
            show_all_accounts = False
        ...
        if User.objects.filter(id=CreatedUserID).exists():
            invited_user = User.objects.get(id=CreatedUserID)
        message = ""
        error = False
        ...
        if UserTable.objects.filter(CompanyID=CompanyID, customer=customer,Active=False).exists():
            is_create = False
        elif UserTable.objects.filter(CompanyID=CompanyID, customer=customer, Active=True).exists():
            is_active_exist = True
        if is_edit == True or usertable_count < NoOfUsers:
            if User.objects.filter(email__iexact=email, is_active=True):
                customer = None
                if is_active_exist and is_edit == False:
                    response_data = {"StatusCode": 6001, "message": "user already added with this email"}
                    return Response(response_data, status=status.HTTP_200_OK)
                
                if is_create:
                    send_mail_invitaion(email, request,"existedUser",userType,CompanyID,invited_user)
                if Customer.objects.filter(user__email=email).exists():
                    customer = Customer.objects.filter(user__email=email).first()
                ...
                if UserTable.objects.filter(CompanyID=CompanyID, customer=customer).exists():
                    user_ins = UserTable.objects.filter(
                        CompanyID=CompanyID, customer=customer).first()
                    user_ins.UpdatedDate = today
                    user_ins.UserType = userType
                    user_ins.ExpiryDate = ExpiryDate
                    user_ins.DefaultAccountForUser = DefaultAccountForUser
                    user_ins.Cash_Account = Cash_Account
                    user_ins.Bank_Account = Bank_Account
                    user_ins.Sales_Account = Sales_Account
                    user_ins.Sales_Return_Account = Sales_Return_Account
                    user_ins.Purchase_Account = Purchase_Account
                    user_ins.Purchase_Return_Account = Purchase_Return_Account
                    user_ins.Action = "M"
                    user_ins.is_web = is_web
                    user_ins.is_mobile = is_mobile
                    user_ins.is_pos = is_pos
                    user_ins.BranchID = BranchID
                    user_ins.show_all_warehouse = show_all_warehouse
                    user_ins.DefaultWarehouse = DefaultWarehouse
                    user_ins.show_all_employees = show_all_employees
                    user_ins.EmployeeID = EmployeeID
                    user_ins.show_all_accounts = show_all_accounts
                    # user_ins.Active = not user_ins.Active
                    
                    # UserTable.objects.filter(
                    #     CompanyID=CompanyID, customer=customer, Active=True
                    # ).update(
                    #     UpdatedDate=today,
                    #     UserType=userType,
                    #     ExpiryDate=ExpiryDate,
                    ...
                    User_Log.objects.create(
                        ...
                        show_all_warehouse=show_all_warehouse,
                        DefaultWarehouse=DefaultWarehouse,
                        Active=user_ins.Active,
                        show_all_employees=show_all_employees,
                        EmployeeID=EmployeeID,
                        show_all_accounts=show_all_accounts,
                    )
                    user_ins.save()
                ...
                send_mail_invitaion(email, request,"NonexistedUser",userType,CompanyID,invited_user)
                ...
                if InviteUsers.objects.filter(
                    ...
                    invited_users.show_all_employees = show_all_employees
                    invited_users.EmployeeID = EmployeeID
                    invited_users.show_all_accounts = show_all_accounts
                    invited_users.save()
                else:
                    if InviteUsers.objects.filter(
...
def send_mail_invitaion1(email, request,type="NonexistedUser"):
    # reset_password_url = "https://viknbooks.com/signup"
    # reset_password_url = "https://vikncodes.in/signup"
    if type == "existedUser":
        reset_password_url = getUrl("signin")
    else:
        reset_password_url = getUrl("signup")
    context = {
        "email": email,
        "domain": request.META["HTTP_ORIGIN"],
        "site_name": "viknbooks.com",
...
def send_mail_invitaion(email, request, service,userType,CompanyID,invited_user):
    token = ""
    if service == "existedUser":
        invite_link = getUrl("signin")
    else:
        invite_link = getUrl("signup")
    context = {
        "email": email,
        "domain": request.META["HTTP_ORIGIN"],
        "site_name": "viknbooks.com",
    ...
    subject = "User Invitation for {title}".format(title='viknbooks.com')
...
def get_user_details(request):
    # data1 = company_insert_query_to_accounts()
    # print(data1)
    user = None
    data = {}
    LastLoginID = None
    if User.objects.filter(pk=request.user.id).exists():
        ...
        if customer and customer.LastLoginCompanyID:
            LastLoginID = customer.LastLoginCompanyID.id
        data = {
            "username":user.username,
            "user_id":user.id,
            "email":user.email,
...
def accounts(request):
    ...
    response.set_cookie(key='is_remember', value=remember_me, domain=domain,
                        secure=True, expires=two_years)
...
@permission_classes((AllowAny,))
@renderer_classes((JSONRenderer,))
def set_browser_uqid(request):
    uqNo = request.GET.get('data')
    response = HttpResponse("Cookie Set")
    domain = getUrl("sub_domain")
    two_years = datetime.datetime.now() + datetime.timedelta(days=730)
    response.set_cookie(key='uqNo', value=uqNo, domain=domain,
                        secure=True, expires=two_years)
    return response
