...
from django.contrib.auth.models import User, Group
from api.v10.users.functions import get_auto_id
...
from api.v10.users.functions import send_email, get_master_auto_id
...
from api.v10.users.serializers import CustomTokenSerializer, SoftwareVersionSerializer
...
def users(request):
    ...
    if serialized1.is_valid():
        ...
        if UserTable.objects.filter(CompanyID=CompanyID, Active=True).exists():
            instances = UserTable.objects.filter(CompanyID=CompanyID,Active=True)
        ...
        if user is not None:
            ...
            if response.status_code == 200:
                ...
                if User.objects.filter(username=username, password=password, is_superuser=False).exists():
                    if UserTable.objects.filter(UserName=username, Password=password, Active=True).exists():
                        CompanyID = get_object_or_404(UserTable.objects.filter(
                            UserName=username, Password=password, Active=True)).CompanyID.id
                ...
                #     VAT_Type = ""
                        ...
                        user_table_instance = get_object_or_404(UserTable.objects.filter(
                            CompanyID=CompanyID, UserName=username, Password=password, Active=True))
...
def companies(request):
    ...
    member_company_instances = models.UserTable.objects.filter(
        customer__user=userId, Active=True)
...
def create_company_user(request, pk):
    ...
    if serialized.is_valid():
        ...
        if password1 == password2:
            if UserTable.objects.filter(CompanyID__Email=email, CompanyID=CompanyID, Active=True).exists():
                error = True
                message += "This email already exists."
            elif UserTable.objects.filter(UserName=username, CompanyID=CompanyID, Active=True).exists():
                error = True
                message += "This username already exists."
...
def get_defaults(request):
    ...
    usertable_count = UserTable.objects.filter(CompanyID=CompanyID,Active=True).count()
    ...
    if UserTable.objects.filter(CompanyID__pk=CompanyID, customer__user=userId, Active=True).exists():
