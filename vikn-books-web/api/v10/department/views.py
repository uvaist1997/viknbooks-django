...
from api.v10.department.serializers import DepartmentRestSerializer, DepartmentSerializer
from api.v10.designation.functions import (
    generate_serializer_errors,
    get_auto_DepartmentID,
    get_auto_id,
)
from brands.models import Department, Department_Log, Designation, Designation_Log
...
def create_department(request):
    ...
    if serialized.is_valid():
        ...
        if Department.objects.filter(pk=ParentDepartment).exists():
        ...
        if not is_nameExist:
            depart_id = Department.objects.create(
...
def department(request, pk):
    ...
    if Department.objects.filter(pk=pk, CompanyID=CompanyID).exists():
        ...
        response_data = {"StatusCode": 6001, "message": "Department Not Found!"}
...
def delete_department(request, pk):
    ...
    if Department.objects.filter(pk=pk, CompanyID=CompanyID).exists():
        ...
        if not Department.objects.filter(
            ParentDepartment=instance, CompanyID=CompanyID
        ).exists():
            Action = "D"
            Department_Log.objects.create(
