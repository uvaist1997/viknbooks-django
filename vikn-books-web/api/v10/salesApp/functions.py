...
def get_auto_idMaster(model, BranchID):
    ...
    if model.objects.filter(BranchID=BranchID).exists():
        max_value = model.objects.filter(
            BranchID=BranchID).aggregate(Max('POSHoldMasterID'))
...
def get_auto_id(model, BranchID):
    ...
    if model.objects.filter(BranchID=BranchID).exists():
        max_value = model.objects.filter(
            BranchID=BranchID).aggregate(Max('POSHoldDetailsID'))
    if max_value:
...
def get_VoucherNoForPOS(CompanyID, BranchID, UserID, VoucherType):
    ...
    if user_count == 1:
        if table.VoucherNoTable.objects.filter(CompanyID=CompanyID, UserID=UserID, VoucherType=VoucherType).exists():
            voucher = table.VoucherNoTable.objects.filter(
                CompanyID=CompanyID, UserID=UserID, VoucherType=VoucherType).first()
...
def get_TokenNo(CompanyID, BranchID):
    ...
    if table.SalesOrderMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Type__in=types).exists():
        last_order = table.SalesOrderMaster.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID, Type__in=types).first()
...
def get_KitchenID(model, BranchID):
    ...
    if model.objects.filter(BranchID=BranchID).exists():
        max_value = model.objects.filter(
            BranchID=BranchID).aggregate(Max('KitchenID'))
    if max_value:
