...
from api.v10.brands.functions import generate_serializer_errors, get_auto_id
from api.v10.brands.serializers import (
    BrandRestSerializer,
    BrandSerializer,
    ListSerializer,
)
from api.v10.brands.tasks import create_random_user_accounts
...
def torunqryTest(request):
    ...
    if PurchaseMaster.objects.annotate(cash_bank=F('CashReceived')+F('BankAmount')).filter(CompanyID__id="cbba36e8-8cb9-4570-9161-2aecf7cf8e8a",cash_bank__lt=F('GrandTotal'), Balance__lt=0).exists():
        instances = PurchaseMaster.objects.annotate(cash_bank=F(
            'CashReceived')+F('BankAmount')).filter(CompanyID__id="cbba36e8-8cb9-4570-9161-2aecf7cf8e8a",cash_bank__lt=F('GrandTotal'), Balance__lt=0)
        vouchers = []
        for i in instances:
            VoucherType = "PI"
            VoucherNo = i.VoucherNo
            VoucherMasterID = i.PurchaseMasterID
            GrandTotal = i.GrandTotal
            CashReceived = i.CashReceived
            LedgerID = i.CashID
            CompanyID = i.CompanyID
            
            postins = LedgerPosting.objects.filter(
                CompanyID=CompanyID,VoucherType=VoucherType, VoucherNo=VoucherNo, VoucherMasterID=VoucherMasterID, LedgerID=LedgerID)
            print(postins)
            for p in postins:
                if converted_float(p.Credit) == converted_float(GrandTotal):
                    print(p.VoucherNo)
                    vouchers.append(p.VoucherNo)
                    # p.Credit = CashReceived
                    # p.save()
    ...
    response_data = {"StatusCode": 6000, "message": "success", "vouchers": vouchers}
