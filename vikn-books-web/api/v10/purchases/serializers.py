...
from main.functions import converted_float, get_GeneralSettings, get_ProductStock
...
class PurchaseMasterRest1Serializer(serializers.ModelSerializer):
    ...
    def get_billwise_status(self, instance):
        ...
        if BillWiseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, InvoiceNo=VoucherNo, TransactionID=PurchaseMasterID, VoucherType="PI").exists():
            instance = BillWiseMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, InvoiceNo=VoucherNo, TransactionID=PurchaseMasterID, VoucherType="PI").first()
    ...
    def get_is_billwised(self, instance):
        ...
        PurchaseMasterID = instance.PurchaseMasterID
        is_billwised = False
        # if BillWiseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, InvoiceNo=VoucherNo, TransactionID=PurchaseMasterID, VoucherType="PI").exclude(Payments=0).exists():
        #     is_billwised = True
        if (
    ...
    def get_RefferenceBillDate(self, instances):
        RefferenceBillDate = ""
        if instances.VenderInvoiceDate:
            RefferenceBillDate = instances.VenderInvoiceDate
        return str(RefferenceBillDate)
...
class PurchaseMasterRestSerializer(serializers.ModelSerializer):
    ...
    def get_is_billwised(self, instance):
        ...
        is_billwised = False
        if BillWiseDetails.objects.filter(CompanyID=CompanyID, BranchID=BranchID, InvoiceNo=InvoiceNo, VoucherType="PI", Payments__gt=0).exclude(PaymentVoucherType="PI").exists():
            is_billwised = True
        return is_billwised
    ...
    def get_PurchaseDetails(self, instances):
        ...
        purchase_details = PurchaseDetails.objects.filter(
            ...
            PurchaseMasterID=instances.PurchaseMasterID,
            BranchID=instances.BranchID,
        ).order_by("PurchaseDetailsID")
        serialized = PurchaseDetailsRestSerializer(
            purchase_details,
            many=True,
            context={
                "CompanyID": CompanyID,
    ...
    def get_PriceCategoryName(self, instances):
        ...
        PriceCategoryName = ""
        if PriceCategory.objects.filter(
    ...
    def get_PurchaseAccountName(self, instances):
        ...
        PurchaseAccountName = ""
        if AccountLedger.objects.filter(
            CompanyID=CompanyID, LedgerID=PurchaseAccount
        ).exists():
    ...
    def get_TotalTax_rounded(self, instances):
        ...
        return converted_float(TotalTax_rounded)
    ...
    def get_GrandTotal_Rounded(self, instances):
        ...
        return converted_float(GrandTotal_Rounded)
...
class PurchaseDetailsRestSerializer(serializers.ModelSerializer):
    ...
    def get_SerialNos(self, purchase_details):
        ...
        PurchaseMasterID = purchase_details.PurchaseMasterID
        PurchaseDetailsID = purchase_details.PurchaseDetailsID
    ...
    def get_ProductCodeVal(self, purchase_details):
        CompanyID = self.context.get("CompanyID")
        ProductID = purchase_details.ProductID
        product = Product.objects.get(CompanyID=CompanyID, ProductID=ProductID)
        ProductCode = product.ProductCode
    ...
    def get_ExpiryDate(self, purchase_details):
        ...
        if ExpiryDate:
            ExpiryDate = "{:%Y-%m}".format(dt.strptime(str(ExpiryDate), "%Y-%m-%d"))
    ...
    def get_Barcode(self, purchase_details):
        CompanyID = self.context.get("CompanyID")
        PriceListID = purchase_details.PriceListID
        BranchID = purchase_details.BranchID
        ProductID = purchase_details.ProductID
    ...
    def get_CostPerItem(self, purchase_details):
        ...
        return converted_float(CostPerItem)
...
class PurchaseMasterForReturnSerializer(serializers.ModelSerializer):
    ...
    def get_PurchaseDetails(self, instances):
        ...
        serialized = PurchaseDetailsForReturnsSerializer(
            purchase_details,
            many=True,
            context={"CompanyID": CompanyID, "PriceRounding": PriceRounding},
        )
    ...
    def get_UnitName(self, purchase_details):
        ...
        if PriceList.objects.filter(
            ...
            UnitName = Unit.objects.get(
                BranchID=BranchID, CompanyID=CompanyID, UnitID=UnitID
            ).UnitName
        else:
            UnitName = ""
...
class PurchasePrintSerializer(serializers.ModelSerializer):
    ...
    def get_Details(self, instances):
        ...
        serialized = PurchaseDetailsPrintSerializer(
            purchase_details,
            many=True,
            context={
                "CompanyID": CompanyID,
...
class FilterPurchaseOrderSerializer(serializers.ModelSerializer):
    is_checked = serializers.SerializerMethodField()

    class Meta:
        model = PurchaseOrderMaster
        fields = ("id", "PurchaseOrderMasterID",
