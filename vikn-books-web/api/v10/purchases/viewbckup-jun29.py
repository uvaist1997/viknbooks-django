...
from api.v10.purchases.serializers import PurchaseMasterSerializer, PurchaseMasterRestSerializer, PurchaseDetailsSerializer,\
...
from api.v10.purchases.functions import get_auto_id, get_auto_idMaster, get_auto_StockRateID, get_auto_StockTransID
...
def create_purchase(request):
    try:
        with transaction.atomic():
            ...
            if is_PurchaseOK:
                ...
                if AllowCashReceiptMorePurchaseAmt == True or AllowCashReceiptMorePurchaseAmt == "true":
                    ...
                    CashAmount = converted_float(CashReceived) + converted_float(Balance)
                else:
                    CashAmount = CashReceived
                ...
                if TaxType == 'VAT':
                    ...
                    if converted_float(SGSTAmount) > 0 or converted_float(CGSTAmount) > 0:
                        LedgerPostingID = get_auto_LedgerPostid(
                            LedgerPosting, BranchID, CompanyID)
                        LedgerPosting.objects.create(
                            LedgerPostingID=LedgerPostingID,
                ...
                if UserTable.objects.filter(CompanyID=CompanyID, customer__user__pk=CreatedUserID, Active=True).exists():
                    DefaultAccountForUser = UserTable.objects.get(
                        CompanyID=CompanyID, customer__user__pk=CreatedUserID, Active=True).DefaultAccountForUser
                    Cash_Account = UserTable.objects.get(
                        CompanyID=CompanyID, customer__user__pk=CreatedUserID, Active=True).Cash_Account
                    Bank_Account = UserTable.objects.get(
                        CompanyID=CompanyID, customer__user__pk=CreatedUserID, Active=True).Bank_Account
                    ...
                    if DefaultAccountForUser:
                        if account_group == 9 and converted_float(CashReceived) > 0 and converted_float(BankAmount) > 0:
                            CashAmount = converted_float(GrandTotal) - converted_float(BankAmount)
                            if converted_float(CashAmount) < converted_float(CashReceived):
                                CashAmount = CashReceived
                            LedgerPostingID = get_auto_LedgerPostid(
                                LedgerPosting, BranchID, CompanyID)
                            LedgerPosting.objects.create(
                        ...
                        elif account_group == 9 and converted_float(CashReceived) == 0 and converted_float(BankAmount) > 0:
                            LedgerPostingID = get_auto_LedgerPostid(
                                LedgerPosting, BranchID, CompanyID)
                            LedgerPosting.objects.create(
                                LedgerPostingID=LedgerPostingID,
                        ...
                        elif account_group == 8 and converted_float(CashReceived) > 0 and converted_float(BankAmount) > 0:
                            LedgerPostingID = get_auto_LedgerPostid(
                                LedgerPosting, BranchID, CompanyID)
                            LedgerPosting.objects.create(
                                LedgerPostingID=LedgerPostingID,
                        ...
                        elif (account_group == 10 or account_group == 29) and converted_float(CashReceived) > 0 and converted_float(BankAmount) > 0 and converted_float(Balance) <= 0:
                            LedgerPostingID = get_auto_LedgerPostid(
                                LedgerPosting, BranchID, CompanyID)
                            LedgerPosting.objects.create(
                                LedgerPostingID=LedgerPostingID,
                            ...
                            if AllowCashReceiptMorePurchaseAmt == True or AllowCashReceiptMorePurchaseAmt == "true":
                                ...
                                BankCash = converted_float(BankAmount) + \
                                    converted_float(CashReceived)
                                LedgerPostingID = get_auto_LedgerPostid(
                                    LedgerPosting, BranchID, CompanyID)
                                LedgerPosting.objects.create(
                                    LedgerPostingID=LedgerPostingID,
                                ...
                                if (converted_float(CashReceived) + converted_float(BankAmount)) >= converted_float(GrandTotal):
                                    if converted_float(BankAmount) == converted_float(GrandTotal):
                                        CashAmount = 0
                                    elif converted_float(BankAmount) < converted_float(GrandTotal):
                            ...
                            if AllowCashReceiptMorePurchaseAmt == True or AllowCashReceiptMorePurchaseAmt == "true":
                                # BankCash = converted_float(BankAmount) + converted_float(CashReceived)
                                LedgerPostingID = get_auto_LedgerPostid(
                                    LedgerPosting, BranchID, CompanyID)
                                LedgerPosting.objects.create(
                                    LedgerPostingID=LedgerPostingID,
                                ...
                                total = converted_float(GrandTotal) + converted_float(Balance)
                                LedgerPostingID = get_auto_LedgerPostid(
                                    LedgerPosting, BranchID, CompanyID)
                                LedgerPosting.objects.create(
                                    LedgerPostingID=LedgerPostingID,
                        ...
                        elif (account_group == 10 or account_group == 29) and converted_float(CashReceived) == 0 and converted_float(BankAmount) == 0:
                            LedgerPostingID = get_auto_LedgerPostid(
                                LedgerPosting, BranchID, CompanyID)
                            LedgerPosting.objects.create(
                                LedgerPostingID=LedgerPostingID,
                            ...
                            # cash = converted_float(GrandTotal) - converted_float(CashReceived)
                            LedgerPostingID = get_auto_LedgerPostid(
                                LedgerPosting, BranchID, CompanyID)
                            LedgerPosting.objects.create(
                                LedgerPostingID=LedgerPostingID,
                            ...
                            # bank = converted_float(GrandTotal) - converted_float(BankAmount)
                            LedgerPostingID = get_auto_LedgerPostid(
                                LedgerPosting, BranchID, CompanyID)
                            LedgerPosting.objects.create(
                                LedgerPostingID=LedgerPostingID,
                            ...
                            # bank = converted_float(GrandTotal) - converted_float(BankAmount)
                            cash_bank = converted_float(BankAmount) + converted_float(CashAmount)
                            LedgerPostingID = get_auto_LedgerPostid(
                                LedgerPosting, BranchID, CompanyID)
                            LedgerPosting.objects.create(
                                LedgerPostingID=LedgerPostingID,
        ...
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'Purchase',
                     'Create', 'Purchase created Failed.', str(e))
        return Response(response_data, status=status.HTTP_200_OK)
...
def edit_purchase(request, pk):
    try:
        ...
        # transaction.set_rollback(False)
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'Purchase',
                     'Edit', 'Purchase Updated Failed.', str(e))
        return Response(response_data, status=status.HTTP_200_OK)
        ...
        if UserID:
            ...
            for i in return_instances:
                ...
                if AccountLedger.objects.filter(LedgerID=LedgerID, BranchID=BranchID, CompanyID=CompanyID).exists():
                    ...
                    #     Parties.objects.get(LedgerID=LedgerID,BranchID=BranchID,CompanyID=CompanyID,PartyType="customer",PartyCode=LedgerCode)
                # try:
