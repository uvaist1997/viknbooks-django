from api.v10.stockPostings.functions import query_get_product_stock
...
from api.v10.priceLists.serializers import PriceListRestSerializer, SinglePriceListRestSerializer
from api.v10.workOrder.serializers import Batch_ListSerializer, UnitListSerializer
...
class ProductBarcodeListRestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'BranchID', 'CompanyID','unit', 'barcode', 'detailID')
...
class SingleProductRestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'ProductID', 'is_Service', 'BranchID', 'ProductCode', 'ProductBarcodeList', 'ProductName', 'DisplayName', 'Description', 'ProductGroupID',
                  ...
                  'IsFinishedProduct', 'IsSales', 'IsPurchase', 'PurchasePrice', 'SalesPrice', 'MRP', 'GST_TaxName', 'VAT_TaxName', 'UnitName', 'Tax1_TaxName', 'Tax2_TaxName', 'Tax3_TaxName',
                  'DefaultUnitID', 'DefaultUnitName', 'DefaultSalesPrice', 'DefaultSalesPrice1', 'DefaultSalesPrice2', 'DefaultSalesPrice3', 'DefaultPurchasePrice',
                  'is_VAT_inclusive', 'is_GST_inclusive', 'is_TAX1_inclusive', 'is_TAX2_inclusive', 'is_TAX3_inclusive', 'PriceListDetails', 'DefaultMRP', "IsKFC", 'DefaultBarcode',
                  "HSNCode", 'Stock', 'Check_positive_Stock','ProductImage2','ProductImage3','ProductImage4','ProductImage5',)
    ...
    def get_ProductBarcodeList(self, instances):
        ...
        BranchID = instances.BranchID
        product_barcode_list = ProductBarcode.objects.filter(
            CompanyID=CompanyID, ProductID=instances, BranchID=BranchID).order_by("ProductBarcodeID")
        serialized = ProductBarcodeListRestSerializer(product_barcode_list, many=True, context={
            "CompanyID": CompanyID, "PriceRounding": PriceRounding})
...
class AddItemBarcodeSerializer(serializers.ModelSerializer):
    ...
    def get_PurchasePrice(self, instances):
        ...
        PurchasePrice = 0
        if ProductBarcode.objects.filter(CompanyID=CompanyID, Barcode=BarCode, ProductID__ProductID=ProductID).exists():
            ins = ProductBarcode.objects.filter(
                CompanyID=CompanyID, Barcode=BarCode, ProductID__ProductID=ProductID).first()
            UnitID = ins.UnitID.UnitID
            if PriceList.objects.filter(CompanyID=CompanyID, ProductID=ProductID, UnitID=UnitID).exists():
                PurchasePrice = PriceList.objects.filter(
                    CompanyID=CompanyID, ProductID=ProductID, UnitID=UnitID).first().PurchasePrice
        elif PriceList.objects.filter(CompanyID=CompanyID, ProductID=ProductID, DefaultUnit=True).exists():
            priceList = PriceList.objects.get(
                CompanyID=CompanyID, ProductID=ProductID, DefaultUnit=True)
            PurchasePrice = priceList.PurchasePrice
            # PurchasePrice = round(PurchasePrice, int(PriceRounding))
    ...
    def get_SalesPrice(self, instances):
        ...
        if ProductBarcode.objects.filter(CompanyID=CompanyID, Barcode=BarCode, ProductID__ProductID=ProductID).exists():
            ...
            UnitID = ins.UnitID.UnitID
            if PriceList.objects.filter(CompanyID=CompanyID,ProductID=ProductID,UnitID=UnitID).exists():
                SalesPrice = PriceList.objects.filter(CompanyID=CompanyID,ProductID=ProductID,UnitID=UnitID).first().SalesPrice
    ...
    def get_PriceListID(self, instances):
        ...
        if check_EnableProductBatchWise:
            ...
            if pricelist_list.filter(Barcode=BarCode).exists():
                ...
                inst = ProductBarcode.objects.filter(
                    CompanyID=CompanyID, BranchID=BranchID, Barcode=BarCode).first()
                UnitID = inst.UnitID.UnitID
                if pricelist_list.filter(ProductID=ProductID, UnitID=UnitID):
                    PriceListID = pricelist_list.filter(ProductID=ProductID, UnitID=UnitID).first().PriceListID
        return PriceListID
