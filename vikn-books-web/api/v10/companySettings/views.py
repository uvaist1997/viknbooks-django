...
from api.v10.branchs.functions import get_ledger_instance
from api.v10.companySettings.functions import generate_serializer_errors
from api.v10.companySettings.serializers import (
    CompanySettingsInitialSerializer,
    CompanySettingsRestSerializer,
    CompanySettingsSerializer,
    EditionSerializer,
    FinancialYearSerializer,
)
from api.v10.general.functions import isValidMasterCardNo
from api.v10.settings.functions import (
    create_or_update_user_type_settings,
    get_user_type_settings,
)
from api.v10.users.serializers import ImageSerializer
from api.v10.users.views import send_mail_invitaion, send_mail_renew_request
...
from main.functions import activity_log, company_insert_query_to_accounts, company_update_query_to_accounts, get_company, get_no_of_days_bwn_dates, userTable_insert_query_to_accounts
...
def create_companySettings_initial(request):
    ...
    try:
        with transaction.atomic():
            ...
            is_registeredGst = data["is_registeredGst"]
            if CompanyLogo == "null":
                CompanyLogo = None
            ...
            if serialized.is_valid() and serialized1.is_valid():
                LUTNumber = serialized.data["LUTNumber"]
                ...
                try:
                    EnableZatca = serialized.data["EnableZatca"]
                except:
                    EnableZatca = False

                is_Gstn_validate = True
                if GSTNumber:
                    if is_registeredVat == "true":
                ...
                if is_Gstn_validate:
                    ...
                    try:
                        Edition = serialized.data["Edition"]
                    except:
                        Edition = "Standard"
                        
                    try:
                        financial_year_period = data["financial_year_period"]
                    except:
                        financial_year_period = None
                    ...
                    if CompanySettings.objects.filter(
                        ...
                        # line added by suhaib
                        Customer.objects.filter(user=owner).update(
                            LastLoginCompanyID=company
                        )
                        # end line
                    ...
                    if is_registeredVat == "true":
                        is_VAT = "true"
                        print("is_registeredVat")
                    if is_registeredGst == "true":
                        is_GST = "true"
                        print("is_registeredGst")
                    # ===========VAT======
                    # GeneralSettings.objects.filter(CompanyID=CompanyID,SettingsType="VAT").update(SettingsValue=is_VAT,
                    #     UpdatedDate=today,
                    #     CreatedUserID=CreatedUserID,
                    #     Action="A"
                    # )
                    ...
                    usertype_instance = UserType.objects.get(
                        CompanyID=CompanyID, UserTypeName="Admin"
                    )
                    ...
                    with open(path, "r+") as json_file:
                        ...
                        # For VAT TAX_TYPE Ledgers
                        vat_on_sales = None
                        vat_on_purchase = None
                        vat_on_expense = None
                        # For VAT GST_TYPE Ledgers
                        central_gst_on_sales = None
                        state_gst_on_sales = None
                        integrated_gst_on_sales = None
                        central_gst_on_purchase = None
                        state_gst_on_purchase = None
                        integrated_gst_on_purchase = None
                        central_gst_on_expense = None
                        state_gst_on_payment = None

                        

                        # For VAT GST_TYPE Ledgers
                        # if CompanyID.Country.Country_Name == "India":
                        central_gst_on_sales = get_ledger_instance(3, 1, company)
                        state_gst_on_sales = get_ledger_instance(10, 1, company)
                        integrated_gst_on_sales = get_ledger_instance(
                            7, 1, company
                        )
                        central_gst_on_purchase = get_ledger_instance(
                            36, 1, company
                        )
                        state_gst_on_purchase = get_ledger_instance(
                            42, 1, company
                        )
                        integrated_gst_on_purchase = get_ledger_instance(
                            39, 1, company
                        )
                        central_gst_on_expense = get_ledger_instance(
                            35, 1, company
                        )
                        state_gst_on_payment = get_ledger_instance(41, 1, company)
                        # else:
                            # For VAT TAX_TYPE Ledgers
                            # if company.Country.Country_Name == "Saudi Arabia":
                        vat_on_sales = get_ledger_instance(
                            55, 1, company)
                        vat_on_purchase = get_ledger_instance(
                            54, 1, company)
                        vat_on_expense = get_ledger_instance(
                            53, 1, company)

                        # For Common Ledgers
                        round_off_sales = get_ledger_instance(78, 1, company)
                        discount_on_sales = get_ledger_instance(74, 1, company)
                        discount_on_purchase = get_ledger_instance(83, 1, company)
                        discount_on_payment = get_ledger_instance(82, 1, company)
                        discount_on_receipt = get_ledger_instance(72, 1, company)
                        discount_on_loyalty = get_ledger_instance(73, 1, company)
                        round_off_purchase = get_ledger_instance(77, 1, company)
                    ...
                    if is_gst:
                        GeneralSettings.objects.filter(
                            CompanyID=CompanyID, SettingsType="GST"
                        ).update(
                            SettingsValue="true",
                            UpdatedDate=today,
                            CreatedUserID=CreatedUserID,
                        ...
                        GeneralSettings.objects.filter(
                            CompanyID=CompanyID, SettingsType="VAT"
                        ).update(
                            SettingsValue="false",
                            UpdatedDate=today,
                            CreatedUserID=CreatedUserID,
                    ...
                    # ================creating Cance Reasons starts here================
                    THIS_FOLDER = os.path.dirname(os.path.abspath(__file__))
                    path = os.path.join(
                        THIS_FOLDER, "company_fixtures/cancel_reasons.json"
                    )
                    ...
                    with open(path, "r+") as json_file:
                        ...
                        CompanyID = get_company(CompanyID)
                        for i in dic:
                            if i["model"] == "brands.CancelReasons":
                                BranchID = i["fields"]["BranchID"]
                                Reason = i["fields"]["Reason"]
                                CancelReasons.objects.create(
                                    CompanyID=CompanyID,
                                    BranchID=BranchID,
                                    Reason=Reason,
                                )
                    # ================creating Cance Reasons ends here================

                    # ================creating defult General Settings starts here================
                    THIS_FOLDER = os.path.dirname(os.path.abspath(__file__))
                    path = os.path.join(
                        THIS_FOLDER, "company_fixtures/user_role_settings.json"
                    )
                    ...
                    with open(path, "r+") as json_file:
                        ...
                        CompanyID = company.id
                        CompanyID = get_company(CompanyID)
                        usertype = UserType.objects.filter(
                            CompanyID=CompanyID, UserTypeName="Admin"
                        ).first()
                        for i in dic:
                            if i["model"] == "brands.UserRoleSettingsModel":
                                BranchID = i["fields"]["BranchID"]
                                parent = i["fields"]["parent"]
                                name = i["fields"]["name"]
                                view_permission = i["fields"]["view_permission"]
                                save_permission = i["fields"]["save_permission"]
                                edit_permission = i["fields"]["edit_permission"]
                                delete_permission = i["fields"]["delete_permission"]
                                print_permission = i["fields"]["print_permission"]
                                UpdatedDate = i["fields"]["UpdatedDate"]
                                UpdatedUserID = i["fields"]["UpdatedUserID"]

                                UserRoleSettingsModel.objects.create(
                                    CompanyID=CompanyID,
                                    BranchID=BranchID,
                                    UserType=usertype,
                                    parent=parent,
                    ...
                    # ================creating defult General Settings ends here================
                    # ================creating defult General Settings ends here================
                    # Inserting Company To Accounts
                    company_insert_query_to_accounts(company)
                    # Inserting UserTable To Accounts
                    userTable_insert_query_to_accounts(user_table)
...
def create_companySettings(request):
    ...
    if serialized.is_valid():
        ...
        # ================creating defult Account Ledger starts here================
        THIS_FOLDER = os.path.dirname(os.path.abspath(__file__))
        path = os.path.join(THIS_FOLDER, "company_fixtures/account_ledgers.json")
        ...
        # ================creating defult Transaction Types starts here================
        THIS_FOLDER = os.path.dirname(os.path.abspath(__file__))
        path = os.path.join(THIS_FOLDER, "company_fixtures/transaction_types.json")
        ...
        # ================creating defult Account Groups starts here================
        THIS_FOLDER = os.path.dirname(os.path.abspath(__file__))
        path = os.path.join(THIS_FOLDER, "company_fixtures/account_groups.json")
        ...
        with open(path, "r+") as json_file:
            ...
            # For VAT TAX_TYPE Ledgers
            if CompanyID.Country.Country_Name == "Saudi Arabia":
                vat_on_sales = get_ledger_instance(55, 1, CompanyID)
                vat_on_purchase = get_ledger_instance(54, 1, CompanyID)
                vat_on_expense = get_ledger_instance(53, 1, CompanyID)

            # For VAT GST_TYPE Ledgers
            elif CompanyID.Country.Country_Name == "India":
                central_gst_on_sales = get_ledger_instance(3, 1, CompanyID)
                state_gst_on_sales = get_ledger_instance(10, 1, CompanyID)
                integrated_gst_on_sales = get_ledger_instance(7, 1, CompanyID)
                central_gst_on_purchase = get_ledger_instance(36, 1, CompanyID)
                state_gst_on_purchase = get_ledger_instance(42, 1, CompanyID)
                integrated_gst_on_purchase = get_ledger_instance(39, 1, CompanyID)
                central_gst_on_expense = get_ledger_instance(35, 1, CompanyID)
                state_gst_on_payment = get_ledger_instance(41, 1, CompanyID)

            # For Common Ledgers
            round_off_sales = get_ledger_instance(78, 1, CompanyID)
            discount_on_sales = get_ledger_instance(74, 1, CompanyID)
            discount_on_purchase = get_ledger_instance(83, 1, CompanyID)
            discount_on_payment = get_ledger_instance(82, 1, CompanyID)
            discount_on_receipt = get_ledger_instance(72, 1, CompanyID)
            discount_on_loyalty = get_ledger_instance(73, 1, CompanyID)
            round_off_purchase = get_ledger_instance(77, 1, CompanyID)
            ...
            for i in dic:
                if i["model"] == "brands.Branch":
                    ...
                    Branch.objects.create(
                        ...
                        country=CompanyID.Country,
                        PostalCode=CompanyID.PostalCode,
                        Email=CompanyID.Email,
                        Phone=CompanyID.Phone,
                        VATNumber=CompanyID.VATNumber,
                        GSTNumber=CompanyID.GSTNumber,
                        is_vat=CompanyID.is_vat,
                        is_gst=CompanyID.is_gst,
                        vat_on_sales=vat_on_sales,
                        vat_on_purchase=vat_on_purchase,
                        vat_on_expense=vat_on_expense,
                        central_gst_on_sales=central_gst_on_sales,
        ...
        # ================creating defult General Settings starts here================
        THIS_FOLDER = os.path.dirname(os.path.abspath(__file__))
        path = os.path.join(THIS_FOLDER, "company_fixtures/general_settings.json")
        ...
        # ================creating defult Price Category starts here================
        THIS_FOLDER = os.path.dirname(os.path.abspath(__file__))
        path = os.path.join(THIS_FOLDER, "company_fixtures/price_category.json")
        ...
        # ================creating defult Price Category starts here================
        THIS_FOLDER = os.path.dirname(os.path.abspath(__file__))
        path = os.path.join(THIS_FOLDER, "company_fixtures/product_category.json")
        ...
        # ================creating defult TaxCategory starts here================
        THIS_FOLDER = os.path.dirname(os.path.abspath(__file__))
        path = os.path.join(THIS_FOLDER, "company_fixtures/tax_categories.json")
        ...
        # Accounts => Masters
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "AccountGroupView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "AccountGroupSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "AccountGroupEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "AccountGroupDelete", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "AccountLedgerView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "AccountLedgerSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "AccountLedgerEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "AccountLedgerDelete", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "BankView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "BankSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "BankEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "BankDelete", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "CustomerView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "CustomerSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "CustomerEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "CustomerDelete", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "SupplierView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "SupplierSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "SupplierEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "SupplierDelete", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "TransactionTypeView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "TransactionTypeSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "TransactionTypeEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Masters", "TransactionTypeDelete", True
        )
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "BankPaymentEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "BankPaymentSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "BankPaymentView", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "BankReceiptEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "BankReceiptSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "BankReceiptView", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "CashPaymentEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "CashPaymentSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "CashPaymentView", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "CashReceiptEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "CashReceiptSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "CashReceiptView", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "JournalEntryEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "JournalEntrySave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Transactions", "JournalEntryView", True
        )
        # Accounts => Reports
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "LedgerReportView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "LedgerReportPrint", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "LedgerReportExport", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "TrialBalanceView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "TrialBalancePrint", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "TrialBalanceExport", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "ProfitAndLossView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "ProfitAndLossPrint", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "ProfitAndLossExport", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "BalanceSheetView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "BalanceSheetPrint", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "BalanceSheetExport", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "DailyReportView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "DailyReportPrint", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "DailyReportExport", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "CashBookView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "CashBookPrint", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "CashBookExport", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "VATReportView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "VATReportPrint", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Accounts_Reports", "VATReportExport", True
        )

        # Inventory => Masters
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "ProductGroupView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "ProductGroupSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "ProductGroupEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "ProductGroupDelete", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "BrandsView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "BrandsSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "BrandsEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "BrandsDelete", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "TaxCategoriesView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "TaxCategoriesSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "TaxCategoriesEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "TaxCategoriesDelete", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "PriceCategoriesView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "PriceCategoriesSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "PriceCategoriesEdit", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "WarehouseView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "WarehouseSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "WarehouseEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "WarehouseDelete", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "ProductsView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "ProductsSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "ProductsEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "ProductsDelete", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "RoutesView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "RoutesSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "RoutesEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "RoutesDelete", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "UnitsView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "UnitsSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "UnitsEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Masters", "UnitsDelete", True
        )
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Transactions", "WorkOrdersView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Transactions", "WorkOrdersSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Transactions", "WorkOrdersEdit", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Transactions", "SaleReturnsView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Transactions", "SaleReturnsSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Transactions", "SaleReturnsEdit", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Transactions", "UsedStocksView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Transactions", "UsedStocksSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Transactions", "UsedStocksEdit", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        # Inventory => Reports
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "StockReportView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "StockReportPrint", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "StockReportExport", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "StockValueReportView", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "PurchaseReportView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "PurchaseReportPrint", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "PurchaseReportExport", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "SalesReportView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "SalesReportPrint", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "SalesReportExport", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "BillWiseReportView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "BillWiseReportPrint", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "BillWiseReportExport", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "UsedStockReportView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Inventory_Reports", "UsedStockReportPrint", True
        )
        create_or_update_user_type_settings(
            User,
            CompanyID,
        ...
        # Payroll => Masters
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Payroll_Masters", "DepartmentsView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Payroll_Masters", "DepartmentsSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Payroll_Masters", "DepartmentsEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Payroll_Masters", "DepartmentsDelete", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Payroll_Masters", "DesignationsView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Payroll_Masters", "DesignationsSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Payroll_Masters", "DesignationsEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Payroll_Masters", "DesignationsDelete", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Payroll_Masters", "EmployeeView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Payroll_Masters", "EmployeeSave", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Payroll_Masters", "EmployeeEdit", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Payroll_Masters", "EmployeeDelete", True
        )

        # Settings
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Settings", "PrinterView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Settings", "BarcodeView", True
        )
        create_or_update_user_type_settings(
            User, CompanyID, BranchID, "Settings", "PrintBarcodeView", True
        )
        # ================creating defult Unit ends here================

        response_data = {"StatusCode": 6000, "data": serialized.data, "id": company.id}
...
def edit_companySettings(request):
    ...
    if serialized.is_valid():
        ...
        if is_Gstn_validate:
            ...
            RegistrationType = serialized.data["RegistrationType"]
            Tax1 = serialized.data["Tax1"]
            Tax2 = serialized.data["Tax2"]
            Tax3 = serialized.data["Tax3"]
            is_vat = serialized.data["is_vat"]
            ...
            instance.EnableZatca = EnableZatca
            instance.LUTNumber = LUTNumber
            # instance.CreatedUserID = CreatedUserID
            instance.save()

            # Update query to Accounts
            company_update_query_to_accounts(instance)

            Branch.objects.filter(CompanyID=CompanyID, BranchID=1).update(
                DisplayName=instance.CompanyName
            )

            is_registeredVat = data["is_vat"]
            is_registeredGst = data["is_gst"]
            print(is_registeredVat, "is_registeredVat")
            print(is_registeredGst, "is_registeredGst")
            ...
            # request , company, log_type, user, source, action, message, description
            response_data = {"StatusCode": 6001, "message": "Invalid GSTNumber!!!"}
...
def companySettings(request):
    ...
    if CompanySettings.objects.filter(pk=CompanyID).exists():
        instance = CompanySettings.objects.get(pk=CompanyID)
        serialized = CompanySettingsRestSerializer(
            instance, context={"request": request}
        )
        response_data = {"StatusCode": 6000, "data": serialized.data}
    else:
        response_data = {"StatusCode": 6001, "message": "CompanySettings Not Found!"}
...
def delete_companySettings(request, pk):
    ...
    instance.DeletedDate = today
    instance.save()

    if UserTable.objects.filter(CompanyID=instance, Active=True).exists():
        instances = UserTable.objects.filter(CompanyID=instance, Active=True)
        for i in instances:
            i.delete()
    ...
    # company_instance = CompanySettings.objects.filter(pk=instance.id, is_deleted=False).last()
    # customer_instance = Customer.objects.get(user__pk=CreatedUserID)
    # customer_instance.LastLoginCompanyID = company_instance
    # customer_instance.save()

    # serialized = CompanySettingsRestSerializer(company_instance, many=False, context={"request": request})
...
def delete_companyPermanantly(request, pk):
    ...
    new_date = None
    instance = get_company(pk)
    if not instance.DeletedDate:
        instance.DeletedDate = today
        instance.save()
    if instance.is_deleted == True and instance.DeletedDate:
        date = instance.DeletedDate
        days = datetime.timedelta(30)
        new_date = date + days
        print(date.date(), new_date.date(), "iiiiiiiiiiiiiiiiiiiiiii")

    if today.date() > new_date.date():
        CompanyName = instance.CompanyName
        CompanyLogo = instance.CompanyLogo
        Address1 = instance.Address1
        Address2 = instance.Address2
        ...
        if models.UserTable.objects.filter(CompanyID=instance, Active=True).exists():
            instances = models.UserTable.objects.filter(
                CompanyID=instance, Active=True)
            for i in instances:
                i.delete()

        if models.User_Log.objects.filter(CompanyID=instance, Active=True).exists():
            instances = models.User_Log.objects.filter(CompanyID=instance,Active=True)
            for i in instances:
                i.delete()
        ...
        if models.ProductCategory_Log.objects.filter(CompanyID=instance).exists():
            instances = models.ProductCategory_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.TransactionTypes_Log.objects.filter(CompanyID=instance).exists():
            instances = models.TransactionTypes_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.GeneralSettings_Log.objects.filter(CompanyID=instance).exists():
            instances = models.GeneralSettings_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()

        if models.DamageStockDetails.objects.filter(CompanyID=instance).exists():
            instances = models.DamageStockDetails.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()

        if models.DamageStockDetails_Log.objects.filter(CompanyID=instance).exists():
            instances = models.DamageStockDetails_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.DamageStockMaster_Log.objects.filter(CompanyID=instance).exists():
            instances = models.DamageStockMaster_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.JournalDetails_Log.objects.filter(CompanyID=instance).exists():
            instances = models.JournalDetails_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.OpeningStockDetails.objects.filter(CompanyID=instance).exists():
            instances = models.OpeningStockDetails.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.OpeningStockMaster.objects.filter(CompanyID=instance).exists():
            instances = models.OpeningStockMaster.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()

        if models.OpeningStockMaster_Log.objects.filter(CompanyID=instance).exists():
            instances = models.OpeningStockMaster_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.PaymentDetails_Log.objects.filter(CompanyID=instance).exists():
            instances = models.PaymentDetails_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.POSHoldDetails_Log.objects.filter(CompanyID=instance).exists():
            instances = models.POSHoldDetails_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.PurchaseDetails_Log.objects.filter(CompanyID=instance).exists():
            instances = models.PurchaseDetails_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.PurchaseMaster_Log.objects.filter(CompanyID=instance).exists():
            instances = models.PurchaseMaster_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()

        if models.PurchaseOrderDetails.objects.filter(CompanyID=instance).exists():
            instances = models.PurchaseOrderDetails.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.PurchaseOrderMaster.objects.filter(CompanyID=instance).exists():
            instances = models.PurchaseOrderMaster.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.PurchaseReturnDetails.objects.filter(CompanyID=instance).exists():
            instances = models.PurchaseReturnDetails.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.PurchaseReturnMaster.objects.filter(CompanyID=instance).exists():
            instances = models.PurchaseReturnMaster.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.ReceiptDetails_Log.objects.filter(CompanyID=instance).exists():
            instances = models.ReceiptDetails_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.SalesOrderDetails_Log.objects.filter(CompanyID=instance).exists():
            instances = models.SalesOrderDetails_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.SalesOrderMaster_Log.objects.filter(CompanyID=instance).exists():
            instances = models.SalesOrderMaster_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()

        if models.SalesReturnDetails.objects.filter(CompanyID=instance).exists():
            instances = models.SalesReturnDetails.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()

        if models.SalesReturnDetails_Log.objects.filter(CompanyID=instance).exists():
            instances = models.SalesReturnDetails_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.SalesReturnMaster_Log.objects.filter(CompanyID=instance).exists():
            instances = models.SalesReturnMaster_Log.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.StockAdjustmentDetails.objects.filter(CompanyID=instance).exists():
            instances = models.StockAdjustmentDetails.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()

        if models.StockAdjustmentDetails_Log.objects.filter(
            CompanyID=instance
        ).exists():
        ...
        if models.StockAdjustmentMaster.objects.filter(CompanyID=instance).exists():
            instances = models.StockAdjustmentMaster.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.StockReceiptDetails.objects.filter(CompanyID=instance).exists():
            instances = models.StockReceiptDetails.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.StockReceiptMaster_ID.objects.filter(CompanyID=instance).exists():
            instances = models.StockReceiptMaster_ID.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.StockTransferDetails.objects.filter(CompanyID=instance).exists():
            instances = models.StockTransferDetails.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.StockTransferMaster_ID.objects.filter(CompanyID=instance).exists():
            instances = models.StockTransferMaster_ID.objects.filter(CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        response_data = {
            "StatusCode": 6001,
            "message": "Cannot Delete CompanySettings!",
        }
        return Response(response_data, status=status.HTTP_200_OK)
...
def check_company_delete(request):
    ...
    if user is not None:
        ...
        response_data = {"StatusCode": 6001, "message": "user credential is wrong!"}
        return Response(response_data, status=status.HTTP_200_OK)
...
def delete_companyTransactions(request, pk):
    ...
    if models.OpeningStockDetails_Log.objects.filter(CompanyID=instance).exists():
        instances = models.OpeningStockDetails_Log.objects.filter(CompanyID=instance)
        for i in instances:
            i.delete()
    ...
    if models.PurchaseOrderDetails_Log.objects.filter(CompanyID=instance).exists():
        instances = models.PurchaseOrderDetails_Log.objects.filter(CompanyID=instance)
        for i in instances:
            i.delete()
    ...
    if models.PurchaseOrderMaster_Log.objects.filter(CompanyID=instance).exists():
        instances = models.PurchaseOrderMaster_Log.objects.filter(CompanyID=instance)
        for i in instances:
            i.delete()
    ...
    if models.PurchaseReturnDetails_Log.objects.filter(CompanyID=instance).exists():
        instances = models.PurchaseReturnDetails_Log.objects.filter(CompanyID=instance)
        for i in instances:
            i.delete()
    ...
    if models.PurchaseReturnMaster_Log.objects.filter(CompanyID=instance).exists():
        instances = models.PurchaseReturnMaster_Log.objects.filter(CompanyID=instance)
        for i in instances:
            i.delete()
    ...
    if models.StockAdjustmentDetails_Log.objects.filter(CompanyID=instance).exists():
        instances = models.StockAdjustmentDetails_Log.objects.filter(CompanyID=instance)
        for i in instances:
            i.delete()
    ...
    if models.StockAdjustmentMaster_Log.objects.filter(CompanyID=instance).exists():
        instances = models.StockAdjustmentMaster_Log.objects.filter(CompanyID=instance)
        for i in instances:
            i.delete()
    ...
    if models.StockReceiptDetails_Log.objects.filter(CompanyID=instance).exists():
        instances = models.StockReceiptDetails_Log.objects.filter(CompanyID=instance)
        for i in instances:
            i.delete()
    ...
    if models.StockReceiptMasterID_Log.objects.filter(CompanyID=instance).exists():
        instances = models.StockReceiptMasterID_Log.objects.filter(CompanyID=instance)
        for i in instances:
            i.delete()
    ...
    if models.StockTransferDetails_Log.objects.filter(CompanyID=instance).exists():
        instances = models.StockTransferDetails_Log.objects.filter(CompanyID=instance)
        for i in instances:
            i.delete()
    ...
    if models.StockTransferMasterID_Log.objects.filter(CompanyID=instance).exists():
        instances = models.StockTransferMasterID_Log.objects.filter(CompanyID=instance)
        for i in instances:
            i.delete()
...
def get_company_count(request):
    ...
    if CompanySettings.objects.filter(CreatedUserID=UserID).exists():
        ...
        response_data = {"StatusCode": 6000, "count": count}
        return Response(response_data, status=status.HTTP_200_OK)
...
@authentication_classes((JWTTokenUserAuthentication,))
@renderer_classes((JSONRenderer,))
def inactive_companyPermanantly(request, pk):
    from brands import models
    ...
    response_data = {"StatusCode": 6000, "message": "Inactive Successfully!"}
    return Response(response_data, status=status.HTTP_200_OK)
...
def create_edition(request):
    data = request.data
    today = datetime.datetime.now()
    serialized = EditionSerializer(data=request.data)
    if serialized.is_valid():
        ...
        if not is_nameExist:
            ...
            response_data = {"StatusCode": 6001, "message": "Edition Already Exist!!!"}
...
def list_edition(request):
    if Edition.objects.all().exists():
        instances = Edition.objects.all()
        serialized = EditionSerializer(instances, many=True)
        response_data = {"StatusCode": 6000, "data": serialized.data}

        return Response(response_data, status=status.HTTP_200_OK)
    else:
        response_data = {"StatusCode": 6001, "message": "Edition Not Found"}
...
def edit_edition(request, pk):
    ...
    today = datetime.datetime.now()
    serialized = EditionSerializer(data=request.data)
    instance = Edition.objects.get(pk=pk)
    ...
    if serialized.is_valid():
        ...
        if edition_ok:
            ...
            if is_nameExist:
                response_data = {
                    "StatusCode": 6001,
                    "message": "Edition Name Already exist",
                }
                return Response(response_data, status=status.HTTP_200_OK)
...
@authentication_classes((JWTTokenUserAuthentication,))
@renderer_classes((JSONRenderer,))
def view_edition(request, pk):
    instance = None
    if Edition.objects.filter(pk=pk).exists():
        instance = Edition.objects.get(pk=pk)
    if instance:
        serialized = EditionSerializer(instance)
        response_data = {"StatusCode": 6000, "data": serialized.data}
    else:
        response_data = {"StatusCode": 6001, "message": "Edition Not Found!"}
...
def delete_edition(request, pk):
    ...
    if Edition.objects.filter(pk=pk).exists():
        instance = Edition.objects.get(pk=pk)
        instance.delete()
        response_data = {"StatusCode": 6000, "message": "Edition Deleted Successfully!"}
    else:
        response_data = {"StatusCode": 6001, "message": "Edition Not Found!"}
...
@authentication_classes((JWTTokenUserAuthentication,))
@renderer_classes((JSONRenderer,))
def check_company_leave(request):
    data = request.data
    CompanyID = data["CompanyID"]
    username = data["username"]
    password = data["password"]
    ...
    if user is not None:
        ...
        if UserTable.objects.filter(
            CompanyID=company, customer__user__username=username, is_owner=False, Active=True
        ).exists():
            user_table_ins = UserTable.objects.get(
                CompanyID=company, customer__user__username=username, is_owner=False,Active=True
            )
            user_table_ins.delete()
            response_data = {
                "StatusCode": 6000,
                "message": "Organization Leave Successfully!",
            }
            return Response(response_data, status=status.HTTP_200_OK)
        else:
            response_data = {
...
def check_company_status(request):
    ...
    if CompanyID:
        ...
        print(ExpiryDate,'ExpiryDateExpiryDateExpiryDateExpiryDateExpiryDateExpiryDateExpiryDateExpiryDateExpiryDate=======')
        ExpiryDays = get_no_of_days_bwn_dates(ExpiryDate)
        user_type = ""
        UserTypeName = ""
        owner_name = ""
        user_details = []
        company_details = []
        user_pic = ""
        if UserTable.objects.filter(CompanyID=company, customer__user__id=user_id, Active=True).exists():
            user_table_ins = UserTable.objects.filter(
                CompanyID=company, customer__user__id=user_id, Active=True).first()
            owner_name = UserTable.objects.filter(
                CompanyID=company, is_owner=True, Active=True).first().customer.user.first_name
            user_type = user_table_ins.UserType.ID
            UserTypeName = user_table_ins.UserType.UserTypeName
            if user_type == 1 and ExpiryDays < 15:
                user_auth_ins = User.objects.get(id=user_id)
                user_customer_ins = Customer.objects.get(user=user_auth_ins)
                user_name = user_auth_ins.username
                date_joined = user_auth_ins.date_joined
                user_first_name = user_auth_ins.first_name
                user_address = user_customer_ins.Address
                # user_pic = user_customer_ins.photo.url
                user_country = ""
                user_state = ""
                if user_customer_ins.Country:
                    user_country = user_customer_ins.Country.Country_Name
                if user_customer_ins.State:
                    user_state = user_customer_ins.State.Name
                user_city = user_customer_ins.City
                
                company_name = company.CompanyName
                company_business_type = ""
                if company.business_type:
                    company_business_type = company.business_type.Name
                company_phone = company.Phone
                company_email = company.Email
                company_address = company.Address1
                company_country = ""
                if company.Country:
                    company_country = company.Country.Country_Name
                if company.State:
                    company_state = company.State.Name
                company_city = company.City
                company_no_of_user = company.NoOfUsers
                company_edition = company.Edition
                company_expiry_date = company.ExpiryDate
                company_register_date = company.CreatedDate.date()
                company_NoOfUsers = company.NoOfUsers
                PostalCode = company.PostalCode
                user_pic = ImageSerializer(user_customer_ins, many=False, context={
                    "request": request}).data.get("photo")
                ...
                user_details.append({
                    ...
                    "user_city":user_city,
                    "company_no_of_user": company_no_of_user,
                ...
                company_details.append({
                    ...
                    "company_NoOfUsers": company_NoOfUsers,
                    "PostalCode": PostalCode
                })
        ...
        response_data = {"StatusCode": 6001, "message": "Company not exist!"}
        return Response(response_data, status=status.HTTP_200_OK)
...
def renew_plan(request):
    ...
    company_expiry_date = data["company_expiry_date"]
    company_address = data["company_address"]
    send_mail_renew_request("hr@vikncodes.com", request, company_name,
                            user_name, user_email, user_phone, company_expiry_date, company_address,"shaheen@vikncodes.com")
    response_data = {
        "StatusCode": 6000,
    }
    return Response(response_data, status=status.HTTP_200_OK)
