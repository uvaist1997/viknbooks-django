from brands.models import LoyaltyProgram,SalesMaster, LoyaltyProgram_Log,Product,ProductGroup,ProductCategory,TransactionTypes,LoyaltyPoint,LoyaltyCustomer
    ...
    if serialized.is_valid():
        ...
        # if not LoyaltyProgram.objects.filter(FromDate__range=[FromDate,ToDate],ToDate__range=[FromDate,ToDate],CardTypeID=CardTypeID).exists():
            ...
            loyaltyprograms = LoyaltyProgram.objects.filter(
                BranchID=BranchID, CompanyID=CompanyID,CardTypeID_id=CardTypeID)
...
def get_customer_point(request,pk):
    ...
    try:
        VoucherMasterID = SalesMaster.objects.get(pk=data['SalesMasterID']).SalesMasterID
    except:
    ...
    serialized1 = ListSerializer(data=request.data)
    # today = datetime.datetime.now()
    print(Type,'PPPPOOII',VoucherMasterID,"===================>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<")
        ...
        if LoyaltyPoint.objects.filter(LoyaltyCustomerID__id=pk,BranchID=BranchID, CompanyID=CompanyID).exists():
            if Type == "M" and VoucherMasterID:
                instances = LoyaltyPoint.objects.filter(
                    LoyaltyCustomerID__id=pk,BranchID=BranchID, CompanyID=CompanyID,VoucherMasterID__lt=VoucherMasterID)
            else:
                instances = LoyaltyPoint.objects.filter(
            ...
            for i in instances:
                if i.Point:
                    if Loyalty_Point_Expire:
                        ...
                        if Type == "M" and VoucherMasterID:
                            if VoucherMasterID == i.VoucherMasterID:
                                print(VoucherMasterID,"VoucherMasterID",i.VoucherMasterID,tot_Point,i.Point)
                            elif not VoucherMasterID == i.VoucherMasterID:
                                tot_Point = converted_float(tot_Point) + converted_float(i.Point) 
                        elif Type == "A":
                            tot_Point = converted_float(tot_Point) + converted_float(i.Point) 
...
def get_loyalty_customer(request):
    ...
    if serialized1.is_valid():
        BranchID = serialized1.data['BranchID']
        if customer_id:
            if LoyaltyCustomer.objects.filter(LoyaltyCustomerID=customer_id,BranchID=BranchID, CompanyID=CompanyID).exists():
                # instance = LoyaltyCustomer.objects.get(LoyaltyCustomerID=customer_id,BranchID=BranchID, CompanyID=CompanyID)
                try:
                    instance = LoyaltyCustomer.objects.get(LoyaltyCustomerID=customer_id,BranchID=BranchID, CompanyID=CompanyID)
                except :
                    instance = LoyaltyCustomer.objects.filter(LoyaltyCustomerID=customer_id,BranchID=BranchID, CompanyID=CompanyID).first()
                # else:
                #     instance = None
