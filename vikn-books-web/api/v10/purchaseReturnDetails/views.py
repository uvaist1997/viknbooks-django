...
from api.v10.purchaseReturnDetails.serializers import PurchaseReturnDetailsSerializer, PurchaseReturnDetailsRestSerializer, PurchaseReturnDetailsDummySerializer, PurchaseReturnDetailsDeletedDummySerializer
from api.v10.brands.serializers import ListSerializer
from api.v10.purchaseReturnDetails.functions import generate_serializer_errors
from rest_framework import status
from api.v10.purchaseReturnDetails.functions import get_auto_id
import datetime
...
def create_purchaseReturnDetails(request):
    ...
    if serialized.is_valid():
        ...
        PurchaseReturnDetailsID = get_auto_id(
            PurchaseReturnDetails, BranchID, DataBase)
...
def list_purchasesReturnDetailsDummy(request):
    ...
    if serialized1.is_valid():
        ...
        if PurchaseReturnDetailsDummy.objects.filter(BranchID=BranchID).exists():
            ...
            instances = PurchaseReturnDetailsDummy.objects.filter(
                BranchID=BranchID, IsDeleted=False)
            deleted_instances = PurchaseReturnDetailsDummy.objects.filter(
            ...
            serialized = PurchaseReturnDetailsDummySerializer(
                instances, many=True, context={"DataBase": DataBase})
            serializedDeleted = PurchaseReturnDetailsDeletedDummySerializer(
