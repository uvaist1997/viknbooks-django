from api.v10.reportQuerys.functions import purchaseReturn_register_report_data
...
from api.v10.purchaseReturns.serializers import PurchaseReturnMasterSerializer, PurchaseReturn1MasterRestSerializer, PurchaseReturnMasterRestSerializer, PurchaseReturnDetailsSerializer, PurchaseReturnDetailsRestSerializer
from api.v10.brands.serializers import ListSerializer
from api.v10.purchaseReturns.functions import generate_serializer_errors
from rest_framework import status
from api.v10.sales.serializers import ListSerializerforReport
from api.v10.purchaseReturns.functions import get_auto_id, get_auto_idMaster
from api.v10.sales.functions import get_BillwiseMasterID, get_auto_stockPostid, SetBatchInSales, handleBillwiseChanges
    ...
    try:
        with transaction.atomic():
            ...
            if is_PurchaseReturnOK:
                ...
                handleBillwiseChanges(CompanyID,BranchID,CashReceived,BankAmount,GrandTotal,LedgerID,VoucherNo,VoucherType,"create",CreditPeriod,PurchaseReturnMasterID,VoucherDate)
                    ...
                    if detailID == 0:
                        ...
                        if product_is_Service == False:
                            if StockPosting.objects.filter(CompanyID=CompanyID, WareHouseID=WarehouseID, VoucherMasterID=PurchaseReturnMasterID, VoucherDetailID=PurchaseReturnDetailsID, BranchID=BranchID, VoucherType="PR", ProductID=ProductID).exists():
                                ...
                                stock_instance.Date = VoucherDate
                                stock_instance.save()
                            else:
                                StockPostingID = get_auto_stockPostid(
                                    StockPosting, BranchID, CompanyID)
    ...
    if instances:
        ...
        # instance = PurchaseReturnMaster.objects.get(CompanyID=CompanyID, pk=pk)
            ...
            if BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo,TransactionID=PurchaseReturnMasterID,VoucherType="PR",Payments__gt=0).exists():
                ...
                # billwise delete 
                if BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo,TransactionID=PurchaseReturnMasterID,VoucherType="PR").exists():
                    BillWiseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InvoiceNo=VoucherNo,TransactionID=PurchaseReturnMasterID,VoucherType="PR").delete()
        ...
        if PurchaseReturnMaster.objects.filter(
            ...
            if UserTable.objects.filter(CompanyID=CompanyID, Active=True).exists():
                user_list = UserTable.objects.filter(
                    CompanyID=CompanyID, Active=True).values_list('customer_id', flat=True)
