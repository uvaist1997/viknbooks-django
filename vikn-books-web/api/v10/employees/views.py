from brands.models import Designation, Employee, Employee_Log, AccountLedger, Country, AccountLedger_Log, LedgerPosting, Category, UserTable
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes,authentication_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from rest_framework_simplejwt.authentication import JWTTokenUserAuthentication
from api.v10.employees.serializers import EmployeeSerializer, EmployeeRestSerializer, CountrySerializer
from api.v10.brands.serializers import ListSerializer
from api.v10.employees.functions import generate_serializer_errors, get_auto_id
from rest_framework import status
from api.v10.employees.functions import get_auto_id, get_EmployeeCode
...
def employees(request):
    ...
    if serialized1.is_valid():
        ...
        EmployeeID = None
        if UserTable.objects.filter(BranchID=BranchID, CompanyID=CompanyID, customer__user__id=CreatedUserID).exists():
            user_tables_ins = UserTable.objects.filter(
                BranchID=BranchID, CompanyID=CompanyID, customer__user__id=CreatedUserID).first()
            show_all_employees = user_tables_ins.show_all_employees
            EmployeeID = user_tables_ins.EmployeeID
        ...
        if show_all_employees == False and EmployeeID:
