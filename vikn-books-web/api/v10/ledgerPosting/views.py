...
from api.v10.purchases.views import list_pagination
...
from api.v10.ledgerPosting.serializers import LedgerPostingSerializer, LedgerPostingRestSerializer, ListSerializerforTrialBalance,\
...
from api.v10.stockPostings.serializers import StockPostingRestSerializer, StockValueAllSerializer, StockValueSingleSerializer
from api.v10.ledgerPosting.functions import DayBook_excel_data, Detail_To_Consolidate, Query_balanceSheet_balancing, Query_balanceSheet_singleSide, Query_profitAndLoss_balancing, export_to_excel_DayBook, export_to_excel_outStandingReport, generate_serializer_errors, outStandingReport_excel_data, profitAndLoss_balancing, swap_negative_AssetCons, swap_negative_AssetDetail, swap_negative_LiablityCons, swap_negative_LiablityDetail
from rest_framework import status
from api.v10.sales.serializers import ListSerializerforReport, ListSerializerforStockReport, ListSerializerforStockValueReport, ListSerializerforStockValueReportSingle
from api.v10.ledgerPosting.functions import balanceSheet_balancing, balanceSheet_excel_data, export_to_excel_balanceSheet, profitAndLoss_excel_data, export_to_excel_profitAndLoss, get_auto_id, convertOrderdDict, get_VoucherName, ledgerReport_excel_data, export_to_excel_ledgerReport, trialBalance_excel_data, export_to_excel_trialBalance
import datetime
...
import pandas as pd
import numpy as np
from api.v10.reportQuerys.functions import query_BalanceSheet_Consolidated_data, query_BalanceSheet_Detailed_data, query_BalanceSheet_NetLoss_or_NetProfit, query_Cash_OR_Bank_Book_report_data, query_ProfitAndLoss_Consolidated_data, query_ProfitAndLoss_Detailed_data, query_ProfitAndLoss_filter, query_closingTrialBalance_report_data, query_ledger_report_data, query_openingBalance_data, query_openingStockBalance_data, query_outStandingReport_report_data, query_purchase_report_data, query_purchaseReturn_report_data, query_sales_report_data, query_salesReturn_report_data, query_salesSummary_report_data, query_stock_report_data, query_stockLedger_report_data, query_trialBalance_report_data
...
def trialBalance(request):
    ...
    if serialized1.is_valid():
        ...
        if LedgerPosting.objects.filter(BranchID=BranchID, CompanyID=CompanyID).exists():
            ...
            templateType,filterMethod,data["CompanyID"], CreatedUserID, PriceRounding, BranchID, ToDate,FromDate)
...
def ledgerReport(request):
    ...
    if serialized1.is_valid():
        ...
        if LedgerPosting.objects.filter(BranchID=BranchID, CompanyID_id=CompanyID).exists():
            ...
            df,details = query_ledger_report_data(CompanyID,BranchID,FromDate,ToDate,PriceRounding,VoucherType,int(value),ManualOpeningBalance,is_fromDate)
...
def profitAndLoss_web(request):
    ...
    if serialized1.is_valid():
        ...
        data = profitAndLoss_excel_data(filterMethod,int(key),
        CompanyID,
        CreatedUserID,
        PriceRounding,
        BranchID,
        ...
        response_data = {
            "StatusCode": 6000,
            "data": data['profitAndLoss_final_data'],
        }
        return Response(response_data, status=status.HTTP_200_OK)
    else:
        response_data = {
...
def balancing_sheet(request):
    ...
    CompanyID = data['CompanyID']
    CompanyID = get_company(CompanyID)
    balancePageType = data['balancePageType']
...
def outstanding_report(request):
    ...
    #         count = len(account_ledgers)
        ...
        df.columns = [                
                'LedgerID','LedgerName', 'Debit','Credit']
