...
from api.v10.reportQuerys.functions import query_BalanceSheet_Consolidated_data, query_BalanceSheet_Detailed_data, query_BalanceSheet_NetLoss_or_NetProfit, query_ProfitAndLoss_Consolidated_data, query_ProfitAndLoss_Detailed_data, query_ProfitAndLoss_filter
import pandas as pd
import json
...
def removeRepetedDataFrame(x,arr=[]):
    print(arr)
    if not x in arr:
        arr.append(x)
        return x
...
def profitAndLoss_excel_data(filterMethod,key, CompanyID, CreatedUserID, PriceRounding, BranchID, ToDate, FromDate, request, ManualOpeningStock, ManualClosingStock):
    print(filterMethod,"^^^^^^^^^^^^profitAndLoss_excel_data^^^^^^^^^^^^^^")
    try:
    ...
    if LedgerPosting.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        ...
        dir_exp_empty_arr = []
        Detail_dirct_exp_df["GroupName"] = Detail_dirct_exp_df["GroupName"] = Detail_dirct_exp_df["GroupName"].apply(lambda x:removeRepetedDataFrame(x,dir_exp_empty_arr))
        ...
        dir_inc_empty_arr = []
        Detail_dirct_inc_df["GroupName"] = Detail_dirct_inc_df["GroupName"] = Detail_dirct_inc_df["GroupName"].apply(lambda x:removeRepetedDataFrame(x,dir_inc_empty_arr))
        ...
        ind_exp_empty_arr = []
        Detail_indirct_exp_df["GroupName"] = Detail_indirct_exp_df["GroupName"] = Detail_indirct_exp_df["GroupName"].apply(lambda x:removeRepetedDataFrame(x,ind_exp_empty_arr))
        ...
        ind_inc_empty_arr = []
        Detail_indirct_inc_df["GroupName"] = Detail_indirct_inc_df["GroupName"] = Detail_indirct_inc_df["GroupName"].apply(lambda x:removeRepetedDataFrame(x,ind_inc_empty_arr))
        ...
        #         # consolidated end
        #         # detailed start
        #         print(Balance,LedgerName,LedgerID,"UUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUVVVVVVVVVVVVVVVVVV")
        ...
        TotalDirectExpense = dirct_exp_df[[str(_('Balance'))]].sum().iloc[0]
        ...
        TotalDirectIncome = dirct_exp_df[[str(_('Balance'))]].sum().iloc[0]
        ...
        dirct_exp_df.columns = [                
        str(_('GroupID')),str(_('GroupName')),str(_('Balance'))]
        TotalInDirectIncome = dirct_exp_df[[str(_('Balance'))]].sum().iloc[0]
        ...
        dirct_exp_df.columns = [                
        str(_('GroupID')),str(_('GroupName')),str(_('Balance'))]
        TotalIndirectExpense = dirct_exp_df[[str(_('Balance'))]].sum().iloc[0]
        json_records = dirct_exp_df.reset_index().to_json(orient ='records')
...
def Query_profitAndLoss_balancing(key, DirExPCons, DirIncCons, inDirExpCons, inDirIncCons, DirectIncomeGreater, DirectExpenseGreater, IndirectExpenseGreater, IndirectIncomeGreater, Indirectdifference, Direct_difference, TotalDirect, TotalIndirect, DirectExpensesData, InDirectExpensesData, DirectIncomeData, IndirectIncomeData, OpeningStock, ClosingStock):
    ...
    if key == 1:
        ...
        # Gross Profit
        print(DirectIncomeGreater,"DirectIncomeGreate===================r",Direct_difference)
    ...
    if key == 2:
        ...
        for (exp_detail, inc_detail) in itertools.zip_longest(DirectExpensesData, DirectIncomeData, fillvalue=""):       
            ...
            if exp_grp_name == '-' and inc_grp_name == '-':
                pass
            else:
        ...
        for (exp_detail, inc_detail) in itertools.zip_longest(InDirectExpensesData, IndirectIncomeData, fillvalue=""):
            ...
            # elif exp_grp_name == '-' and inc_grp_name == '':
            # elif not exp_grp_name == '-' and inc_grp_name == '':
