...
from api.v10.stockPostings.serializers import get_voucherNo
...
def check_groupforBalanceSheet(AccountGroupUnder, CompanyID):
    if not AccountGroupUnder == 0:
        if AccountGroup.objects.filter(CompanyID=CompanyID, AccountGroupID=AccountGroupUnder).exists():
            group_instance = AccountGroup.objects.get(CompanyID=CompanyID, AccountGroupID=AccountGroupUnder)
            AccountGroupUnder = group_instance.AccountGroupUnder
            AccountGroupName = group_instance.AccountGroupName
...
class TrialBalanceSerializer(serializers.ModelSerializer):
    ...
    def get_Debit(self, account_ledger):
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, BranchID=BranchID).exists():
            ...
            for ledgerpost_instance in ledgerpost_instances:
                ...
                Debit += converted_float(ledgerpost_instance.Debit)
                Credit += converted_float(ledgerpost_instance.Credit)

        Tot = converted_float(Debit) - converted_float(Credit)
        if converted_float(Tot) > 0:
            TotalDebit = Tot

        return converted_float(abs(TotalDebit))
    ...
    def get_Credit(self, account_ledger):
        ...
        return converted_float(abs(TotalCredit))
...
class LedgerReportAllSerializer(serializers.ModelSerializer):
    ...
    def get_Debit(self, account_ledger):
        ...
        Debit = 0
        print(ToDate, BranchID, CompanyID, LedgerID,
              "ToDate, BranchID, CompanyID,LedgerID")
        if LedgerPosting.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, BranchID=BranchID, Date__lte=ToDate).exists():
            ledgerpost_instances = LedgerPosting.objects.filter(
                CompanyID=CompanyID, LedgerID=LedgerID, BranchID=BranchID, Date__lte=ToDate)
            for ledgerpost_instance in ledgerpost_instances:
                Debit += ledgerpost_instance.Debit

        print(PriceRounding, type(PriceRounding))
        return converted_float(round(Debit, PriceRounding))
    ...
    def get_Credit(self, account_ledger):
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, BranchID=BranchID, Date__lte=ToDate).exists():
            ...
            return converted_float(round(Credit, PriceRounding))
    ...
    def get_Balance(self, account_ledger):
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, BranchID=BranchID, Date__lte=ToDate).exists():
            ...
            Credit = 0
            Debit = 0
            print("ledger instance length print here >>>>>>>>>>>>>>")
            print(len(ledgerpost_instances))
            for ledgerpost_instance in ledgerpost_instances:
                Debit += converted_float(ledgerpost_instance.Debit)
                Credit += converted_float(ledgerpost_instance.Credit)

            Balance = converted_float(Debit) - converted_float(Credit)
            return converted_float(round(Balance, PriceRounding))
...
class LedgerReportLedgerWiseSerializer(serializers.ModelSerializer):
    ...
    def get_RelatedLedgerName(self, instance):
        ...
        BranchID = instance.BranchID
        RelatedLedgerName = ""
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=RelatedLedgerID).exists():
            AccountLedgerInstances = AccountLedger.objects.get(
                CompanyID=CompanyID, LedgerID=RelatedLedgerID)
...
class LedgerReportGroupSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LedgerPosting
        fields = ('LedgerID', 'LedgerName', 'Date',
                  'Debit', 'Credit', 'GroupUnder')
...
class LedgerReportSerializer(serializers.ModelSerializer):
    ...
    def get_Debit(self, account_ledger):
        ...
        print(CompanyID,
              ...
              ToDate,
              LedgerID)
        if LedgerPosting.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID, Date__lte=ToDate).exists():
            ledgerpost_instances = LedgerPosting.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID, Date__lte=ToDate)
            ...
            amount = converted_float(Debit_sum) - converted_float(Credit_sum)
            if amount > 0:
                Debit = amount
            # for ledgerpost_instance in ledgerpost_instances:
...
class ProfitAndLossSerializer(serializers.ModelSerializer):
    ...
    def get_Balance(self, account_ledger):
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID, Date__gte=FromDate, Date__lte=ToDate).exists():
            ledgerpost_instances = LedgerPosting.objects.filter(
                CompanyID=CompanyID, LedgerID=LedgerID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate)
            for ledgerpost_instance in ledgerpost_instances:
                Debit += ledgerpost_instance.Debit
                Credit += ledgerpost_instance.Credit
            ...
            # if account_ledger:
            #     check = check_groupforProfitandLoss(account_ledger.AccountGroupUnder, CompanyID)
            #     if check == "Direct Income" or check == "Indirect Income":
