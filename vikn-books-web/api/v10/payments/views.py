...
from api.v10.reportQuerys.functions import query_payment_report_data
...
from api.v10.payments.serializers import BillwiseDetailsSerializer, BillwiseMasterSerializer, PaymentDetailsListSerializer, PaymentMasterSerializer, PaymentMasterRestSerializer, PaymentDetailsSerializer, PaymentDetailsRestSerializer
from api.v10.payments.serializers import ListSerializer
from api.v10.payments.functions import export_to_excel_paymentReport, generate_serializer_errors, paymentReport_excel_data
from rest_framework import status
from api.v10.payments.functions import get_auto_id, get_auto_idMaster, get_auto_VoucherNo
...
from api.v10.sales.functions import get_BillwiseDetailsID, get_Genrate_VoucherNo
...
def create_payment(request):
    try:
        with transaction.atomic():
            ...
            insts = PaymentMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, VoucherNo__iexact=VoucherNo)
            if insts:
            ...
            if billwiseDetails_datas:
                for b in billwiseDetails_datas:
                    ...
                    if BillWiseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, BillwiseMasterID=BillwiseMasterID).exists():
                        ...
                        all_details = BillWiseDetails.objects.filter(
...
def billwise_list_customer(request):
    ...
    if call_type == "create":
        if BillWiseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, CustomerID=LedgerID, InvoiceAmount__gt=F('Payments')).exists():
            ...
            # data = serialized.data
            data = sorted(serialized.data, key=lambda k: (
                k['created_date']))
            response_data = {
                "StatusCode": 6000,
                "data": data
    ...
    elif call_type == "list_type":
        if BillWiseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, CustomerID=LedgerID, VoucherType=PaymentVoucherType, InvoiceAmount__gt=F('Payments')).exists():
            bill_wise_instances = BillWiseMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, CustomerID=LedgerID, VoucherType=PaymentVoucherType, InvoiceAmount__gt=F('Payments')).order_by("-VoucherDate")
            serialized = BillwiseMasterSerializer(bill_wise_instances, many=True, context={
                "CompanyID": CompanyID, "PaymentVoucherNo": PaymentVoucherNo, "PaymentVoucherType": PaymentVoucherType, "call_type": call_type})
        ...
        if PaymentDetails.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            if page_number and items_per_page:
                instances = PaymentDetails.objects.filter(
                    CompanyID=CompanyID, BranchID=BranchID).order_by("-PaymentMasterID")
                count = len(instances)
                ledger_sort_pagination = list_pagination(
                    instances,
                    items_per_page,
