...
def billwise_report_data(CompanyID, BranchID, FromDate, ToDate, CustomerID, RouteID):
    ...
    if  not int(RouteID) == 0 and not int(CustomerID) == 0:
        cursor.execute('''
            select P."PartyName",'Sales Invoice' AS Voucher, "VoucherDate", "InvoiceNo",SM."OrderNo" as OrderNo,P."CreditPeriod" as terms,
            "DueDate","RouteName", (SELECT (now()::date - SM."Date"::date)) AS aging, "InvoiceAmount" - "Payments" AS DueAmount
            ...
            UNION
            select P."PartyName",'Sales Return' AS Voucher, SM."VoucherDate", "InvoiceNo",'' as OrderNo,P."CreditPeriod" as terms,
            "DueDate","RouteName", (SELECT (now()::date - SM."VoucherDate"::date)) AS aging, ("InvoiceAmount" - "Payments") * -1 AS DueAmount
            from public.billwise_master AS B
        ...
        cursor.execute('''
            ...
            P."CreditPeriod" as terms,"DueDate","RouteName", (SELECT (now()::date - SM."Date"::date)) AS aging,"InvoiceAmount" - "Payments" AS DueAmount
            from public.billwise_master AS B
            LEFT OUTER JOIN public."salesMasters_salesMaster" AS SM ON B."CustomerID"= SM."LedgerID" 
            AND B."CompanyID_id" = SM."CompanyID_id" AND SM."SalesMasterID" = "TransactionID" AND "VoucherType" = 'SI'
            LEFT OUTER JOIN public.parties_parties AS P ON P."LedgerID" = SM."LedgerID" AND P."CompanyID_id" = SM."CompanyID_id"
            ...
            P."CreditPeriod" as terms,"DueDate","RouteName", (SELECT (now()::date - SM."VoucherDate"::date)) AS aging,("InvoiceAmount" - "Payments") * -1 AS DueAmount
            from public.billwise_master AS B
            LEFT OUTER JOIN public."salesReturnMasters_salesReturnMaster" AS SM ON B."CustomerID"= SM."LedgerID" 
            AND B."CompanyID_id" = SM."CompanyID_id" AND SM."SalesReturnMasterID" = "TransactionID" AND "VoucherType" = 'SR'
            LEFT OUTER JOIN public.parties_parties AS P ON P."LedgerID" = SM."LedgerID" AND P."CompanyID_id" = SM."CompanyID_id"
    ...
    if cursor.rowcount == 0:
        ...
        for name, group in grouped:
            ...
            new_row = pd.DataFrame({'PartyName': name, 'Type': 'Total {}'.format(name),
                                    'Date': '', 'InvoiceNo': "-", 'PO': '', 'Terms': '', 'DueDate': "", 'Class': "",
                                    'aging': '', 'OpenBalance': Total}, index=[0])
...
def dashboard_data(CompanyID, BranchID, FromDate, ToDate,Group):
    ...
    cursor.execute('''
        ...
        SELECT "AccountGroupName","LedgerName" ,  "Debit" - "Credit" AS Balace,DATE_TRUNC('month',"Date") AS Date
            FROM public."accountLedger_accountLedger" AL  
        INNER JOIN public."accountgroup_accountgroup" AS A ON AL."AccountGroupUnder" = A."AccountGroupID" AND AL."CompanyID_id" = A."CompanyID_id" 
        LEFT OUTER JOIN public."ledgerPostings_ledgerPosting" as L ON AL."LedgerID" = L."LedgerID" AND AL."CompanyID_id" = L."CompanyID_id" AND AL."BranchID" = L."BranchID"
        AND L."Date" > %(FromDate)s AND L."Date" < %(ToDate)s WHERE AL."AccountGroupUnder" IN (SELECT "AccountGroupID" FROM GroupInMainGroup) AND AL."BranchID" = %(BranchID)s
        ...
        cursor.execute('''
            SELECT "RouteName",SUM(GrandTotal) AS GrandTotal,SUM(CashSales) AS CashSales,SUM(BankSales) AS BankSales, SUM(Receipt) AS Receipt FROM (
                SELECT * ,(SELECT ROUND(COALESCE(SUM("NetAmount"),0),2) FROM public."receiptDetailses_receiptDetails" AS D 
                        ...
                        WHERE D."CompanyID_id" = 'e837c6a7-288a-4199-8b61-cc7905e54451' AND "Date" between '2022-01-01' and '2022-10-01' AND D."LedgerID" = t."LedgerID") AS Receipt
                FROM (
                select 
