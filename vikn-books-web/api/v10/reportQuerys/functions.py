...
from brands.models import CompanySettings, Parties, UserTable
...
def query_sales_report_data(
    ...
    Country = CompanySettings.objects.get(id=CompanyID)
    CountryName = "India"
    if Country:
        CountryName = Country.Country.Country_Name

    dic = {"BranchID": BranchID,"CompanyID": CompanyID,"ToDate": ToDate,"FromDate": FromDate,"PriceRounding": PriceRounding,"UserID": UserID,"EmployeeID": EmployeeID,"CustomerID": CustomerID,"filterValue":filterValue,"CountryName":CountryName}
    ...
    cursor.execute(
        """
            ...
            CASE WHEN  
            S."EmployeeID" >0  THEN(SELECT COALESCE("FirstName", '') FROM public.employees_employees WHERE "EmployeeID" = S."EmployeeID" AND "CompanyID_id" = S."CompanyID_id")      
            WHEN S."EmployeeID" =0 THEN ''
            END AS SalesMan,
            
            CASE WHEN %(CountryName)s = 'India' THEN
            (SELECT COALESCE("GSTNumber", '') FROM public."parties_parties" AS C WHERE "CompanyID_id" = %(CompanyID)s AND C."LedgerID" = S."LedgerID" fetch first 1 rows only)
            ELSE
            (SELECT COALESCE("VATNumber", '') FROM public."parties_parties" AS C WHERE "CompanyID_id" = %(CompanyID)s AND C."LedgerID" = S."LedgerID" fetch first 1 rows only)
            END,
            ...
            Cash=0.00
            WHEN %(filterValue)s = 'partial' THEN
            Cash > 0 and Cash < GrandTotal
            ELSE
            Cash > 0 or Cash=0
            END
     """,
...
def query_sales_both_report_data(
    ...
    cursor.execute(
        """
            ...
            CASE WHEN %(CountryName)s = 'India' THEN
            (SELECT COALESCE("GSTNumber", '') FROM public."parties_parties" AS C WHERE "CompanyID_id" = S."CompanyID_id" AND C."LedgerID" = S."LedgerID" fetch first 1 rows only)
            ELSE
            (SELECT COALESCE("VATNumber", '') FROM public."parties_parties" AS C WHERE "CompanyID_id" = S."CompanyID_id" AND C."LedgerID" = S."LedgerID" fetch first 1 rows only)
            END,
...
def query_purchase_report_data(
    ...
    dic = {"BranchID": BranchID,"CompanyID": CompanyID,"ToDate": ToDate,"FromDate": FromDate,"PriceRounding": PriceRounding,"UserID": UserID,"EmployeeID": EmployeeID,"CustomerID": CustomerID,"filterValue":filterValue,"ReffNo":ReffNo,"CountryName":CountryName}
    ...
    cursor.execute(
        ...
        CASE WHEN %(CountryName)s = 'India' THEN
        (SELECT COALESCE("GSTNumber", '') FROM public."parties_parties" AS C WHERE "CompanyID_id" = P."CompanyID_id" AND C."LedgerID" = P."LedgerID" fetch first 1 rows only)
        ELSE
        (SELECT COALESCE("VATNumber", '') FROM public."parties_parties" AS C WHERE "CompanyID_id" = P."CompanyID_id" AND C."LedgerID" = P."LedgerID" fetch first 1 rows only)
        END,
        ...
        -- (select
        --     CASE WHEN length("VATNumber") > 0 THEN "VATNumber" WHEN length("GSTNumber") > 0 THEN "GSTNumber" END AS TaxNo 
        --     from public."parties_parties" where "CompanyID_id" = P."CompanyID_id" AND "PartyName" = AL."LedgerName")
...
def query_purchaseReturn_report_data(
    ...
    dic = {"BranchID": BranchID,"CompanyID": CompanyID,"ToDate": ToDate,"FromDate": FromDate,"PriceRounding": int(PriceRounding),"UserID": UserID,"EmployeeID": EmployeeID,"CustomerID": CustomerID,"filterValue":filterValue,"ReffNo":ReffNo,"CountryName":CountryName}
    cursor.execute(
        '''SELECT * FROM (SELECT  
        P."id" as id ,           
        P."VoucherNo" as VoucherNo ,           
...
def query_outStandingReport_report_data(
    ...
    if RouteLedgers and VoucherType == "null":
        cursor.execute(
            """ select "LedgerID","LedgerName",round(Debit,2),round(Credit,2) FROM(  
        ...
        cursor.execute(
            """ select "LedgerID","LedgerName",round(Debit,2),round(Credit,2) FROM(  
            SELECT LP."LedgerID",Al."LedgerName",  
            Case WHEN (SUM(LP."Debit") -SUM(LP."Credit") < 0 ) THEN round(SUM(LP."Credit" - LP."Debit"),2) Else 0 end AS Credit ,    
            Case WHEN(SUM(LP."Debit") -SUM(LP."Credit") > 0 ) THEN round(SUM(LP."Debit" - LP."Credit"),2) Else 0 end AS Debit  
            from public."ledgerPostings_ledgerPosting" AS LP  
            INNER JOIN public.parties_parties AS P ON LP."BranchID" = P."BranchID"  and lp."LedgerID" = P."LedgerID"
                AND P."CompanyID_id" = LP."CompanyID_id"
                    INNER JOIN public."accountLedger_accountLedger" AS AL ON LP."LedgerID" = AL."LedgerID" 
...
def query_stock_report_data(
    ...
    if ProductFilter == "1":
        cursor.execute(
            ...
            (SELECT COALESCE(round(SUM(SP."QtyIn"),2) - round(SUM(SP."QtyOut"),2),0)
            ...
            (SELECT COALESCE("UnitName", '') FROM public."units_unit" AS UN WHERE UN."CompanyID_id" = P."CompanyID_id" AND UN."UnitID" = PL."UnitID" AND PL."DefaultUnit" = 'true'  ) AS BaseUnitName,

            (SELECT coalesce("MultiFactor", '0') FROM public."pricelist_pricelist" AS UN WHERE UN."CompanyID_id" = P."CompanyID_id" AND UN."ProductID" = PL."ProductID" AND UN."UnitInReports" = 'true')AS StockInBaseUnit,
            


            (SELECT COALESCE("UnitName", '') FROM public."units_unit" AS UN INNER JOIN public.pricelist_pricelist as AA ON  AA."BranchID" = '1' AND 
            ...
            PG."GroupName",
            (SELECT COALESCE("CategoryName", '') FROM public."productcategory_productcategory" AS C WHERE C."CompanyID_id" = P."CompanyID_id" AND C."ProductCategoryID" = PG."CategoryID" )
            AS CategoryName,
            (SELECT COALESCE("BrandName", '') FROM public."brands_brand" AS B WHERE B."CompanyID_id" = P."CompanyID_id" AND B."BrandID" = P."BrandID" )
            AS BrandName,
        ...
        cursor.execute(
            ...
            (SELECT coalesce("MultiFactor", '0') FROM public."pricelist_pricelist" AS UN WHERE UN."CompanyID_id" = P."CompanyID_id" AND UN."ProductID" = PL."ProductID" AND UN."UnitInReports" = 'true') AS MultiFactor,
    ...
    print("================================")
    print("countttttt",cursor.rowcount)
    print(cursor)
    ...
    convert_dict = {'id': str}
    df = df.astype(convert_dict)
    # df[str(_("CurrentStock"))] = df[str(_("CurrentStock"))].apply(
    #     lambda x: int(999) if x == None else x)

    # if df['CurrentStock'].eq(0).any():
    #     sum_column = 0
    # else:
    #     sum_column = df["StockInBaseUnit"] / df["CurrentStock"]
    #     # sum_column = [y*x if x > 1 else y/x for x,y in zip(df['CurrentStock'],df['StockInBaseUnit'])]
        
    # df["CurrentStock"] = sum_column
    if df['CurrentStock'].eq(0).any() or df['StockInBaseUnit'].eq(0).any():
        print("FIRST STOCK REPORT IN CHECKING ****************************")
        test = [0 if x == None else x if x>1 else print(y,x,"UUUUYYII") for x,y in zip(df['CurrentStock'],df['StockInBaseUnit'])]
        df[str(_("CurrentStock"))] = 0
    else:
        # df[str(_("CurrentStock"))] = [0 if x == None else x if x>1 else y/x for x,y in zip(df['CurrentStock'],df['StockInBaseUnit'])]
        df[str(_("CurrentStock"))] = [0 if x == None or y == None else y/x if x>1 else y/x for x,y in zip(df['CurrentStock'],df['StockInBaseUnit'])]
...
def query_stockLedger_report_data(
    ...
    if ProductID > 0 and WareHouseID > 0:
        cursor.execute(
            ...
            WHEN "VoucherType" = 'ES' THEN (SELECT "VoucherNo" FROM public."stock_management_master" WHERE "BranchID" = '1' AND "StockMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id")  
            WHEN "VoucherType" = 'SS' THEN (SELECT "VoucherNo" FROM public."stock_management_master" WHERE "BranchID" = '1' AND "StockMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id")  
            WHEN "VoucherType" = 'DS' THEN (SELECT "VoucherNo" FROM public."stock_management_master" WHERE "BranchID" = '1' AND "StockMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id")  
            WHEN "VoucherType" = 'US' THEN (SELECT "VoucherNo" FROM public."stock_management_master" WHERE "BranchID" = '1' AND "StockMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id" )  
        ...
        "Date",W."WarehouseName",  
			...
			WHEN "VoucherType" = 'ES' THEN (SELECT "Stock" FROM public."stock_management_details" WHERE "CompanyID_id" = S."CompanyID_id" AND "StockDetailsID" = S."VoucherDetailID")
			WHEN "VoucherType" = 'SS' THEN (SELECT "Stock" FROM public."stock_management_details" WHERE "CompanyID_id" = S."CompanyID_id" AND "StockDetailsID" = S."VoucherDetailID")
			WHEN "VoucherType" = 'DS' THEN (SELECT "Stock" FROM public."stock_management_details" WHERE "CompanyID_id" = S."CompanyID_id" AND "StockDetailsID" = S."VoucherDetailID")
			WHEN "VoucherType" = 'US' THEN (SELECT "Stock" FROM public."stock_management_details" WHERE "CompanyID_id" = S."CompanyID_id" AND "StockDetailsID" = S."VoucherDetailID")
			...
			-- WHEN "VoucherType" = 'ES' THEN (SELECT "FreeQty" FROM public."stock_management_details" WHERE "CompanyID_id" = S."CompanyID_id" AND "StockDetailsID" = S."VoucherDetailID")
			-- WHEN "VoucherType" = 'SS' THEN (SELECT "FreeQty" FROM public."stock_management_details" WHERE "CompanyID_id" = S."CompanyID_id" AND "StockDetailsID" = S."VoucherDetailID")
			-- WHEN "VoucherType" = 'DS' THEN (SELECT "FreeQty" FROM public."stock_management_details" WHERE "CompanyID_id" = S."CompanyID_id" AND "StockDetailsID" = S."VoucherDetailID")
			-- WHEN "VoucherType" = 'US' THEN (SELECT "FreeQty" FROM public."stock_management_details" WHERE "CompanyID_id" = S."CompanyID_id" AND "StockDetailsID" = S."VoucherDetailID")
            ...
            --------END-----------
            CASE  
            WHEN "VoucherType" = 'SI' THEN (SELECT "id" FROM public."salesMasters_salesMaster" WHERE "BranchID" = '1' AND "SalesMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id" )  
            WHEN "VoucherType" = 'SR' THEN (SELECT "id" FROM public."salesReturnMasters_salesReturnMaster" WHERE "BranchID" = '1' AND "SalesReturnMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id")  
            WHEN "VoucherType" = 'PI' THEN (SELECT "id" FROM public."purchaseMasters_purchaseMaster" WHERE "BranchID" = '1' AND "PurchaseMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id")  
            WHEN "VoucherType" = 'PR' THEN (SELECT "id" FROM public."purchaseReturnMasters_purchaseReturnMaster" WHERE "BranchID" = '1' AND "PurchaseReturnMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id")  
            WHEN "VoucherType" = 'OS' THEN (SELECT "id" FROM public."openingStockMasters_openingStockMaster" WHERE "BranchID" = '1' AND "OpeningStockMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id")  
            WHEN "VoucherType" = 'WO' THEN (SELECT "id" FROM public."WorkOrderMasters_WorkOrderMaster" WHERE "BranchID" = '1' AND "WorkOrderMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id")  
            WHEN "VoucherType" = 'ST' THEN (SELECT "id" FROM public."stockTransferMaster_ID_stockTransferMaster_ID" WHERE "BranchID" = '1' AND "StockTransferMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id")  
            WHEN "VoucherType" = 'ES' THEN (SELECT "id" FROM public."stock_management_master" WHERE "BranchID" = '1' AND "StockMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id")  
            WHEN "VoucherType" = 'SS' THEN (SELECT "id" FROM public."stock_management_master" WHERE "BranchID" = '1' AND "StockMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id")  
            WHEN "VoucherType" = 'DS' THEN (SELECT "id" FROM public."stock_management_master" WHERE "BranchID" = '1' AND "StockMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id")  
            WHEN "VoucherType" = 'US' THEN (SELECT "id" FROM public."stock_management_master" WHERE "BranchID" = '1' AND "StockMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id" )  
            WHEN "VoucherType" = 'SA' THEN (SELECT "id" FROM public."stock_management_master" WHERE "BranchID" = '1' AND "StockMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id" )  
            END AS id,
            (SELECT "UnitName" FROM public.units_unit WHERE "CompanyID_id" = S."CompanyID_id" AND "UnitID" =PR."UnitID") as BaseunitName,
            U."username",
            CASE
			WHEN "VoucherType" = 'SI' THEN (SELECT "LedgerID" FROM public."accountLedger_accountLedger" WHERE "CompanyID_id" = S."CompanyID_id" AND "LedgerID" = (SELECT "LedgerID" FROM public."salesMasters_salesMaster" WHERE "BranchID" = '1' AND "SalesMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id"))  
            WHEN "VoucherType" = 'SR' THEN (SELECT "LedgerID" FROM public."accountLedger_accountLedger" WHERE "CompanyID_id" = S."CompanyID_id" AND "LedgerID" = (SELECT "LedgerID" FROM public."salesReturnMasters_salesReturnMaster" WHERE "BranchID" = '1' AND "SalesReturnMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id"))
            WHEN "VoucherType" = 'PI' THEN (SELECT "LedgerID" FROM public."accountLedger_accountLedger" WHERE "CompanyID_id" = S."CompanyID_id" AND "LedgerID" = (SELECT "LedgerID" FROM public."purchaseMasters_purchaseMaster" WHERE "BranchID" = '1' AND "PurchaseMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id"))
            WHEN "VoucherType" = 'PR' THEN (SELECT "LedgerID" FROM public."accountLedger_accountLedger" WHERE "CompanyID_id" = S."CompanyID_id" AND "LedgerID" = (SELECT "LedgerID" FROM public."purchaseReturnMasters_purchaseReturnMaster" WHERE "BranchID" = '1' AND "PurchaseReturnMasterID"="VoucherMasterID" AND "CompanyID_id" = S."CompanyID_id"))
			ELSE '0' END AS LedgerID
...
def query_openingBalanceDifference_data(
    ...
    if ManualOpeningBalance:
        ...
        cursor.execute(
            """
            SELECT COALESCE(SUM("Debit"),0) - COALESCE(SUM("Credit"),0) AS OpeningDifference               
...
def query_ledger_report_data(CompanyID, BranchID, FromDate, ToDate, PriceRounding, VoucherType, value, ManualOpeningBalance,is_fromDate):
    cursor = connection.cursor()
    dic = {
        "value": value,
        "BranchID": BranchID,
    ...
    if VoucherType == "all_ledger":
        ...
        cursor.execute(
            ...
            FROM public."accountLedger_accountLedger" AS A   
            INNER JOIN "public"."ledgerPostings_ledgerPosting" AS L ON L."CompanyID_id" = A."CompanyID_id" AND L."LedgerID" = A."LedgerID" AND L."BranchID" = A."BranchID" AND
            
            CASE WHEN %(is_fromDate)s = true THEN
            L."Date" BETWEEN %(FromDate)s AND %(ToDate)s
            ELSE
            L."Date" <= %(ToDate)s
            END              

            WHERE A."CompanyID_id" = %(CompanyID)s AND A."BranchID" = %(BranchID)s GROUP BY L."LedgerID",A."LedgerName" ) AS TEMP WHERE Balance != 0 ORDER BY "LedgerName"   
            """,
            dic,
        )
        data = cursor.fetchall()
        ...
        cursor.execute(
            ...
            ,round(L."Debit",2)   
            ,round(L."Credit",2)
            ,CASE WHEN "VoucherType" = 'SI' THEN(SELECT "id" FROM public."salesMasters_salesMaster" WHERE "BranchID" = L."BranchID" AND "SalesMasterID" = "VoucherMasterID" AND "CompanyID_id" = L."CompanyID_id")   
            WHEN "VoucherType" = 'SR' THEN(SELECT "id" FROM public."salesReturnMasters_salesReturnMaster" WHERE "BranchID" = L."BranchID" AND "SalesReturnMasterID" = "VoucherMasterID" AND "CompanyID_id" = L."CompanyID_id")   
            WHEN "VoucherType" = 'PI' THEN(SELECT "id" FROM public."purchaseMasters_purchaseMaster" WHERE "BranchID" = L."BranchID" AND "PurchaseMasterID" = "VoucherMasterID" AND "CompanyID_id" = L."CompanyID_id")   
            WHEN "VoucherType" = 'PR' THEN(SELECT "id" FROM public."purchaseReturnMasters_purchaseReturnMaster" WHERE "BranchID" = L."BranchID" AND "PurchaseReturnMasterID" = "VoucherMasterID" AND "CompanyID_id" = L."CompanyID_id") 
            END AS id
    ...
    df = pd.DataFrame(data)
    if VoucherType == "all_ledger":
        df.columns = [str(_("LedgerID")),str(_("LedgerName")), str(_("Balance"))]
    elif VoucherType == "group_wise":
        df.columns = [
            str(_("LedgerName")),
            str(_("Debit")),
...
def query_updateLedgerBalance(CompanyID, BranchID):
    ...
    cursor.execute(
        """
        UPDATE public."accountLedger_accountLedger" AS A SET "Balance" = (SELECT COALESCE(SUM("Debit" - "Credit") ,0) 
...
def query_stockValue_report_data(
    ...
    cursor.execute(
        ...
        (SELECT COALESCE("WarehouseName", '') FROM public."warehouse_warehouse" AS WH WHERE WH."CompanyID_id" = P."CompanyID_id" AND WH."WarehouseID" = %(WareHouseID)s)
        ELSE ''
        ...
        --- AVG ----        
            ...
            "PurchasePrice" FROM public.pricelist_pricelist AS PL WHERE PL."ProductID" = P."ProductID" AND PL."DefaultUnit" = 'true' AND "CompanyID_id" = P."CompanyID_id") 
    ...
    convert_dict = {'id': str}
    df = df.astype(convert_dict)
    # df[str(_("Stock"))] = df[str(_("Stock"))].apply(
    #     lambda x: int(999) if x == None else x)

    # df[str(_("Stock"))] = [y*x if x > 1 else y/x for x,y in zip(df['Stock'],df['StockInBaseUnit'])]
    if df['Stock'].eq(0).any() or df['StockInBaseUnit'].eq(0).any():
        df[str(_("Stock"))] = 0
    else:    
        df[str(_("Stock"))] = [0 if x == None or y == None else y/x if x>1 else y/x for x,y in zip(df['Stock'],df['StockInBaseUnit'])]

    # if df['Stock'].eq(0).any():
    #     sum_column = 0
    # else:
    #     sum_column = df["StockInBaseUnit"] / df["Stock"]
    #     # sum_column = [y*x if x > 1 else y/x for x,y in zip(df['Stock'],df['StockInBaseUnit'])]
...
def query_closingTrialBalance_report_data(
    templateType,filterMethod,CompanyID, UserID, PriceRounding, BranchID, ToDate, FromDate
):
    exp_cursor = connection.cursor()
    # inc_cursor = connection.cursor()
    asset_liabilty_cursor = connection.cursor()
    ...
    exp_cursor.execute('''
        WITH RECURSIVE GroupInMainGroup AS (
                ...
                FROM public.accountgroup_accountgroup
                WHERE
                "AccountGroupID" in (40,3,4,41) AND "CompanyID_id" = %(CompanyID)s
                UNION
                SELECT
                e."AccountGroupID",
                e."AccountGroupUnder",
                ...
                SELECT "AccountGroupName","LedgerName" ,  
                (SELECT ROUND(coalesce(SUM("Debit"), 0)- (coalesce(SUM("Credit"), 0)),2) 
                FROM public."ledgerPostings_ledgerPosting" WHERE ("Date" < %(FromDate)s )
    ...
    #     WHERE AL."AccountGroupUnder" IN (SELECT "AccountGroupID" FROM GroupInMainGroup) AND AL."BranchID" = %(BranchID)s
    #     AND AL."CompanyID_id" = %(CompanyID)s
    #     ) AS TEMP WHERE (ABS(Credit) + ABS(Debit) + ABS(Closing) + ABS(Opening)) != 0;
    ...
    df.columns = [str(_("AccountGroupName")), str(
        _("LedgerName")),str(_("OpeningBalance")), str(_("TotalDebit")),str(_("TotalCredit")),str(_("ClosingBalance")),str(_("Total"))]
    ...
    # Taiking Debit & Credit From Total
    df[str(_("Debit"))] = df[str(_("Total"))].apply(
        lambda x: round(float(x), PriceRounding) if x >= 0 else 0)
    df[str(_("Credit"))] = df[str(_("Total"))].apply(lambda x: round(
        abs(float(x)), PriceRounding) if x <= 0 else 0)
    
    # Taiking Debit & Credit From OpeningBalance
    df[str(_("OpeningDebit"))] = df[str(_("OpeningBalance"))].apply(
        lambda x: round(float(x), PriceRounding) if x >= 0 else 0)
    df[str(_("OpeningCredit"))] = df[str(_("OpeningBalance"))].apply(lambda x: round(
        abs(float(x)), PriceRounding) if x <= 0 else 0)

    # Taiking Debit & Credit From ClosingBalance
    df[str(_("ClosingDebit"))] = df[str(_("ClosingBalance"))].apply(
        lambda x: round(float(x), PriceRounding) if x >= 0 else 0)
    df[str(_("ClosingCredit"))] = df[str(_("ClosingBalance"))].apply(lambda x: round(
        abs(float(x)), PriceRounding) if x <= 0 else 0)
    ...
    if TotalAvgValueOpening > 0:
        debit = float(TotalAvgValueOpening)
        df.loc[len(df.index)] = ["Opening Stock", "Opening Stock Value", 0,0, 0,0,debit,credit,0,0,0,0]
    elif TotalAvgValueOpening < 0:
        credit = float(TotalAvgValueOpening)
        df.loc[len(df.index)] = ["Opening Stock", "Opening Stock Value", 0,0, 0,0,debit,credit,0,0,0,0]
    else:
        print("ELSE",TotalAvgValueOpening)
    ...
    if Debit_total > Credit_total:
        diffrence = Debit_total
        credit = Debit_total - Credit_total
        df.loc[tot_index] = ["Loss", "Loss", 0,0,0,0,debit, credit,0,0,0,0]
    elif Debit_total < Credit_total:
        diffrence = Credit_total
        debit = Credit_total - Debit_total
        df.loc[tot_index] = ["Profit", "Profit",0,0,0,0, debit, credit,0,0,0,0]
    elif Debit_total == Credit_total:
        diffrence = Debit_total
    
    df.loc[tot_index+1] = ["Total", "Total", 0,0,0,0,diffrence, diffrence,0,0,0,0]
    ...
    if templateType == "Closing Trial Balance" or templateType == "Trial Balance":
        grouped = df.groupby(['AccountGroupName'],sort=False)
        merged_df = []
    
        for name, group in grouped:
            group.loc[-1] = [name,"","","","","","","","","","",""]  # adding a row
            group.index = group.index + 1  # shifting index
            group = group.sort_index()
            merged_df.append(group)
        
        df = pd.concat(merged_df)
        # df = grouped.aggregate(np.sum).reset_index()
...
def query_Cash_OR_Bank_Book_report_data(value, CompanyID, BranchID, FromDate, ToDate, PriceRounding, VoucherType, RouteLedgers, ManualOpeningBalance):
    ...
    END AS "Narration"        
        ...
        ,round(L."Debit",2)        
        ,round(L."Credit",2),
            CASE WHEN "VoucherType" ='SI' THEN (SELECT "id" FROM public."salesMasters_salesMaster" WHERE "BranchID" = '1' AND "SalesMasterID" = "VoucherMasterID" AND "CompanyID_id" = L."CompanyID_id")         
            WHEN "VoucherType" ='SR' THEN (SELECT "id" FROM public."salesReturnMasters_salesReturnMaster" WHERE "BranchID" = '1' AND "SalesReturnMasterID" = "VoucherMasterID" AND "CompanyID_id" = L."CompanyID_id")        
            WHEN "VoucherType" ='PI' THEN (SELECT "id" FROM public."purchaseMasters_purchaseMaster" WHERE "BranchID" = '1' AND "PurchaseMasterID" = "VoucherMasterID" AND "CompanyID_id" = L."CompanyID_id")        
            WHEN "VoucherType" ='PR' THEN (SELECT "id" FROM public."purchaseReturnMasters_purchaseReturnMaster" WHERE "BranchID" = '1' AND "PurchaseReturnMasterID" = "VoucherMasterID" AND "CompanyID_id" = L."CompanyID_id")        
            WHEN "VoucherType" ='CR' THEN (SELECT "id" FROM public."receiptMasters_receiptMaster" WHERE "BranchID" = '1' AND "ReceiptMasterID" = "VoucherMasterID" AND "VoucherType"='CR' AND "CompanyID_id" = L."CompanyID_id")        
            WHEN "VoucherType" ='BR' THEN (SELECT "id" FROM public."receiptMasters_receiptMaster" WHERE "BranchID" = '1' AND "ReceiptMasterID" = "VoucherMasterID" AND "VoucherType"='BR' AND "CompanyID_id" = L."CompanyID_id")        
            WHEN "VoucherType" ='CP' THEN (SELECT "id" FROM public."paymentMasters_paymentMaster" WHERE "BranchID" = '1' AND "PaymentMasterID" = "VoucherMasterID" AND "VoucherType"='CP' AND "CompanyID_id" = L."CompanyID_id")        
            WHEN "VoucherType" ='BP' THEN (SELECT "id" FROM public."paymentMasters_paymentMaster" WHERE "BranchID" = '1' AND "PaymentMasterID" = "VoucherMasterID" AND "VoucherType"='BP' AND "CompanyID_id" = L."CompanyID_id")        
            WHEN "VoucherType" ='LOB' THEN (SELECT "id" FROM public."accountLedger_accountLedger" WHERE "BranchID" = '1' AND "LedgerID" = "VoucherMasterID" AND "CompanyID_id" = L."CompanyID_id")        
            WHEN "VoucherType" ='OS' THEN (SELECT "id" FROM public."openingStockMasters_openingStockMaster" WHERE "BranchID" = '1' AND "OpeningStockMasterID" = "VoucherMasterID" AND "CompanyID_id" = L."CompanyID_id")          
            WHEN "VoucherType" ='JL' THEN (SELECT "id" FROM public."journalMaster_journalMaster" WHERE "BranchID" = '1' AND "JournalMasterID" = "VoucherMasterID" AND "CompanyID_id" = L."CompanyID_id")   
            WHEN "VoucherType" ='EX' THEN (SELECT "id" FROM public."expenseMasters_expenseMaster" WHERE "BranchID" = '1' AND "ExpenseMasterID" = "VoucherMasterID" AND "CompanyID_id" = L."CompanyID_id")  
            END AS "id"     
    ...
    df.columns = [str(_('VoucherNo')), str(_('Narration')), str(_('Date')), str(_('Particulars')), str(
        _('LedgerName')), str(_('VoucherType')), str(_('Receipts')), str(_('Payments')), str(_('id'))]
    # convert date format(millisecond) to str in dataframe
    df['Date'] = df['Date'].astype(str)
    # concert uuid to string
    convert_dict = {'id': str}
...
def query_receipt_report_data(CompanyID, BranchID, FromDate, ToDate, PriceRounding, CashLedgers, VoucherType):
    ...
    # adding Heading to frame
    df.columns = [
        str(('VoucherNo')), str(('Date')), str(('LedgerName')), str(('Amount')), str(('Discount')), str(('NetAmount')), str(('VoucherType')), str(('Narration')), str(('id'))]
    # convert date format(millisecond) to str in dataframe
    df['Date'] = df['Date'].astype(str)
    # concert uuid to string
    convert_dict = {'id': str}
...
def billwise_profit_report_data(CompanyID, BranchID, FromDate, ToDate, UserID, WareHouseID, CustomerID, RouteID):
    ...
    # adding Heading to frame
    df.columns = [
        str(('Date')), str(('InvoiceNo')), str(('GrossAmount')), str(('TotalTax')), str(('NetAmount')), str(('NetCost')), str(('id'))]
...
def sales_register_report_data(CompanyID, BranchID, FromDate, ToDate, WarehouseID, UserID, LedgerID, ProductID, ProductGroupID, ProductCategoryID, ProductCode, Barcode):
    ...
    if int(ProductID) > 0:
        cursor.execute('''
            SELECT
            s."id","Date", "VoucherNo" as InvoiceNo,P."ProductCode", P."ProductName", PL."Barcode", round(SD."Qty",2),
            (SELECT COALESCE("UnitName", '') FROM public."units_unit" AS UN WHERE UN."CompanyID_id" = P."CompanyID_id" AND UN."UnitID" = PL."UnitID"  ) AS UnitName,
    ...
    df.columns = [
        str(('id')),str(('Date')), str(('InvoiceNo')), str(('ProductCode')), str(('ProductName')), str(('Barcode')), str(('Qty')),str(('UnitName')), str(('SalesPrice')), str(('GrossAmount')), str(('VATAmount')), str(('NetAmount')), str(('Cost'))]
...
def purchase_register_report_data(CompanyID, BranchID, FromDate, ToDate, WarehouseID, UserID, LedgerID, ProductID, ProductGroupID, ProductCategoryID, ProductCode, Barcode):
    ...
    if int(ProductID) > 0:
        cursor.execute('''
            SELECT 
            PM."id","Date", "VoucherNo" as InvoiceNo,P."ProductCode", P."ProductName", PL."Barcode", round(PD."Qty",2),
    ...
    df.columns = [
        str(('id')),str(('Date')), str(('InvoiceNo')), str(('ProductCode')), str(('ProductName')), str(('Barcode')), str(('Qty')), str(('UnitPrice')), str(('GrossAmount')), str(('VATAmount')), str(('NetAmount')), str(('Cost'))]
...
def salesReturn_register_report_data(CompanyID, BranchID, FromDate, ToDate, WarehouseID, UserID, LedgerID, ProductID, ProductGroupID, ProductCategoryID, ProductCode, Barcode):
    ...
    if int(ProductID) > 0:
        cursor.execute('''
            SELECT 
            SR."id","VoucherDate" as Date, "VoucherNo" as InvoiceNo,P."ProductCode", P."ProductName", PL."Barcode", round(SD."Qty",2),
            round(SD."UnitPrice",2) as SalesPrice, round(SD."GrossAmount",2), round(SD."VATAmount",2), round(SD."NetAmount",2), round(SD."CostPerPrice",2) as Cost
    ...
    df.columns = [
        str(('id')),str(('Date')), str(('InvoiceNo')), str(('ProductCode')), str(('ProductName')), str(('Barcode')), str(('Qty')), str(('SalesPrice')), str(('GrossAmount')), str(('VATAmount')), str(('NetAmount')), str(('Cost'))]
...
def purchaseReturn_register_report_data(CompanyID, BranchID, FromDate, ToDate, WarehouseID, UserID, LedgerID, ProductID, ProductGroupID, ProductCategoryID, ProductCode, Barcode):
    ...
    if int(ProductID) > 0:
        cursor.execute('''
            SELECT 
            PM."id","VoucherDate" as Date, "VoucherNo" as InvoiceNo,P."ProductCode", P."ProductName", PL."Barcode", round(PD."Qty",2),
...
def stock_transfer_report_data(CompanyID,BranchID,FromDate,ToDate,PriceRounding,UserID,WarehouseFromID,WarehouseToID,VoucherType):
    ...
    # adding Heading to frame
    df.columns = [
        str(_('id')),str(_('VoucherNo')), str(_('Date')), str(_('User')), str(_('WarehouseFrom')), str(_('WarehouseTo')), str(_('TotalQty')), str(_('GrandTotal'))]
    # convert date format(millisecond) to str in dataframe
    df['Date'] = df['Date'].astype(str)
     # concert uuid to string
    convert_dict = {'id': str}
...
def query_BalanceSheet_NetLoss_or_NetProfit(ToDate,FromDate,BranchID,CompanyID,OPENINGSTOCK,CLOSINGSTOCK):
    ...
    print(DIRECTEXPENCES+OPENINGSTOCK,"DIRECTEXPENCES*******============DIRECTEXPENCES")
    ...
    print(DIRECTEXPENCES,"CHECKING>>>>.................")
    INDIRECTEXPENCES = GrossLoss + INDIRECTEXPENCES
    INDIRECTINCOME = GrossProfit + INDIRECTINCOME
    
    print(DIRECTEXPENCES,INDIRECTEXPENCES)
    print("DIRECTINCOME111*******============DIRECTINCOME")
    print (DIRECTINCOME,INDIRECTINCOME)
...
def billwise_report_data(CompanyID, BranchID, FromDate, ToDate, CustomerID, RouteID):
    if Parties.objects.filter(CompanyID=CompanyID, BranchID=BranchID, PartyID=CustomerID).exists():
        CustomerID = Parties.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID, PartyID=CustomerID).first().LedgerID
    cursor = connection.cursor()
    dic = {'CompanyID': CompanyID, 'BranchID': BranchID, 'FromDate': FromDate, 'ToDate': ToDate,"CustomerID": CustomerID, "RouteID": RouteID}

    if  not int(RouteID) == 0 and not int(CustomerID) == 0:
        cursor.execute('''
            select P."PartyName",'Sales Invoice' AS Voucher, "VoucherDate", "InvoiceNo",SM."OrderNo" as OrderNo,"DueDate" - (select CURRENT_DATE) as terms,
            "DueDate","RouteName", (SELECT (now()::date - SM."Date"::date)) AS aging, "InvoiceAmount" - "Payments" AS DueAmount,SM."id"
            from public.billwise_master AS B
            LEFT OUTER JOIN public."salesReturnMasters_salesReturnMaster" AS SM ON B."CustomerID"= SM."LedgerID" 
            AND B."CompanyID_id" = SM."CompanyID_id" AND SM."SalesMasterID" = "TransactionID" AND "VoucherType" = 'SR'
            LEFT OUTER JOIN public.parties_parties AS P ON P."LedgerID" = SM."LedgerID" AND P."CompanyID_id" = SM."CompanyID_id"
            LEFT OUTER JOIN public.route_route AS R ON P."RouteID" = R."RouteID" AND P."CompanyID_id" = R."CompanyID_id"
                where B."CompanyID_id" = %(CompanyID)s AND B."BranchID" = %(CompanyID)s  AND B."Payments" < B."InvoiceAmount" AND P."PartyName" is not null
                AND SM."CustomerID" = %(CustomerID)s AND P."RouteID" = %(RouteID)s
            UNION
            select P."PartyName",'Sales Return' AS Voucher, SM."VoucherDate", "InvoiceNo",'' as OrderNo,"DueDate" - (select CURRENT_DATE) as terms,
            "DueDate","RouteName", (SELECT (now()::date - SM."VoucherDate"::date)) AS aging, ("InvoiceAmount" - "Payments") * -1 AS DueAmount,SM."id"
            from public.billwise_master AS B
            LEFT OUTER JOIN public."salesMasters_salesMaster" AS SM ON B."CustomerID"= SM."LedgerID" 
            AND B."CompanyID_id" = SM."CompanyID_id" AND SM."SalesReturnMasterID" = "TransactionID" AND "VoucherType" = 'SI'
            LEFT OUTER JOIN public.parties_parties AS P ON P."LedgerID" = SM."LedgerID" AND P."CompanyID_id" = SM."CompanyID_id"
            LEFT OUTER JOIN public.route_route AS R ON P."RouteID" = R."RouteID" AND P."CompanyID_id" = R."CompanyID_id"
                where B."CompanyID_id" = %(CompanyID)s AND B."BranchID" = %(CompanyID)s  AND B."Payments" < B."InvoiceAmount" AND P."PartyName" is not null
                AND SM."CustomerID" = %(CustomerID)s AND P."RouteID" = %(RouteID)s
        ...
        cursor.execute('''
            ...
            from public.billwise_master AS B
            LEFT OUTER JOIN public."salesMasters_salesMaster" AS SM ON B."CustomerID"= SM."LedgerID" 
            AND B."CompanyID_id" = SM."CompanyID_id" AND SM."SalesMasterID" = "TransactionID" AND "VoucherType" = 'SI'
            LEFT OUTER JOIN public.parties_parties AS P ON P."LedgerID" = SM."LedgerID" AND P."CompanyID_id" = SM."CompanyID_id"
            LEFT OUTER JOIN public.route_route AS R ON P."RouteID" = R."RouteID" AND P."CompanyID_id" = R."CompanyID_id"
                where B."CompanyID_id" = %(CompanyID)s AND B."BranchID" = %(BranchID)s  AND B."Payments" < B."InvoiceAmount" AND P."PartyName" is not null
                AND SM."LedgerID" = %(CustomerID)s
            UNION
            select P."PartyName",'Sales Return' AS Voucher, SM."VoucherDate", "InvoiceNo",'' as OrderNo,"DueDate" - (select CURRENT_DATE) as terms,
            "DueDate","RouteName", (SELECT (now()::date - SM."VoucherDate"::date)) AS aging,("InvoiceAmount" - "Payments") * -1 AS DueAmount,SM."id"
            from public.billwise_master AS B
            LEFT OUTER JOIN public."salesReturnMasters_salesReturnMaster" AS SM ON B."CustomerID"= SM."LedgerID" 
            AND B."CompanyID_id" = SM."CompanyID_id" AND SM."SalesReturnMasterID" = "TransactionID" AND "VoucherType" = 'SR'
            LEFT OUTER JOIN public.parties_parties AS P ON P."LedgerID" = SM."LedgerID" AND P."CompanyID_id" = SM."CompanyID_id"
            LEFT OUTER JOIN public.route_route AS R ON P."RouteID" = R."RouteID" AND P."CompanyID_id" = R."CompanyID_id"
                where B."CompanyID_id" = %(CompanyID)s AND B."BranchID" = %(BranchID)s  AND B."Payments" < B."InvoiceAmount" AND P."PartyName" is not null
                AND SM."LedgerID" = %(CustomerID)s
        ...
        cursor.execute('''
            select P."PartyName",'Sales Invoice' AS Voucher, "VoucherDate", "InvoiceNo",SM."OrderNo" as OrderNo,
            "DueDate" - (select CURRENT_DATE) as terms,"DueDate","RouteName", (SELECT (now()::date - SM."Date"::date)) AS aging,"InvoiceAmount" - "Payments" AS DueAmount,SM."id"
            from public.billwise_master AS B
            LEFT OUTER JOIN public."salesMasters_salesMaster" AS SM ON B."CustomerID"= SM."LedgerID" 
            AND B."CompanyID_id" = SM."CompanyID_id" AND SM."SalesMasterID" = "TransactionID" AND "VoucherType" = 'SI'
            LEFT OUTER JOIN public.parties_parties AS P ON P."LedgerID" = SM."LedgerID" AND P."CompanyID_id" = SM."CompanyID_id"
            LEFT OUTER JOIN public.route_route AS R ON P."RouteID" = R."RouteID" AND P."CompanyID_id" = R."CompanyID_id"
                where B."CompanyID_id" = %(CompanyID)s AND B."BranchID" = %(BranchID)s  AND B."Payments" < B."InvoiceAmount" AND P."PartyName" is not null
                AND P."RouteID" = %(RouteID)s
            UNION
            select P."PartyName",'Sales Return' AS Voucher, SM."VoucherDate", "InvoiceNo",'' as OrderNo,
            "DueDate" - (select CURRENT_DATE) as terms,"DueDate","RouteName", (SELECT (now()::date - SM."VoucherDate"::date)) AS aging,("InvoiceAmount" - "Payments") * -1 AS DueAmount,SM."id"
            from public.billwise_master AS B
            LEFT OUTER JOIN public."salesReturnMasters_salesReturnMaster" AS SM ON B."CustomerID"= SM."LedgerID" 
            AND B."CompanyID_id" = SM."CompanyID_id" AND SM."SalesReturnMasterID" = "TransactionID" AND "VoucherType" = 'SR'
            LEFT OUTER JOIN public.parties_parties AS P ON P."LedgerID" = SM."LedgerID" AND P."CompanyID_id" = SM."CompanyID_id"
    ...
    if cursor.rowcount == 0:
        ...
        # adding Heading to frame
        df.columns = [
            str(('PartyName')), str(('Type')), str(('Date')), str(
                ('InvoiceNo')), str(('PO')), str(('Terms')), str(('DueDate')), str(('Class')),
              str(('aging')),str(('OpenBalance')),str(('id'))]

        grouped = df.groupby(['PartyName'])
        result = []
        num = 0
        TotalBalance = 0
        for name, group in grouped:
            new_row = pd.DataFrame({'PartyName': name, 'Type': name,
                                    'Date': '', 'InvoiceNo': "", 'PO': '', 'Terms': '', 'DueDate': "", 'Class': "",
                                 'aging': '','OpenBalance': 0,'id': '' }, index=[0])
            group = pd.concat([new_row, group.loc[:]]).reset_index(drop=True)
            
            Total = group['OpenBalance'].sum()
            
            new_row = pd.DataFrame({'PartyName': name, 'Type': 'Total {}'.format(name),
                                    'Date': '', 'InvoiceNo': "-", 'PO': '', 'Terms': '', 'DueDate': "", 'Class': "",
                                    'aging': '', 'OpenBalance': Total,'id': ''}, index=[0])
            group = group.append(new_row, ignore_index=True)
            if num > 0:
                totalRow = len(group.index)
                group.iloc[1: totalRow]   
            result.append(group)
            num += 1
            TotalBalance += Total
        ...
        convert_dict = {'id': str}
        df = df.astype(convert_dict)
        df = df.drop("PartyName", axis=1)
        json_records = df.reset_index().to_json(orient='records')
        details = json.loads(json_records)
        df.index = np.arange(1, len(df)+1)
        df.loc[len(df), 'Type'] = "Total"
    return df, details


def dashboard_data(CompanyID, BranchID, FromDate, ToDate,Group):
    cursor = connection.cursor()
    dic = {'CompanyID': CompanyID, 'BranchID': BranchID, 'FromDate': FromDate,
           'ToDate': ToDate,'Group': Group}
    ...
    cursor.execute('''
        WITH RECURSIVE GroupInMainGroup AS (
            ...
            FROM public.accountgroup_accountgroup
            WHERE
            "AccountGroupID" in (%(Group)s) AND "CompanyID_id" = %(CompanyID)s
            UNION
            SELECT
            e."AccountGroupID",
            e."AccountGroupUnder",
            ...
            SELECT TO_CHAR(Date, 'month') AS "month",Date,SUM(Balace) as Balace
            FROM(  
                
            SELECT "AccountGroupName","LedgerName" ,  "Debit" - "Credit" AS Balace,DATE_TRUNC('month',"Date") AS Date,"Date" as d
                FROM public."accountLedger_accountLedger" AL  
            INNER JOIN public."accountgroup_accountgroup" AS A ON AL."AccountGroupUnder" = A."AccountGroupID" AND AL."CompanyID_id" = A."CompanyID_id" 
            LEFT OUTER JOIN public."ledgerPostings_ledgerPosting" as L ON AL."LedgerID" = L."LedgerID"
                AND AL."CompanyID_id" = L."CompanyID_id" AND AL."BranchID" = L."BranchID"
            WHERE L."Date" BETWEEN %(FromDate)s AND %(ToDate)s AND AL."AccountGroupUnder" IN (SELECT "AccountGroupID" FROM GroupInMainGroup) AND AL."BranchID" = %(BranchID)s
            AND AL."CompanyID_id" = %(CompanyID)s --GROUP BY DATE_TRUNC('month',"Date")
            ) AS TEMP WHERE Balace != 0 GROUP BY Date ORDER BY Date;
        ''', dic)
...
def quick_report_data(CompanyID, BranchID, FromDate, ToDate, type,Group):
    ...
    if type == "routes":
        cursor.execute('''
            SELECT "RouteName",SUM(GrandTotal) AS GrandTotal,SUM(CashSales) AS CashSales,SUM(BankSales) AS BankSales, SUM(Receipt) AS Receipt FROM (
                SELECT * ,(SELECT ROUND(COALESCE(SUM("NetAmount"),0),2) FROM public."receiptDetailses_receiptDetails" AS D 
                        INNER JOIN public."receiptMasters_receiptMaster" AS R ON R."ReceiptMasterID" =D."ReceiptMasterID" AND D."CompanyID_id" =R."CompanyID_id" 
                        WHERE D."CompanyID_id" = %(CompanyID)s AND "Date" between %(FromDate)s and %(ToDate)s AND D."LedgerID" = t."LedgerID") AS Receipt
                FROM (
                select 
                S."LedgerID","PartyName","RouteName",
                round(SUM(S."GrandTotal"),2) AS GrandTotal,
                round(SUM(
                    (Select COALESCE(SUM("Debit" - "Credit"),0) FROM public."ledgerPostings_ledgerPosting" where "VoucherType"='SI'      
                            AND "VoucherMasterID"=s."SalesMasterID" AND "BranchID" = S."BranchID"
                    AND "CompanyID_id" = S."CompanyID_id"
                ...
                (Select COALESCE(SUM("Debit" - "Credit"),0) FROM public."ledgerPostings_ledgerPosting" where "VoucherType"='SI'      
                AND "VoucherMasterID"=s."SalesMasterID" AND "BranchID" = S."BranchID"
                AND "CompanyID_id" = S."CompanyID_id" AND "LedgerID" IN       
                (Select "LedgerID" FROM public."accountLedger_accountLedger" where "AccountGroupUnder"=8 AND "CompanyID_id" = S."CompanyID_id"))),2) AS BankSales
                        
                        
                from public.route_route AS R 
                INNER JOIN public.parties_parties AS P on P."RouteID" = R."RouteID" AND P."CompanyID_id" = R."CompanyID_id"
                INNER JOIN public."salesMasters_salesMaster" AS S ON S."LedgerID" = P."LedgerID" AND P."CompanyID_id" = S."CompanyID_id" 
                where S."CompanyID_id" = %(CompanyID)s AND "Date" between %(FromDate)s and %(ToDate)s
                    GROUP BY S."LedgerID","PartyName","RouteName"
                    )as t) AS tm GROUP BY "RouteName"
            ''', dic)
    else:
        cursor.execute('''
            select "LedgerID","LedgerName",--- "Credit"
            (select COALESCE(SUM("Debit" ),0) AS Amount from public."ledgerPostings_ledgerPosting" 
            where "CompanyID_id" = A."CompanyID_id" and "LedgerID" = A."LedgerID" and "Date" between %(FromDate)s and %(ToDate)s)
            from public."accountLedger_accountLedger" as A where "CompanyID_id" = %(CompanyID)s
            and "AccountGroupUnder" = %(Group)s
            ''', dic)
    ...
    if cursor.rowcount == 0:
        ...
        if type == "routes":
            df.columns = [
                str(('RouteName')), str(('GrandTotal')), str(('CashSales')), str(('BankSales')), str(('Receipt'))]
            CreditSales = df["GrandTotal"] - \
                (df["CashSales"] + df["BankSales"] + df["Receipt"])
            df["CreditSales"] = CreditSales
        else:
            df.columns = [
                str(('LedgerID')), str(('LedgerName')), str(('Total'))]
...
def quick_report_pdf_excel(CompanyID, BranchID, FromDate, ToDate):
    dic = {'CompanyID': CompanyID, 'BranchID': BranchID, 'FromDate': FromDate,
           'ToDate': ToDate}
    ...
    if cursor1.rowcount == 0:
        ...
        df1.columns = [
            str(('RouteName')), str(('GrandTotal')), str(('CashSales')), str(('BankSales')), str(('Receipt'))]
        CreditSales = df1["GrandTotal"] - \
            (df1["CashSales"] + df1["BankSales"] + df1["Receipt"])
        df1["CreditSales"] = CreditSales
        json_records = df1.reset_index().to_json(orient='records')
        details1 = json.loads(json_records)
        df1.columns = [['Routes'] * 6, df1.columns]
    # -----------------------------------------------------------------------------------------
        
    cursor2 = connection.cursor()
    cursor2.execute('''
            select "LedgerName",
            (select round(COALESCE(SUM("Debit" ),0),2) AS Amount from public."ledgerPostings_ledgerPosting"
            where "CompanyID_id" = A."CompanyID_id" and "LedgerID" = A."LedgerID" and "Date" between %(FromDate)s and %(ToDate)s)
            from public."accountLedger_accountLedger" as A where "CompanyID_id" = %(CompanyID)s
            and "AccountGroupUnder" = '43'
            ''', dic)
    ...
    if cursor2.rowcount == 0:
        ...
        df2.columns = [
             str(('LedgerName')), str(('Total'))]
        
        json_records = df2.reset_index().to_json(orient='records')
        details2 = json.loads(json_records)
        df2.columns = [['Expenses'] * 2, df2.columns]
    ...
    if cursor3.rowcount == 0:
        ...
        details3 = json.loads(json_records)
        df3.columns = [['Supplier'] * 2, df3.columns]
    ...
    return df, details1,details2,details3


def product_summary_report_data(CompanyID, BranchID, FromDate, ToDate, ProductID, CategoryID,GroupID,BrandID,PriceListID,LedgerID):
    print(CompanyID, BranchID, FromDate, ToDate, ProductID, CategoryID,GroupID,BrandID,PriceListID,LedgerID)
    cursor = connection.cursor()
    dic = {'CompanyID': CompanyID, 'BranchID': BranchID, 'FromDate': FromDate,
           'ToDate': ToDate, 'ProductID': ProductID, 'CategoryID': CategoryID, 'GroupID': GroupID, 'BrandID': BrandID, 'PriceListID': PriceListID, 'LedgerID': LedgerID}
    
    if int(ProductID) > 0 and int(LedgerID) > 0 and int(PriceListID) > 0:
        cursor.execute('''
            select p."ProductID",
            p."ProductName",
            m."LedgerID",
            ...
            FROM public."purchaseReturnMasters_purchaseReturnMaster" as rm
                inner join public."purchaseReturnDetails_purchaseReturnDetail" as rd on rm."PurchaseReturnMasterID" = rd."PurchaseReturnMasterID"
                    and rm."CompanyID_id" =rd."CompanyID_id"
            where rd."ProductID" = p."ProductID" and rm."CompanyID_id" =p."CompanyID_id"
                and rd."PriceListID" = d."PriceListID" and rm."LedgerID" = m."LedgerID" ) AS ReturnQty ,
            (SELECT COALESCE(SUM("GrossAmount"),0)
            FROM public."purchaseReturnMasters_purchaseReturnMaster" as rm
                inner join public."purchaseReturnDetails_purchaseReturnDetail" as rd on rm."PurchaseReturnMasterID" = rd."PurchaseReturnMasterID"
                    and rm."CompanyID_id" =rd."CompanyID_id"
            where rd."ProductID" = p."ProductID" and rm."CompanyID_id" =p."CompanyID_id"
                and rd."PriceListID" = d."PriceListID" and rm."LedgerID" = m."LedgerID") as ReturnAmount,

           (select CONCAT(COALESCE(round("Qty" + "FreeQty"), 2) , ' ',"UnitName")
            from public."purchaseDetailses_purchaseDetails"  pd
                inner join public.pricelist_pricelist as ppl on ppl."PriceListID" = pd."PriceListID" and ppl."CompanyID_id" = pd."CompanyID_id"
                inner join public.units_unit as ut on ppl."UnitID" = ut."UnitID" and ppl."CompanyID_id" = ut."CompanyID_id"
                Inner Join public."purchaseMasters_purchaseMaster" as pm on pm."PurchaseMasterID" = pd."PurchaseMasterID" and pm."CompanyID_id"= pd."CompanyID_id" and pm."LedgerID" =m."LedgerID"
            where pd."CompanyID_id"= p."CompanyID_id" and pd."ProductID" = p."ProductID"  and pd."PriceListID" = pl."PriceListID"
            order by pd."PurchaseMasterID" desc limit 1 ) as lastqty,

            (select "GrossAmount"
            from public."purchaseDetailses_purchaseDetails" as pd
                inner join public.pricelist_pricelist as ppl on ppl."PriceListID" = pd."PriceListID" and ppl."CompanyID_id" = pd."CompanyID_id"
                Inner Join public."purchaseMasters_purchaseMaster" as pm on pm."PurchaseMasterID" = pd."PurchaseMasterID" and pm."CompanyID_id"= pd."CompanyID_id" and pm."LedgerID" =m."LedgerID"
            where pd."CompanyID_id"= p."CompanyID_id" and pd."ProductID" = p."ProductID"  and pd."PriceListID" = pl."PriceListID"
            order by pd."PurchaseMasterID" desc limit 1 ) as lastgrossamount,
            'PI' as VoucherType,
            p."id"

        from public.products_product as p
            inner join public."purchaseDetailses_purchaseDetails" as d on d."ProductID" = p."ProductID" and d."CompanyID_id"= p."CompanyID_id"
            Inner Join public."purchaseMasters_purchaseMaster" as m on m."PurchaseMasterID" = d."PurchaseMasterID" and m."CompanyID_id"= p."CompanyID_id"
            inner join public."accountLedger_accountLedger" as a on a."LedgerID" = m."LedgerID" and a."CompanyID_id"= p."CompanyID_id"
            inner join public.pricelist_pricelist as pl on  pl."CompanyID_id" = d."CompanyID_id"
            inner join public.units_unit as u on pl."UnitID" = u."UnitID" and pl."CompanyID_id" = u."CompanyID_id"
        where p."CompanyID_id" = %(CompanyID)s and p."ProductID" = %(ProductID)s and m."LedgerID" = %(LedgerID)s and pl."PriceListID" = %(PriceListID)s and m."Date" between %(FromDate)s and %(ToDate)s 
        and d."PriceListID" = pl."PriceListID"
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
        )
        ...
        SUM("GrossAmount") as GrossAmount,
        --Purchase return
        (SELECT COALESCE(SUM("Qty"),0) FROM public."salesReturnMasters_salesReturnMaster" as rm 
        inner join public."salesReturnDetails_salesReturnDetail" as rd on rm."SalesReturnMasterID" = rd."SalesReturnMasterID"
        and rm."CompanyID_id" =rd."CompanyID_id" where rd."ProductID" = p."ProductID" and rm."CompanyID_id" =p."CompanyID_id"
        and rd."PriceListID" = d."PriceListID" and rm."LedgerID" = m."LedgerID" ) AS ReturnQty ,
        (SELECT COALESCE(SUM("GrossAmount"),0) FROM public."salesReturnMasters_salesReturnMaster" as rm
        inner join public."salesReturnDetails_salesReturnDetail" as rd on rm."SalesReturnMasterID" = rd."SalesReturnMasterID"
        and rm."CompanyID_id" =rd."CompanyID_id" where rd."ProductID" = p."ProductID" and rm."CompanyID_id" =p."CompanyID_id"
        and rd."PriceListID" = d."PriceListID" and rm."LedgerID" = m."LedgerID") as ReturnAmount,

        (select CONCAT(COALESCE(round("Qty" + "FreeQty"), 2) , ' ',"UnitName")
            from public."salesDetails_salesDetail"  pd
                inner join public.pricelist_pricelist as ppl on ppl."PriceListID" = pd."PriceListID" and ppl."CompanyID_id" = pd."CompanyID_id"
                inner join public.units_unit as ut on ppl."UnitID" = ut."UnitID" and ppl."CompanyID_id" = ut."CompanyID_id"
                Inner Join public."salesMasters_salesMaster" as pm on pm."SalesMasterID" = pd."SalesMasterID" and pm."CompanyID_id"= pd."CompanyID_id" and pm."LedgerID" =m."LedgerID"
            where pd."CompanyID_id"= p."CompanyID_id" and pd."ProductID" = p."ProductID"  and pd."PriceListID" = pl."PriceListID"
            order by pd."SalesMasterID" desc limit 1 ) as lastqty,

            (select "GrossAmount"
            from public."salesDetails_salesDetail" as pd
                inner join public.pricelist_pricelist as ppl on ppl."PriceListID" = pd."PriceListID" and ppl."CompanyID_id" = pd."CompanyID_id"
                Inner Join public."salesMasters_salesMaster" as pm on pm."SalesMasterID" = pd."SalesMasterID" and pm."CompanyID_id"= pd."CompanyID_id" and pm."LedgerID" =m."LedgerID"
            where pd."CompanyID_id"= p."CompanyID_id" and pd."ProductID" = p."ProductID"  and pd."PriceListID" = pl."PriceListID"
            order by pd."SalesMasterID" desc limit 1 ) as lastgrossamount,
            'SI' as VoucherType,
            p."id"
        
        from public.products_product as p
        inner join public."salesDetails_salesDetail" as d on d."ProductID" = p."ProductID" and d."CompanyID_id"= p."CompanyID_id"
        Inner Join public."salesMasters_salesMaster" as m on m."SalesMasterID" = d."SalesMasterID" and m."CompanyID_id"= p."CompanyID_id"
        inner join public."accountLedger_accountLedger" as a on a."LedgerID" = m."LedgerID" and a."CompanyID_id"= p."CompanyID_id"
        inner join public.pricelist_pricelist as pl on pl."CompanyID_id" = d."CompanyID_id"
        inner join public.units_unit as u on pl."UnitID" = u."UnitID" and pl."CompanyID_id" = u."CompanyID_id"
        where p."CompanyID_id" = %(CompanyID)s and p."ProductID" = %(ProductID)s and m."LedgerID" = %(LedgerID)s and pl."PriceListID" = %(PriceListID)s and m."Date" between %(FromDate)s and %(ToDate)s 
        ...
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id")
            ''', dic)
    elif int(ProductID) > 0 and int(LedgerID) > 0:
        cursor.execute('''
            select p."ProductID",
            p."ProductName",
            m."LedgerID",
        ...
        from public.products_product as p
            ...
            Inner Join public."purchaseMasters_purchaseMaster" as m on m."PurchaseMasterID" = d."PurchaseMasterID" and m."CompanyID_id"= p."CompanyID_id"
            inner join public."accountLedger_accountLedger" as a on a."LedgerID" = m."LedgerID" and a."CompanyID_id"= p."CompanyID_id"
            inner join public.pricelist_pricelist as pl on pl."PriceListID" = d."PriceListID" and pl."CompanyID_id" = d."CompanyID_id"
            inner join public.units_unit as u on pl."UnitID" = u."UnitID" and pl."CompanyID_id" = u."CompanyID_id"
        where p."CompanyID_id" = %(CompanyID)s and p."ProductID" = %(ProductID)s and m."LedgerID" = %(LedgerID)s and m."Date" between %(FromDate)s and %(ToDate)s
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
        )
        ...
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id")
            ''', dic)
    elif int(CategoryID) > 0 and int(LedgerID) > 0:
        cursor.execute('''
           select p."ProductID",
            p."ProductName",
            m."LedgerID",
        ...
        from public.products_product as p
            ...
            inner join public.pricelist_pricelist as pl on pl."PriceListID" = d."PriceListID" and pl."CompanyID_id" = d."CompanyID_id"
            inner join public.units_unit as u on pl."UnitID" = u."UnitID" and pl."CompanyID_id" = u."CompanyID_id"
            inner join public.productgroup_productgroup as pg on pg."ProductGroupID" = p."ProductGroupID" and pg."CompanyID_id" = p."CompanyID_id"
            inner join public.productcategory_productcategory as pc on pc."CompanyID_id" = p."CompanyID_id"
          
        where p."CompanyID_id" = %(CompanyID)s and p."ProductGroupID" = pg."ProductGroupID" and pc."ProductCategoryID" = %(CategoryID)s and m."LedgerID" = %(LedgerID)s and m."Date" between %(FromDate)s and %(ToDate)s
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
        )
        ...
        select p."ProductID",
            ...
            --Purchase return
            (SELECT COALESCE(SUM("Qty"),0)
            FROM public."salesReturnMasters_salesReturnMaster" as rm
                inner join public."salesReturnDetails_salesReturnDetail" as rd on rm."SalesReturnMasterID" = rd."SalesReturnMasterID"
                    and rm."CompanyID_id" =rd."CompanyID_id"
            where rd."ProductID" = p."ProductID" and rm."CompanyID_id" =p."CompanyID_id"
                and rd."PriceListID" = d."PriceListID" and rm."LedgerID" = m."LedgerID" ) AS ReturnQty ,
        ...
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
            ...
            ''', dic)
    elif int(GroupID) > 0 and int(LedgerID) > 0:
        cursor.execute('''
            select p."ProductID",
            p."ProductName",
            m."LedgerID",
        ...
        from public.products_product as p
            ...
            inner join public.pricelist_pricelist as pl on pl."PriceListID" = d."PriceListID" and pl."CompanyID_id" = d."CompanyID_id"
            inner join public.units_unit as u on pl."UnitID" = u."UnitID" and pl."CompanyID_id" = u."CompanyID_id"
            inner join public.productgroup_productgroup as pg on pg."CompanyID_id" = p."CompanyID_id"
          
        where p."CompanyID_id" = %(CompanyID)s and p."ProductGroupID"  = pg."ProductGroupID" and pg."ProductGroupID" = %(GroupID)s and m."LedgerID" = %(LedgerID)s and m."Date" between %(FromDate)s and %(ToDate)s
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
        )
        ...
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
            ...
            ''', dic)
    elif int(BrandID) > 0 and int(LedgerID) > 0:
        cursor.execute('''
            select p."ProductID",
            p."ProductName",
            m."LedgerID",
        ...
        from public.products_product as p
            ...
            inner join public.pricelist_pricelist as pl on pl."PriceListID" = d."PriceListID" and pl."CompanyID_id" = d."CompanyID_id"
            inner join public.units_unit as u on pl."UnitID" = u."UnitID" and pl."CompanyID_id" = u."CompanyID_id"
            inner join public.brands_brand as br on br."CompanyID_id" = p."CompanyID_id"
          
        where p."CompanyID_id" = %(CompanyID)s and p."BrandID"  = br."BrandID" and br."BrandID" = %(BrandID)s and m."LedgerID" = %(LedgerID)s and m."Date" between %(FromDate)s and %(ToDate)s
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
        )
        ...
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
            ...
            ''', dic)
    elif int(ProductID) > 0 and int(PriceListID) > 0:
        
        cursor.execute('''
            select "ProductID","ProductName","LedgerID","LedgerName",1 as PriceListID,(SELECT "UnitName" from public.pricelist_pricelist as pl
            inner join public.units_unit as u on pl."UnitID" = u."UnitID" and pl."CompanyID_id" = u."CompanyID_id" where pl."CompanyID_id" = t."CompanyID_id" and "PriceListID" = %(PriceListID)s) as unitname,
            SUM(pqty) as qty,SUM(FreeQty) as FreeQty,SUM(GrossAmount) as grossamount,SUM(prqty) as returnqty,SUM(returnamount) as returnamount
            ...
            --lastqty
                ...
                from public."purchaseDetailses_purchaseDetails"  pd
                    inner join public.pricelist_pricelist as ppl on ppl."PriceListID" = pd."PriceListID" and ppl."CompanyID_id" = pd."CompanyID_id"
                    inner join public.units_unit as ut on ppl."UnitID" = ut."UnitID" and ppl."CompanyID_id" = ut."CompanyID_id"
                    Inner Join public."purchaseMasters_purchaseMaster" as pm on pm."PurchaseMasterID" = pd."PurchaseMasterID" and pm."CompanyID_id"= pd."CompanyID_id" and pm."LedgerID" =t."LedgerID"
                where pd."CompanyID_id"= t."CompanyID_id" and pd."ProductID" = t."ProductID"  and pd."PriceListID" = %(PriceListID)s
                order by pd."PurchaseMasterID" desc limit 1 ) as lastqty,
        --lastgrossamount
                (select "GrossAmount"
                from public."purchaseDetailses_purchaseDetails" as pd
        ...
        select p."ProductID",
                ...
                "UnitName",
                SUM("Qty") as Qty,
                --multiply with multifactor
                Case when 
                (SELECT "MultiFactor" from public.pricelist_pricelist where "CompanyID_id" = p."CompanyID_id" and "PriceListID" = %(PriceListID)s) < 1 then
                (SUM("Qty") * pl."MultiFactor" )*(SELECT "MultiFactor" from public.pricelist_pricelist where "CompanyID_id" = p."CompanyID_id" and "PriceListID" = %(PriceListID)s)
                else 
                (SUM("Qty") * pl."MultiFactor" ) / (SELECT "MultiFactor" from public.pricelist_pricelist where "CompanyID_id" = p."CompanyID_id" and "PriceListID" = %(PriceListID)s)
                end as pqty,
                SUM("FreeQty") as FreeQty,
                ...
                Case when 
                (SELECT "MultiFactor" from public.pricelist_pricelist where "CompanyID_id" = p."CompanyID_id" and "PriceListID" = %(PriceListID)s) < 1 then
                (SELECT COALESCE(SUM("Qty"),0) * "MultiFactor"
                FROM public."purchaseReturnMasters_purchaseReturnMaster" as rm
                    inner join public."purchaseReturnDetails_purchaseReturnDetail" as rd on rm."PurchaseReturnMasterID" = rd."PurchaseReturnMasterID"
                        and rm."CompanyID_id" =rd."CompanyID_id"
                where rd."ProductID" = p."ProductID" and rm."CompanyID_id" =p."CompanyID_id"
                    and rd."PriceListID" = d."PriceListID" and rm."LedgerID" = m."LedgerID" )
                    *(SELECT "MultiFactor" from public.pricelist_pricelist where "CompanyID_id" = p."CompanyID_id" and "PriceListID" = %(PriceListID)s)
                    else 
                (SELECT COALESCE(SUM("Qty"),0) * "MultiFactor"
                FROM public."purchaseReturnMasters_purchaseReturnMaster" as rm
                    inner join public."purchaseReturnDetails_purchaseReturnDetail" as rd on rm."PurchaseReturnMasterID" = rd."PurchaseReturnMasterID"
                        and rm."CompanyID_id" =rd."CompanyID_id"
                where rd."ProductID" = p."ProductID" and rm."CompanyID_id" =p."CompanyID_id"
                    and rd."PriceListID" = d."PriceListID" and rm."LedgerID" = m."LedgerID" ) / (SELECT "MultiFactor" from public.pricelist_pricelist where "CompanyID_id" = p."CompanyID_id" and "PriceListID" = %(PriceListID)s)
                end
                    AS prqty ,
            ...
            where p."CompanyID_id" = %(CompanyID)s and  p."ProductID" = %(ProductID)s --and pl."PriceListID" = %(PriceListID)s 
            and m."Date" between %(FromDate)s and %(ToDate)s 
            and d."PriceListID" = pl."PriceListID"
            group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id",pl."MultiFactor"
            )
            
            )
            as t group by("ProductID","ProductName","LedgerID","LedgerName","CompanyID_id","id")
        ...
        --lastqty
                    ...
                    from public."salesDetails_salesDetail"  pd
                        inner join public.pricelist_pricelist as ppl on ppl."PriceListID" = pd."PriceListID" and ppl."CompanyID_id" = pd."CompanyID_id"
                        inner join public.units_unit as ut on ppl."UnitID" = ut."UnitID" and ppl."CompanyID_id" = ut."CompanyID_id"
                        Inner Join public."salesMasters_salesMaster" as pm on pm."SalesMasterID" = pd."SalesMasterID" and pm."CompanyID_id"= pd."CompanyID_id" and pm."LedgerID" =t."LedgerID"
                    where pd."CompanyID_id"= t."CompanyID_id" and pd."ProductID" = t."ProductID"  and pd."PriceListID" = %(PriceListID)s
                    order by pd."SalesMasterID" desc limit 1 ) as lastqty,
        --lastgrossamount
                    (select "GrossAmount"
        ...
        select p."ProductID",
                    ...
                    FROM public."salesReturnMasters_salesReturnMaster" as rm
                        inner join public."salesReturnDetails_salesReturnDetail" as rd on rm."SalesReturnMasterID" = rd."SalesReturnMasterID"
                            and rm."CompanyID_id" =rd."CompanyID_id"
                    where rd."ProductID" = p."ProductID" and rm."CompanyID_id" =p."CompanyID_id"
                        and rd."PriceListID" = d."PriceListID" and rm."LedgerID" = m."LedgerID" ) / (SELECT "MultiFactor" from public.pricelist_pricelist where "CompanyID_id" = p."CompanyID_id" and "PriceListID" = 1)
                    end
                        AS prqty ,
        ...
        where p."CompanyID_id" = %(CompanyID)s and  p."ProductID" = %(ProductID)s and m."Date" between %(FromDate)s and %(ToDate)s
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
        )
        ...
        from public.products_product as p
            ...
            inner join public.units_unit as u on pl."UnitID" = u."UnitID" and pl."CompanyID_id" = u."CompanyID_id"
            inner join public.productcategory_productcategory as pc on pc."CompanyID_id" = p."CompanyID_id"
            inner join public.productgroup_productgroup as pg on pg."ProductGroupID" = p."ProductGroupID" and pg."CategoryID" = pc."ProductCategoryID" and pg."CompanyID_id" = p."CompanyID_id"
          
        where p."CompanyID_id" = %(CompanyID)s and  p."ProductGroupID" = pg."ProductGroupID" and pc."ProductCategoryID" = %(CategoryID)s  and m."Date" between %(FromDate)s and %(ToDate)s
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
        )
        ...
        select p."ProductID",
         ...
         from public.products_product as p
            inner join public."salesDetails_salesDetail" as d on d."ProductID" = p."ProductID" and d."CompanyID_id"= p."CompanyID_id"
            Inner Join public."salesMasters_salesMaster" as m on m."PurchaseMasterID" = d."PurchaseMasterID" and m."CompanyID_id"= p."CompanyID_id"
            inner join public."accountLedger_accountLedger" as a on a."LedgerID" = m."LedgerID" and a."CompanyID_id"= p."CompanyID_id"
            inner join public.pricelist_pricelist as pl on pl."PriceListID" = d."PriceListID" and pl."CompanyID_id" = d."CompanyID_id"
            inner join public.units_unit as u on pl."UnitID" = u."UnitID" and pl."CompanyID_id" = u."CompanyID_id"
            inner join public.productcategory_productcategory as pc on pc."CompanyID_id" = p."CompanyID_id"
        ...
        cursor.execute('''
            ...
            where p."CompanyID_id" = %(CompanyID)s and p."ProductGroupID"  = pg."ProductGroupID" and pg."ProductGroupID" = %(GroupID)s  and m."Date" between %(FromDate)s and %(ToDate)s
            group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
            )
        ...
        cursor.execute('''
            ...
            where p."CompanyID_id" = %(CompanyID)s and p."BrandID"  = br."BrandID" and br."BrandID" = %(BrandID)s and m."Date" between %(FromDate)s and %(ToDate)s
            group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
            )
        ...
        from public.products_product as p
            ...
            inner join public.pricelist_pricelist as pl on pl."PriceListID" = d."PriceListID" and pl."CompanyID_id" = d."CompanyID_id"
            inner join public.units_unit as u on pl."UnitID" = u."UnitID" and pl."CompanyID_id" = u."CompanyID_id"
        where p."CompanyID_id" = %(CompanyID)s and m."LedgerID" = %(LedgerID)s  and m."Date" between %(FromDate)s and %(ToDate)s
        group by (p."ProductID",p."ProductName",m."LedgerID","LedgerName",d."PriceListID","UnitName",p."CompanyID_id",pl."PriceListID",p."id"
        )
    ...
    df = pd.DataFrame(data)
    df.columns = [
        str(('ProductID')), str(('ProductName')), str(('LedgerID')), str(('LedgerName')), str(('PriceListID')), str(('UnitName')),
        str(('Qty')), str(('FreeQty')), str(('GrossAmount')), str(('ReturnQty')), str(('ReturnAmount')), str(('LastQty')),
        str(('LastGrossAmount')), str(('VoucherType')), str(('id'))]
    ...
    # df.loc[str(_("Total"))] = (
    #     df[["Qty", "FreeQty","GrossAmount","ReturnQty","ReturnAmount","LastGrossAmount", "VoucherType"]].sum().reindex(
    #         df.columns, fill_value="")
    # )

    json_records = df.reset_index().to_json(orient='records')
    details = json.loads(json_records)
    df1 = df[df.VoucherType == 'SI']
    df2 = df[df.VoucherType == 'PI']
    df = [df1,df2]
    return df, details
