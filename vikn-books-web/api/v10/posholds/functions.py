...
def get_VoucherNoForPOS(CompanyID,BranchID,UserID,VoucherType):
    ...
    seperator = ""
    PreFix = VoucherType
    user_count = table.UserTable.objects.filter(
    ...
    new_VoucherNo = str(PreFix) + str(seperator) + str(new_invoiceNo) 
...
def query_sales_report_rassassy_data(
    ...
    CreditSales = df["GrandTotal"] - \
        (df["CashSale"] + df["BankSale"])
    df["CreditSale"] = CreditSales
    
    # df.loc[str(_('Total'))] = df[['GrandTotal', 'CashSale', 'BankSale', 'CreditSale']].sum().reindex(df.columns, fill_value='')
...
def query_dining_report_rassassy_data(
    ...
    # df.loc[str(_('Total'))] = df[['GrandTotal', 'CashSale', 'BankSale',
    #                               'CreditSale']].sum().reindex(df.columns, fill_value='')
    # concert uuid to string
    convert_dict = {'id': str}
    df = df.astype(convert_dict)
    # adding decimal point
    ...
    if filterVal:
        cursor.execute(
            """
                ...
                (Select "LedgerID" FROM public."accountLedger_accountLedger" where "AccountGroupUnder"=8 AND "CompanyID_id" = S."CompanyID_id"))),2) AS BankSales,
                (SELECT "TableName" from public."pos_table_pos_table" AS t WHERE S."Table_id" = t.id) as TableName,
                (SELECT "OrderTime" from public."salesOrderMasters_salesOrderMaster" AS so WHERE so."InvoiceID" = S."SalesMasterID" AND so."CompanyID_id" = S."CompanyID_id") as OrderTime 
...
def query_product_report_rassassy_data(
    ...
    if filterVal:
        cursor.execute(
            """
                    ...
                    (SELECT "ProductName" FROM public."products_product" AS p where p."CompanyID_id" = s."CompanyID_id" and p."ProductID" = s."ProductID") as ProductName,
                    s."Rate" as  cost,
                    (SELECT "UnitPrice" FROM public."salesDetails_salesDetail" AS sd where sd."CompanyID_id" = s."CompanyID_id" and sd."ProductID" = s."ProductID" and sd."SalesMasterID"=s."VoucherMasterID" and sd."SalesDetailsID"=s."VoucherDetailID") as rate,
                    
                    sum(case when  s."VoucherType" ='SI' then s."QtyOut"
                    else 0 end) as noOfSold

                    FROM public."stockPosting_stockPosting" AS s
                    WHERE s."Date" BETWEEN %(FromDate)s AND %(ToDate)s AND s."CompanyID_id" = %(CompanyID)s AND s."BranchID" = '1' AND "VoucherType" = 'SI' AND "ProductID" = %(filterVal)s
                    GROUP BY "date", "ProductID"  , "cost" ,"CompanyID_id","VoucherMasterID" ,"VoucherDetailID"   
                    Order by "Date"
            """,
            dic,
        )
        ...
        cursor.execute(
            """
                    ...
                    FROM public."stockPosting_stockPosting" AS s
                    WHERE s."Date" BETWEEN %(FromDate)s AND %(ToDate)s AND s."CompanyID_id" = %(CompanyID)s AND s."BranchID" = '1' AND "VoucherType" = 'SI'
                    GROUP BY "date", "ProductID"  , "cost","CompanyID_id","VoucherMasterID","VoucherDetailID"   
                    Order by "Date"
            """,
            dic,
    ...
    df["GrandTotal"] = df["rate"] * df["noOfSold"]
    # concert uuid to string
    # convert_dict = {'id': str}
    # df = df.astype(convert_dict)
    # adding decimal point
    # df["cost"] = df["cost"].apply(lambda x: round(
    #     abs(x), PriceRounding))
