...
from api.v10.accountLedgers.functions import get_LedgerCode, get_auto_id as generated_ledgerID,get_auto_LedgerPostid
from api.v10.products.functions import get_ProductCode, get_auto_AutoBarcode, get_auto_AutoBatchCode, get_auto_priceListid, update_stock,get_auto_id as get_auto_ProductID
from api.v10.ledgerPosting.functions import convertOrderdDict
from api.v10.users.serializers import MyCompaniesSerializer, CompaniesSerializer
...
from api.v10.loyaltyCustomer.functions import get_auto_id as get_auto_loyaltyID
...
def pos_product_list(request):
    ...
    if GroupID:
        ...
        if Product.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            ...
            CompanyID=CompanyID, BranchID=BranchID)
    if type == "veg":
        instances = instances.filter(VegOrNonVeg="veg")
    if instances:
...
def create_pos_salesOrder(request):
    try:
            ...
            if is_SaleOrderOK:
                ...
                for saleOrdersDetail in saleOrdersDetails:
                    ...
                    if ProductID:
                        ...
                        flavour = None
                        if Flavour and Flavours.objects.filter(id=Flavour).exists():
                            flavour = Flavours.objects.get(id=Flavour)
        ...
        activity_id = activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'posholds Sales Order',
...
def create_pos_salesInvoice(request):
    try:
        with transaction.atomic():
            ...
            if UserTable.objects.filter(CompanyID=CompanyID, customer__user__pk=CreatedUserID, Active=True).exists():
...
def kitchen_print(request):
    ...
    try:
        KitchenPrint = data['KitchenPrint']
    except:
        KitchenPrint = False
...
def get_defaults(request):
    ...
    if usertable_count > NoOfUsers:
        ...
        last_user_table_instance = administrations_models.UserTable.objects.filter(CompanyID=CompanyID, Active=True)[
            corrent_user:]
        administrations_models.UserTable.objects.exclude(
            CompanyID=CompanyID, pk__in=last_user_table_instance, Active=True).delete()
    ...
    if administrations_models.UserTable.objects.filter(CompanyID__pk=CompanyID, customer__user=userId, Active=True).exists():
        ...
        DefaultAccountForUser = administrations_models.UserTable.objects.get(
            CompanyID=CompanyID, customer__user__pk=userId, Active=True).DefaultAccountForUser
    ...
    try:
        with transaction.atomic():
            ...
            if not is_nameExist:
                ...
                account_ledger.as_on_date = as_on_date
                account_ledger.Action = "M"
                ...
                instance.GST_Treatment = GST_Treatment
                instance.VAT_Treatment = VAT_Treatment
                instance.save()
...
def printer_create(request):
    ...
    Type = data['Type']
    StatusCode = 6001
    message = "POS Printer already exist"
    if POS_Printer.objects.filter(CompanyID=CompanyID,PrinterName=PrinterName).exists():
        StatusCode = 6001
        message = "Printer Name already exist"
    elif POS_Printer.objects.filter(CompanyID=CompanyID, IPAddress=IPAddress).exists():
        StatusCode = 6001
        message = "IPAddress already exist"
    else:
...
def product_create(request):
    try:
        with transaction.atomic():
            ...
            variant_ins = None
            if Variant and ProductVariants.objects.filter(id=Variant).exists():
    ...
    try:
        with transaction.atomic():
            ...
            if not is_nameExist:
                ...
                instance.ProductImage = productImage1
                ...
                pricelist = PriceList.objects.get(
                    ProductID=ProductID, CompanyID=CompanyID,DefaultUnit=True)
...
def rassassy_reports(request):
    ...
    print("===============================================final")
    print(df['GrandTotal'].sum())
    sum_values = {}
    if not ReportType == "ProductReport":
        GrandTotal_sum = df['GrandTotal'].sum()
        Cash_sum = df['CashSale'].sum()
        Bank_sum = df['BankSale'].sum()
        Credit_sum = df['CreditSale'].sum()
        sum_values = {
            "GrandTotal_sum": GrandTotal_sum,
            "Cash_sum": Cash_sum,
            "Bank_sum": Bank_sum,
...
def default_values(request):
    ...
    user_type = ""
    if UserTable.objects.filter(CompanyID=company, customer__user=userId, Active=True).exists():
        user_table_instance = UserTable.objects.get(CompanyID=company, customer__user=userId,Active=True)
        user_type = user_table_instance.UserType.ID
        
    VAT = get_general_settings(BranchID, company, "VAT")
    GST = get_general_settings(BranchID, company, "GST")
    QtyDecimalPoint = get_general_settings(
        BranchID, company, "QtyDecimalPoint")
    PriceDecimalPoint = get_general_settings(
        BranchID, company, "PriceDecimalPoint")
    RoundingFigure = get_general_settings(
        BranchID, company, "RoundingFigure")
...
@authentication_classes((JWTTokenUserAuthentication,))
@renderer_classes((JSONRenderer,))
def pos_product_list_paginated(request):
    today = datetime.datetime.now()
    data = request.data
    ...
    if instances:
        ...
        serialized = RasassyProduct_Serializer(product_sort_pagination, many=True, context={
            "request": request, "CompanyID": CompanyID, "PriceRounding": PriceRounding})
