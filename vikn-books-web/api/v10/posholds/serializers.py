...
from api.v10.products.functions import get_stock
from api.v8.posholds.serializers import QRCodeSerializer
...
from main.functions import ConvertedTime, converted_float, get_ProductStock, get_ProductWareHouseStock
...
class POSHoldMasterRestSerializer(serializers.ModelSerializer):
    ...
    def get_POSHoldDetails(self, instances):
        poshold_details = table.POSHoldDetails.objects.filter(
            POSHoldMasterID=instances.POSHoldMasterID, BranchID=instances.BranchID)
...
class POS_Role_Serializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ('id', 'RoleName', 'show_dining', 'show_take_away',
                  'show_online', 'show_car', 'show_kitchen')
...
class POS_Table_Serializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'title', 'description', 'PriceCategory', 'Status', 'priceid', 'SalesOrderID',
                  'SalesMasterID', 'SalesOrderGrandTotal', 'SalesGrandTotal', 'OrderTime', 'IsActive')

    def get_id(self, instances):
        id = instances.id
        return str(id)

    def get_PriceCategory(self, instances):
        PriceCategory = None
        if instances.PriceCategory:
            PriceCategory = instances.PriceCategory.id
        return str(PriceCategory)
    ...
    def get_SalesMasterID(self, instances):
        ...
        if table.SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Table__id=table_id, Status="Invoiced").exists():
            sales_invoice = table.SalesMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, Table__id=table_id, Status="Invoiced").first()
    ...
    def get_SalesOrderID(self, instances):
        ...
        if table.SalesOrderMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Table__id=table_id, Status="Ordered").exists():
            sales_order = table.SalesOrderMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, Table__id=table_id, Status="Ordered").first()
...
class POS_Product_Serializer(serializers.ModelSerializer):
    ...
    price_list_details = serializers.SerializerMethodField()
    VariantName = serializers.SerializerMethodField()
    ...
    VegOrNonVeg = serializers.SerializerMethodField()

    class Meta:
        model = table.Product
        fields = ('id', 'ProductID', 'ProductName', 'DefaultUnitID', 'DefaultUnitName', 'DefaultSalesPrice', 'DefaultPurchasePrice',
                  'GST_SalesTax', 'SalesTax', 'GST_TaxName', 'VAT_TaxName', 'GST_ID', 'is_inclusive', 'VatID', 'Description', 'price_list_details',
                  'ProductImage', 'ProductImage2', 'ProductImage3', 'VegOrNonVeg', 'VariantName',
                  'Variant', 'ProductGroupID', 'GroupName')

    def get_ProductImage(self, instances):
        ProductImage = instances.ProductImage
        request = self.context.get("request")
        base_url = request.META['HTTP_HOST']
        if not ProductImage:
            ProductImage = ""
        else:
            ProductImage = "https://" + base_url + ProductImage.url
        return ProductImage
        
    def get_VegOrNonVeg(self, instances):
        VegOrNonVeg = instances.VegOrNonVeg
        if not VegOrNonVeg:
            VegOrNonVeg = ""
        return VegOrNonVeg
    
    def get_is_inclusive(self, instances):
        is_inclusive = instances.is_inclusive
        if not is_inclusive:
            is_inclusive = False
        return is_inclusive
    ...
    def get_GroupName(self, instances):
        CompanyID = self.context.get("CompanyID")
        ProductGroupID = instances.ProductGroupID
        BranchID = instances.BranchID
        GroupName = ""
        if table.ProductGroup.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductGroupID=ProductGroupID).exists():
            GroupName = table.ProductGroup.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, ProductGroupID=ProductGroupID).first().GroupName
        return GroupName

    
    def get_Variant(self, instances):
        CompanyID = self.context.get("CompanyID")
    ...
    def get_VariantName(self, instances):
        CompanyID = self.context.get("CompanyID")
        Variant = instances.Variant
        VariantName = ""
        if Variant:
    ...
    def get_price_list_details(self, instances):
        CompanyID = self.context.get("CompanyID")
        PriceRounding = self.context.get("PriceRounding")
        ProductID = instances.ProductID
        BranchID = instances. BranchID
        data = []
        if table.PriceList.objects.filter(CompanyID=CompanyID, ProductID=ProductID).exists():
            price_ins = table.PriceList.objects.filter(
                CompanyID=CompanyID, ProductID=ProductID)
            serialized = PriceListDetailsSerializer(price_ins, many=True, context={
...
class POS_Order_Serializer(serializers.ModelSerializer):
    ...
    def get_SalesGrandTotal(self, instances):
        ...
        if table.SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, SalesMasterID=InvoiceID).exists():
            GrandTotal = table.SalesMaster.objects.get(
                CompanyID=CompanyID, BranchID=BranchID, SalesMasterID=InvoiceID).GrandTotal
    ...
    def get_SalesID(self, instances):
        ...
        SalesID = ""
        if table.SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, SalesMasterID=InvoiceID).exists():
            SalesID = table.SalesMaster.objects.get(
                CompanyID=CompanyID, BranchID=BranchID, SalesMasterID=InvoiceID).id
        return str(SalesID)
...
class POS_Sales_PrintSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'Tax_no', 'CRNo', 'Mobile', 'City', 'State', 'Country', 'PostalCode', 'GrossAmt_print', 'VATAmount_print', 'NetTotal_print', 'TotalTax_print', 'Balance_print', 'TotalDiscount_print', 'GrandTotal_print', 'qr_image', 'BranchID', 'VoucherNo', 'Date',
                  ...
                  'Balance', 'SalesDetails', 'CompanyDetails', 'OrderPhone', 'TokenNumber', 'CreatedDate', 'OrderType')

    
    def get_OrderType(self, instances):
        CompanyID = self.context.get("CompanyID")
        ...
        if table.SalesOrderMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, InvoiceID=InvoiceID).exists():
            order_instance = table.SalesOrderMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, InvoiceID=InvoiceID).first()
            OrderType = order_instance.Type
        return OrderType
    
    def get_CreatedDate(self, instances):
        CreatedDate = instances.CreatedDate
        converted_CreatedDate = ConvertedTime(CreatedDate)
        return converted_CreatedDate
    ...
    def get_Tax_no(self, instances):
        ...
        if table.Parties.objects.filter(LedgerID=instances.LedgerID, CompanyID=CompanyID).exists():
            Tax_no = table.Parties.objects.get(
                LedgerID=instances.LedgerID, CompanyID=CompanyID).VATNumber
    ...
    def get_CRNo(self, instances):
        ...
        CR_no = ""
        if table.Parties.objects.filter(LedgerID=instances.LedgerID, CompanyID=CompanyID).exists():
            CR_no = table.Parties.objects.get(
                LedgerID=instances.LedgerID, CompanyID=CompanyID).CRNo
    ...
    def get_Mobile(self, instances):
        ...
        Mobile = ""
        if table.Parties.objects.filter(LedgerID=instances.LedgerID, CompanyID=CompanyID).exists():
            Mobile = table.Parties.objects.get(
        ...
        if table.Parties.objects.filter(LedgerID=LedgerID, CompanyID=CompanyID).exists():
            if table.Parties.objects.get(LedgerID=LedgerID, CompanyID=CompanyID).Country:
                ...
                if table.Country.objects.filter(pk=pk).exists():
                    Country_Name = table.Country.objects.get(
                        pk=pk).Country_Name
    ...
    #         qr_image = table.QrCode.objects.get(
    #             voucher_type="SI", master_id=pk).url
    #     return qr_image
    ...
    def get_qr_image(self, instances):
        ...
        qr_code = None
        if table.QrCode.objects.filter(voucher_type="SI", master_id=pk).exists():
            qr_code = table.QrCode.objects.get(voucher_type="SI", master_id=pk)
...
class POS_SalesDetailsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'unq_id', 'SalesDetailsID', 'BranchID', 'ProductName',
                  ...
                  'unitPriceRounded', 'quantityRounded', 'netAmountRounded','ProductDescription')
    ...
    def get_ProductDescription(self, sales_details):
        CompanyID = self.context.get("CompanyID")
        ProductID = sales_details.ProductID
        BranchID = sales_details.BranchID
        product = table.Product.objects.get(
    ...
    def get_TotalTax(self, purchase_details):
        ...
        TotalTax = (converted_float(TAX1Amount) + converted_float(TAX2Amount) + converted_float(TAX3Amount) +
                    converted_float(VATAmount) + converted_float(IGSTAmount) + converted_float(SGSTAmount) + converted_float(CGSTAmount) + converted_float(KFCAmount))
...
class POS_SalesOrder_PrintSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'Tax_no', 'CRNo', 'Mobile', 'City', 'State', 'Country', 'PostalCode', 'GrossAmt_print', 'VATAmount_print', 'NetTotal_print', 'TotalTax_print', 'TotalDiscount_print', 'GrandTotal_print', 'BranchID', 'VoucherNo', 'Date',
                  ...
                  'BillDiscPercent', 'BillDiscAmt', 'VATAmount', 'SGSTAmount', 'CGSTAmount', 'IGSTAmount', 'PrintCustomerName',
                  'SalesDetails', 'CompanyDetails', 'TokenNumber', 'OrderType')


    def get_OrderType(self, instances):
        CompanyID = self.context.get("CompanyID")
        OrderType = instances.Type
        return OrderType
...
class POS_SalesOrderDetailsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'unq_id', 'BranchID', 'ProductName',
                  ...
                  'CGSTAmount', 'IGSTPerc', 'IGSTAmount', 'NetAmount', 'CreatedUserID', 'TotalTax',
                  'unitPriceRounded', 'quantityRounded', 'netAmountRounded', 'ProductDescription', "Flavour")
...
class POS_LedgerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'LedgerID', 'LedgerName', 'OpeningBalance',
                  'CustomerLedgerBalance', 'AccountGroupUnder')
...
class POS_Table_Serializer1(serializers.ModelSerializer):
    ...
    class Meta:
        model = table.POS_Table
        fields = ('id', 'TableName', 'IsActive',
                  'PriceCategoryID', 'Position', 'sales_price')
...
class PriceListDetailsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'UnitName', 'SalesPrice', 'PriceListID',
                  'PurchasePrice', 'UnitID', 'index', 'MultiFactor')

    def get_UnitName(self, instances):
        CompanyID = self.context.get("CompanyID")
        BranchID = instances.BranchID
        UnitName = ""
        if table.Unit.objects.filter(CompanyID=CompanyID, BranchID=BranchID, UnitID=instances.UnitID).exists():
            UnitName = table.Unit.objects.get(
                CompanyID=CompanyID, BranchID=BranchID, UnitID=instances.UnitID).UnitName
        return str(UnitName)

    def get_index(self, instances):
        index = 0
        return index
...
class ProductVariantSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = table.ProductVariants
        fields = ('id', 'VariantID', 'VariantName', 'Amount')
        
        
class POS_partySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = table.Parties
        fields = ('id', 'PartyImage', 'FirstName', 'DisplayName', 'OpeningBalance', 'RouteName',
                  'LastName', 'PhoneNumber', 'Email', 'Address1', 'CustomerName', 'WebURL',
                  'CreditPeriod', 'PriceCategoryID', 'PanNumber', 'CreditLimit', 'RouteID',
                  'CRNo', 'BankName1', 'AccountName1', 'AccountNo1', 'IBANOrIFSCCode1', 'as_on_date',
                  'PriceCategoryName', 'GST_Treatment', 'VAT_Treatment')
        
    def get_PriceCategoryName(self, instances):
        CompanyID = self.context.get("CompanyID")
        PriceCategoryID = instances.PriceCategoryID
        BranchID = instances.BranchID
        if table.PriceCategory.objects.filter(CompanyID=CompanyID, PriceCategoryID=PriceCategoryID, BranchID=BranchID).exists():
            route_detail = table.PriceCategory.objects.get(
                CompanyID=CompanyID, PriceCategoryID=PriceCategoryID, BranchID=BranchID)
            PriceCategoryName = route_detail.PriceCategoryName
        else:
            PriceCategoryName = ""
    ...
    def get_as_on_date(self, instances):
        ...
        LedgerID = instances.LedgerID
        if table.AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID).exists():
            as_on_date = table.AccountLedger.objects.filter(
                CompanyID=CompanyID, LedgerID=LedgerID).first().as_on_date
        return as_on_date
    
    def get_CustomerName(self, instances):
        CustomerName = instances.PartyName
        return CustomerName
    ...
    def get_RouteName(self, instances):
        ...
        BranchID = instances.BranchID
        if table.Route.objects.filter(CompanyID=CompanyID, RouteID=RouteID, BranchID=BranchID).exists():
            route_detail = table.Route.objects.get(
    ...
    def get_FirstName(self, instances):
        FirstName = instances.FirstName
        if not FirstName:
            FirstName = instances.PartyName
        return FirstName
        
    def get_LastName(self, instances):
        LastName = instances.LastName
        if not LastName:
            LastName = ""
        return LastName
...
class PrintSerializer(serializers.ModelSerializer):
    class Meta:
        model = table.POS_Printer
        fields = '__all__'


class RasassyProduct_Serializer(serializers.ModelSerializer):
    DefaultUnitName = serializers.SerializerMethodField()
    DefaultSalesPrice = serializers.SerializerMethodField()
    ProductImage = serializers.SerializerMethodField()
    ...
    class Meta:
        model = table.Product
        fields = ('id', 'ProductName', 'DefaultUnitName', 'DefaultSalesPrice',
