from brands.models import LedgerPosting, UserTable, UserType, Customer, User_Log
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes,authentication_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from rest_framework_simplejwt.authentication import JWTTokenUserAuthentication
from api.v10.userTable.serializers import UserTableSerializer, UserTableSerializerRestSerializer, ListSerializer
from api.v10.userTable.functions import generate_serializer_errors
...
def create_user_table(request):
    ...
    if serialized.is_valid():
        ...
        if usertable_count < NoOfUsers:
            if UserTable.objects.filter(CompanyID=CompanyID, customer=customer, Active=True).exists():
                UserTable.objects.filter(CompanyID=CompanyID, customer=customer, Active=True).update(
                    UpdatedDate=today,
                    UserType=UserTypeID,
...
def edit_user_table(request, pk):
    ...
    if serialized.is_valid():
        ...
        if UserTable.objects.filter(CompanyID=CompanyID, pk=pk, Active=True).exists():
            UserTable.objects.filter(CompanyID=CompanyID, pk=pk, Active=True).update(
                UpdatedDate=today,
                UserType=UserTypeID,
                ExpiryDate=ExpiryDate,
                DefaultAccountForUser=DefaultAccountForUser,
...
def user_table(request, pk):
    ...
    instance = None
    if UserTable.objects.filter(CompanyID=CompanyID, pk=pk, Active=True).exists():
        instance = UserTable.objects.get(pk=pk, Active=True)
    if instance:
        serialized = UserTableSerializerRestSerializer(
...
def delete_user_table(request, pk):
    ...
    if instances:
        for instance in instances:
            ...
            if LedgerPosting.objects.filter(CompanyID=instance.CompanyID, CreatedUserID=user_id).exists():
                instance.save()
            else:
                instance.delete()
            User_Log.objects.create(
...
def leave_user_table(request, pk):
    ...
    instance = UserTable.objects.get(
        CompanyID__pk=pk, customer__user=user_id, Active=True)
    UserType = instance.UserType
    ExpiryDate = instance.ExpiryDate
    DefaultAccountForUser = instance.DefaultAccountForUser
    Cash_Account = instance.Cash_Account
