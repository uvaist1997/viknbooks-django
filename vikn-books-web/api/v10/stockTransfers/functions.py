...
def get_auto_id(model, BranchID, CompanyID):
    ...
    if model.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        instance = model.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID).order_by("StockTransferDetailsID").last()
        StockTransferDetailsID = instance.StockTransferDetailsID + 1
    return StockTransferDetailsID
