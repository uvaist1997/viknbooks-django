...
from main.functions import generate_random_nos
...
def get_VoucherNoForPOS(CompanyID,BranchID,UserID,VoucherType):
    ...
    if user_count == 1:
        if table.VoucherNoTable.objects.filter(CompanyID=CompanyID,UserID=UserID,VoucherType=VoucherType).exists():
            voucher = table.VoucherNoTable.objects.filter(CompanyID=CompanyID,UserID=UserID,VoucherType=VoucherType).first()
...
def device_code_exist():
    device_code = generate_random_nos(10)
    if table.POS_Devices.objects.filter(DeviceCode=device_code).exists():
        device_code_exist()
    return device_code
