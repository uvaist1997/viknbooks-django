import numpy as np
from api.v10.reportQuerys.functions import query_receipt_report_data
...
from api.v10.receipts.serializers import ReceiptDetailsListSerializer, ReceiptPrintSerializer, ReceiptMasterSerializer, ReceiptMasterRestSerializer, ReceiptDetailsSerializer, ReceiptDetailsRestSerializer
from api.v10.payments.serializers import ListSerializer, PaymentPrintSerializer
from api.v10.receipts.functions import export_to_excel_receiptReport, generate_serializer_errors, get_voucher_type, receiptReport_excel_data
from rest_framework import status
from api.v10.receipts.functions import get_auto_id, get_auto_idMaster
...
def create_receipt(request):
    ...
    try:
        with transaction.atomic():
            ...
            try:
                over_payment = data['over_payment']
            except:
                over_payment = 0
...
def edit_receipt(request, pk):
    try:
        with transaction.atomic():
            ...
            receiptMaster_instance.OverPayment = over_payment
        ...
        if ReceiptDetails.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            if page_number and items_per_page:
                instances = ReceiptDetails.objects.filter(
                    CompanyID=CompanyID, BranchID=BranchID).order_by("-ReceiptMasterID")
                count = len(instances)
                ledger_sort_pagination = list_pagination(
                    instances,
                    items_per_page,
