...
from api.v10.purchaseDetails.serializers import PurchaseDetailsSerializer, PurchaseDetailsRestSerializer, PurchaseDetailsDummySerializer, PurchaseDetailsDeletedDummySerializer
from api.v10.brands.serializers import ListSerializer
from api.v10.purchaseDetails.functions import generate_serializer_errors
from rest_framework import status
from api.v10.purchaseDetails.functions import get_auto_id
import datetime
...
def list_purchasesDetailsDummy(request):
    ...
    if serializedList.is_valid():
        ...
        if PurchaseDetailsDummy.objects.filter(BranchID=BranchID).exists():
            ...
            instances = PurchaseDetailsDummy.objects.filter(
                BranchID=BranchID, IsDeleted=False)

            deleted_instances = PurchaseDetailsDummy.objects.filter(
            ...
            serialized = PurchaseDetailsDummySerializer(
                instances, many=True, context={"DataBase": DataBase})
            serializedDeleted = PurchaseDetailsDeletedDummySerializer(
