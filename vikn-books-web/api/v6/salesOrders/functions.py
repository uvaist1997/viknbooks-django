...
def get_auto_idMaster(model,BranchID, CompanyID):
    ...
    latest_auto_id =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).aggregate(Max('SalesOrderMasterID'))
    

    if model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        max_value =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).aggregate(Max('SalesOrderMasterID'))
...
def get_auto_id(model,BranchID,CompanyID):
    ...
    # latest_auto_id =  model.objects.all().order_by("-CreatedDate")[:1]
    latest_auto_id =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).aggregate(Max('SalesOrderDetailsID'))
    

    if model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        max_value =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).aggregate(Max('SalesOrderDetailsID'))
