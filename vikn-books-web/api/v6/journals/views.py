...
from api.v6.journals.serializers import JournalMasterSerializer, JournalMasterRestSerializer, JournalDetailsSerializer, JournalDetailsRestSerializer
from api.v6.brands.serializers import ListSerializer
from api.v6.journals.functions import generate_serializer_errors
from rest_framework import status
from api.v6.journals.functions import get_auto_id, get_auto_idMaster
from api.v6.sales.functions import get_auto_VoucherNo
import datetime
        ...
        with transaction.atomic():
            ...
            if is_JournalOK:
                ...
                def remove_non_zero_equal_debit_and_credit(data):
                    ...
                    for item in data:
                        if item[1] != 0:
                            to_be_removed = list(filter(lambda x: x[-1] == item[1], data))
                ...
                def set_debit_and_credit_list():
                    ...
                    non_zero_entries = sorted(filter(lambda x: x[1] != 0 or x[-1] != 0, debit_credit_list)) # removes items with both debit and credit being 0
                    remove_non_zero_equal_debit_and_credit(non_zero_entries)
                    # print(non_zero_entries, final_data)
                    debit_list = list(filter(lambda x: x[1] != 0, non_zero_entries))
                    credit_list = list(filter(lambda x: x[-1] != 0, non_zero_entries))
                ...
                for f in final_data:
                    ...
                    if not debit == 0:
                        ...
                        Amount = credit
                        DrOCr = "Cr"
                    update_ledger = UpdateLedgerBalance(CompanyID,BranchID,f['LedgerID'],JournalMasterID,VoucherType,Amount,DrOCr,"create")
...
def edit_journal(request, pk):
    try:
        with transaction.atomic():
            ...
            update_ledger = UpdateLedgerBalance(CompanyID,BranchID,0,JournalMasterID,VoucherType,0,"Cr","update")
