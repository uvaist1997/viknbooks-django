...
def get_auto_id(model,BranchID,CompanyID):
    ...
    latest_auto_id =  model.objects.filter(CompanyID=CompanyID).aggregate(Max('ProductGroupID'))

    if model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        max_value =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).aggregate(Max('ProductGroupID'))
