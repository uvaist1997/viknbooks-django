...
def get_auto_idMaster(model,BranchID,CompanyID):
    ...
    latest_auto_id =  model.objects.filter(CompanyID=CompanyID).aggregate(Max('StockTransferMasterID'))
    
    if model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        max_value =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).aggregate(Max('StockTransferMasterID'))
...
def get_auto_id(model,BranchID,CompanyID):
    ...
    # latest_auto_id =  model.objects.all().order_by("-CreatedDate")[:1]
    latest_auto_id =  model.objects.filter(CompanyID=CompanyID).aggregate(Max('StockTransferDetailsID'))
    
    if model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        max_value =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).aggregate(Max('StockTransferDetailsID'))
