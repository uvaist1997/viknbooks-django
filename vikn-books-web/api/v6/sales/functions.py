...
from api.v6.sales.serializers import SalesGSTReportSerializer,GST_CDNR_Serializer,GST_B2B_Serializer
...
def get_Genrate_VoucherNo(model,BranchID,CompanyID,VoucherType):
    ...
    if model.objects.filter(CompanyID=CompanyID, BranchID=BranchID,VoucherNo=VoucherNo).exists():
        instances = model.objects.filter(CompanyID=CompanyID, BranchID=BranchID)
...
def gstr1_data(CompanyID,BranchID,FromDate,ToDate,PriceRounding):
            ...
            if not SalesMasterID in unq_master_ids:
                ...
                for s in sales_gsts:
                    if not s in unq_sales_gsts:
                        ...
                        taxable_value = round(float(taxable_value),int(PriceRounding))
    ...
    if SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,shipping_tax_amount__gt=0,Date__gte=FromDate,Date__lte=ToDate,TaxType__in=tax_types).exists():
        ...
        for i in jsnDatas:
            ...
            if not SalesMasterID in unq_master_ids:
                ...
                GrandTotal = round(float(GrandTotal),int(PriceRounding))
                ShippingCharge = round(float(ShippingCharge),int(PriceRounding))
            ...
            if not SalesMasterID in unq_master_ids:
                ...
                for s in sales_gsts:
                    if not s in unq_sales_gsts and float(GrandTotal) < 250000:
                        ...
                        if ApplicableTaxRate == "":
                            ApplicableTaxRate = 0
                        ApplicableTaxRate = round(float(ApplicableTaxRate),int(PriceRounding))
                        # reate
                        ...
                        if not str(PlaceOfSupply)+str(rate) in rate_placeofsupplay_arr:
                            ...
                            b2cs_data = [i for i in final_data_b2c_small if i['rate_placeofsupplay'] == str(PlaceOfSupply)+str(rate)]
                            curent_taxable_value = b2cs_data[0]['taxable_value']
                            new_taxable_value = float(curent_taxable_value) + float(old_taxable_value)
            ...
            if not SalesReturnMasterID in unq_master_ids:
                ...
                for s in sales_gsts:
                    if not s in unq_sales_gsts and float(GrandTotal) < 250000:
                        ...
                        old_taxable_value = taxable_value
                        rate = float(s) * -1
                        print(str(PlaceOfSupply)+str(rate),'----',taxable_value,)
                        if not str(PlaceOfSupply)+str(rate) in rate_placeofsupplay_arr:
                            final_data_b2c_small.append({
                                "rate_placeofsupplay":str(PlaceOfSupply)+str(rate),
            ...
            for uq in uqc_ins:
                if Unit.objects.filter(CompanyID=CompanyID,BranchID=BranchID,UQC=uq).exists():
                    ...
                    if not TotalQuantity == 0 and not TotalNetAmount == 0:
                         # ======
                        TotalNetAmount1 = round(float(TotalNetAmount),int(PriceRounding))
                        TotalTaxableValue1 = round(float(TotalTaxableValue),int(PriceRounding))
                        TotalIGSTAmount1 = round(float(TotalIGSTAmount),int(PriceRounding))
                        TotalCGSTAmount1 = round(float(TotalCGSTAmount),int(PriceRounding))
                        TotalSGSTAmount1 = round(float(TotalSGSTAmount),int(PriceRounding))
    ...
    if SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,shipping_tax_amount__gt=0,Date__gte=FromDate,Date__lte=ToDate,).exists():
        ...
        for s in sales_ins:
            ...
            ShippingCharge1 = round(float(s.ShippingCharge),int(PriceRounding))
            ...
            if HSN_Code and SalesTax:
                ...
                if not str(HSN_Code)+str(SalesTax) in rate_saletax:
                    ...
                    # final_data_HSN_summary  
                    superplayers = [user for user in final_data_HSN_summary if user['rate_saletax'] == str(HSN_Code)+str(SalesTax)]
    ...
    if SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate).exists():
        sales_instances = SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate).order_by('SalesMasterID')
        cancelled_sales_instances = SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate,Status="Cancelled")
        ...
        print(first_instance.VoucherNo,last_instance.VoucherNo,cancelled_count,total_count,"SalesMasterID")
    ...
    if SalesReturnMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherDate__gte=FromDate,VoucherDate__lte=ToDate).exists():
        sales_instances = SalesReturnMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherDate__gte=FromDate,VoucherDate__lte=ToDate).order_by('SalesReturnMasterID')
        first_instance = sales_instances.first()
        last_instance = sales_instances.last()
        total_count = len(sales_instances)
        final_data_docs.append({
...
def b2cs_excel(wb,b2cs_data):
    ...
    # write column headers in sheet
    for col_num in range(len(columns)):
        b2cs.write(row_num, col_num, columns[col_num],b2cs_column_header_style)
...
def cdnur_excel(wb,cdnur_data):
    ...
    ws.write(2, 5, Total_invoice_value_cdnur, decimal_cdnur_subheader_value_style)
    ws.write(2, 6, "", cdnur_subheader_value_style)
    ws.write(2, 7, "", cdnur_subheader_value_style)
    ws.write(2, 8, Total_taxable_value_cdnur, decimal_cdnur_subheader_value_style)
    ...
    for i in cdnur_data:
        ...
        ws.write(row_num, 6, ApplicableTaxRate,decimal_cdnur_subheader_value_style)
        ws.write(row_num, 7, Rate,decimal_cdnur_subheader_value_style)
        ws.write(row_num, 8, taxable_value,decimal_cdnur_subheader_value_style)
...
def hsn_excel(wb,hsn_data):
    ...
    ws.write(2, 6, total_taxable_value_hsn, decimal_cdnur_subheader_value_style)
    ...
    #column header names, you can use your own headers here
    columns = ['HSN', 'Description', 'UQC', 'Total Quantity','Total Value','Rate', 'Taxable Value','IGST Amount', 'CGST Amount','SGST Amount','Cess Amount' ]
    ...
    for i in hsn_data:
        ...
        ws.write(row_num, 3, TotalQuantity, decimal_cdnur_subheader_value_style)
        ws.write(row_num, 4, TotalNetAmount, decimal_cdnur_subheader_value_style)
        ws.write(row_num, 5, Rate, decimal_cdnur_subheader_value_style)
        ws.write(row_num, 6, TotalTaxableValue, decimal_cdnur_subheader_value_style)
        ws.write(row_num, 7, TotalIGSTAmount, decimal_cdnur_subheader_value_style)
        ws.write(row_num, 8, TotalCGSTAmount, decimal_cdnur_subheader_value_style)
        ws.write(row_num, 9, TotalSGSTAmount, decimal_cdnur_subheader_value_style)
...
def default_b2cl_excel(wb,b2cl_data):
    ...
    #column header names, you can use your own headers here
    columns = ['Invoice  number', 'Invoice Date', 'Invoice value', 'Applicable % of Tax Rate','Place of Supply(POS)', 'Rate', 'Taxable Value', 'Cess Amount' ]
...
def b2ba_excel(wb,b2ba_data):
 ...
 # color light blue third header style
    ...
    #column header names, you can use your own headers here
    columns = ['GSTIN/UIN of Recipient', 'Receiver Name', 'Original Invoice Number', 'Original Invoice date','Revised Invoice Number', 'Revised Invoice date', 'Invoice Value','Place Of Supply', 'Reverse Charge', 'Applicable % of Tax Rate', 'Invoice Type', 'E-Commerce GSTIN', 'Rate', 'Taxable Value', 'Cess Amount' ]
...
def b2cla_excel(wb,b2ba_data):
 ...
 # color light blue third header style
    ...
    #column header names, you can use your own headers here
    columns = ['Original Invoice Number','Original Invoice date','Original Place Of Supply','Revised Invoice Number','Revised Invoice date','Invoice Value','Applicable % of Tax Rate','Rate','Taxable Value','Cess Amount','E-Commerce GSTIN', ]
...
def b2csa_excel(wb,b2ba_data):
 ...
 # color light blue third header style
    ...
    #column header names, you can use your own headers here
    columns = ['Financial Year','Original Month','Place Of Supply','Type','Applicable % of Tax Rate','Rate','Taxable Value','Cess Amount','E-Commerce GSTIN' ]
...
def cdnra_excel(wb,b2ba_data):
 ...
 # color light blue third header style
    ...
    #column header names, you can use your own headers here
    columns = ['GSTIN/UIN of Recipient','Receiver Name','Original Note Number','Original Note Date','Revised Note Number','Revised Note Date','Note Type','Place Of Supply','Reverse Charge','Note Supply Type','Note Value','Applicable % of Tax Rate','Rate','Taxable Value','Cess Amount', ]
    ...
    for col_num in range(len(columns)):
        if col_num < 4:           
            ws.write(row_num, col_num, columns[col_num],orange_column_header_style)
        else:           
            ws.write(row_num, col_num, columns[col_num],l_blue_column_header_style)
...
def cdnura_excel(wb,cdnura_data):
 ...
 # color light blue third header style
    ...
    #column header names, you can use your own headers here
    columns = ['UR Type','Original Note Number', 'Original Note Date','Revised Note Number','Revised Note Date','Note Type','Place Of Supply','Note Value','Applicable % of Tax Rate','Rate','Taxable Value','Cess Amount']
...
def expa_excel(wb,expa_data):
 ...
 # color light blue third header style
    ...
    #column header names, you can use your own headers here
    columns = ['Export Type','Original Invoice Number','Original Invoice date','Revised Invoice Number','Revised Invoice date','Invoice Value','Port Code','Shipping Bill Number','Shipping Bill Date','Rate','Taxable Value','Cess Amount',]
...
def at_excel(wb,at_data):
    ...
    #column header names, you can use your own headers here
    columns = ['Place Of Supply','Applicable % of Tax Rate','Rate','Gross Advance Received','Cess Amount']
...
def ata_excel(wb,ata_data):
 ...
 # color light blue third header style
    ...
    # header
    ws.write(0, 0, "Summary For Amended Tax Liability(Advance Received) ", header_style) 
    ws.write_merge(0,0,1,2,'Original details',seond_header_style)
    ...
    #column header names, you can use your own headers here
    columns = ['Financial Year','Original Month','Original Place Of Supply','Applicable % of Tax Rate','Rate','Gross Advance Received','Cess Amount',]
...
def atadj_excel(wb,atadj_data):
 ...
 # color light blue third header style
    ...
    #column header names, you can use your own headers here
    columns = ['Place Of Supply','Applicable % of Tax Rate','Rate','Gross Advance Adjusted','Cess Amount']
...
def atadja_excel(wb,exemp_data):
 ...
 # color light blue third header style
    ...
    #column header names, you can use your own headers here
    columns = ['Financial Year','Original Month','Original Place Of Supply','Applicable % of Tax Rate','Rate','Gross Advance Adjusted','Cess Amount',]
...
def exemp_excel(wb,exemp_data):
 ...
 # color light blue third header style
    ...
    # header
    ws.write(0, 0, "Summary For Nil rated, exempted and non GST outward supplies (8)", header_style) 
...
def docs_excel(wb,docs_data):
 ...
 # color light blue third header style
    ...
    # header
    ws.write(0, 0, "Summary of documents issued during the tax period (13)", header_style) 
    ...
    #column header names, you can use your own headers here
    columns = ['Nature of Document','Sr. No. From','Sr. No. To','Total Number','Cancelled',]
...
def export_to_excel_sales_gst(wb,data,FromDate,ToDate):
    ...
    columns = ['Date','Voucher No','Particulars','GSTIN/UIN','Voucher Type','Tax Type','Taxable Amount','SGST','CGST','IGST','Cess','KFC','Total Tax Amount']
    row_num = 0
    # write column headers in sheet
