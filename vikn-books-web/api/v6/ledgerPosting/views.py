...
from api.v6.ledgerPosting.serializers import LedgerPostingSerializer, LedgerPostingRestSerializer, ListSerializerforTrialBalance,\
...
from api.v6.stockPostings.serializers import StockPostingRestSerializer, StockValueAllSerializer, StockValueSingleSerializer
from api.v6.ledgerPosting.functions import generate_serializer_errors
from rest_framework import status
from api.v6.sales.serializers import ListSerializerforReport, ListSerializerforStockReport, ListSerializerforStockValueReport, ListSerializerforStockValueReportSingle
from api.v6.ledgerPosting.functions import balanceSheet_excel_data,export_to_excel_balanceSheet,profitAndLoss_excel_data,export_to_excel_profitAndLoss,get_auto_id, convertOrderdDict, get_VoucherName,ledgerReport_excel_data,export_to_excel_ledgerReport,trialBalance_excel_data,export_to_excel_trialBalance
import datetime
...
def DayBook(request):
    ...
    if LedgerPosting.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Date=Date).exists():
        ...
        for ta in test_arr:
            ...
            not_dupli_list_byID = []
            [not_dupli_list_byID.append(x) for x in list_byID if x not in not_dupli_list_byID]
            ...
            vouch_list = []
            if ta == "SI":
                sales_instances = SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID,SalesMasterID__in=not_dupli_list_byID)
                sales_instances_modif = SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID,SalesMasterID__in=not_dupli_list_byID_modi)
                sales_instances_delt = SalesMaster_Log.objects.filter(CompanyID=CompanyID,Action="D", BranchID=BranchID,TransactionID__in=not_dupli_list_byID_delt)
                ...
                sales_grand_total = sales_instances.aggregate(Sum('GrandTotal'))
                grand_total = sales_grand_total['GrandTotal__sum']
                sales_grand_total_modif = sales_instances_modif.aggregate(Sum('GrandTotal'))
                grand_total_modif = sales_grand_total_modif['GrandTotal__sum']
                sales_grand_total_delt = sales_instances_delt.aggregate(Sum('GrandTotal'))
                grand_total_delt = sales_grand_total_delt['GrandTotal__sum']
                vouch_list.append(ta)
            elif ta == "PI":
                purchase_instances = PurchaseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID,PurchaseMasterID__in=not_dupli_list_byID)
                purchase_instances_modif = PurchaseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID,PurchaseMasterID__in=not_dupli_list_byID_modi)
                purchase_instances_delt = PurchaseMaster_Log.objects.filter(CompanyID=CompanyID,Action="D", BranchID=BranchID,TransactionID__in=not_dupli_list_byID_delt)
                ...
                purchase_grand_total = purchase_instances.aggregate(Sum('GrandTotal'))
                grand_total = purchase_grand_total['GrandTotal__sum']
                purchase_grand_total_modif = purchase_instances_modif.aggregate(Sum('GrandTotal'))
                grand_total_modif = purchase_grand_total_modif['GrandTotal__sum']
                purchase_grand_total_delt = purchase_instances_delt.aggregate(Sum('GrandTotal'))
                grand_total_delt = purchase_grand_total_delt['GrandTotal__sum']
                vouch_list.append(ta)
            elif ta == "SR":
                sales_return_instances = SalesReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID,SalesReturnMasterID__in=not_dupli_list_byID)
                sales_return_instances_modif = SalesReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID,SalesReturnMasterID__in=not_dupli_list_byID_modi)
                sales_return_instances_delt = SalesReturnMaster_Log.objects.filter(CompanyID=CompanyID,Action="D", BranchID=BranchID,TransactionID__in=not_dupli_list_byID_delt)
                ...
                sales_return_grand_total = sales_return_instances.aggregate(Sum('GrandTotal'))
                grand_total = sales_return_grand_total['GrandTotal__sum']
                sales_return_grand_total_modif = sales_return_instances_modif.aggregate(Sum('GrandTotal'))
                grand_total_modif = sales_return_grand_total_modif['GrandTotal__sum']
                sales_return_grand_total_delt = sales_return_instances_delt.aggregate(Sum('GrandTotal'))
                grand_total_delt = sales_return_grand_total_delt['GrandTotal__sum']
                vouch_list.append(ta)
            elif ta == "PR":
                purchase_return_instances = PurchaseReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID,PurchaseReturnMasterID__in=not_dupli_list_byID)
                purchase_return_instances_modif = PurchaseReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID,PurchaseReturnMasterID__in=not_dupli_list_byID_modi)
                purchase_return_instances_delt = PurchaseReturnMaster_Log.objects.filter(CompanyID=CompanyID,Action="D", BranchID=BranchID,TransactionID__in=not_dupli_list_byID_delt)
                ...
                purchase_return_grand_total = purchase_return_instances.aggregate(Sum('GrandTotal'))
                grand_total = purchase_return_grand_total['GrandTotal__sum']
                purchase_return_grand_total_modif = purchase_return_instances_modif.aggregate(Sum('GrandTotal'))
                grand_total_modif = purchase_return_grand_total_modif['GrandTotal__sum']
                purchase_return_grand_total_delt = purchase_return_instances_delt.aggregate(Sum('GrandTotal'))
                grand_total_delt = purchase_return_grand_total_delt['GrandTotal__sum']
                vouch_list.append(ta)
            elif ta == "CP" or ta == "BP":
                payment_instances = PaymentMaster.objects.filter(CompanyID=CompanyID,VoucherType=ta, BranchID=BranchID,PaymentMasterID__in=not_dupli_list_byID)
                payment_instances_modif = PaymentMaster.objects.filter(CompanyID=CompanyID,VoucherType=ta, BranchID=BranchID,PaymentMasterID__in=not_dupli_list_byID_modi)
                payment_instances_delt = PaymentMaster_Log.objects.filter(CompanyID=CompanyID,VoucherType=ta,Action="D", BranchID=BranchID,TransactionID__in=not_dupli_list_byID_delt)
                ...
                payment_grand_total_modif = payment_instances_modif.aggregate(Sum('TotalAmount'))
                grand_total_modif = payment_grand_total_modif['TotalAmount__sum']
                payment_grand_total_delt = payment_instances_delt.aggregate(Sum('TotalAmount'))
                grand_total_delt = payment_grand_total_delt['TotalAmount__sum']
                vouch_list.append(ta)
            elif ta == "CR" or ta == "BR":
                receipt_instances = ReceiptMaster.objects.filter(CompanyID=CompanyID,VoucherType=ta, BranchID=BranchID,ReceiptMasterID__in=not_dupli_list_byID)
                receipt_instances_modif = ReceiptMaster.objects.filter(CompanyID=CompanyID,VoucherType=ta, BranchID=BranchID,ReceiptMasterID__in=not_dupli_list_byID_modi)
                receipt_instances_delt = ReceiptMaster_Log.objects.filter(CompanyID=CompanyID,VoucherType=ta,Action="D", BranchID=BranchID,TransactionID__in=not_dupli_list_byID_delt)
                ...
                receipt_grand_total = receipt_instances.aggregate(Sum('TotalAmount'))
                grand_total = receipt_grand_total['TotalAmount__sum']
                receipt_grand_total_modif = receipt_instances_modif.aggregate(Sum('TotalAmount'))
                grand_total_modif = receipt_grand_total_modif['TotalAmount__sum']
                receipt_grand_total_delt = receipt_instances_delt.aggregate(Sum('TotalAmount'))
            ...
            for al in account_ledger_insts:
                ...
                [not_dupli_list_byID_detail.append(x) for x in list_byID_detail if x not in not_dupli_list_byID_detail]
                ...
                detail_amt = float(total_debit_daybk_) - float(total_credit_modi_)
...
def ledgerReport_excel(request):
    ...
    print(CompanyID,CreatedUserID,PriceRounding,BranchID,FromDate,ToDate,ID,value,ManualOpeningBalance,'****************')
    ...
    data = ledgerReport_excel_data(CompanyID,CreatedUserID,PriceRounding,BranchID,FromDate,ToDate,ID,value,ManualOpeningBalance)
...
def trialBalance_excel(request):
    ...
    #creating workbook
    wb = xlwt.Workbook(encoding='utf-8')
    data = trialBalance_excel_data(CompanyID,CreatedUserID,PriceRounding,BranchID,ToDate,request)
...
def profitAndLoss_excel(request):
    ...
    #creating workbook
    wb = xlwt.Workbook(encoding='utf-8')
    data = profitAndLoss_excel_data(key,CompanyID,CreatedUserID,PriceRounding,BranchID,ToDate,FromDate,request,ManualOpeningStock,ManualClosingStock)
...
def balanceSheet_excel(request):
    ...
    #creating workbook
    wb = xlwt.Workbook(encoding='utf-8')
    data = balanceSheet_excel_data(CompanyID,CreatedUserID,PriceRounding,BranchID,ToDate,FromDate,request)
    ManualOpeningStock = 0
    ManualClosingStock = 0
    profit_loss_data = profitAndLoss_excel_data(CompanyID,CreatedUserID,PriceRounding,BranchID,ToDate,FromDate,request,ManualOpeningStock,ManualClosingStock)
    # ===============  adding Ledger Report sheet ============
    header_style = xlwt.easyxf('pattern: pattern solid, fore_colour light_blue;'
                              'font: colour white, bold True;')
