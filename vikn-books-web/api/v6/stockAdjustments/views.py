...
from api.v6.stockAdjustments.serializers import StockAdjustmentMasterSerializer, StockAdjustmentMasterRestSerializer, StockAdjustmentDetailsSerializer, StockAdjustmentDetailsRestSerializer
from api.v6.brands.serializers import ListSerializer
from api.v6.stockAdjustments.functions import generate_serializer_errors, get_VoucherNo
from rest_framework import status
from api.v6.stockAdjustments.functions import get_auto_id, get_auto_idMaster
import datetime
from main.functions import get_company, activity_log
from api.v6.stockPostings.functions import get_auto_excessMasterID, get_auto_excessDetailsID, get_auto_shortageMasterID, get_auto_shortageDetailsID
                    ...
                    if GeneralSettings.objects.filter(CompanyID=CompanyID, BranchID=BranchID, SettingsType="EnableProductBatchWise").exists():
                        ...
                        if check_EnableProductBatchWise == "True" or check_EnableProductBatchWise == True:
                            if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode).exists():
                                ...
                                batch_insts.StockIn = float(
                                    StockIn) + float(ExcessStock)
                                batch_insts.save()
                            else:
                    ...
                    if GeneralSettings.objects.filter(CompanyID=CompanyID, BranchID=BranchID, SettingsType="EnableProductBatchWise").exists():
                        ...
                        if check_EnableProductBatchWise == "True" or check_EnableProductBatchWise == True:
                            if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode).exists():
                                ...
                                batch_insts.StockOut = float(
                                    StockOut) + float(abs_ShortageStock)
                                batch_insts.save()
                            else:
                                BatchCode = get_auto_AutoBatchCode(
                                    Batch, BranchID, CompanyID)
