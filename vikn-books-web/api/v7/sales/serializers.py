...
from brands.models import CompanySettings,QrCode,Country,State,LoyaltyPoint,SalesEstimateMaster, SalesEstimateDetails, SalesMaster, SalesDetails, Product, AccountLedger, Warehouse, PriceList, SerialNumbers, TransactionTypes, Unit,\
    ...
    SalesOrderMaster, SalesOrderDetails
from api.v7.priceLists.serializers import PriceListRestSerializer
from api.v7.workOrder.serializers import Batch_ListSerializer
...
class SalesMasterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('LoyaltyCustomerID', 'id', 'BranchID', 'Action', 'VoucherNo', 'Date',
                  'CreditPeriod', 'LedgerID', 'OldLedgerBalance','GST_Treatment','VAT_Treatment','Status',
...
class SalesMasterRestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id','Tax_no','CRNo','Mobile','City','State','Country','PostalCode','GrossAmt_print','VATAmount_print','TotalTaxableAmount_print','NetTotal_print','TotalTax_print','Balance_print', 'TotalDiscount_print','GrandTotal_print','qr_image','ShippingCharge','shipping_tax_amount','TaxTypeID','SAC','SalesTax','LoyaltyCustomerID','LoyaltyCustomerName','TotalTaxableAmount', 'SalesMasterID', 'BranchID', 'Action', 'VoucherNo', 'Date',
                  'CreditPeriod', 'LedgerID', 'LedgerName', 'OldLedgerBalance', 'CashID', 'BankID','Country_of_Supply','State_of_Supply','VAT_Treatment',
                  'PriceCategoryID', 'EmployeeID', 'SalesAccount', 'SalesAccountName', 'CustomerName', 'Address1', 'Address2', 'Address3','PrintAddress',
                  ...
                  'TaxID', 'TaxType', 'BillDiscPercent', 'BillDiscAmt', 'VATAmount', 'SGSTAmount', 'CGSTAmount', 'IGSTAmount','Status','Attention','PrintCustomerName',
                  'TAX1Amount', 'TAX2Amount', 'TAX3Amount', 'KFCAmount', 'Balance', 'TransactionTypeID', 'DetailID', 'is_customer', 'SalesDetails','LoyaltyPoint','LoyaltyValue',
                  'ShipAddress1','ShipAddress2','ShipCity','ShipStateName','ShipCountryName')
    ...
    def get_ShipAddress1(self, instances):
        ...
        if Parties.objects.filter(LedgerID=LedgerID, CompanyID=CompanyID).exists():
            ShipAddress1 = Parties.objects.filter(
                LedgerID=LedgerID, CompanyID=CompanyID).first().Address1_Shipping
        return ShipAddress1
    ...
    def get_ShipAddress2(self, instances):
        ...
        LedgerID = instances.LedgerID
        if Parties.objects.filter(LedgerID=LedgerID, CompanyID=CompanyID).exists():
            ShipAddress2 = Parties.objects.filter(
                LedgerID=LedgerID, CompanyID=CompanyID).first().Address2_Shipping
        return ShipAddress2
    ...
    def get_ShipCity(self, instances):
        ...
        LedgerID = instances.LedgerID
        if Parties.objects.filter(LedgerID=LedgerID, CompanyID=CompanyID).exists():
            ShipCity = Parties.objects.filter(
                LedgerID=LedgerID, CompanyID=CompanyID).first().City_Shipping
        return ShipCity
    ...
    def get_ShipStateName(self, instances):
        ...
        LedgerID = instances.LedgerID
        if Parties.objects.filter(LedgerID=LedgerID, CompanyID=CompanyID).exists():
            State_Shipping = Parties.objects.filter(
                LedgerID=LedgerID, CompanyID=CompanyID).first().State_Shipping
            if State_Shipping:
    ...
    def get_ShipCountryName(self, instances):
        ...
        LedgerID = instances.LedgerID
        if Parties.objects.filter(LedgerID=LedgerID, CompanyID=CompanyID).exists():
            Country_Shipping = Parties.objects.filter(
                LedgerID=LedgerID, CompanyID=CompanyID).first().Country_Shipping
            if Country_Shipping:
    ...
    def get_LoyaltyPoint(self, instances):
        ...
        if LoyaltyPoint.objects.filter(CompanyID=CompanyID, VoucherMasterID=instances.SalesMasterID, BranchID=BranchID,VoucherType="SI",is_Radeem=False).exists():
            instance = LoyaltyPoint.objects.get(CompanyID=CompanyID, VoucherMasterID=instances.SalesMasterID, BranchID=BranchID,VoucherType="SI",is_Radeem=False)
...
class SalesMasterRest1Serializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'SalesMasterID', 'VoucherNo', 'Date','TotalTax_rounded','GrandTotal_Rounded',
                   'LedgerName', 'CustomerName', 'TotalTax','Status',
                  'GrandTotal')
...
class SalesDetailsRestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SalesDetails
        fields = ('id','NetAmount_print','VATPerc_print','VATAmount_print', 'unq_id', 'SalesDetailsID', 'BranchID', 'Action', 'SalesMasterID', 'ProductID', 'ProductName','ProductTaxID',
                  'Qty', 'FreeQty', 'UnitPrice', 'ReturnQty', 'InclusivePrice', 'ActualUnitPrice', 'SerialNos','is_kfc',
                  'RateWithTax', 'CostPerPrice', 'PriceListID', 'UnitName', 'DiscountPerc', 'DiscountAmount', 'AddlDiscPerc', 'AddlDiscAmt',
                  'GrossAmount', 'TaxableAmount', 'VATPerc', 'VATAmount', 'SGSTPerc', 'SGSTAmount', 'CGSTPerc', 'Description','ProductTaxName',
                  'CGSTAmount', 'IGSTPerc', 'IGSTAmount', 'KFCAmount', 'KFCPerc', 'NetAmount', 'CreatedDate', 'CreatedUserID', 'Flavour',
                  ...
                  'is_VAT_inclusive', 'is_GST_inclusive', 'is_TAX1_inclusive', 'is_TAX2_inclusive', 'is_TAX3_inclusive','UnitList','is_inclusive',
                  'unitPriceRounded', 'quantityRounded', 'actualSalesPrice', 'netAmountRounded', 'BatchCode', 'ProductCode', 'HSNCode',
                  'GST_Inclusive','Vat_Inclusive','SalesPrice','ActualProductTaxName','ActualProductTaxID')
...
class SerialNumberSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SerialNumbers
        fields = ('SerialNoID', 'SerialNo', 'ItemCode',
                  'SalesMasterID', 'SalesDetailsID')
...
class SalesGSTReportSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SalesMaster
        fields = ('id', 'SalesMasterID','TotalTaxableAmount','SGSTAmount','CGSTAmount','IGSTAmount', 'BranchID', 'Action', 'VoucherNo', 'TaxType', 'Date', 'CreditPeriod','SalesType','TotalQty',
                  'LedgerID', 'PriceCategoryID', 'EmployeeID', 'SalesAccount', 'CustomerName', 'TaxableValue','KFCAmount','TotalTax')
    ...
    def get_TaxableValue(self, instances):
        ...
        SalesMasterID = instances.SalesMasterID
        BranchID = instances.BranchID
        sales_details = SalesDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,SalesMasterID=SalesMasterID)
...
class SalesMasterReportSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SalesMaster
        fields = ('id', 'SalesMasterID', 'BranchID', 'Action', 'VoucherNo', 'Date', 'CashID', 'BankID', 'CashName', 'BankName',
                  'CreditPeriod', 'LedgerID', 'LedgerName', 'LedgerBalance', 'OldLedgerBalance', 'AccountGroupUnder','GST_Treatment','VAT_Treatment',
                  'PriceCategoryID', 'EmployeeID', 'SalesAccount', 'SalesAccountName', 'CustomerName', 'Address1', 'Address2', 'Address3',
                  'Notes', 'FinacialYearID', 'TotalGrossAmt', 'AddlDiscPercent', 'AddlDiscAmt', 'TotalDiscount', 'TotalTax','Country_of_Supply','State_of_Supply',
                  'NetTotal', 'AdditionalCost', 'GrandTotal', 'RoundOff', 'CashReceived', 'CashAmount', 'BankAmount', 'WarehouseID', 'WareHouseName', 'TableID', 'SeatNumber', 'NoOfGuests', 'INOUT', 'TokenNumber', 'CardTypeID', 'CardTypeName', 'CardNumber', 'IsActive', 'IsPosted', 'SalesType', 'CreatedDate', 'UpdatedDate', 'CreatedUserID',
                  'TaxID', 'TaxType', 'BillDiscPercent', 'BillDiscAmt', 'VATAmount', 'SGSTAmount', 'CGSTAmount', 'IGSTAmount','Country_of_Supply_name','State_of_Supply_name',
    ...
    def get_CashSales(self, instances):
        ...
        TotalDebit = 0
        TotalCredit = 0
        account_instance = AccountLedger.objects.filter(CompanyID=CompanyID, BranchID=BranchID, AccountGroupUnder=9)
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID, BranchID=BranchID, AccountGroupUnder=9).exists():
            ...
            if LedgerPosting.objects.filter(CompanyID=CompanyID, LedgerID__in=ledger_ids, Date__gte=FromDate, Date__lte=ToDate, VoucherType="SI",VoucherMasterID=instances.SalesMasterID).exists():
                ledger_ins = LedgerPosting.objects.filter(
                    CompanyID=CompanyID, LedgerID__in=ledger_ids, Date__gte=FromDate, Date__lte=ToDate, VoucherType="SI",VoucherMasterID=instances.SalesMasterID)
                for i in ledger_ins:
                    print(i.id,"forloop")
    ...
    def get_BankSales(self, instances):
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID, BranchID=BranchID, AccountGroupUnder=8).exists():
            account_instance = AccountLedger.objects.filter(CompanyID=CompanyID, BranchID=BranchID, AccountGroupUnder=8)
            ledger_ids = account_instance.values_list('LedgerID', flat=True)
    ...
    def get_CreditSales(self, instances):
        ...
        if AccountLedger.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, BranchID=BranchID, AccountGroupUnder__in=[8, 9]).exists():
            account_instance = AccountLedger.objects.filter(CompanyID=CompanyID, BranchID=BranchID, AccountGroupUnder__in=[8,9])
            ledger_ids = account_instance.values_list('LedgerID', flat=True)
            if LedgerPosting.objects.filter(CompanyID=CompanyID, LedgerID__in=ledger_ids, Date__gte=FromDate, Date__lte=ToDate, VoucherType="SI",VoucherMasterID=instances.SalesMasterID).exists():
                ledger_ins = LedgerPosting.objects.filter(
                    CompanyID=CompanyID, LedgerID__in=ledger_ids, Date__gte=FromDate, Date__lte=ToDate, VoucherType="SI",VoucherMasterID=instances.SalesMasterID)
            ...
            account_instance = AccountLedger.objects.filter(CompanyID=CompanyID, BranchID=BranchID)
            ledger_ids = account_instance.values_list('LedgerID', flat=True)
            if LedgerPosting.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, Date__gte=FromDate, Date__lte=ToDate, VoucherType="SI",VoucherMasterID=instances.SalesMasterID).exists():
                ledger_ins = LedgerPosting.objects.filter(
                    CompanyID=CompanyID, LedgerID=LedgerID, Date__gte=FromDate, Date__lte=ToDate, VoucherType="SI",VoucherMasterID=instances.SalesMasterID)
                for i in ledger_ins:
                    TotalDebit += float(i.Debit)
                    TotalCredit += float(i.Credit)
                    print(i.pk,"************************************")
            ...
            CreditSales = float(TotalDebit) - float(TotalCredit)
            GrandTotal = instances.GrandTotal
            print(TotalDebit,TotalCredit , CreditSales,'float(ELSEEEEEEEEEE) - float(CreditSales)')
...
class SalesDetailsForReturnsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'unq_id', 'SalesDetailsID', 'BranchID', 'Action', 'SalesMasterID', 'ProductID', 'ProductName',
                  'Qty', 'FreeQty', 'UnitPrice', 'InclusivePrice', 'ReturnQty', 'BatchCode_list', 'ActualUnitPrice', 'HSNCode',
                  'RateWithTax', 'CostPerPrice', 'PriceListID', 'UnitName', 'DiscountPerc', 'DiscountAmount', 'AddlDiscPerc', 'AddlDiscAmt',
                  'GrossAmount', 'TaxableAmount', 'VATPerc', 'VATAmount', 'SGSTPerc', 'SGSTAmount', 'CGSTPerc','SerialNos','ProductCode','is_kfc',
                  'CGSTAmount', 'IGSTPerc', 'IGSTAmount', 'NetAmount', 'CreatedDate', 'CreatedUserID', 'Flavour', 'BatchCode','is_inclusive',
...
class StockSerializer(serializers.ModelSerializer):
    ...
    def get_Cost(self, instance):
        ...
        if StockPosting.objects.filter(ProductID=ProductID, CompanyID=CompanyID, QtyIn__gt=0, Date__lte=ToDate, VoucherType__in=VoucherTypes).exists():
            stock_ins = StockPosting.objects.filter(
                ProductID=ProductID, CompanyID=CompanyID, QtyIn__gt=0, Date__lte=ToDate, VoucherType__in=VoucherTypes)
            TotalQtyInRate = 0
            TotalQtyIn = 0
            TotalQtyInRate = stock_ins.aggregate(
    ...
    def get_Qty(self, instance):
        ...
        if WareHouseID > 0:
            ...
            if StockPosting.objects.filter(ProductID=ProductID, CompanyID=CompanyID, Date__lte=ToDate).exists():
                stock_ins = StockPosting.objects.filter(
...
class SupplierVsProductReportSerializer(serializers.ModelSerializer):
    ...
    def get_AvgSalesRate(self, instances):
        ...
        if SalesDetails.objects.filter(CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID).exists():
            salesDetails_ins = SalesDetails.objects.filter(
                CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID)
            MultiFactor = PriceList.objects.get(
                CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID, DefaultUnit=True).MultiFactor
            AvgSalesRate = 0
            for s in salesDetails_ins:
                oneAvg = (float(s.Qty) * float(MultiFactor) *
                          float(s.UnitPrice)) / (float(s.Qty) * float(MultiFactor))
                AvgSalesRate += oneAvg
        return AvgSalesRate
    ...
    def get_TotalPurchaseAmount(self, instances):
        ...
        if PurchaseDetails.objects.filter(CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID).exists():
            purchaseDetails_ins = PurchaseDetails.objects.filter(
                CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID)
            TotalPurchaseAmount = purchaseDetails_ins.aggregate(
                Sum('GrossAmount'))
    ...
    def get_TotalPurchaseReturn(self, instances):
        ...
        if PurchaseReturnDetails.objects.filter(CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID).exists():
            ...
            TotalPurchaseReturn = purchaseReturnDetails_ins.aggregate(
                Sum('Qty'))
    ...
    def get_LastPurchasePrice(self, instances):
        ...
        if PurchaseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
            purchase_master_ids = PurchaseMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).values_list('PurchaseMasterID', flat=True)
    ...
    def get_TotalSales(self, instances):
        ...
        if SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
            sales_master_ids = SalesMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).values_list('SalesMasterID', flat=True)
            if SalesDetails.objects.filter(CompanyID=CompanyID, ProductID=ProductID, SalesMasterID__in=sales_master_ids, BranchID=BranchID).exists():
                detail_ins = SalesDetails.objects.filter(
                    CompanyID=CompanyID, ProductID=ProductID, SalesMasterID__in=sales_master_ids, BranchID=BranchID)
    ...
    def get_TotalSalesAmount(self, instances):
        ...
        if SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
            ...
            if SalesDetails.objects.filter(CompanyID=CompanyID, ProductID=ProductID, SalesMasterID__in=sales_master_ids, BranchID=BranchID).exists():
                salesDetails_ins = SalesDetails.objects.filter(
                    CompanyID=CompanyID, ProductID=ProductID, SalesMasterID__in=sales_master_ids, BranchID=BranchID)
                TotalSalesAmount = salesDetails_ins.aggregate(
                    Sum('GrossAmount'))
                TotalSalesAmount = TotalSalesAmount['GrossAmount__sum']
        return TotalSalesAmount
        ...
        if PurchaseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
            ...
            if PurchaseDetails.objects.filter(CompanyID=CompanyID, ProductID=ProductID, PurchaseMasterID__in=purchase_master_ids, BranchID=BranchID).exists():
                detail_ins = PurchaseDetails.objects.filter(
                    CompanyID=CompanyID, ProductID=ProductID, PurchaseMasterID__in=purchase_master_ids, BranchID=BranchID)
    ...
    def get_TotalSalesReturn(self, instances):
        ...
        if SalesReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
            salesReturn_master_ids = SalesReturnMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).values_list('SalesReturnMasterID', flat=True)
            if SalesReturnDetails.objects.filter(CompanyID=CompanyID, ProductID=ProductID, SalesReturnMasterID__in=salesReturn_master_ids, BranchID=BranchID).exists():
                detail_ins = SalesReturnDetails.objects.filter(
                    CompanyID=CompanyID, ProductID=ProductID, SalesReturnMasterID__in=salesReturn_master_ids, BranchID=BranchID)
    ...
    def get_TotalSalesReturnAmount(self, instances):
        ...
        if SalesReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
            ...
            if SalesReturnDetails.objects.filter(CompanyID=CompanyID, ProductID=ProductID, SalesReturnMasterID__in=salesReturn_master_ids, BranchID=BranchID).exists():
                detail_ins = SalesReturnDetails.objects.filter(
                    CompanyID=CompanyID, ProductID=ProductID, SalesReturnMasterID__in=salesReturn_master_ids, BranchID=BranchID)
                TotalSalesReturnAmount = detail_ins.aggregate(
                    Sum('GrossAmount'))
                TotalSalesReturnAmount = TotalSalesReturnAmount['GrossAmount__sum']
        return TotalSalesReturnAmount
    ...
    def get_AvgSalesReturnRate(self, instances):
        ...
        if SalesReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
            ...
            if SalesReturnDetails.objects.filter(CompanyID=CompanyID, ProductID=ProductID, SalesReturnMasterID__in=purchase_master_ids, BranchID=BranchID).exists():
                detail_ins = SalesReturnDetails.objects.filter(
                    CompanyID=CompanyID, ProductID=ProductID, SalesReturnMasterID__in=purchase_master_ids, BranchID=BranchID)
                MultiFactor = PriceList.objects.get(
                    CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID, DefaultUnit=True).MultiFactor
                AvgSalesReturnRate = 0
                for s in detail_ins:
    ...
    def get_TotalPurchaseReturn(self, instances):
        ...
        if PurchaseReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
            purchaseReturn_master_ids = PurchaseReturnMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).values_list('PurchaseReturnMasterID', flat=True)
            if PurchaseReturnDetails.objects.filter(CompanyID=CompanyID, ProductID=ProductID, PurchaseReturnMasterID__in=purchaseReturn_master_ids, BranchID=BranchID).exists():
                detail_ins = PurchaseReturnDetails.objects.filter(
                    CompanyID=CompanyID, ProductID=ProductID, PurchaseReturnMasterID__in=purchaseReturn_master_ids, BranchID=BranchID)
    ...
    def get_TotalPurchaseReturnAmount(self, instances):
        ...
        if PurchaseReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
            ...
            if PurchaseReturnDetails.objects.filter(CompanyID=CompanyID, ProductID=ProductID, PurchaseReturnMasterID__in=purchaseReturn_master_ids, BranchID=BranchID).exists():
                detail_ins = PurchaseReturnDetails.objects.filter(
                    CompanyID=CompanyID, ProductID=ProductID, PurchaseReturnMasterID__in=purchaseReturn_master_ids, BranchID=BranchID)
                TotalPurchaseReturnAmount = detail_ins.aggregate(
                    Sum('GrossAmount'))
                TotalPurchaseReturnAmount = TotalPurchaseReturnAmount['GrossAmount__sum']
        return TotalPurchaseReturnAmount
...
class StockValueInventoryFlowSerializer(serializers.ModelSerializer):
    ...
    def get_Cost(self, instance):
        ...
        if StockPosting.objects.filter(ProductID=ProductID, CompanyID=CompanyID, QtyIn__gt=0, Date__gte=FromDate, Date__lte=ToDate, VoucherType="PI").exists():
            stock_ins = StockPosting.objects.filter(
                ProductID=ProductID, CompanyID=CompanyID, QtyIn__gt=0, Date__gte=FromDate, Date__lte=ToDate, VoucherType="PI")
...
class FilterOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'SalesOrderMasterID',
                  'VoucherNo', 'Date', 'is_checked')
...
class BillWiseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SalesMaster
        fields = ('id', 'Date', 'VoucherNo','TotalGrossAmt','VATAmount','NetAmount','TotalTax','NetCost','Profit','GrandTotal')
    ...
    def get_NetCost(self, instance):
        ...
        salesdetail_ins = SalesDetails.objects.filter(CompanyID=CompanyID, BranchID=BranchID, SalesMasterID=SalesMasterID)
        NetCost = 0
        if salesdetail_ins:
...
class GST_B2B_Serializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ( 'Customer_GST_No','CustomerName','VoucherNo','Date','GrandTotal','PlaceOfSupply','ApplicableTaxRate','ReverseCharge',
            'InvoiceType','E_Commerce_GSTIN','CESS_amount','SalesMasterID','Type','ShippingCharge','shipping_tax_amount','SalesTax','PlaceOfSupply_id')
    ...
    def get_PlaceOfSupply_id(self, instances):
        ...
        if not State_of_Supply:
            if Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=LedgerID).exists():
                State_of_Supply = Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=LedgerID).first().State
        if not State_of_Supply:
    ...
    def get_CustomerName(self, instances):
        ...
        # LedgerID = SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,SalesMasterID=SalesMasterID).first().LedgerID
        if Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=LedgerID).exists():
            CustomerName = Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=LedgerID).first().PartyName
    ...
    def get_Customer_GST_No(self, instances):
        ...
        # LedgerID = SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,SalesMasterID=SalesMasterID).first().LedgerID
        if Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=LedgerID).exists():
            Customer_GST_No = Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=LedgerID).first().GSTNumber
    ...
    def get_InvoiceType(self, instances):
        ...
        if instances.VATAmount > 0:
            if not VAT_Treatment:
                if Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=LedgerID).exists():
                    InvoiceType = Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=LedgerID).first().VAT_Treatment
            InvoiceType = get_vatTreatment(InvoiceType)
        else:
            if not GST_Treatment:
                if Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=LedgerID).exists():
                    InvoiceType = Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=LedgerID).first().GST_Treatment
...
class GST_CDNR_Serializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ( 'Customer_GST_No','CustomerName','VoucherNo','VoucherDate','GrandTotal','PlaceOfSupply','ApplicableTaxRate','ReverseCharge',
            'InvoiceType','E_Commerce_GSTIN','CESS_amount','SalesReturnMasterID','Type','RefferenceBillNo','RefferenceBillDate',
            'DocumentType','UR_Type')
