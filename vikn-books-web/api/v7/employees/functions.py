...
def get_auto_id(model,BranchID,CompanyID):
    ...
    latest_auto_id =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).aggregate(Max('EmployeeID'))

    if model.objects.filter(BranchID=BranchID,CompanyID=CompanyID).exists():
        max_value =  model.objects.filter(BranchID=BranchID,CompanyID=CompanyID).aggregate(Max('EmployeeID'))
...
def get_EmployeeCode(model,BranchID,CompanyID): 
    latest_EmployeeCode =  model.objects.filter(BranchID=BranchID,CompanyID=CompanyID).order_by("-id")[:1]
