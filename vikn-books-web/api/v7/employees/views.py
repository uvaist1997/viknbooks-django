...
from api.v7.employees.serializers import EmployeeSerializer, EmployeeRestSerializer, CountrySerializer
from api.v7.brands.serializers import ListSerializer
from api.v7.employees.functions import generate_serializer_errors, get_auto_id
from rest_framework import status
from api.v7.employees.functions import get_auto_id, get_EmployeeCode
...
def sales_man_list(request):
    ...
    if serialized1.is_valid():
        BranchID = serialized1.data['BranchID']
        sales_designation = ["Sales Man","Salesman", "sales man", "salesman","Sales man"]
        desig_ids = []
        if Designation.objects.filter(BranchID=BranchID, CompanyID=CompanyID,DesignationName__in=sales_designation).exists():
            designation_ins = Designation.objects.filter(BranchID=BranchID, CompanyID=CompanyID,DesignationName__in=sales_designation)
