...
from api.v7.priceCategories.serializers import PriceCategorySerializer, PriceCategoryRestSerializer
from api.v7.brands.serializers import ListSerializer
from api.v7.priceCategories.functions import generate_serializer_errors
from rest_framework import status
from api.v7.priceCategories.functions import get_auto_id
import datetime
    ...
    if serialized.is_valid():
        ...
        if category_ok:
            ...
            if is_nameExist:
                ...
                #request , company, log_type, user, source, action, message, description
                activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'PriceCategory',
                             'Edit', 'PriceCategory Update successfully.', 'PriceCategory UpdatedDate successfully.')
                response_data = {
                    "StatusCode": 6000,
                    "data": serialized.data
                }
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'PriceCategory', 'Edit',
                     'PriceCategory field not valid.', generate_serializer_errors(serialized._errors))
        response_data = {
            "StatusCode": 6001,
            "message": generate_serializer_errors(serialized._errors)
        }
