from api.v7.brands.tasks import create_random_user_accounts
...
from api.v7.brands.serializers import BrandSerializer, BrandRestSerializer, ListSerializer
from api.v7.brands.functions import generate_serializer_errors
...
def torunqryTest(request):
    try:
        with transaction.atomic():
            ...
            CompanyID = "54ba4163-584f-4d3f-bb02-120b9800ba97"
            BranchID = 1
            data = request.data
            CreatedUserID = data['CreatedUserID']
            CompanyID = CompanySettings.objects.get(id=CompanyID)
            if PurchaseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
                print("successsssaaaaaaaaaaaaaaaaaaannuu")
