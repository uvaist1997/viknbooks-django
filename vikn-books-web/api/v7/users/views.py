...
from django.contrib.auth.models import User, Group
from api.v7.users.functions import get_auto_id
from brands import models
from api.v7.general.functions import get_current_role
...
from api.v7.users.functions import send_email, get_master_auto_id
...
from api.v7.users.serializers import CustomTokenSerializer, SoftwareVersionSerializer
        ...
        if response.status_code == 200 and result["success"] == 6000:
            ...
            if Customer.objects.filter(user__username=username).exists():
                customer_ins = Customer.objects.filter(
                    user__username=username).first()
                customer_ins.LastLoginToken = LastToken
                LastLoginCompanyID = customer_ins.LastLoginCompanyID
                customer_ins.save()
...
def get_defaults(request):
    ...
    if UserTable.objects.filter(CompanyID__pk=CompanyID, customer__user=userId).exists():
        ...
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>-------------------------}")
        print(user_table_instance.UserType)
