...
from api.v7.stockPostings.serializers import StockPostingRestSerializer, StockValueAllSerializer, StockValueSingleSerializer
from api.v7.ledgerPosting.functions import generate_serializer_errors
from rest_framework import status
from api.v7.sales.serializers import ListSerializerforReport, ListSerializerforStockReport, ListSerializerforStockValueReport, ListSerializerforStockValueReportSingle
from api.v7.ledgerPosting.functions import balanceSheet_excel_data, export_to_excel_balanceSheet, profitAndLoss_excel_data, export_to_excel_profitAndLoss, get_auto_id, convertOrderdDict, get_VoucherName, ledgerReport_excel_data, export_to_excel_ledgerReport, trialBalance_excel_data, export_to_excel_trialBalance
import datetime
...
def profitAndLoss_web(request):
    ...
    if serialized1.is_valid():
        ...
        print(CompanyID, CreatedUserID, PriceRounding, BranchID, ToDate,
              FromDate, ManualOpeningStock, ManualClosingStock)
...
def balanceSheet_web(request):
    ...
    if serialized1.is_valid():
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate).exists():
            ...
            TotalLiabilitis = 0
            test_arr = []
            print(CompanyID, BranchID, FromDate, ToDate, 'test_arr')
...
def trialBalance_excel(request):
    ...
    data = trialBalance_excel_data(
        CompanyID, CreatedUserID, PriceRounding, BranchID, ToDate, request)
