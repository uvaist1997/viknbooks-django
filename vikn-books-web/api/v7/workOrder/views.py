...
from api.v7.purchases.serializers import PurchaseMasterSerializer, PurchaseMasterRestSerializer, PurchaseDetailsSerializer,\
    PurchaseDetailsRestSerializer, PurchaseMasterReportSerializer, PurchaseMasterForReturnSerializer
from api.v7.workOrder import serializers
from api.v7.brands.serializers import ListSerializer
from api.v7.purchases.functions import generate_serializer_errors
from api.v7.products.functions import get_auto_AutoBatchCode
from rest_framework import status
from api.v7.sales.serializers import ListSerializerforReport
from api.v7.workOrder.functions import get_auto_id, get_auto_idMaster, get_auto_StockRateID, get_auto_StockTransID
...
from api.v7.purchaseReturns.serializers import PurchaseReturnMasterRestSerializer
import re,sys, os
        ...
        with transaction.atomic():
            ...
            if not WorkOrderMaster.objects.filter(BranchID=BranchID, CompanyID=CompanyID, VoucherNo__iexact=VoucherNo).exclude(VoucherNo=instanceVoucherNo):
                ...
                if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=instance.WorkOrderMasterID,BranchID=BranchID, VoucherType="WO").exists():
                    stk_ins = StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=instance.WorkOrderMasterID,BranchID=BranchID, VoucherType="WO")
                    total_count_stock = stk_ins.count()
                    null_count = stk_ins.filter(VoucherDetailID__isnull=True).count()

                    if total_count_stock == null_count:
                        stk_ins.delete()
                    else:
                        instance_qty_sum = float(instance.ProductQty)
                        ...
                        stk_ins_master.QtyIn = float(stk_ins_master.QtyIn) - float(instance_Qty)
                ...
                if WorkOrderDetails.objects.filter(CompanyID=CompanyID, BranchID=BranchID, WorkOrderMasterID=instance.WorkOrderMasterID).exists():
                    ...
                    for i in Work_ins:
                        ...
                        if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=instance.WorkOrderMasterID,VoucherDetailID=i.WorkOrderDetailsID,ProductID=i.ProductID,BranchID=BranchID, VoucherType="WO").exists():
                            stock_inst = StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=instance.WorkOrderMasterID,VoucherDetailID=i.WorkOrderDetailsID,ProductID=i.ProductID,BranchID=BranchID, VoucherType="WO").first()
                            print("instance_Qty//////////")
                            print(instance_Qty)
                ...
                if StockPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherType="WO",VoucherMasterID=instance.WorkOrderMasterID,QtyIn__gt=0).exists():
                    stock_master_ins = StockPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherType="WO",VoucherMasterID=instance.WorkOrderMasterID,QtyIn__gt=0).first()
                            ...
                            if not deleted_pk == '' or not deleted_pk == 0:
                                if WorkOrderDetails.objects.filter(CompanyID=CompanyID, pk=deleted_pk).exists():
                                    ...
                                    if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=WorkOrderMasterID_Deleted,VoucherDetailID=WorkOrderDetailsID_Deleted,ProductID=ProductID_Deleted,BranchID=BranchID, VoucherType="WO").exists():
                                        stock_instances_delete = StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=WorkOrderMasterID_Deleted,VoucherDetailID=WorkOrderDetailsID_Deleted,ProductID=ProductID_Deleted,BranchID=BranchID, VoucherType="WO")
