...
from api.v7.accountLedgers.serializers import AccountLedgerSerializer, AccountLedgerRestSerializer, LedgerReportSerializer, AccountLedgerListSerializer
from api.v7.brands.serializers import ListSerializer
from api.v7.accountLedgers.serializers import ListSerializerforPayment
from api.v7.accountLedgers.functions import generate_serializer_errors
from rest_framework import status
from api.v7.banks.functions import get_auto_Bankid
from web.functions import get_auto_EmployeeID
from api.v7.accountLedgers.functions import get_auto_id, get_LedgerCode, get_auto_LedgerPostid, get_auto_idfor_party,get_shippingAddress
    ...
    if serialized1.is_valid():
        ...
        if AccountLedger.objects.filter(BranchID=BranchID, CompanyID=CompanyID, LedgerID=LedgerID).exists():
            ...
            if Attention or Address1 or City or CustomerStateName or PostalCode:
                BillingAddress = str(Attention) + str(",") + str(Address1)  + str(",") + str(City) + str(",") + str(CustomerStateName) + str(',') + str(PostalCode)
