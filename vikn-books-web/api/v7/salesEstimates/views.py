from brands.models import VoucherNoTable,SalesEstimateMaster, SalesEstimateMaster_Log, SalesEstimateDetails, SalesEstimateDetails_Log, GeneralSettings
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes,authentication_classes
from rest_framework.response import Response
from rest_framework_simplejwt.authentication import JWTTokenUserAuthentication
from api.v7.salesEstimates.serializers import SalesEstimateMasterSerializer, SalesEstimateMasterRestSerializer, SalesEstimateDetailsSerializer, SalesEstimateDetailsRestSerializer,SalesEstimateMaster1RestSerializer
from api.v7.brands.serializers import ListSerializer
from api.v7.salesEstimates.functions import generate_serializer_errors
from rest_framework import status
from api.v7.salesEstimates.functions import get_auto_id, get_auto_idMaster
import datetime
...
def create_salesEstimate(request):
    try:
        with transaction.atomic():
            ...
            insts = SalesEstimateMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
            if insts:
                for i in insts:
        ...
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'Sales Estimate',
...
def edit_salesEstimate(request, pk):
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        err_descrb = str(exc_type) + str(fname) + str(exc_tb.tb_lineno)
        print("errrrrrrrrrrrrrrrrrorrrrrrrrrrrrrrrrrrrrrrr")
        print(str(e))
        print(err_descrb)
        response_data = {
            "StatusCode": 6001,
