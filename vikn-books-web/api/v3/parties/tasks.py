...
from api.v3.parties.utils import DataframeUtil
from api.v3.parties.functions import get_auto_id, get_auto_idLedger, get_PartyCode
...
def import_party_task(self, input_excel, CompanyID, CreatedUserID, BranchID):
    ...
    for index, row in dataframe.iterrows():
        ...
        if row.get('PartyCode'):
            ...
            PartyCode = get_PartyCode(models.Parties, BranchID, CompanyID, PartyType.lower())
        ...
        if not models.Parties.objects.filter(BranchID=BranchID, CompanyID=CompanyID, PartyName__iexact=PartyName).exists():
            ...
            if Country:
                if models.Country.objects.filter(Country_Name__iexact=Country).exists():
                    Country = models.Country.objects.get(Country_Name__iexact=Country).id
                else:
                    Country_create = models.Country.objects.create(Country_Name=Country)
                    Country = Country_create.id
            if State:
                if models.State.objects.filter(Country__pk=Country,State_Name__iexact=State).exists():
                    State = models.State.objects.get(Country__pk=Country,State_Name__iexact=State).id
                else:
                    State_create = models.State.objects.create(Country__pk=Country,State_Name__iexact=State)
                    State = State_create.id

            if RouteName:
                if models.Route.objects.filter(CompanyID=CompanyID, RouteName__iexact=RouteName).exists():
                    RouteID = models.Route.objects.get(CompanyID=CompanyID, RouteName__iexact=RouteName).RouteID
                else:
