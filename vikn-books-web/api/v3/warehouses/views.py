from brands.models import Warehouse, Warehouse_Log, SalesMaster, PurchaseMaster, StockPosting, UserTable, GeneralSettings
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from api.v3.warehouses.serializers import WarehouseSerializer, WarehouseRestSerializer
    ...
    if serialized1.is_valid():
        ...
        if Warehouse.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            ...
            if GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ExpiryDays").exists():
                general_ins = GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ExpiryDays").first()
            ...
            if GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ShowTotalTax").exists():
                general_ins = GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="ShowTotalTax").first()
