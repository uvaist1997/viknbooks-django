...
from api.v3.priceLists.serializers import PriceListSerializer, PriceListRestSerializer, ListSerializer
from api.v3.priceLists.functions import generate_serializer_errors
from rest_framework import status
from api.v3.priceLists.functions import get_auto_id
from api.v3.products.functions import get_auto_AutoBarcode
import datetime
        ...
        if GeneralSettings.objects.filter(CompanyID=CompanyID, BranchID=BranchID, SettingsType="EnableProductBatchWise").exists():
            ...
            if check_EnableProductBatchWise == "True" or check_EnableProductBatchWise == True:
                if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID).exists():
                    ...
                    for i in batch_ins:
                        print(i.BatchCode)
                        stock = float(i.StockIn) - float(i.StockOut)
...
def single_priceList(request):
    ...
    if PriceList.objects.filter(CompanyID=CompanyID, PriceListID=PriceListID, BranchID=BranchID).exists():
        ...
        Batch_Stock = 0
        if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID, PriceListID=PriceListID, PurchasePrice=PurchasePrice).exists():
            BatchIns = Batch.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID, PriceListID=PriceListID, PurchasePrice=PurchasePrice).first()
            BatchCode = BatchIns.BatchCode
            BatchQtyIn = BatchIns.StockIn
            BatchQtyOut = BatchIns.StockOut
            Batch_Stock = float(BatchQtyIn) - float(BatchQtyOut)
        else:
