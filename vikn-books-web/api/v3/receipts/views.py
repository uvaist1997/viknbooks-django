...
from api.v3.receipts.serializers import ReceiptMasterSerializer, ReceiptMasterRestSerializer, ReceiptDetailsSerializer, ReceiptDetailsRestSerializer
from api.v3.payments.serializers import ListSerializer
from api.v3.receipts.functions import generate_serializer_errors
from rest_framework import status
from api.v3.receipts.functions import get_auto_id, get_auto_idMaster
...
def edit_receipt(request, pk):
    ...
    if LedgerPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=ReceiptMasterID, BranchID=BranchID).exists():
        ...
        ledgerPostInstances = LedgerPosting.objects.filter(
            CompanyID=CompanyID, VoucherMasterID=ReceiptMasterID, BranchID=BranchID)
    ...
    if ReceiptDetails.objects.filter(CompanyID=CompanyID, ReceiptMasterID=ReceiptMasterID, BranchID=BranchID).exists():
        ...
        receiptDetailInstances = ReceiptDetails.objects.filter(
            CompanyID=CompanyID, ReceiptMasterID=ReceiptMasterID, BranchID=BranchID)
