...
from api.v3.purchaseOrders.serializers import PurchaseOrderMasterSerializer, PurchaseOrderMasterRestSerializer, PurchaseOrderDetailsSerializer, PurchaseOrderDetailsRestSerializer
from api.v3.brands.serializers import ListSerializer
from api.v3.purchaseOrders.functions import generate_serializer_errors
from rest_framework import status
from api.v3.purchaseOrders.functions import get_auto_id, get_auto_idMaster
import datetime
...
def create_purchaseOrder(request):
    ...
    if not is_voucherExist:
        ...
        PurchaseOrderMasterID = get_auto_idMaster(
            PurchaseOrderMaster, BranchID, DataBase)
        ...
        for purchaseOrderDetail in purchaseOrderDetails:
            ...
            PurchaseOrderDetailsID = get_auto_id(
                PurchaseOrderDetails, BranchID, DataBase)
...
def edit_purchaseOrder(request, pk):
    ...
    if PurchaseOrderDetails.objects.filter(PurchaseOrderMasterID=PurchaseOrderMasterID, BranchID=BranchID).exists():
        ...
        purchaseOrdrDetailInstances = PurchaseOrderDetails.objects.filter(
    ...
    #request , company, log_type, user, source, action, message, description
    activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'PurchaseOrder',
                 'Edit', 'Purchase Order updated Successfully.', 'Purchase Order updated Successfully')
