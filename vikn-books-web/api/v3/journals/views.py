...
from api.v3.journals.serializers import JournalMasterSerializer, JournalMasterRestSerializer, JournalDetailsSerializer, JournalDetailsRestSerializer
from api.v3.brands.serializers import ListSerializer
from api.v3.journals.functions import generate_serializer_errors
from rest_framework import status
from api.v3.journals.functions import get_auto_id, get_auto_idMaster
from api.v3.sales.functions import get_auto_VoucherNo
import datetime
...
def edit_journal(request, pk):
    ...
    if LedgerPosting.objects.filter(VoucherMasterID=JournalMasterID, BranchID=BranchID, CompanyID=CompanyID).exists():
        ledgerPostInstances = LedgerPosting.objects.filter(
            VoucherMasterID=JournalMasterID, BranchID=BranchID, CompanyID=CompanyID)
