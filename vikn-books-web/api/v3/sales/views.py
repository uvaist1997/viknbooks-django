from brands.models import SalesMaster, SalesMaster_Log, SalesDetails, SalesDetails_Log, StockPosting, LedgerPosting,\
    ...
    UsedStockMaster, GeneralSettings, CompanySettings,WorkOrderMaster,Batch
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from api.v3.sales.serializers import SalesMasterSerializer, SalesMasterRestSerializer, SalesDetailsSerializer, SalesDetailsRestSerializer,BatchSerializer,\
...
from api.v3.purchases.serializers import PurchaseMasterRestSerializer, PurchasePrintSerializer
from api.v3.salesReturns.serializers import SalesReturnMasterRestSerializer, SalesReturnMasterReportSerializer
from api.v3.purchaseReturns.serializers import PurchaseReturnMasterRestSerializer, PurchaseReturnPrintSerializer
from api.v3.sales.functions import generate_serializer_errors
from rest_framework import status
from api.v3.sales.functions import get_auto_id, get_auto_idMaster, get_auto_stockPostid
...
from api.v3.stockPostings.serializers import StockPostingSerializer, StockPostingRestSerializer, ExcessStockMasterSerializer, ShortageStockMasterSerializer, DamageStockMasterSerializer, UsedStockMasterSerializer, UsedStockDetailSerializer
from api.v3.companySettings.serializers import CompanySettingsRestSerializer
        ...
        if SalesDetails:
            for salesdetail in salesdetails:
                ...
                if ProductID:
                    ...
                    product_purchasePrice = PriceList.objects.get(CompanyID=CompanyID, BranchID=BranchID,PriceListID=PriceListID).PurchasePrice
                ...
                if check_EnableProductBatchWise == "True" or check_EnableProductBatchWise == True:
                    if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode).exists():
                        if check_AllowUpdateBatchPriceInSales == "True" or check_AllowUpdateBatchPriceInSales == True:
                            ...
                            NewStock = float(StockOut) + float(Qty)
                            batch_ins.StockOut = NewStock
                            batch_ins.SalesPrice = UnitPrice
                            batch_ins.save()
                        else:
                            batch_ins = Batch.objects.get(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode)
                            StockOut = batch_ins.StockOut
        ...
        if Product.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            ...
            if not ProductCode.isdecimal():
                ...
                if temp.match(ProductCode):
                    ...
                    ProductCode = str(float(ProductCode) + 1).zfill(len(ProductCode))
            else:
...
def generate_printReport(request):
    from api.v3.sales.serializers import SalesPrintSerializer
    from api.v3.salesReturns.serializers import SalesReturnMasterPrintSerializer
    data = request.data
    CompanyID = data['CompanyID']
    CompanyID = get_company(CompanyID)
    CreatedUserID = data['CreatedUserID']
...
def batchwise_report(request):
    ...
    print(ProductID,"WWWWWWWWWWWWWWW")
