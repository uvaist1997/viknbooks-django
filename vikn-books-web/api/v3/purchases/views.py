from brands.models import PurchaseMaster, PurchaseMaster_Log, PurchaseDetails, PurchaseDetails_Log, StockPosting, LedgerPosting, StockPosting_Log,\
    LedgerPosting_Log, PurchaseDetailsDummy, StockRate, PriceList, StockTrans, ProductGroup, Brand, Unit, Warehouse, PurchaseReturnMaster, OpeningStockMaster, GeneralSettings, Product, Batch
...
from api.v3.purchases.functions import get_auto_id, get_auto_idMaster, get_auto_StockRateID, get_auto_StockTransID
                    ...
                    if check_EnableProductBatchWise == "True" or check_EnableProductBatchWise == True:
                        if check_BatchCriteria == "PurchasePrice":
                            if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID,ProductID=ProductID, PurchasePrice=Batch_purchasePrice).exists():
                                batch_ins = Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID,ProductID=ProductID, PurchasePrice=Batch_purchasePrice).last()
                            ...
                            if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID, PurchasePrice=Batch_purchasePrice,ExpiryDate=ExpiryDate).exists():
                                batch_ins = Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID, PurchasePrice=Batch_purchasePrice,ExpiryDate=ExpiryDate).last()
                                StockIn = batch_ins.StockIn
                                BatchCode = batch_ins.BatchCode
                                NewStock = float(StockIn) + float(Qty)
                                batch_ins.StockIn = NewStock
...
def edit_purchase(request, pk):
    ...
    if PurchaseDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PurchaseMasterID=PurchaseMasterID).exists():
        purch_ins = PurchaseDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PurchaseMasterID=PurchaseMasterID)
        for i in purch_ins:
        ...
        if ProductID:
            ...
            if GeneralSettings.objects.filter(CompanyID=CompanyID, BranchID=BranchID, SettingsType="EnableProductBatchWise").exists():
                ...
                if GeneralSettings.objects.filter(CompanyID=CompanyID, SettingsType="BatchCriteria").exists():
                    check_BatchCriteria = GeneralSettings.objects.get(CompanyID=CompanyID, SettingsType="BatchCriteria").SettingsValue
                ...
                if check_EnableProductBatchWise == "True" or check_EnableProductBatchWise == True:
                    if check_BatchCriteria == "PurchasePrice":
                        if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode, PurchasePrice=Batch_purchasePrice).exists():
                            batch_ins = Batch.objects.get(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode, PurchasePrice=Batch_purchasePrice)
                            StockIn = batch_ins.StockIn
                            print(StockIn)
                            BatchCode = batch_ins.BatchCode
                            NewStock = float(StockIn) + float(Qty_detail)
                            if ExpiryDate:
                                batch_ins.ExpiryDate = ExpiryDate
                            if ManufactureDate:
                                batch_ins.ManufactureDate = ManufactureDate
                        ...
                        if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode, SalesPrice=SalesPrice).exists():
                            batch_ins = Batch.objects.get(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode, SalesPrice=SalesPrice)
                            StockIn = batch_ins.StockIn
                            BatchCode = batch_ins.BatchCode
                            NewStock = float(StockIn) + float(Qty_detail)
                            batch_ins.StockIn = NewStock
