...
from api.v3.accountLedgers.serializers import AccountLedgerSerializer, AccountLedgerRestSerializer, LedgerReportSerializer, AccountLedgerListSerializer
from api.v3.brands.serializers import ListSerializer
from api.v3.accountLedgers.serializers import ListSerializerforPayment
from api.v3.accountLedgers.functions import generate_serializer_errors
from rest_framework import status
from api.v3.banks.functions import get_auto_Bankid
from web.functions import get_auto_EmployeeID
from api.v3.accountLedgers.functions import get_auto_id, get_LedgerCode, get_auto_LedgerPostid, get_auto_idfor_party
        ...
        if not employees_exist and not parties_exist and not salesOrderMaster_exist and not purchaseOrderMaster_exist and not LPInstances:
            ...
            if LedgerPosting.objects.filter(LedgerID=LedgerID, BranchID=BranchID, VoucherType="LOB", CompanyID=CompanyID).exists():
                ledgerPostInstances = LedgerPosting.objects.filter(
                    LedgerID=LedgerID, BranchID=BranchID, VoucherType="LOB", CompanyID=CompanyID)
        ...
        if AccountLedger.objects.filter(BranchID=BranchID, CompanyID=CompanyID).exists():
            if UserTable.objects.filter(CompanyID=CompanyID, customer__user__pk=CreatedUserID, DefaultAccountForUser=True).exists():
                ...
                account_groupunder_instances = AccountLedger.objects.filter(
                    BranchID=BranchID, CompanyID=CompanyID, AccountGroupUnder__in=[10, 29])
                ...
                instances = AccountLedger.objects.filter(
                    BranchID=BranchID, CompanyID=CompanyID, AccountGroupUnder__in=[8, 9, 10, 29])
