...
from api.v3.purchaseReturns.serializers import PurchaseReturnMasterSerializer, PurchaseReturnMasterRestSerializer, PurchaseReturnDetailsSerializer, PurchaseReturnDetailsRestSerializer
from api.v3.brands.serializers import ListSerializer
from api.v3.purchaseReturns.functions import generate_serializer_errors
from rest_framework import status
from api.v3.sales.serializers import ListSerializerforReport
from api.v3.purchaseReturns.functions import get_auto_id, get_auto_idMaster
        ...
        for purchaseReturnDetail in purchaseReturnDetails:
            ...
            if StockRate.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID, WareHouseID=WarehouseID, Qty__gt=0).exists():
                stockRate_instances = StockRate.objects.filter(
                    CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID, WareHouseID=WarehouseID, Qty__gt=0)
            ...
            if StockRate.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID, Cost=Cost, WareHouseID=WarehouseID, Qty__lte=0).exists():
                stockRate_instance = StockRate.objects.filter(
                    CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID, Cost=Cost, WareHouseID=WarehouseID, Qty__lte=0).first()
                if float(changQty) > 0:
                    stockRate_instance.Qty = float(
                        stockRate_instance.Qty) - float(changQty)
                    stockRate_instance.save()
    ...
    for purchaseReturnDetail in purchaseReturnDetails:
        ...
        if detailID == 1:
            ...
            if StockRate.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Cost=Cost, ProductID=ProductID, WareHouseID=WarehouseID, PriceListID=PriceListID).exists():
                ...
                if StockTrans.objects.filter(StockRateID=StockRateID, DetailID=PurchaseReturnDetailsID, MasterID=PurchaseReturnMasterID, VoucherType=VoucherType, BranchID=BranchID).exists():
                    stockTra_in = StockTrans.objects.filter(StockRateID=StockRateID, DetailID=PurchaseReturnDetailsID,
                                                            MasterID=PurchaseReturnMasterID, VoucherType=VoucherType, BranchID=BranchID).first()
