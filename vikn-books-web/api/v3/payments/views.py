...
from api.v3.payments.serializers import PaymentMasterSerializer, PaymentMasterRestSerializer, PaymentDetailsSerializer, PaymentDetailsRestSerializer
from api.v3.payments.serializers import ListSerializer
from api.v3.payments.functions import generate_serializer_errors
from rest_framework import status
from api.v3.payments.functions import get_auto_id, get_auto_idMaster, get_auto_VoucherNo
...
def edit_payment(request, pk):
    ...
    if LedgerPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=PaymentMasterID, BranchID=BranchID, VoucherType="CP").exists():
        ledgerPostInstances = LedgerPosting.objects.filter(
            CompanyID=CompanyID, VoucherMasterID=PaymentMasterID, BranchID=BranchID, VoucherType="CP")
    ...
    if PaymentDetails.objects.filter(CompanyID=CompanyID, PaymentMasterID=PaymentMasterID, BranchID=BranchID).exists():
        paymentDetailInstances = PaymentDetails.objects.filter(
            CompanyID=CompanyID, PaymentMasterID=PaymentMasterID, BranchID=BranchID)
