...
from api.v5.masterTypes.serializers import MasterTypeSerializer, MasterTypeRestSerializer
from api.v5.brands.serializers import ListSerializer
from api.v5.masterTypes.functions import generate_serializer_errors
from rest_framework import status
from api.v5.masterTypes.functions import get_auto_id
...
def masterTypes1(request):
    ...
    if serialized1.is_valid():
        ...
        if MasterType.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            ...
            instances = MasterType.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID).exclude(MasterTypeID__in=[3,6,7,8])
