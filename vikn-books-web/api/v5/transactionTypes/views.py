...
from api.v5.transactionTypes.functions import generate_serializer_errors
from rest_framework import status
from api.v5.transactionTypes.functions import get_auto_id
import datetime
...
def transactionTypesByMasterName(request):
    ...
    if serialized1.is_valid():
        ...
        if MasterType.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Name=MasterName).exists():
            ...
            if MasterTypeID == 2:
                instances = TransactionTypes.objects.filter(
                    CompanyID=CompanyID, BranchID=BranchID, MasterTypeID=MasterTypeID).exclude(Name__in=["Cash","Bounced"])
            else:
                instances = TransactionTypes.objects.filter(
                    CompanyID=CompanyID, BranchID=BranchID, MasterTypeID=MasterTypeID).exclude(TransactionTypesID=14)
