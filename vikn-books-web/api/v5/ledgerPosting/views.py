...
def profitAndLoss_web(request):
    ...
    if serialized1.is_valid():
        print("PRINTTTTTTTTTTTTTTTTUVAISTTTTTTTTTT")
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate).exists():
            ...
            if Product.objects.filter(CompanyID=CompanyID, BranchID=BranchID, InventoryType="StockItem").exists():
                ...
                if StockPosting.objects.filter(ProductID__in=product_ids, BranchID=BranchID, Date__gte=FromDate, CompanyID=CompanyID, VoucherType__in="OS").exists():
                    stock_instances_new_os = StockPosting.objects.filter(
                        ProductID__in=product_ids, BranchID=BranchID, Date__gte=FromDate, CompanyID=CompanyID, VoucherType__in="OS", QtyIn__gt=0)
                    TotalRateIn_new_os = stock_instances_new_os.aggregate(
                        total_sum=Sum(F('QtyIn') * F('Rate')))
                    sum_QtyIn_os = stock_instances_new_os.aggregate(
                        Sum('QtyIn'))
            ...
            if Product.objects.filter(CompanyID=CompanyID, BranchID=BranchID, InventoryType="StockItem").exists():
                ...
                if StockPosting.objects.filter(ProductID__in=product_ids, BranchID=BranchID, Date__lte=ToDate, CompanyID=CompanyID, VoucherType__in=voucherTypes).exists():
                    ...
                    TotalRateIn_new = stock_instances_new.aggregate(
                        total_sum=Sum(F('QtyIn') * F('Rate')))
                    sum_QtyIn = stock_instances_new.aggregate(Sum('QtyIn'))
                    sum_QtyIn = sum_QtyIn['QtyIn__sum']
                    TotalQtyIn_new = StockPosting.objects.filter(
                        ProductID__in=product_ids, BranchID=BranchID, Date__lte=ToDate, CompanyID=CompanyID).aggregate(Sum('QtyIn'))
                    TotalQtyOut_new = StockPosting.objects.filter(
                        ProductID__in=product_ids, BranchID=BranchID, Date__lte=ToDate, CompanyID=CompanyID).aggregate(Sum('QtyOut'))
                    TotalQtyIn_new = TotalQtyIn_new['QtyIn__sum']
                    TotalQtyOut_new = TotalQtyOut_new['QtyOut__sum']
                    stock_new = float(TotalQtyIn_new) - \
                        float(TotalQtyOut_new)
                    
                    AvgRate_new = 0
                    if float(sum_QtyIn) > 0:
                        AvgRate_new = float(
                            TotalRateIn_new['total_sum']) / float(sum_QtyIn)
                    TotalAvgValueClosing += (float(stock_new)
                                             * float(AvgRate_new))
                    # new end
