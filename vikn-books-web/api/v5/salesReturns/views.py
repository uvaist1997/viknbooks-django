...
from api.v5.salesReturns.serializers import SalesReturnMasterSerializer, SalesReturnMasterRestSerializer,\
...
from api.v5.salesReturns.functions import generate_serializer_errors, sales_return_point
from rest_framework import status
from api.v5.salesReturns.functions import get_auto_id, get_auto_idMaster
from api.v5.sales.functions import get_auto_stockPostid, get_actual_point
                ...
                if is_LoyaltyCustomer:
                    if LoyaltyProgram.objects.filter(BranchID=BranchID, CompanyID=CompanyID, FromDate__lte=today_date, ToDate__gte=today_date).exists():
                        ...
                        l_instances = LoyaltyPoint.objects.filter(
                            LoyaltyCustomerID__id=loyalty_instance.pk, BranchID=BranchID, CompanyID=CompanyID)
                        for i in l_instances:
                            CurrentPoint += float(i.Point)
                        if float(CurrentPoint) >= float(actual_point):
                            return_point = float(
                                CurrentPoint) - float(actual_point)
                            print(
                                return_point, 'IFFFFFFFFFFreturn_pointreturn_pointreturn_point')
       ...
       # =========Loyalty Point ==========
        if LoyaltyPoint.objects.filter(LoyaltyCustomerID=LoyaltyCustomerID, VoucherMasterID=SalesReturnMasterID, VoucherType="SR", BranchID=BranchID, CompanyID=CompanyID).exists():
            instances = LoyaltyPoint.objects.filter(
                LoyaltyCustomerID=LoyaltyCustomerID, VoucherMasterID=SalesReturnMasterID, VoucherType="SR", BranchID=BranchID, CompanyID=CompanyID)
            for i in instances:
