...
from api.v2.damageStock.serializers import DamageStockMasterRestSerializer, DamageStockDetailsRestSerializer
from api.v2.brands.serializers import ListSerializer
from api.v2.damageStockDetails.functions import generate_serializer_errors
from rest_framework import status
from api.v2.damageStock.functions import get_auto_id, get_auto_idMaster
# from api.v2.damageStockDetails.functions import get_auto_idDetails
...
def create_damageStock(request):
    ...
    DamageStockMasterID = get_auto_idMaster(DamageStockMaster,BranchID, CompanyID)
    ...
    for damageStockDetail in damageStockDetails:
        ...
        DamageStockDetailsID = get_auto_id(DamageStockDetails,BranchID, CompanyID)
    ...
    dummies = DamageStockDetailsDummy.objects.filter(BranchID=BranchID,CompanyID=CompanyID)
...
def edit_damageStock(request,pk):
    ...
    for damageStockDetail in damageStockDetails:
        ...
        damageStockDetails_instance = DamageStockDetails.objects.get(pk=pk,CompanyID=CompanyID)
...
def damageStockMaster(request,pk):
    ...
    if DamageStockMaster.objects.filter(pk=pk,CompanyID=CompanyID).exists():
        ...
        serialized = DamageStockMasterRestSerializer(instances,context = {"CompanyID": CompanyID })
        response_data = {
            "StatusCode" : 6000,
...
def list_damageStockMaster(request):
    ...
    if serialized1.is_valid():
        ...
        if DamageStockMaster.objects.filter(BranchID=BranchID,CompanyID=CompanyID).exists():
            ...
            instances = DamageStockMaster.objects.filter(BranchID=BranchID,CompanyID=CompanyID)

            serialized = DamageStockMasterRestSerializer(instances,many=True,context = {"CompanyID": CompanyID })
...
def delete_damageStockMaster(request,pk):
    ...
    if DamageStockMaster.objects.filter(pk=pk,CompanyID=CompanyID).exists():
        ...
        detail_instances = DamageStockDetails.objects.filter(DamageStockMasterID=DamageStockMasterID,BranchID=BranchID,CompanyID=CompanyID)
