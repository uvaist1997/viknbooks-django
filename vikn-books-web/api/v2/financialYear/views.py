...
from api.v2.financialYear.serializers import FinancialYearSerializer
...
def create_financial_year(request):
    ...
    if not administrations_models.FinancialYear.objects.filter(IsClosed=False,CompanyID=CompanyID).exists():
        if serialized.is_valid():
            ...
            test = administrations_models.FinancialYear.objects.filter(IsClosed=True,CompanyID=CompanyID)
            date_arr = []
            for i in test:
            ...
            if str(FromDate) in date_arr or str(ToDate) in date_arr:
                ...
                if days[0] == '364' or days[0] == '365':
                    auto_id = get_auto_id_financial_year(administrations_models.FinancialYear, CompanyID)
...
def edit_financial_year(request,pk):
    ...
    today = datetime.datetime.now()
    serialized = FinancialYearSerializer(data=request.data)
    instance = administrations_models.FinancialYear.objects.get(pk=pk,CompanyID=CompanyID)
...
def financial_years(request):
    ...
    CompanyID = data['CompanyID']
    CompanyID = get_company(CompanyID)
    instances = administrations_models.FinancialYear.objects.filter(CompanyID=CompanyID)
    closed_instances = administrations_models.FinancialYear.objects.filter(CompanyID=CompanyID,IsClosed=True).last()
    serialized = FinancialYearSerializer(instances,many=True)
    closed_serialized = FinancialYearSerializer(closed_instances,many=True)
    response_data = {
        "StatusCode" : 6000,
