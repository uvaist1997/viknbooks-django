...
from api.v2.masterTypes.serializers import MasterTypeSerializer, MasterTypeRestSerializer
from api.v2.brands.serializers import ListSerializer
from api.v2.masterTypes.functions import generate_serializer_errors
from rest_framework import status
from api.v2.masterTypes.functions import get_auto_id
...
def create_masterType(request):
    ...
    if serialized.is_valid():
        ...
        mastertypes = MasterType.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
...
def masterTypes(request):
    ...
    if serialized1.is_valid():
        ...
        if MasterType.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
            ...
            instances = MasterType.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
