...
from api.v2.stockPostings.serializers import StockPostingSerializer, StockPostingRestSerializer,ExcessStockMasterSerializer,ShortageStockMasterSerializer,DamageStockMasterSerializer,UsedStockMasterSerializer,UsedStockDetailSerializer
from api.v2.brands.serializers import ListSerializer
from api.v2.stockPostings.functions import generate_serializer_errors
from rest_framework import status
from api.v2.stockPostings.functions import get_auto_id, get_auto_excessMasterID,get_auto_excessDetailsID,get_auto_shortageMasterID,get_auto_shortageDetailsID,get_auto_damageMasterID,get_auto_damageDetailsID,get_auto_usedMasterID,\
...
def create_excess_stock(request):
    ...
    if not ExcessStockMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherNo__iexact=VoucherNo).exists():
        ...
        ExcessStockMasterID = get_auto_excessMasterID(ExcessStockMaster,BranchID,CompanyID)
        ...
        for d in Details:
            ...
            ExcessStockDetailsID = get_auto_excessDetailsID(ExcessStockDetails,BranchID,CompanyID)
            ...
            pricelist_ins = PriceList.objects.get(PriceListID=PriceListID,CompanyID=CompanyID,BranchID=BranchID)
            ...
            if StockRate.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Cost=CostPerItem,ProductID=ProductID,WareHouseID=WarehouseID,PriceListID=PriceListID).exists():
                stockRateInstance = StockRate.objects.get(CompanyID=CompanyID,BranchID=BranchID,Cost=CostPerItem,ProductID=ProductID,WareHouseID=WarehouseID,PriceListID=PriceListID)
                
                StockRateID = stockRateInstance.StockRateID
                stockRateInstance.Qty = float(stockRateInstance.Qty) + float(ExcessStock)
                ...
                if StockTrans.objects.filter(StockRateID=StockRateID,DetailID=ExcessStockDetailsID,MasterID=ExcessStockMasterID,VoucherType=VoucherType,BranchID=BranchID,IsActive=True).exists():
                    stockTra_in = StockTrans.objects.filter(StockRateID=StockRateID,DetailID=ExcessStockDetailsID,MasterID=ExcessStockMasterID,VoucherType=VoucherType,BranchID=BranchID,IsActive=True).first()
...
def list_excessStock(request):
    ...
    if serialized1.is_valid():
        ...
        if ExcessStockMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
            instances = ExcessStockMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
            serialized = ExcessStockMasterSerializer(instances,many=True,context={"CompanyID" : CompanyID,
...
def delete_excessStock(request,pk):
    ...
    if ExcessStockMaster.objects.filter(CompanyID=CompanyID,pk=pk).exists():
        ...
        if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=ExcessStockMasterID,BranchID=BranchID,VoucherType="ES").exists():
            ...
            stockPostingInstances = StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=ExcessStockMasterID,BranchID=BranchID,VoucherType="ES")
        ...
        detail_instances = ExcessStockDetails.objects.filter(CompanyID=CompanyID,ExcessStockMasterID=ExcessStockMasterID)
        ...
        for detail_instance in detail_instances:
            ...
            if StockTrans.objects.filter(CompanyID=CompanyID,DetailID=ExcessStockDetailsID,MasterID=ExcessStockMasterID,BranchID=BranchID,VoucherType="ES",IsActive=True).exists():
                stockTrans_instances = StockTrans.objects.filter(CompanyID=CompanyID,DetailID=ExcessStockDetailsID,MasterID=ExcessStockMasterID,BranchID=BranchID,VoucherType="ES",IsActive=True)
...
def edit_excessStock(request,pk):
    ...
    excessStockMaster_instance = ExcessStockMaster.objects.get(pk=pk,CompanyID=CompanyID)
    ...
    if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=ExcessStockMasterID,BranchID=BranchID,VoucherType=VoucherType).exists():
        stockPostingInstances = StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=ExcessStockMasterID,BranchID=BranchID,VoucherType=VoucherType)
        for stockPostingInstance in stockPostingInstances:
            stockPostingInstance.delete()
    ...
    if ExcessStockDetails.objects.filter(ExcessStockMasterID=ExcessStockMasterID,BranchID=BranchID,CompanyID=CompanyID).exists():
        excessStockDetails_instances = ExcessStockDetails.objects.filter(ExcessStockMasterID=ExcessStockMasterID,BranchID=BranchID,CompanyID=CompanyID)
    ...
    if StockTrans.objects.filter(CompanyID=CompanyID,VoucherType=VoucherType,MasterID=ExcessStockMasterID,BranchID=BranchID,IsActive=True).exists():
        trans_ins = StockTrans.objects.filter(CompanyID=CompanyID,VoucherType=VoucherType,MasterID=ExcessStockMasterID,BranchID=BranchID,IsActive=True)
        for stk in trans_ins:
            StockRateID = stk.StockRateID
            stockQty = stk.Qty

            rate_ins = StockRate.objects.filter(CompanyID=CompanyID,StockRateID=StockRateID,BranchID=BranchID).first()
    ...
    activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'ExcessStock', 'Edit', 'ExcessStock Updated successfully.', 'ExcessStock Updated successfully.')
...
def create_shortage_stock(request):
    ...
    if not ShortageStockMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherNo__iexact=VoucherNo).exists():
        ...
        ShortageStockMasterID = get_auto_shortageMasterID(ShortageStockMaster,BranchID,CompanyID)
        ...
        for d in Details:
            ...
            ShortageStockDetailsID = get_auto_shortageDetailsID(ShortageStockDetails,BranchID,CompanyID)
            ...
            if StockRate.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Cost=CostPerItem,ProductID=ProductID,WareHouseID=WarehouseID,PriceListID=PriceListID).exists():
                ...
                StockRateID = stockRateInstance.StockRateID
                stockRateInstance.Qty = float(stockRateInstance.Qty) - float(ShortageStock)
                stockRateInstance.save()
                ...
                if StockTrans.objects.filter(StockRateID=StockRateID,DetailID=ShortageStockDetailsID,MasterID=ShortageStockMasterID,VoucherType=VoucherType,BranchID=BranchID,IsActive=True).exists():
                    stockTra_in = StockTrans.objects.filter(StockRateID=StockRateID,DetailID=ShortageStockDetailsID,MasterID=ShortageStockMasterID,VoucherType=VoucherType,BranchID=BranchID,IsActive=True).first()
...
def list_shortageStock(request):
    ...
    if serialized1.is_valid():
        ...
        if ShortageStockMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
            instances = ShortageStockMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
            serialized = ShortageStockMasterSerializer(instances,many=True,context={"CompanyID" : CompanyID,
                "PriceRounding" : PriceRounding})
            response_data = {
                "StatusCode" : 6000,
                "data" : serialized.data
...
def delete_shortageStock(request,pk):
    ...
    if ShortageStockMaster.objects.filter(CompanyID=CompanyID,pk=pk).exists():
        ...
        if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=ShortageStockMasterID,BranchID=BranchID,VoucherType="SS").exists():
            ...
            stockPostingInstances = StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=ShortageStockMasterID,BranchID=BranchID,VoucherType="SS")
        ...
        detail_instances = ShortageStockDetails.objects.filter(CompanyID=CompanyID,ShortageStockMasterID=ShortageStockMasterID)
        ...
        for detail_instance in detail_instances:
            ...
            if StockTrans.objects.filter(CompanyID=CompanyID,DetailID=ShortageStockDetailsID,MasterID=ShortageStockMasterID,BranchID=BranchID,VoucherType="SS",IsActive=True).exists():
                stockTrans_instances = StockTrans.objects.filter(CompanyID=CompanyID,DetailID=ShortageStockDetailsID,MasterID=ShortageStockMasterID,BranchID=BranchID,VoucherType="SS",IsActive=True)
...
def edit_shortageStock(request,pk):
    ...
    shortageStockMaster_instance = ShortageStockMaster.objects.get(pk=pk,CompanyID=CompanyID)
    ...
    if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=ShortageStockMasterID,BranchID=BranchID,VoucherType=VoucherType).exists():
        stockPostingInstances = StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=ShortageStockMasterID,BranchID=BranchID,VoucherType=VoucherType)
        for stockPostingInstance in stockPostingInstances:
            stockPostingInstance.delete()
    ...
    if ShortageStockDetails.objects.filter(ShortageStockMasterID=ShortageStockMasterID,BranchID=BranchID,CompanyID=CompanyID).exists():
        shortageStockDetails_instances = ShortageStockDetails.objects.filter(ShortageStockMasterID=ShortageStockMasterID,BranchID=BranchID,CompanyID=CompanyID)
    ...
    if StockTrans.objects.filter(CompanyID=CompanyID,VoucherType=VoucherType,MasterID=ShortageStockMasterID,BranchID=BranchID,IsActive=True).exists():
        trans_ins = StockTrans.objects.filter(CompanyID=CompanyID,VoucherType=VoucherType,MasterID=ShortageStockMasterID,BranchID=BranchID,IsActive=True)
        for stk in trans_ins:
            StockRateID = stk.StockRateID
            stockQty = stk.Qty
    ...
    #request , company, log_type, user, source, action, message, description
    activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'ShortageStock', 'Edit', 'ShortageStock Updated successfully.', 'ShortageStock Updated successfully.')
...
def create_damage_stock(request):
    ...
    if not DamageStockMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherNo__iexact=VoucherNo).exists():
        ...
        DamageStockMasterID = get_auto_damageMasterID(DamageStockMaster,BranchID,CompanyID)
        ...
        for d in Details:
            ...
            DamageStockDetailsID = get_auto_damageDetailsID(DamageStockDetails,BranchID,CompanyID)
            ...
            if StockRate.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Cost=CostPerItem,ProductID=ProductID,WareHouseID=WarehouseID,PriceListID=PriceListID).exists():
                ...
                StockRateID = stockRateInstance.StockRateID
                stockRateInstance.Qty = float(stockRateInstance.Qty) - float(DamageStock)
                stockRateInstance.save()
                ...
                if StockTrans.objects.filter(StockRateID=StockRateID,DetailID=DamageStockDetailsID,MasterID=DamageStockMasterID,VoucherType=VoucherType,BranchID=BranchID,IsActive=True).exists():
                    stockTra_in = StockTrans.objects.filter(StockRateID=StockRateID,DetailID=DamageStockDetailsID,MasterID=DamageStockMasterID,VoucherType=VoucherType,BranchID=BranchID,IsActive=True).first()
...
def list_damageStock(request):
    ...
    if serialized1.is_valid():
        ...
        if DamageStockMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
            instances = DamageStockMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
            serialized = DamageStockMasterSerializer(instances,many=True,context={"CompanyID" : CompanyID,
                "PriceRounding" : PriceRounding})
            response_data = {
                "StatusCode" : 6000,
                "data" : serialized.data
...
def delete_damageStock(request,pk):
    ...
    if DamageStockMaster.objects.filter(CompanyID=CompanyID,pk=pk).exists():
        ...
        if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=DamageStockMasterID,BranchID=BranchID,VoucherType="DS").exists():
            ...
            stockPostingInstances = StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=DamageStockMasterID,BranchID=BranchID,VoucherType="DS")
        ...
        detail_instances = DamageStockDetails.objects.filter(CompanyID=CompanyID,DamageStockMasterID=DamageStockMasterID)
        ...
        for detail_instance in detail_instances:
            ...
            if StockTrans.objects.filter(CompanyID=CompanyID,DetailID=DamageStockDetailsID,MasterID=DamageStockMasterID,BranchID=BranchID,VoucherType="DS",IsActive=True).exists():
                stockTrans_instances = StockTrans.objects.filter(CompanyID=CompanyID,DetailID=DamageStockDetailsID,MasterID=DamageStockMasterID,BranchID=BranchID,VoucherType="DS",IsActive=True)
...
def edit_damageStock(request,pk):
    ...
    damageStockMaster_instance = DamageStockMaster.objects.get(pk=pk,CompanyID=CompanyID)
    ...
    if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=DamageStockMasterID,BranchID=BranchID,VoucherType=VoucherType).exists():
        stockPostingInstances = StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=DamageStockMasterID,BranchID=BranchID,VoucherType=VoucherType)
        for stockPostingInstance in stockPostingInstances:
            stockPostingInstance.delete()
    ...
    if DamageStockDetails.objects.filter(DamageStockMasterID=DamageStockMasterID,BranchID=BranchID,CompanyID=CompanyID).exists():
        damageStockDetails_instances = DamageStockDetails.objects.filter(DamageStockMasterID=DamageStockMasterID,BranchID=BranchID,CompanyID=CompanyID)
    ...
    if StockTrans.objects.filter(CompanyID=CompanyID,VoucherType=VoucherType,MasterID=DamageStockMasterID,BranchID=BranchID,IsActive=True).exists():
        trans_ins = StockTrans.objects.filter(CompanyID=CompanyID,VoucherType=VoucherType,MasterID=DamageStockMasterID,BranchID=BranchID,IsActive=True)
        for stk in trans_ins:
            StockRateID = stk.StockRateID
            stockQty = stk.Qty
    ...
    #request , company, log_type, user, source, action, message, description
    activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'DamageStock', 'Edit', 'DamageStock Updated successfully.', 'ShortageStock Updated successfully.')
...
def create_used_stock(request):
    ...
    if not UsedStockMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherNo__iexact=VoucherNo).exists():
        ...
        UsedStockMasterID = get_auto_usedMasterID(UsedStockMaster,BranchID,CompanyID)
        ...
        for d in Details:
            ...
            UsedStockDetailsID = get_auto_usedDetailsID(UsedStockDetails,BranchID,CompanyID)
            ...
            if StockRate.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Cost=CostPerItem,ProductID=ProductID,WareHouseID=WarehouseID,PriceListID=PriceListID).exists():
                ...
                StockRateID = stockRateInstance.StockRateID
                stockRateInstance.Qty = float(stockRateInstance.Qty) - float(UsedStock)
                stockRateInstance.save()
                ...
                if StockTrans.objects.filter(StockRateID=StockRateID,DetailID=UsedStockDetailsID,MasterID=UsedStockMasterID,VoucherType=VoucherType,BranchID=BranchID,IsActive=True).exists():
                    stockTra_in = StockTrans.objects.filter(StockRateID=StockRateID,DetailID=UsedStockDetailsID,MasterID=UsedStockMasterID,VoucherType=VoucherType,BranchID=BranchID,IsActive=True).first()
...
def list_usedStock(request):
    ...
    if serialized1.is_valid():
        ...
        if UsedStockMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
            instances = UsedStockMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
            serialized = UsedStockMasterSerializer(instances,many=True,context={"CompanyID" : CompanyID,
                "PriceRounding" : PriceRounding})
            response_data = {
                "StatusCode" : 6000,
                "data" : serialized.data
...
def delete_usedStock(request,pk):
    ...
    if UsedStockMaster.objects.filter(CompanyID=CompanyID,pk=pk).exists():
        ...
        if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=UsedStockMasterID,BranchID=BranchID,VoucherType="US").exists():
            ...
            stockPostingInstances = StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=UsedStockMasterID,BranchID=BranchID,VoucherType="US")
        ...
        detail_instances = UsedStockDetails.objects.filter(CompanyID=CompanyID,UsedStockMasterID=UsedStockMasterID)
        ...
        for detail_instance in detail_instances:
            ...
            if StockTrans.objects.filter(CompanyID=CompanyID,DetailID=UsedStockDetailsID,MasterID=UsedStockMasterID,BranchID=BranchID,VoucherType="US",IsActive=True).exists():
                stockTrans_instances = StockTrans.objects.filter(CompanyID=CompanyID,DetailID=UsedStockDetailsID,MasterID=UsedStockMasterID,BranchID=BranchID,VoucherType="US",IsActive=True)
...
def edit_usedStock(request,pk):
    ...
    usedStockMaster_instance = UsedStockMaster.objects.get(pk=pk,CompanyID=CompanyID)
    ...
    if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=UsedStockMasterID,BranchID=BranchID,VoucherType=VoucherType).exists():
        stockPostingInstances = StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=UsedStockMasterID,BranchID=BranchID,VoucherType=VoucherType)
        for stockPostingInstance in stockPostingInstances:
            stockPostingInstance.delete()

    if UsedStockDetails.objects.filter(UsedStockMasterID=UsedStockMasterID,BranchID=BranchID,CompanyID=CompanyID).exists():
        usedStockDetails_instances = UsedStockDetails.objects.filter(UsedStockMasterID=UsedStockMasterID,BranchID=BranchID,CompanyID=CompanyID)
    ...
    if StockTrans.objects.filter(CompanyID=CompanyID,VoucherType=VoucherType,MasterID=UsedStockMasterID,BranchID=BranchID,IsActive=True).exists():
        trans_ins = StockTrans.objects.filter(CompanyID=CompanyID,VoucherType=VoucherType,MasterID=UsedStockMasterID,BranchID=BranchID,IsActive=True)
        for stk in trans_ins:
            StockRateID = stk.StockRateID
            stockQty = stk.Qty
    ...
    #request , company, log_type, user, source, action, message, description
    activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'UsedStock', 'Edit', 'UsedStock Updated successfully.', 'ShortageStock Updated successfully.')
