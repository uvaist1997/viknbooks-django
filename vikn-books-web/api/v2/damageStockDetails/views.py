...
from api.v2.damageStockDetails.serializers import DamageStockDetailsSerializer, DamageStockDetailsRestSerializer, DamageStockDetailsDummySerializer
from api.v2.damageStockMaster.serializers import DamageStockMasterSerializer, DamageStockMasterRestSerializer
...
from api.v2.damageStockDetails.functions import get_auto_id
from api.v2.damageStockMaster.functions import get_auto_idMaster
...
def create_damageStock(request):
    ...
    if stockDetailSerialized.is_valid() and stockMasterSerialized.is_valid():
        ...
        DamageStockMasterID = get_auto_idMaster(DamageStockMaster,BranchID,DataBase)
        DamageStockDetailsID = get_auto_id(DamageStockDetails,BranchID, DataBase)
...
def list_damageStockDetailsDummy(request):
    ...
    if serialized1.is_valid():
        ...
        if DamageStockDetailsDummy.objects.filter(BranchID=BranchID).exists():
            instances = DamageStockDetailsDummy.objects.filter(BranchID=BranchID)
            serialized = DamageStockDetailsDummySerializer(instances,many=True)
