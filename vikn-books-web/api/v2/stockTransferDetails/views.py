...
from api.v2.stockTransferDetails.serializers import StockTransferDetailsSerializer, StockTransferDetailsRestSerializer, StockTransferDetailsDummySerializer
from api.v2.brands.serializers import ListSerializer
from api.v2.stockTransferDetails.functions import generate_serializer_errors
from rest_framework import status
from api.v2.stockTransferDetails.functions import get_auto_id
import datetime
...
def list_stockTransferDetails(request):
    ...
    if serialized1.is_valid():
        ...
        if StockTransferDetails.objects.filter(BranchID=BranchID).exists():
            ...
            serialized = StockTransferDetailsRestSerializer(instances,many=True)
...
def list_stockTransferDetailsDummy(request):
    ...
    if serialized1.is_valid():
        ...
        if StockTransferDetailsDummy.objects.filter(BranchID=BranchID).exists():
            ...
            instances = StockTransferDetailsDummy.objects.filter(BranchID=BranchID)

            serialized = StockTransferDetailsDummySerializer(instances,many=True)
