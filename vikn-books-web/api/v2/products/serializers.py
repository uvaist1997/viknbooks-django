...
from brands.models import Product, PriceList, Brand, ProductGroup, TaxCategory, Unit, StockPosting,Batch
...
class ProductRestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'ProductID', 'BranchID', 'ProductCode', 'ProductName', 'DisplayName', 'Description', 'ProductGroupID',
                  ...
                  'is_VAT_inclusive', 'is_GST_inclusive', 'is_TAX1_inclusive', 'is_TAX2_inclusive', 'is_TAX3_inclusive','Stock', 'PriceListDetails','DefaultMRP',"IsKFC","HSNCode")
    ...
    def get_Stock(self, instances):
        ...
        if StockPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID=ProductID):
            stock_ins = StockPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID=ProductID)
            total_stockIN = 0
            total_stockOUT = 0
            for s in stock_ins:
...
class ProductSearchShortcutSerializer(serializers.ModelSerializer):
    ...
    def get_Quantity(self, instances):
        ...
        if StockPosting.objects.filter(CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID, WareHouseID=WarehouseID).exists():
            stockpostIns = StockPosting.objects.filter(CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID, WareHouseID=WarehouseID)
            for i in stockpostIns:
                TotalQtyIn += i.QtyIn
                TotalQtyOut += i.QtyOut
