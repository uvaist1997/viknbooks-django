...
def get_auto_id(model,BranchID,CompanyID):
    ...
    latest_auto_id =  model.objects.filter(CompanyID=CompanyID).aggregate(Max('ProductID'))
    
    max_value = 0
    if model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        max_value =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).aggregate(Max('ProductID'))
...
def get_auto_priceListid(model,BranchID,CompanyID):
    ...
    PriceListID = None
    # latest_auto_id =  model.objects.filter(CompanyID=CompanyID).order_by("-CreatedDate")[:1]
    latest_auto_id =  model.objects.filter(CompanyID=CompanyID).aggregate(Max('PriceListID'))
    

    if model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        max_value =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).aggregate(Max('PriceListID'))
...
def get_ProductCode(model,BranchID,CompanyID):
    ...
    if model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        ...
        latest_ProductCode =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).order_by("ProductID").last()
...
def get_auto_AutoBarcode(model,BranchID,CompanyID):
    ...
    AutoBarcode = 5000
    if model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        latest_AutoBarcode =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).order_by("PriceListID").last()
...
def get_auto_BatchNo(model,BranchID,CompanyID):
    ...
    BatchNo = 5000
    if model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        latest_BatchNo =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).order_by("BatchNo").last()
...
def get_auto_AutoBatchCode(model, BranchID, CompanyID):
    ...
    if model.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        ...
        CompanyID=CompanyID, BranchID=BranchID).last()
