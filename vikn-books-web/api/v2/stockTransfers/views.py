from brands.models import StockTransferMaster_ID, StockTransferMasterID_Log, StockTransferDetails, StockTransferDetails_Log,\
StockTransferDetailsDummy, StockRate, StockTrans, StockPosting, StockPosting_Log, PriceList
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from api.v2.stockTransfers.serializers import StockTransferMaster_IDSerializer, StockTransferMaster_IDRestSerializer, StockTransferDetailsSerializer, StockTransferDetailsRestSerializer
from api.v2.brands.serializers import ListSerializer
from api.v2.stockTransfers.functions import generate_serializer_errors
from rest_framework import status
from api.v2.stockTransfers.functions import get_auto_id, get_auto_idMaster
from api.v2.purchases.functions import get_auto_StockRateID, get_auto_StockTransID
from api.v2.sales.functions import get_auto_stockPostid, get_auto_VoucherNo
import datetime
...
def create_stockTransfer(request):
    ...
    StockTransferMasterID = get_auto_idMaster(StockTransferMaster_ID,BranchID, CompanyID)
    ...
    for stockTransferDetail in stockTransferDetails:
        ...
        StockTransferDetailsID = get_auto_id(StockTransferDetails,BranchID,CompanyID)
        ...
        if StockRate.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Cost=Cost,ProductID=ProductID,WareHouseID=WarehouseFromID,PriceListID=PriceListID).exists():
            stockRateInstance = StockRate.objects.get(CompanyID=CompanyID,BranchID=BranchID,Cost=Cost,ProductID=ProductID,WareHouseID=WarehouseFromID,PriceListID=PriceListID)
        ...
        if StockRate.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Cost=Cost,ProductID=ProductID,WareHouseID=WarehouseToID,PriceListID=PriceListID).exists():
            stockRateInstance = StockRate.objects.get(CompanyID=CompanyID,BranchID=BranchID,Cost=Cost,ProductID=ProductID,WareHouseID=WarehouseToID,PriceListID=PriceListID)
    ...
    activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Stock Transfer', 'Create', 'Stock Transfer created successfully.', 'Stock Transfer saved successfully.')
...
def edit_stockTransfer(request,pk):
    ...
    stockTransferMaster_instance = StockTransferMaster_ID.objects.get(CompanyID=CompanyID,pk=pk)
    ...
    if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=StockTransferMasterID,BranchID=BranchID,VoucherType=VoucherType).exists():
        stockPostingInstances = StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=StockTransferMasterID,BranchID=BranchID,VoucherType=VoucherType)
            ...
            if PriceList.objects.filter(CompanyID=CompanyID,ProductID=ProductID_Deleted,DefaultUnit=True).exists():
                ...
                if not deleted_pk == '' or not deleted_pk == 0:
                    if StockTransferDetails.objects.filter(CompanyID=CompanyID,pk=deleted_pk).exists():
                        deleted_detail = StockTransferDetails.objects.filter(CompanyID=CompanyID,pk=deleted_pk)
                        ...
                        if StockRate.objects.filter(CompanyID=CompanyID,ProductID=ProductID_Deleted,PriceListID=PriceListID_Deleted,Cost=Cost_Deleted,WareHouseID=WarehouseFromID_Deleted).exists():
                            stockRate_instance = StockRate.objects.get(CompanyID=CompanyID,ProductID=ProductID_Deleted,PriceListID=PriceListID_Deleted,Cost=Cost_Deleted,WareHouseID=WarehouseFromID_Deleted)
                        ...
                        if StockRate.objects.filter(CompanyID=CompanyID,ProductID=ProductID_Deleted,PriceListID=PriceListID_Deleted,Cost=Cost_Deleted,WareHouseID=WarehouseToID_Deleted).exists():
                            stockRate_instance = StockRate.objects.get(CompanyID=CompanyID,ProductID=ProductID_Deleted,PriceListID=PriceListID_Deleted,Cost=Cost_Deleted,WareHouseID=WarehouseToID_Deleted)
                            StockRateID = stockRate_instance.StockRateID
                            if StockTrans.objects.filter(CompanyID=CompanyID,DetailID=StockTransferDetailsID_Deleted,MasterID=StockTransferMasterID_Deleted,BranchID=BranchID,
                                VoucherType=VoucherType,IsActive=True,StockRateID=StockRateID).exists():
                                stockTrans_instance = StockTrans.objects.get(CompanyID=CompanyID,DetailID=StockTransferDetailsID_Deleted,MasterID=StockTransferMasterID_Deleted,BranchID=BranchID,
    ...
    for stockTransferDetail in stockTransferDetails:
        ...
        if detailID == 0:
            stockTransferDetail_instance = StockTransferDetails.objects.get(pk=pk)
        ...
        if detailID == 1:
            ...
            stockTrans_instances = StockTrans.objects.filter(CompanyID=CompanyID,DetailID=StockTransferDetailsID,BranchID=BranchID,VoucherType=VoucherType,IsActive=True)
            for stockTrans_instance in stockTrans_instances:
                ...
                if StockRate.objects.filter(CompanyID=CompanyID,StockRateID=stockRateID,BranchID=BranchID,WareHouseID=WarehouseFromID).exists():
                    stockRate_instances = StockRate.objects.filter(CompanyID=CompanyID,StockRateID=stockRateID,BranchID=BranchID,WareHouseID=WarehouseFromID)
                ...
                if StockRate.objects.filter(CompanyID=CompanyID,StockRateID=stockRateID,BranchID=BranchID,WareHouseID=WarehouseToID).exists():
                    stockRate_instances = StockRate.objects.filter(CompanyID=CompanyID,StockRateID=stockRateID,BranchID=BranchID,WareHouseID=WarehouseToID)
    ...
    #request , company, log_type, user, source, action, message, description
    activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Stock Transfer', 'Edit', 'Stock Transfer Updated successfully.', 'Stock Transfer Updated successfully.')
...
def list_stockTransferMasterID(request):
    ...
    if serialized1.is_valid():
        ...
        if StockTransferMaster_ID.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
            instances = StockTransferMaster_ID.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
            serialized = StockTransferMaster_IDRestSerializer(instances,many=True,context = {"CompanyID": CompanyID,
                "PriceRounding" : PriceRounding})

            #request , company, log_type, user, source, action, message, description
            activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Stock Transfer', 'List', 'Stock Transfer List Viewed successfully.', 'Stock Transfer List Viewed successfully')
...
def stock_transfer_pagination(request):
    ...
    if serialized1.is_valid():
        ...
        if page_number and items_per_page:
            stock_transfer_object = StockTransferMaster_ID.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
            ...
            stock_transfer_serializer = StockTransferMaster_IDRestSerializer(
...
def stockTransferMasterID(request,pk):
    ...
    if StockTransferMaster_ID.objects.filter(CompanyID=CompanyID,pk=pk).exists():
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Stock Transfer', 'View', 'Stock Transfer Single Viewed successfully.', 'Stock Transfer Single Viewed successfully.')
        response_data = {
            "StatusCode" : 6000,
            "data" : serialized.data
        }
...
def delete_stockTransferMasterID(request,pk):
    ...
    if StockTransferMaster_ID.objects.filter(CompanyID=CompanyID,pk=pk).exists():
        ...
        detail_instances = StockTransferDetails.objects.filter(CompanyID=CompanyID,StockTransferMasterID=StockTransferMasterID,BranchID=BranchID)
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'Stock Transfer', 'Delete', 'Stock Transfer Deleted successfully.', 'Stock Transfer Deleted successfully.')
        response_data = {
            "StatusCode" : 6000,
            "title" : "Success",
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Warning', CreatedUserID, 'Stock Transfer', 'Delete', 'Stock Transfer Deleted Failed.', 'Stock Transfer Not Found')
        response_data = {
            "StatusCode" : 6001,
            "title" : "Failed",
