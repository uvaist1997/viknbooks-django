...
from api.v2.dashboard import serializers
from api.v2.ledgerPosting.serializers import ListSerializerforProfitAndLoss, ProfitAndLossSerializer
from api.v2.dashboard.functions import generate_serializer_errors
from rest_framework import status
from api.v2.dashboard.functions import get_auto_id
...
def customer(request):
    ...
    if models.Parties.objects.filter(CompanyID=CompanyID,PartyType="customer").exists():
        instances = models.Parties.objects.filter(CompanyID=CompanyID,PartyType="customer")
    serialized = serializers.CustomerSerializer(instances,many=True,context = {"CompanyID": CompanyID })
    data = []
    count = 0
...
def supplier(request):
    ...
    if models.Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PartyType="supplier").exists():
        instances = models.Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PartyType="supplier")
    serialized = serializers.CustomerSerializer(instances,many=True,context = {"CompanyID": CompanyID ,"BranchID":BranchID})
    data = []
    count = 0
    for i in serialized.data:
        if count <= 9:
...
def accounts(request):
    ...
    if models.Bank.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        ...
        instances = models.AccountLedger.objects.filter(CompanyID=CompanyID,BranchID=BranchID,AccountGroupUnder__in=[8,9])
    serialized = serializers.BankSerializer(instances,many=True,context = {"CompanyID": CompanyID ,"BranchID":BranchID})
    data = []
    count = 0
    for i in serialized.data:
        if count <= 9:
...
def profit_and_loss(request):
    ...
    if serialized1.is_valid():
        ...
        to_date = datetime.datetime.strptime(FromDate, "%Y-%m-%d").date()+datetime.timedelta(31)
        for month in range(12):
            ...
            if models.LedgerPosting.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Date__gte=from_date, Date__lte=to_date).exists():
                ...
                if models.Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InventoryType="StockItem").exists():
                    product_instances_OS = models.Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InventoryType="StockItem")
                    ...
                    for pi in product_instances_OS:
                        ...
                        if models.StockPosting.objects.filter(ProductID=ProductID,Date__lt=from_date,CompanyID=CompanyID).exists():
                            stock_instance_OS = models.StockPosting.objects.filter(ProductID=ProductID,Date__lt=from_date,CompanyID=CompanyID)
                            QtyInRate = 0
                            QtyOutTot = 0
                            QtyInTot = 0
                        ...
                        if models.StockPosting.objects.filter(ProductID=ProductID,Date__gte=from_date, Date__lte=to_date,CompanyID=CompanyID,VoucherType="OS").exists():
                            stock_instance_OS = models.StockPosting.objects.filter(ProductID=ProductID,Date__gte=from_date, Date__lte=to_date,CompanyID=CompanyID,VoucherType="OS")
                            QtyInRate = 0
                            QtyOutTot = 0
                            QtyInTot = 0
                            for si in stock_instance_OS:
                ...
                if models.Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InventoryType="StockItem").exists():
                    product_instances_CS = models.Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InventoryType="StockItem")
                    ...
                    for pi in product_instances_CS:
                        ...
                        if models.StockPosting.objects.filter(ProductID=ProductID,Date__lte=to_date,CompanyID=CompanyID).exists():
                            stock_instance_CS = models.StockPosting.objects.filter(ProductID=ProductID,Date__lte=to_date,CompanyID=CompanyID)
                            QtyInRate = 0
                            QtyOutTot = 0
                            QtyInTot = 0
