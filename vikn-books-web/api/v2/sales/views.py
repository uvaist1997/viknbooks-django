...
from api.v2.sales.serializers import SalesMasterSerializer, SalesMasterRestSerializer, SalesDetailsSerializer, SalesDetailsRestSerializer,\
...
from api.v2.purchases.serializers import PurchaseMasterRestSerializer, PurchasePrintSerializer
from api.v2.salesReturns.serializers import SalesReturnMasterRestSerializer, SalesReturnMasterReportSerializer
from api.v2.purchaseReturns.serializers import PurchaseReturnMasterRestSerializer, PurchaseReturnPrintSerializer
from api.v2.sales.functions import generate_serializer_errors
from rest_framework import status
from api.v2.sales.functions import get_auto_id, get_auto_idMaster, get_auto_stockPostid
    ...
    if VoucherType == "SI":
        if SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            ...
            if not OldVoucherNo.isdecimal():
                ...
                new_VoucherNo = str(int(OldVoucherNo) + 1).zfill(len(OldVoucherNo))
            response_data = {
                "StatusCode": 6000,
        ...
        if AccountGroup.objects.filter(CompanyID=CompanyID).exists():
            ...
            if not OldGroupCode.isdecimal():
                ...
                new_GroupCode = str(int(OldGroupCode) + 1).zfill(len(OldGroupCode))
        ...
        if Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
            ...
            if not ProductCode.isdecimal():
                ...
                ProductCode = str(int(ProductCode) + 1).zfill(len(ProductCode))
...
def generate_printReport(request):
    from api.v2.sales.serializers import SalesPrintSerializer
    from api.v2.salesReturns.serializers import SalesReturnMasterPrintSerializer
    data = request.data
    CompanyID = data['CompanyID']
    CompanyID = get_company(CompanyID)
    CreatedUserID = data['CreatedUserID']
    ...
    if serialized1.is_valid():
        ...
        if SalesMaster.objects.filter(CompanyID=CompanyID, Date__gte=FromDate, Date__lte=ToDate).exists():
            ...
            response_data = {
                "StatusCode": 6000,
                "data": final_array,
                "TotalProfit": round(TotalProfit,PriceRounding),
                "TotalCost": round(TotalCost,PriceRounding),
                "TotalNetAmt": round(TotalNetAmt,PriceRounding),
                "TotalVatAmt": round(TotalVatAmt,PriceRounding),
                "TotalGrossAmt": round(TotalGrossAmt,PriceRounding),
            }
            return Response(response_data, status=status.HTTP_200_OK)
        else:
