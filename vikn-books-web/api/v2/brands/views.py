...
from api.v2.brands.serializers import BrandSerializer, BrandRestSerializer, ListSerializer
from api.v2.brands.functions import generate_serializer_errors
...
def torunqry(request):
    ...
    for s in sales_master_ins:
        ...
        for d in sales_details_ins:
            ...
            PurchasePrice = PriceList.objects.get(PriceListID=PriceListID_det,BranchID=BranchID,CompanyID=CompanyID,ProductID=d.ProductID).PurchasePrice
...
def torunqryPurchase(request):
    ...
    for s in purchase_master_ins:
        ...
        for d in purchase_details_ins:
            ...
            MultiFactor = PriceList.objects.get(CompanyID=CompanyID,PriceListID=d.PriceListID,BranchID=BranchID).MultiFactor
...
def torunqryOpeningStock(request):
    ...
    openingStock_master_ins = OpeningStockMaster.objects.filter(CompanyID=CompanyID)
    VoucherType = "OS"
...
def torunqryPyment(request):
    ...
    for p in payment_master_ins:
        ...
        payment_detail_ins = PaymentDetails.objects.filter(PaymentMasterID=PaymentMasterID,CompanyID=CompanyID)
...
def torunqryReciept(request):
    ...
    for p in receipt_master_ins:
        ...
        UpdatedDate = p.UpdatedDate
        MasterLedgerID = p.LedgerID
        receipt_detail_ins = ReceiptDetails.objects.filter(ReceiptMasterID=ReceiptMasterID,CompanyID=CompanyID)
...
def torunqryForPriceList(request):
    ...
    for i in salesDetails_ins:
        ...
        if PriceList.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID=ProductID,UnitID=UnitID).exists():
            pricelist_ins = PriceList.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID=ProductID,UnitID=UnitID)
    ...
    openingStockDetails_ins = OpeningStockDetails.objects.filter(CompanyID=CompanyID)
