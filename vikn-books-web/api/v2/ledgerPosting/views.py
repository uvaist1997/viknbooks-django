...
from api.v2.ledgerPosting.serializers import LedgerPostingSerializer, LedgerPostingRestSerializer, ListSerializerforTrialBalance,\
...
from api.v2.stockPostings.serializers import StockPostingRestSerializer, StockValueAllSerializer, StockValueSingleSerializer
from api.v2.ledgerPosting.functions import generate_serializer_errors
from rest_framework import status
from api.v2.sales.serializers import ListSerializerforReport, ListSerializerforStockReport, ListSerializerforStockValueReport, ListSerializerforStockValueReportSingle
from api.v2.ledgerPosting.functions import get_auto_id, convertOrderdDict, get_VoucherName
import datetime
from django.db.models import Sum
from django.shortcuts import render
from main.functions import get_company
from api.v2.companySettings.serializers import CompanySettingsRestSerializer
                ...
                if Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,InventoryType="StockItem").exists():
                    ...
                    for pi in product_instances_CS:
                        ...
                        if StockPosting.objects.filter(ProductID=ProductID,Date__lte=ToDate,CompanyID=CompanyID,VoucherType="OS").exists():
                            stock_instance_CS = StockPosting.objects.filter(ProductID=ProductID,Date__lte=ToDate,CompanyID=CompanyID,VoucherType="OS")
                            QtyInRate = 0
                            QtyOutTot = 0
                            QtyInTot = 0
                            for si in stock_instance_CS:
...
def cash_book(request):
    ...
    if not CashLedgers:
        CashLedgers = []
        cash_ledger_ins = AccountLedger.objects.filter(CompanyID=CompanyID,BranchID=BranchID,AccountGroupUnder=9)
