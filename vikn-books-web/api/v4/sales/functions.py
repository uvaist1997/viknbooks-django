...
def get_stock_value(CompanyID,BranchID,FromDate,ToDate,WarehouseID,PriceRounding):
    GrandTotalCost = 0
    if StockPosting.objects.filter(CompanyID=instance.CompanyID, BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate).exists():
        stock_instances = StockPosting.objects.filter(CompanyID=instance.CompanyID, BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate)
        ...
        product_instances = Product.objects.filter(
            CompanyID=instance.CompanyID, BranchID=BranchID, ProductID__in=product_arry)
    ...
    if loyalty_program:
        ...
        if RadeemPoint:
            ...
            if LoyaltyPoint.objects.filter(BranchID=instance.BranchID,CompanyID=instance.CompanyID,LoyaltyCustomerID=loyalty_customer,VoucherType="SI",ExpiryDate__gte=sale_date,is_Radeem=False).exists():
                ...
                print(radeem_point,'2POOOOOOOOOOOOOOOOOOOO##############################OOOOOOOOOIIIIIIIIINNNNNNNNNNNTTTTTTT')
                ...
                if i.Point:
                    if Loyalty_Point_Expire:
                        ...
                        if Action == "M":
                            if is_edit:
                                tot_Point = float(tot_Point) + float(i.Point) 
                                print(("3 i.ExpiryDate Kayaryyyyyyyyyyyyyy"))
                        elif Action == "A":
                            tot_Point = float(tot_Point) + float(i.Point) 
                            print(("4 i.ExpiryDate Kayaryyyyyyyyyyyyyy"))
                    print(("i.ExpiryDate Kayaryyyyyyyyyyyyyy()_()_()_()_"))
...
def edit_LoyaltyCalculation(instance,loyalty_customer,details,Loyalty_Point_Expire,RadeemPoint):
    ...
    if RadeemPoint:
        ...
        radeem_value = radeem[1]
        radeem_point = int(radeem[0]) * -1
        if int(GrandTotal) >= int(radeem_value):
            ...
            LoyaltyPoint_Log.objects.create(
                BranchID=instance.BranchID,
                LoyaltyPointID=instance.LoyaltyPointID,
                Value=radeem_value,
                VoucherType="SI",
                VoucherMasterID=instance.SalesMasterID,
                Point=radeem_point,
