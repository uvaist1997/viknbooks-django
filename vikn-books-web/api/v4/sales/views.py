...
from api.v4.sales.serializers import SalesEstimateForOrderSerializer, SalesMasterSerializer, SalesMasterRestSerializer, SalesDetailsSerializer,\
    ...
    SalesSummaryReportSerializer,SupplierVsProductReportSerializer,SalesGSTReportSerializer,ProductVsSuppliersReportSerializer,FilterOrderSerializer
from api.v4.brands.serializers import ListSerializer
from api.v4.salesOrders.serializers import SalesOrderDetailsRestSerializer
from api.v4.purchases.serializers import PurchaseMasterRestSerializer, PurchasePrintSerializer
from api.v4.salesReturns.serializers import SalesReturnMasterRestSerializer, SalesReturnMasterReportSerializer
from api.v4.purchaseReturns.serializers import PurchaseReturnMasterRestSerializer, PurchaseReturnPrintSerializer
from api.v4.sales.functions import set_LoyaltyCalculation,edit_LoyaltyCalculation,generate_serializer_errors, get_month,get_stock_value
from rest_framework import status
from api.v4.sales.functions import get_auto_id, get_auto_idMaster, get_auto_stockPostid,get_date_list
...
from api.v4.companySettings.serializers import CompanySettingsRestSerializer
from api.v4.products.functions import get_auto_AutoBatchCode
from django.db.models import Max
from api.v4.salesOrders.serializers import SalesOrderMasterRestSerializer
    ...
    if is_SaleOK:
        ...
        if LoyaltyCustomerID:
            if LoyaltyCustomer.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LoyaltyCustomerID=LoyaltyCustomerID).exists():
                loyalty_customer = LoyaltyCustomer.objects.get(CompanyID=CompanyID,BranchID=BranchID,LoyaltyCustomerID=LoyaltyCustomerID)
...
def edit_sales(request, pk):
    ...
    if SerialNumbers.objects.filter(CompanyID=CompanyID, SalesMasterID=SalesMasterID, BranchID=BranchID).exists():
    ...
    if is_LoyaltyCustomer:
        ...
        try:
            ...
            RadeemPoint = None
        edit_LoyaltyCalculation(salesMaster_instance,loyalty_customer,details,Loyalty_Point_Expire,RadeemPoint)
            # ===========================
    ...
    if SalesMaster.objects.filter(CompanyID=CompanyID, pk=pk).exists():
        ...
        for detail_instance in detail_instances:
            ...
            if SerialNumbers.objects.filter(CompanyID=CompanyID, BranchID=BranchID,SalesMasterID=SalesMasterID,SalesDetailsID=SalesDetailsID).exists():
                serial_instances = SerialNumbers.objects.filter(CompanyID=CompanyID, BranchID=BranchID,SalesMasterID=SalesMasterID,SalesDetailsID=SalesDetailsID)
    ...
    if serialized1.is_valid():
        ...
        if UserID:
            ...
            if SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate, CreatedUserID=UserID).exists():
                ...
                print(jsnDatas,"OOOOOOOIIIII")
                for i in jsnDatas:
                    TaxableValue = i['TaxableValue']
                    TotalQty = i['TotalQty']

                    Total_TaxableValue += TaxableValue
                    Total_TotalQty += TotalQty
...
def generate_printReport(request):
    from api.v4.sales.serializers import SalesPrintSerializer
    from api.v4.salesReturns.serializers import SalesReturnMasterPrintSerializer
    data = request.data
    CompanyID = data['CompanyID']
    CompanyID = get_company(CompanyID)
    CreatedUserID = data['CreatedUserID']
...
def stock_report(request):
    ...
    if serialized1.is_valid():
        ...
        if stockPosting_instances:
            ...
            qurried_instances = stockPosting_instances.values('ProductID').annotate(in_stock_quantity=Sum('QtyIn') - Sum('QtyOut')).order_by('ProductID')
...
def stock_ledger_report(request):
    ...
    if serialized1.is_valid():
        ...
        if StockPosting.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID).exists():
            stockPosting_instances = StockPosting.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID).order_by('Date', 'StockPostingID')
            if WareHouseID > 0:
                stockPosting_instances = stockPosting_instances.filter(WareHouseID=WareHouseID).order_by('Date', 'StockPostingID')
            # if page_number and items_per_page:
            #     stockPosting_instances = StockPosting.objects.filter(
        ...
        if stockPosting_instances:
            ...
            if report_type == 11:
                ...
                for monthly in monthly_dates:
                    ...
                    stock_datas = stockPosting_instances.filter(Date__in=monthly)
                    stockPost_arry = []
                    product_arry = []
    ...
    if serialized1.is_valid():
        ...
        if filterValue == 1:
            if Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PartyID=PartyID).exists():
                party_LedgerID = Parties.objects.get(CompanyID=CompanyID,BranchID=BranchID,PartyID=PartyID).LedgerID
                ...
                if SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate,LedgerID=party_LedgerID).exists():
                    salesMaster_instance = SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate,LedgerID=party_LedgerID)
                    sales_master_ids = salesMaster_instance.values_list('SalesMasterID',flat=True)
                    if SalesDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,SalesMasterID__in=sales_master_ids).exists():
                        salesDetail_instances = SalesDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,SalesMasterID__in=sales_master_ids)
                        sales_productids = salesDetail_instances.values_list('ProductID',flat=True)
                ...
                if PurchaseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate,LedgerID=party_LedgerID).exists():
                    purchaseMaster_instance = PurchaseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate,LedgerID=party_LedgerID)
                    purchase_master_ids = purchaseMaster_instance.values_list('PurchaseMasterID',flat=True)
                    if PurchaseDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PurchaseMasterID__in=purchase_master_ids).exists():
                        purchaseDetail_instances = PurchaseDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PurchaseMasterID__in=purchase_master_ids)
                        purchase_productids = purchaseDetail_instances.values_list('ProductID',flat=True)
                        purchase_diff_ids = list(set(purchase_productids) - set(productids))
                ...
                if SalesReturnMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID, VoucherDate__gte=FromDate, VoucherDate__lte=ToDate,LedgerID=party_LedgerID).exists():
                    salesReturnMaster_instance = SalesReturnMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID, VoucherDate__gte=FromDate, VoucherDate__lte=ToDate,LedgerID=party_LedgerID)
                    salesReturn_master_ids = salesReturnMaster_instance.values_list('SalesReturnMasterID',flat=True)
                    if SalesReturnDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,SalesReturnMasterID__in=salesReturn_master_ids).exists():
                        salesReturnDetail_instances = SalesReturnDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,SalesReturnMasterID__in=salesReturn_master_ids)
                        salesReturn_productids = salesReturnDetail_instances.values_list('ProductID',flat=True)
                        salesReturn_diff_ids = list(set(salesReturn_productids) - set(productids))
                ...
                if PurchaseReturnMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID, VoucherDate__gte=FromDate, VoucherDate__lte=ToDate,LedgerID=party_LedgerID).exists():
                    purchaseReturnMaster_instance = PurchaseReturnMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID, VoucherDate__gte=FromDate, VoucherDate__lte=ToDate,LedgerID=party_LedgerID)
                    purchaseReturn_master_ids = purchaseReturnMaster_instance.values_list('PurchaseReturnMasterID',flat=True)
                    if PurchaseReturnDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PurchaseReturnMasterID__in=purchaseReturn_master_ids).exists():
                        purchaseReturnDetail_instances = PurchaseReturnDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PurchaseReturnMasterID__in=purchaseReturn_master_ids)
                        purchaseReturn_productids = purchaseReturnDetail_instances.values_list('ProductID',flat=True)
                        purchaseReturn_diff_ids = list(set(purchaseReturn_productids) - set(productids))
                        productids.extend(purchaseReturn_diff_ids)

                
                if page_number and items_per_page:
                    prodct_ins = Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID__in=productids)
                    party_sort_pagination = list_pagination(
                        prodct_ins,
                        items_per_page,
                        page_number
                    )
                serialized = SupplierVsProductReportSerializer(party_sort_pagination, many=True, context={"CompanyID": CompanyID, "PriceRounding": PriceRounding, "FromDate": FromDate,"ToDate":ToDate})
                response_data = {
                    "StatusCode": 6000,
                    "data": serialized.data,
            ...
            if SalesDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID=ProductID).exists():
                salesDetail_instances = SalesDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID=ProductID)
                sales_master_ids = salesDetail_instances.values_list('SalesMasterID',flat=True)
                sales_masterIns = SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,SalesMasterID__in=sales_master_ids)
            ...
            if PurchaseDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID=ProductID).exists():
                purchaseDetail_instances = PurchaseDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID=ProductID)
                purchase_master_ids = purchaseDetail_instances.values_list('PurchaseMasterID',flat=True)
                purchase_masterIns = PurchaseMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PurchaseMasterID__in=purchase_master_ids)
                purchase_partyIds = purchase_masterIns.values_list('LedgerID',flat=True)
            ...
            if SalesReturnDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID=ProductID).exists():
                salesReturnDetail_instances = SalesReturnDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID=ProductID)
                salesReturn_master_ids = salesReturnDetail_instances.values_list('SalesReturnMasterID',flat=True)
                salesReturn_masterIns = SalesReturnMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,SalesReturnMasterID__in=salesReturn_master_ids)
                salesReturn_partyIds = salesReturn_masterIns.values_list('LedgerID',flat=True)
            ...
            if PurchaseReturnDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID=ProductID).exists():
                purchaseReturnDetail_instances = PurchaseReturnDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID=ProductID)
                purchaseReturn_master_ids = purchaseReturnDetail_instances.values_list('PurchaseReturnMasterID',flat=True)
                purchaseReturn_masterIns = PurchaseReturnMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PurchaseReturnMasterID__in=purchaseReturn_master_ids)
                purchaseReturn_partyIds = purchaseReturn_masterIns.values_list('LedgerID',flat=True)
                purchaseReturn_diff_ids = list(set(purchaseReturn_partyIds) - set(ledger_ids))
            ...
            if AccountLedger.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID__in=ledger_ids,AccountGroupUnder=29).exists():
                party_ledger_ins = AccountLedger.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID__in=ledger_ids,AccountGroupUnder=29)
            ...
            if Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID__in=party_ids).exists():
                if page_number and items_per_page:
                    party_ins = Parties.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID__in=party_ids)
                    party_sort_pagination = list_pagination(
                        party_ins,
                        items_per_page,
                        page_number
...
def inventory_flow_report(request):
    ...
    if serialized1.is_valid():
        ...
        if StockPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__lt=FromDate).exists():
            stockPostInstance = StockPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__lt=FromDate)
            if not WarehouseID == 0:
                stockPostInstance = stockPostInstance.filter(WareHouseID=WarehouseID)
            opening_stock_qtyinSum = stockPostInstance.aggregate(Sum('QtyIn'))
            opening_stock_qtyoutSum = stockPostInstance.aggregate(Sum('QtyOut'))
            opening_stock_qtyinSum = opening_stock_qtyinSum['QtyIn__sum']
            opening_stock_qtyoutSum = opening_stock_qtyoutSum['QtyOut__sum']
            opening_stock = float(opening_stock_qtyinSum) - float(opening_stock_qtyoutSum)
            my_dict = {
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=69,VoucherType="PI",Date__gte=FromDate,Date__lte=ToDate).exists():
            ledgerPostInstance = LedgerPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=69,VoucherType="PI",Date__gte=FromDate,Date__lte=ToDate)
        ...
        if StockPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate,VoucherType="ST").exists():
            stock_ins = StockPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate,VoucherType="ST")
            if not WarehouseID == 0:
            ...
            stock_transfr_sum = float(stock_transfer_qtyinSum) - float(stock_transfer_qtyoutSum)
        ...
        # stock value
        stock_value = get_stock_value(CompanyID,BranchID,FromDate,ToDate,WarehouseID,PriceRounding)
        my_dict = {
        ...
        if StockPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate,VoucherType__in=Voucher_types).exists():
            stock_ins = StockPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate,VoucherType__in=Voucher_types)
            if not WarehouseID == 0:
                stock_ins = stock_ins.filter(WareHouseID=WarehouseID)
            ...
            stock_transfer_qtyoutSum = stock_transfer_qtyoutSum['QtyOut__sum']
            stock_adjust_sum = float(stock_adjust_qtyinSum) - float(stock_transfer_qtyoutSum)
        ...
        account_ledgers_supp = AccountLedger.objects.filter(CompanyID=CompanyID,BranchID=BranchID,AccountGroupUnder=29,IsActive=True)
        supplier_ids = account_ledgers_supp.values_list('LedgerID',flat=True)
        account_ledgers_cust = AccountLedger.objects.filter(CompanyID=CompanyID,BranchID=BranchID,AccountGroupUnder=10,IsActive=True)
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__lt=ToDate,LedgerID__in=supplier_ids).exists():
            ledger_supp_ins = LedgerPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__lt=ToDate,LedgerID__in=supplier_ids)
            ...
            supp_balance = float(ledger_supp_Debit_sum) - float(ledger_supp_Credit_sum)
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__lt=ToDate,LedgerID__in=customer_ids).exists():
            ledger_cust_ins = LedgerPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__lt=ToDate,LedgerID__in=customer_ids)
            ...
            cust_balance = float(ledger_cust_Debit_sum) - float(ledger_cust_Credit_sum)
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate,LedgerID__in=supplier_ids,VoucherType__in=Pay_voucherTypes).exists():
            ledger_supp_pay_ins = LedgerPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__gte=FromDate,Date__lt=ToDate,LedgerID__in=supplier_ids,VoucherType__in=Pay_voucherTypes)
            ledger_supp_pay_Debit_sum = ledger_supp_pay_ins.aggregate(Sum('Debit'))
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate,LedgerID__in=customer_ids,VoucherType__in=Pay_voucherTypes).exists():
            ledger_cust_pay_ins = LedgerPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__gte=FromDate,Date__lt=ToDate,LedgerID__in=customer_ids,VoucherType__in=Pay_voucherTypes)
            ledger_supp_pay_Credit_sum = ledger_cust_pay_ins.aggregate(Sum('Credit'))
...
def sales_grand_totals(request):
    ...
    for d in dates_list:
        ...
        if SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__month=month,Date__year=year).exists():
            sales_instances = SalesMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,Date__month=month,Date__year=year)
...
def sales_filterd_sales_order(request):
    ...
    if SalesOrderMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=OrderCustomerID,IsInvoiced="N").exists():
        sales_order_instances = SalesOrderMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=OrderCustomerID,IsInvoiced="N")
        if OrderFromDate and OrderToDate:
            sales_order_instances = sales_order_instances.filter(Date__gte=OrderFromDate,Date__lte=OrderToDate)
        if sales_order_instances:
            serialized = FilterOrderSerializer(sales_order_instances,many=True)
...
def get_order_details(request):
    ...
    if SalesOrderMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherNo__in=order_vouchers,IsInvoiced="N").exists():
        sales_order_instances = SalesOrderMaster.objects.filter(CompanyID=CompanyID,BranchID=BranchID,VoucherNo__in=order_vouchers,IsInvoiced="N")
        master_ids = sales_order_instances.values_list('SalesOrderMasterID',flat=True)
        sales_order_details = SalesOrderDetails.objects.filter(CompanyID=CompanyID,BranchID=BranchID,SalesOrderMasterID__in=master_ids)
