...
def get_auto_id(model,BranchID,CompanyID):
    ...
    if model.objects.filter(BranchID=BranchID,CompanyID=CompanyID).exists():
        max_value =  model.objects.filter(BranchID=BranchID,CompanyID=CompanyID).aggregate(Max('LeaveTypeID')) 

        if max_value:
            max_leaveType = max_value.get('LeaveTypeID__max', 0)
            
            LeaveTypeID = max_leaveType + 1
