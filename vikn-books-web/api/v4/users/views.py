...
from django.contrib.auth.models import User, Group
from api.v4.users.functions import get_auto_id
from brands import models
from api.v4.general.functions import get_current_role
...
from api.v4.users.functions import send_email,get_master_auto_id
...
from api.v4.users.serializers import CustomTokenSerializer, SoftwareVersionSerializer
...
def create_general_settings(request):
    ...
    # ======Uvais=====
    print(data['loyalty_taskList'],'loyalty_taskListloyalty_taskList')
    loyalty_point_value = ""
    for i in data['loyalty_taskList']:
        print(i["LoyaltyPoint"],'OPOPOPO')
        if i["LoyaltyPoint"] and i["LoyaltyValue"]:
            loyalty_point_value += str(i["LoyaltyPoint"])+str('-')+str(i["LoyaltyValue"]+str(','))
    ...
    if serialized.is_valid():
        ...
        is_Loyalty_SalesReturn_MinimumSalePrice = serialized.data['is_Loyalty_SalesReturn_MinimumSalePrice']
...
def get_defaults(request):
    ...
    if administrations_models.GeneralSettings.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        ...
        print(loyalty_taskList,"YUYYUYUYUYUYUYUYUYUYUYU")
        ...
        dic = {
            # "Free_Quantity_In_Sales": Free_Quantity_In_Sales,
            "loyalty_taskList":loyalty_taskList,
            "EnableLoyaltySettings":EnableLoyaltySettings,
            "Loyalty_Point_Expire":Loyalty_Point_Expire,
            "is_Loyalty_SalesReturn_MinimumSalePrice":is_Loyalty_SalesReturn_MinimumSalePrice,
            # "Free_Quantity_In_Purchase": Free_Quantity_In_Purchase,
