...
from api.v4.transactionTypes.serializers import TransactionTypesSerializer, TransactionTypesRestSerializer, ListSerializerByMasterName
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from api.v4.loyaltyCustomer.serializers import LoyaltyCustomerSerializer, LoyaltyCustomerRestSerializer
from api.v4.brands.serializers import ListSerializer
from api.v4.loyaltyCustomer.functions import generate_serializer_errors
from rest_framework import status
from api.v4.loyaltyCustomer.functions import get_auto_id, get_auto_DepartmentID
from main.functions import get_company, activity_log
import datetime
from api.v4.accountLedgers.serializers import AccountLedgerSerializer, AccountLedgerRestSerializer, LedgerReportSerializer, AccountLedgerListSerializer
        ...
        if not MobileNo_ok:
            if not CardNumber_ok:
                ...
                if TransactionTypes.objects.filter(pk=CardStatusID).exists():
                    card_status_instance = TransactionTypes.objects.get(pk=CardStatusID)
                ...
                if AccountLedger.objects.filter(pk=AccountLedgerID).exists():
                    sundry_debtor_instance = AccountLedger.objects.get(pk=AccountLedgerID)
              
                if not is_firstnameExist:
                    # serialized.save(BrandID=BrandID,CreatedDate=today,ModifiedDate=today)
                    LoyaltyCustomer.objects.create(
...
def card_types(request):
    ...
    if serialized1.is_valid():
        ...
        BranchID = serialized1.data['BranchID']
        MasterTypeID = MasterType.objects.get(BranchID=BranchID, CompanyID=CompanyID,Name="Loyalty Card Type").MasterTypeID
...
def card_status(request):
    ...
    if serialized1.is_valid():
        ...
        MasterTypeID = MasterType.objects.get(BranchID=BranchID, CompanyID=CompanyID,Name="Loyalty Card Status").MasterTypeID
        if TransactionTypes.objects.filter(BranchID=BranchID, CompanyID=CompanyID,MasterTypeID=MasterTypeID).exists():
            instances = TransactionTypes.objects.filter(
                BranchID=BranchID, CompanyID=CompanyID,MasterTypeID=MasterTypeID)
...
def sundry_debtors(request):
    ...
    if serialized1.is_valid():
        ...
        AccountGroupID = AccountGroup.objects.get(CompanyID=CompanyID,AccountGroupName="Sundry Debtors").AccountGroupID
