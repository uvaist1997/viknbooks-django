...
def get_auto_idMaster(model,BranchID,CompanyID):
    ...
    if model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        max_value =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).aggregate(Max('PurchaseOrderMasterID'))
...
def get_auto_id(model,BranchID,CompanyID):
    ...
    if model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
        max_value =  model.objects.filter(CompanyID=CompanyID,BranchID=BranchID).aggregate(Max('PurchaseOrderDetailsID'))
