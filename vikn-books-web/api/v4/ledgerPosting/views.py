...
from api.v4.ledgerPosting.serializers import LedgerPostingSerializer, LedgerPostingRestSerializer, ListSerializerforTrialBalance,\
...
from api.v4.stockPostings.serializers import StockPostingRestSerializer, StockValueAllSerializer, StockValueSingleSerializer
from api.v4.ledgerPosting.functions import generate_serializer_errors
from rest_framework import status
from api.v4.sales.serializers import ListSerializerforReport, ListSerializerforStockReport, ListSerializerforStockValueReport, ListSerializerforStockValueReportSingle
from api.v4.ledgerPosting.functions import get_auto_id, convertOrderdDict, get_VoucherName
import datetime
        ...
        if LedgerPosting.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate).exists():
            ...
            if Product.objects.filter(CompanyID=CompanyID, BranchID=BranchID, InventoryType="StockItem").exists():
                ...
                for pi in product_instances_OS:
                    ...
                    if StockPosting.objects.filter(ProductID=ProductID,BranchID=BranchID, Date__lt=FromDate, CompanyID=CompanyID,VoucherType__in=voucherTypes).exists():
                        stock_instances_new_os = StockPosting.objects.filter(ProductID=ProductID,BranchID=BranchID, Date__lt=FromDate, CompanyID=CompanyID,VoucherType__in=voucherTypes,QtyIn__gt=0)
                        TotalRateIn_new_os = stock_instances_new_os.aggregate(total_sum=Sum(F('QtyIn') * F('Rate')))
                        sum_QtyIn_os = stock_instances_new_os.aggregate(Sum('QtyIn'))
                        sum_QtyIn_os = sum_QtyIn_os['QtyIn__sum']
                        TotalQtyIn_new_os = StockPosting.objects.filter(ProductID=ProductID,BranchID=BranchID, Date__lt=FromDate, CompanyID=CompanyID).aggregate(Sum('QtyIn'))
                        TotalQtyOut_new_os = StockPosting.objects.filter(ProductID=ProductID,BranchID=BranchID, Date__lt=FromDate, CompanyID=CompanyID).aggregate(Sum('QtyOut'))
                        TotalQtyIn_new_os = TotalQtyIn_new_os['QtyIn__sum']
                        TotalQtyOut_new_os = TotalQtyOut_new_os['QtyOut__sum']
                        stock_new_os = float(TotalQtyIn_new_os) - float(TotalQtyOut_new_os)
                        
                        AvgRate_new = 0
                        if float(sum_QtyIn_os) > 0:
                            AvgRate_new = float(TotalRateIn_new_os['total_sum']) / float(sum_QtyIn_os)
                        TotalAvgValueOpening += (float(stock_new_os) * float(AvgRate_new))
                    ...
                    if StockPosting.objects.filter(ProductID=ProductID,BranchID=BranchID, Date__gte=FromDate, CompanyID=CompanyID,VoucherType__in=voucherTypes).exists():
                        stock_instances_new_os = StockPosting.objects.filter(ProductID=ProductID,BranchID=BranchID, Date__gte=FromDate, CompanyID=CompanyID,VoucherType__in=voucherTypes,QtyIn__gt=0)
                        TotalRateIn_new_os = stock_instances_new_os.aggregate(total_sum=Sum(F('QtyIn') * F('Rate')))
                        sum_QtyIn_os = stock_instances_new_os.aggregate(Sum('QtyIn'))
                        sum_QtyIn_os = sum_QtyIn_os['QtyIn__sum']
                        TotalQtyIn_new_os = StockPosting.objects.filter(ProductID=ProductID,BranchID=BranchID, Date__gte=FromDate, CompanyID=CompanyID).aggregate(Sum('QtyIn'))
                        TotalQtyOut_new_os = StockPosting.objects.filter(ProductID=ProductID,BranchID=BranchID, Date__gte=FromDate, CompanyID=CompanyID).aggregate(Sum('QtyOut'))
                        TotalQtyIn_new_os = TotalQtyIn_new_os['QtyIn__sum']
                        TotalQtyOut_new_os = TotalQtyOut_new_os['QtyOut__sum']
                        stock_new_os = float(TotalQtyIn_new_os) - float(TotalQtyOut_new_os)
            ...
            if Product.objects.filter(CompanyID=CompanyID, BranchID=BranchID, InventoryType="StockItem").exists():
                ...
                for pi in product_instances_CS:
                    ProductID = pi.ProductID
                    # new start
                    if StockPosting.objects.filter(ProductID=ProductID,BranchID=BranchID, Date__lte=ToDate, CompanyID=CompanyID,VoucherType__in=voucherTypes).exists():
                        stock_instances_new = StockPosting.objects.filter(ProductID=ProductID,BranchID=BranchID, Date__lte=ToDate, CompanyID=CompanyID,VoucherType__in=voucherTypes,QtyIn__gt=0)
                        TotalRateIn_new = stock_instances_new.aggregate(total_sum=Sum(F('QtyIn') * F('Rate')))
                        sum_QtyIn = stock_instances_new.aggregate(Sum('QtyIn'))
                        sum_QtyIn = sum_QtyIn['QtyIn__sum']
                        TotalQtyIn_new = StockPosting.objects.filter(ProductID=ProductID,BranchID=BranchID, Date__lte=ToDate, CompanyID=CompanyID).aggregate(Sum('QtyIn'))
                        TotalQtyOut_new = StockPosting.objects.filter(ProductID=ProductID,BranchID=BranchID, Date__lte=ToDate, CompanyID=CompanyID).aggregate(Sum('QtyOut'))
                        TotalQtyIn_new = TotalQtyIn_new['QtyIn__sum']
                        TotalQtyOut_new = TotalQtyOut_new['QtyOut__sum']
                        stock_new = float(TotalQtyIn_new) - float(TotalQtyOut_new)
                        
                        AvgRate_new = 0
                        if float(sum_QtyIn) > 0:
                            AvgRate_new = float(TotalRateIn_new['total_sum']) / float(sum_QtyIn)
                        TotalAvgValueClosing += (float(stock_new) * float(AvgRate_new))
                    # new end
