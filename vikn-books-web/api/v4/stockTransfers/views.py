...
from api.v4.stockTransfers.serializers import StockTransferMaster_IDSerializer, StockTransferMaster_IDRestSerializer,\
StockTransferDetailsSerializer, StockTransferDetailsRestSerializer,StockTransferRegisterReportSerializer
from api.v4.brands.serializers import ListSerializer
from api.v4.stockTransfers.functions import generate_serializer_errors
from rest_framework import status
from api.v4.stockTransfers.functions import get_auto_id, get_auto_idMaster
from api.v4.purchases.functions import get_auto_StockRateID, get_auto_StockTransID
from api.v4.sales.functions import get_auto_stockPostid, get_auto_VoucherNo
import datetime
...
def report_stockTransfer_register(request):
    ...
    if StockTransferMaster_ID.objects.filter(CompanyID=CompanyID, BranchID=BranchID,Date__gte=FromDate,Date__lte=ToDate).exists():
        ...
        if WareHouseFrom == 0 and WareHouseTo == 0:
            ...
            report_instance = instance.filter(WarehouseFromID=WareHouseFrom,WarehouseToID=WareHouseTo)
