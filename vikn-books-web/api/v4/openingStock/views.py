...
from api.v4.openingStock.serializers import OpeningStockMasterSerializer, OpeningStockMasterRestSerializer, OpeningStockDetailsSerializer, OpeningStockDetailsRestSerializer,OpeningStockFilterSerializer, OpeningStockReportSerializer
from api.v4.brands.serializers import ListSerializer
from api.v4.openingStock.functions import generate_serializer_errors
from rest_framework import status
from api.v4.openingStock.functions import get_auto_id, get_auto_idMaster
    ...
    if serialized1.is_valid():
        ...
        if filterValue == 2:
            if Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
                product_instances = Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID)
        else:
            if GroupID:
                if Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductGroupID=GroupID).exists():
                    product_instances = Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductGroupID=GroupID)
        ...
        if product_instances:
            ...
            serialized = OpeningStockFilterSerializer(party_sort_pagination, many=True, context={"CompanyID": CompanyID,"PriceRounding" : PriceRounding})
...
def openingStocks_xls_read(request):
    ...
    if serialized1.is_valid():
        ...
        if PriceList.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
            autobarcode_product_id_list = PriceList.objects.filter(CompanyID=CompanyID,BranchID=BranchID,AutoBarcode__in=AutoBarcode_list).values_list('ProductID', flat="True")

            if Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID).exists():
                product_instances = Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID__in=autobarcode_product_id_list)
