...
def sales_return_point(instance,product_id,TaxableAmount,BranchID,CompanyID,is_MinimumSalePrice):
    tot_taxable_amnt = 0
    if instance.ProductType == "All_product":
        ...
        products = Product.objects.filter(BranchID=BranchID,CompanyID=CompanyID,ProductGroupID__in=ProductGroupIDs)
        for i in products:
            if i:
                if i.ProductID == product_id:
                    print('All_product MONE')
        ...
        group_instance = ProductGroup.objects.filter(BranchID=BranchID,CompanyID=CompanyID,CategoryID__in=ProductCategoryIDs)
        ProductGroupIDs = []
