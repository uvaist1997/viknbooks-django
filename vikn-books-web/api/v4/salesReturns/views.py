from brands.models import SalesReturnMaster, SalesReturnMaster_Log, SalesReturnDetails, SalesReturnDetails_Log,\
    ...
    LoyaltyPoint,LoyaltyPoint_Log
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from api.v4.salesReturns.serializers import SalesReturnMasterSerializer, SalesReturnMasterRestSerializer,\
...
from api.v4.salesReturns.functions import generate_serializer_errors,sales_return_point
from rest_framework import status
from api.v4.salesReturns.functions import get_auto_id, get_auto_idMaster
from api.v4.sales.functions import get_auto_stockPostid,get_actual_point
    ...
    if is_SalesReturnOK:
        ...
        if is_LoyaltyCustomer:
            if LoyaltyProgram.objects.filter(BranchID=BranchID,CompanyID=CompanyID,FromDate__lte=today_date,ToDate__gte=today_date).exists():
                instance = LoyaltyProgram.objects.get(BranchID=BranchID,CompanyID=CompanyID,FromDate__lte=today_date,ToDate__gte=today_date)
                ...
                print(loyalty_customer,tot_taxable_amnt,"tot_TaxableAmount actual_point",actual_point)
                CurrentPoint = 0
                loyalty_instance = LoyaltyCustomer.objects.get(BranchID=BranchID,CompanyID=CompanyID,LoyaltyCustomerID=loyalty_customer.LoyaltyCustomerID)
                ...
                if float(CurrentPoint) >= float(actual_point):
                    ...
                    if actual_point:
                        LoyaltyPointID = get_point_auto_id(LoyaltyPoint, BranchID, CompanyID)
                        ExpiryDate = None
...
def edit_salesReturn(request, pk):
    ...
    if is_LoyaltyCustomer:
        if LoyaltyProgram.objects.filter(BranchID=BranchID,CompanyID=CompanyID,FromDate__lte=today_date,ToDate__gte=today_date).exists():
            ...
            CurrentPoint = 0
            loyalty_instance = LoyaltyCustomer.objects.get(BranchID=BranchID,CompanyID=CompanyID,LoyaltyCustomerID=loyalty_customer.LoyaltyCustomerID)
            if LoyaltyPoint.objects.filter(BranchID=BranchID,CompanyID=CompanyID,LoyaltyCustomerID=loyalty_customer.LoyaltyCustomerID,VoucherMasterID=SalesReturnMasterID,VoucherType=VoucherType).exists():
                loyalty_point_instance = LoyaltyPoint.objects.get(BranchID=BranchID,CompanyID=CompanyID,LoyaltyCustomerID=loyalty_customer.LoyaltyCustomerID,VoucherMasterID=SalesReturnMasterID,VoucherType=VoucherType)
            ...
            if float(CurrentPoint) >= float(actual_point):
                ...
                if actual_point:
                    ...
                    LoyaltyPoint.objects.create(
                        ...
                        ExpiryDate=ExpiryDate,
                        LoyaltyCustomerID=loyalty_customer,
                        LoyaltyProgramID=loyalty_instance,
                        is_Radeem=False,
