...
class SalesReturnMasterRestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SalesReturnMaster
        fields = ('id','LoyaltyCustomerID','SalesReturnMasterID','BranchID','Action','VoucherNo','VoucherDate','RefferenceBillNo',
    ...
    def get_LoyaltyCustomerID(self, instances):
        ...
        try:
            ...
            print(instances.LoyaltyCustomerID.LoyaltyCustomerID,"^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        except:
