...
from api.v8.openingStock.serializers import OpeningStockMasterSerializer, OpeningStockMasterRestSerializer, OpeningStockDetailsSerializer, OpeningStockDetailsRestSerializer, OpeningStockFilterSerializer, OpeningStockReportSerializer, OpeningStockMaster1RestSerializer
from api.v8.brands.serializers import ListSerializer
from api.v8.openingStock.functions import generate_serializer_errors
from rest_framework import status
from api.v8.openingStock.functions import get_auto_id, get_auto_idMaster
                ...
                for deleted_Data in deleted_datas:
                    ...
                    if not deleted_pk == '' or not deleted_pk == 0:
                        if OpeningStockDetails.objects.filter(CompanyID=CompanyID, pk=deleted_pk).exists():
                            deleted_detail = OpeningStockDetails.objects.filter(
