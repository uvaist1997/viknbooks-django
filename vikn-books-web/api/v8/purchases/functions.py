...
from brands.models import PurchaseReturnDetails, VoucherNoTable, State, PurchaseMaster, AccountLedger_Log, PurchaseMaster_Log, PurchaseDetails, PurchaseDetails_Log, StockPosting, LedgerPosting, StockPosting_Log,\
...
from api.v8.products.functions import get_auto_AutoBatchCode
...
def get_auto_idMaster(model, BranchID, CompanyID):
    ...
    if model.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        max_value = model.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID).aggregate(Max('PurchaseMasterID'))
...
def get_auto_id(model, BranchID, CompanyID):
    ...
    latest_auto_id = model.objects.filter(
        CompanyID=CompanyID).aggregate(Max('PurchaseDetailsID'))

    if model.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        max_value = model.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID).aggregate(Max('PurchaseDetailsID'))
...
def get_auto_StockRateID(model, BranchID, CompanyID):
    ...
    # latest_auto_id =  model.objects.filter(CompanyID=CompanyID).order_by("-CreatedDate")[:1]
    latest_auto_id = model.objects.filter(
        CompanyID=CompanyID).aggregate(Max('StockRateID'))

    if model.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        max_value = model.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID).aggregate(Max('StockRateID'))
...
def get_auto_StockTransID(model, BranchID, CompanyID):
    ...
    # latest_auto_id =  model.objects.filter(CompanyID=CompanyID).order_by("-CreatedDate")[:1]
    latest_auto_id = model.objects.filter(
        CompanyID=CompanyID).aggregate(Max('StockTransID'))

    if model.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
        max_value = model.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID).aggregate(Max('StockTransID'))
    ...
    for tx in tax_types:
        if PurchaseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate, TaxType=tx).exists():
            ...
            purchase_details = PurchaseDetails.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, PurchaseMasterID__in=purchase_ids)
        ...
        if PurchaseReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, VoucherDate__gte=FromDate, VoucherDate__lte=ToDate, TaxType=tx).exists():
            ...
            purchaseReturn_details = PurchaseReturnDetails.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, PurchaseReturnMasterID__in=purchaseReturn_ids)
            ...
            tax_list = purchaseReturn_details.values_list(
                'IGSTPerc', flat=True)
            tax_list = set(tax_list)
                ...
                if gouprd_details:
                    ...
                    for g in gouprd_details:
                        ...
                        VoucherDate = purchaseReturn_instances.get(
                            PurchaseReturnMasterID=PurchaseReturnMasterID).VoucherDate
                        InvoiceNo = purchaseReturn_instances.get(
                            PurchaseReturnMasterID=PurchaseReturnMasterID).RefferenceBillNo
                        LedgerID = purchaseReturn_instances.get(
                            PurchaseReturnMasterID=PurchaseReturnMasterID).LedgerID
                        ProductID = g.ProductID
                        party = ""
                        GSTN = ""
                        City = ""
...
def export_to_excel_purchase_gst(wb, data, FromDate, ToDate):
    ...
    columns = ['Date', 'Voucher No', 'Refference Bill No', 'Vender Invoice Date', 'Particulars', 'GSTIN/UIN',
               'Voucher Type', 'Tax Type', 'Taxable Amount', 'SGST', 'CGST', 'IGST', 'Cess', 'Total Tax Amount', 'Grand Total', ]
    row_num = 0
    # write column headers in sheet
...
def export_to_excel_purchase(wb, data, FromDate, ToDate, title):
    ws = wb.add_sheet("Purchase Report")

    columns = ['Voucher No', 'Voucher Date', 'Ledger Name',
               'Gross Amount', 'Total Tax', 'Bill Discount', 'Grand Total', ]
    # write column headers in sheet
    ...
    for j in data_list:
        ...
        ws.write(data_row, 3, float(TotalGrossAmt), value_decimal_style)
        ws.write(data_row, 4, float(TotalTax), value_decimal_style)
        ws.write(data_row, 5, float(BillDiscAmt), value_decimal_style)
        ws.write(data_row, 6, float(GrandTotal), value_decimal_style)
        data_row += 1
...
def purchase_excel_data(CompanyID, BranchID, FromDate, ToDate, PriceRounding, ReffNo):
    ...
    if ReffNo:
        if PurchaseMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate, RefferenceBillNo=ReffNo).exists():
            ...
            serialized_purchase = PurchaseMasterRest1Serializer(instances, many=True, context={
...
def export_to_excel_purchaseRetun(wb, data, FromDate, ToDate, title):
...
def purchaseReturn_excel_data(CompanyID, BranchID, FromDate, ToDate, PriceRounding, ReffNo):
    purchase_data = []
    purchaseReturn_data = []
    Total_netAmt_purchase = 0
    Total_GrossAmt_purchase = 0
...
def SetBatch(CompanyID, check_EnableProductBatchWise, check_BatchCriteria, BranchID, ProductID, PriceListID,SalesPrice,Batch_purchasePrice,
    ExpiryDate,ManufactureDate,qty_batch,WarehouseID,today,CreatedUserID,BatchCode,action,VoucherType):
    message = ""
    if check_EnableProductBatchWise == "True" or check_EnableProductBatchWise == True:
        if CompanyID.business_type.Name == "Pharmacy":
            if action == "create":
                if table.Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exists():
                    if table.Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exclude(ProductID=ProductID).exists():
                        message = "Batch Already exist with some other Product"
                    elif table.Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exclude(PriceListID=PriceListID).exists():
                        message = "Batch Already exist with some other unit"
                    elif table.Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exclude(ManufactureDate=ManufactureDate).exists():
                        message = "Batch Already exist with some other ManufactureDate"
                    elif table.Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exclude(ExpiryDate=ExpiryDate).exists():
                        message = "Batch Already exist with some other ExpiryDate"
                    else:
                        if table.Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode,PurchasePrice=Batch_purchasePrice).exists():
                            batch_ins = table.Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode,PurchasePrice=Batch_purchasePrice).first()
                            current_stock = batch_ins.StockIn
                            batch_ins.StockIn = float(current_stock) + float(qty_batch)
                            batch_ins.SalesPrice = SalesPrice
                            batch_ins.save()
                        else:
                            message = "Batch Already exist with some other Purchase Price"
                else:
                    # BatchCode = get_auto_AutoBatchCode(
                ...
                if table.Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exists():
                    batch = table.Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).first()
                    if not table.StockPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchID=BatchCode).exclude(VoucherType="PI").exists():
                        current_stock = batch.StockIn
                        batch.StockIn = float(current_stock) + float(qty_batch)
            ...
            if check_BatchCriteria == "PurchasePrice":
                if table.Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID, PriceListID=PriceListID, PurchasePrice=Batch_purchasePrice).exists():
                ...
                if table.Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID, PriceListID=PriceListID, PurchasePrice=Batch_purchasePrice, ExpiryDate=ExpiryDate).exists():
                    batch_ins = table.Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID,
                                                     PriceListID=PriceListID, PurchasePrice=Batch_purchasePrice, ExpiryDate=ExpiryDate).last()
                    StockIn = batch_ins.StockIn
                    BatchCode = batch_ins.BatchCode
                    SalesPrice = batch_ins.SalesPrice
