...
from api.v8.accountGroups.serializers import AccountGroupSerializer, AccountGroupRestSerializer
from api.v8.accountGroups.functions import generate_serializer_errors
from rest_framework import status
from api.v8.accountGroups.functions import get_auto_id
...
def create_accountGroup(request):
    ...
    if serialized.is_valid():
        ...
        if not AccountGroup.objects.filter(CompanyID=CompanyID,AccountGroupName=AccountGroupName):
            ...
            activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'AccountGroup',
                         'Create', 'AccountGroup created successfully.', 'AccountGroup saved successfully.')
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'AccountGroup', 'Create',
                     'AccountGroup field not valid.', generate_serializer_errors(serialized._errors))
...
def edit_accountGroup(request, pk):
    ...
    if AccountGroup.objects.filter(pk=pk).exists():
        ...
        if serialized.is_valid():
            ...
            #request , company, log_type, user, source, action, message, description
            activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'AccountGroup',
                         'Edit', 'AccountGroup Updated successfully.', 'AccountGroup saved successfully.')
            ...
            #request , company, log_type, user, source, action, message, description
            activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'AccountGroup', 'Edit',
                         'AccountGroup Updated Failed.', generate_serializer_errors(serialized._errors))
            response_data = {
                "StatusCode": 6001,
                "message": generate_serializer_errors(serialized._errors)
            }
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'AccountGroup',
                     'Edit', 'AccountGroup Updated Failed.', 'AccountGroup Not Found!.')
        response_data = {
            "StatusCode": 6001,
...
def accountGroups(request):
    ...
    if page_number and items_per_page:
        instances = AccountGroup.objects.filter(
            CompanyID=CompanyID).exclude(AccountGroupID__in=exclude_item).order_by('-AccountGroupID')
        count = len(instances)
        ...
        ledger_sort_pagination = AccountGroup.objects.filter(
            CompanyID=CompanyID).exclude(AccountGroupID__in=exclude_item).order_by('-AccountGroupID')
    ...
    activity_log(request._request, CompanyID, 'Information', CreatedUserID,
                 'AccountGroup', 'View', 'AccountGroup List', 'The user viewed AccountGroups')
    response_data = {
        "StatusCode": 6000,
        "data": serialized.data,
        "count": count,
...
def search_accountGroups(request):
    ...
    if serialized1.is_valid():
        ...
        if AccountGroup.objects.filter(CompanyID=CompanyID).exists():
            instances = AccountGroup.objects.filter(
                CompanyID=CompanyID).exclude(AccountGroupID__in=exclude_item)
            if length < 3:
                if param == "name":
                    instances = instances.filter(
                        (Q(AccountGroupName__icontains=product_name)))[:10]
                elif param == "code":
                    instances = instances.filter(
                        (Q(GroupCode__icontains=product_name)))[:10]
                else:
                    instances = instances.filter(
                        (Q(AccountGroupUnder__in=product_name)))[:10]
            else:
                if param == "name":
                    instances = instances.filter(
            ...
            serialized = AccountGroupRestSerializer(instances, many=True, context={
...
def accountGroup(request, pk):
    ...
    if AccountGroup.objects.filter(pk=pk, CompanyID=CompanyID).exists():
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'AccountGroup',
                     'View', 'AccountGroup Single page', 'The user viewed AccountGroup')
        response_data = {
            "StatusCode": 6000,
            "data": serialized.data
        }
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'AccountGroup',
                     'View', 'AccountGroup Single page', 'AccountGroup Not Found!')
        response_data = {
            "StatusCode": 6001,
            "message": "Account Group Not Found!"
        }
...
def delete_accountGroup(request, pk):
    ...
    if AccountGroup.objects.filter(pk=pk, CompanyID=CompanyID).exists():
        ...
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Information', CreatedUserID, 'AccountGroup',
                     'Delete', 'AccountGroup Deleted', 'AccountGroup Deleted Successfully!')
    else:
        #request , company, log_type, user, source, action, message, description
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'AccountGroup', 'Delete',
                     'AccountGroup Deleted failed', 'AccountGroup Deleted failed,AccountGroup Not Found!')
        response_data = {
            "StatusCode": 6001,
            "title": "Failed",
            "message": "Account Group Not Found!"
