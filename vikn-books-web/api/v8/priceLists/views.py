...
from api.v8.priceLists.serializers import PriceListSerializer, PriceListRestSerializer, ListSerializer
from api.v8.priceLists.functions import generate_serializer_errors
from rest_framework import status
from api.v8.priceLists.functions import get_auto_id
from api.v8.products.functions import get_auto_AutoBarcode
import datetime
    ...
    if serialized1.is_valid():
        ...
        if PriceList.objects.filter(CompanyID=CompanyID, BranchID=BranchID, ProductID=ProductID).exists():
            ...
            if Product.objects.filter(CompanyID=CompanyID,ProductID=ProductID,BranchID=BranchID).exists():
                is_inclusive = Product.objects.get(CompanyID=CompanyID,ProductID=ProductID,BranchID=BranchID).is_inclusive
            ...
            response_data = {
                ...
                "Tax1_SalesTax": float(Tax1_SalesTax),
                "Tax2_PurchaseTax": float(Tax2_PurchaseTax),
                "Tax2_SalesTax": float(Tax2_SalesTax),
                "Tax3_PurchaseTax": float(Tax3_PurchaseTax),
                "Tax3_SalesTax": float(Tax3_SalesTax),
    ...
    if PriceList.objects.filter(CompanyID=CompanyID, PriceListID=PriceListID, BranchID=BranchID).exists():
        ...
        if StockPosting.objects.filter(CompanyID=CompanyID, PriceListID=PriceListID, BranchID=BranchID, WareHouseID=WarehouseID).exists():
            if Date:
                ...
                CompanyID=CompanyID, PriceListID=PriceListID, BranchID=BranchID, WareHouseID=WarehouseID,Date__lte=Date)
...
def single_priceList_batch(request):
    ...
    if Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exists():
        batch_ins = Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).first()
        batch_pricelistID = batch_ins.PriceListID
        batch_salesPrice = batch_ins.SalesPrice
        batch_purchasePrice = batch_ins.PurchasePrice
        if PriceList.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PriceListID=batch_pricelistID).exists():
            batch_MultiFactor = PriceList.objects.get(CompanyID=CompanyID,BranchID=BranchID,PriceListID=batch_pricelistID).MultiFactor

    if PriceList.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PriceListID=PriceListID).exists():
        price_ins = PriceList.objects.get(CompanyID=CompanyID,BranchID=BranchID,PriceListID=PriceListID)
    ...
    if float(batch_MultiFactor) < float(unit_MultiFactor):
        SalesPrice = float(batch_salesPrice) * float(unit_MultiFactor)
        PurchasePrice = float(batch_purchasePrice) * float(unit_MultiFactor)
    elif float(batch_MultiFactor) > float(unit_MultiFactor):
        SalesPrice = float(batch_salesPrice) / float(batch_MultiFactor)
        PurchasePrice = float(batch_purchasePrice) / float(batch_MultiFactor)
    else:
