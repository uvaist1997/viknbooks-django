...
class MyCompaniesSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('CompanyName', 'id', 'company_type', 'ExpiryDate', 'Permission',
                  "CompanyLogo", "Edition", "IsPosUser", "StateName", "Email")
    ...
    def get_Edition(self, instances):
        if UserTable.objects.get(pk=instances.pk).CompanyID.Edition:
            Edition = UserTable.objects.get(
                pk=instances.pk).CompanyID.Edition
            return Edition
        else:
            return ""
    ...
    def get_Email(self, instances):
        if UserTable.objects.get(pk=instances.pk).CompanyID.Email:
            Email = UserTable.objects.get(
                pk=instances.pk).CompanyID.Email
            return Email
        else:
            return ""
...
class GeneralSettingsSerializer(serializers.Serializer):
    ...
    # EnableBranch = serializers.BooleanField()
