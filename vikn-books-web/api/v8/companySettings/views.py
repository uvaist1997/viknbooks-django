from brands.models import UserRoleSettingsModel,CancelReasons, Edition, CompanySettings, CompanySettings_Log, Designation, Department, AccountLedger, TransactionTypes, Warehouse, UserType, AccountGroup, Bank, Branch, Brand, Flavours, Color, GeneralSettings, MasterType, PriceCategory, ProductCategory, ProductGroup, Route, TaxCategory, TaxType, Country, Unit, FinancialYear, FinancialYear_Log, Unit, State, BusinessType, UserTable, Customer, User_Log
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from api.v8.companySettings.serializers import CompanySettingsRestSerializer, CompanySettingsInitialSerializer, FinancialYearSerializer
from api.v8.companySettings.functions import generate_serializer_errors
from rest_framework import status
from api.v8.companySettings.serializers import CompanySettingsSerializer, EditionSerializer
from administrations.functions import get_auto_id_financial_year
from django.shortcuts import get_object_or_404
from django.contrib.auth.models import User
from main.functions import get_company, activity_log
from api.v8.settings.functions import create_or_update_user_type_settings, get_user_type_settings
import datetime
import json
import os
from django.contrib.auth import authenticate
from api.v8.general.functions import isValidMasterCardNo
from api.v8.settings.functions import create_or_update_user_type_settings
    ...
    if serialized.is_valid() and serialized1.is_valid():
        ...
        if is_Gstn_validate:
            ...
            with open(path, 'r+') as json_file:
                ...
                CompanyID = company.id
                CompanyID = get_company(CompanyID)
                usertype = UserType.objects.filter(CompanyID=CompanyID,UserTypeName="Admin").first()
                for i in dic:
...
def edit_companySettings(request):
    ...
    if serialized.is_valid():
        ...
        if is_Gstn_validate:
            ...
            serialized = CompanySettingsRestSerializer(instance, many=False, context={"request": request})
            data = serialized.data
            response_data = {
                "StatusCode": 6000,
                "data": data
...
def delete_companyPermanantly(request, pk):
    ...
    if today.date() > new_date.date():
        ...
        if models.TaxCategory_Log.objects.filter(CompanyID=instance).exists():
            instances = models.TaxCategory_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()

        if models.ProductCategory.objects.filter(CompanyID=instance).exists():
            instances = models.ProductCategory.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.ProductGroup_Log.objects.filter(CompanyID=instance).exists():
            instances = models.ProductGroup_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.PriceCategory_Log.objects.filter(CompanyID=instance).exists():
            instances = models.PriceCategory_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.AccountGroup_Log.objects.filter(CompanyID=instance).exists():
            instances = models.AccountGroup_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.AccountLedger_Log.objects.filter(CompanyID=instance).exists():
            instances = models.AccountLedger_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.MasterType_Log.objects.filter(CompanyID=instance).exists():
            instances = models.MasterType_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()

        if models.TransactionTypes.objects.filter(CompanyID=instance).exists():
            instances = models.TransactionTypes.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.GeneralSettings.objects.filter(CompanyID=instance).exists():
            instances = models.GeneralSettings.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.DamageStockMaster.objects.filter(CompanyID=instance).exists():
            instances = models.DamageStockMaster.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.Designation_Log.objects.filter(CompanyID=instance).exists():
            instances = models.Designation_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.Department_Log.objects.filter(CompanyID=instance).exists():
            instances = models.Department_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.FinancialYear_Log.objects.filter(CompanyID=instance).exists():
            instances = models.FinancialYear_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.JournalDetails.objects.filter(CompanyID=instance).exists():
            instances = models.JournalDetails.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.JournalMaster_Log.objects.filter(CompanyID=instance).exists():
            instances = models.JournalMaster_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.LedgerPosting_Log.objects.filter(CompanyID=instance).exists():
            instances = models.LedgerPosting_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.PaymentDetails.objects.filter(CompanyID=instance).exists():
            instances = models.PaymentDetails.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.PaymentMaster_Log.objects.filter(CompanyID=instance).exists():
            instances = models.PaymentMaster_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()

        if models.POSHoldDetails.objects.filter(CompanyID=instance).exists():
            instances = models.POSHoldDetails.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.POSHoldMaster_Log.objects.filter(CompanyID=instance).exists():
            instances = models.POSHoldMaster_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()

        if models.PurchaseDetails.objects.filter(CompanyID=instance).exists():
            instances = models.PurchaseDetails.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.PurchaseMaster.objects.filter(CompanyID=instance).exists():
            instances = models.PurchaseMaster.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.ReceiptDetails.objects.filter(CompanyID=instance).exists():
            instances = models.ReceiptDetails.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.ReceiptMaster_Log.objects.filter(CompanyID=instance).exists():
            instances = models.ReceiptMaster_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.SalesDetails_Log.objects.filter(CompanyID=instance).exists():
            instances = models.SalesDetails_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.SalesMaster_Log.objects.filter(CompanyID=instance).exists():
            instances = models.SalesMaster_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()

        if models.SalesOrderDetails.objects.filter(CompanyID=instance).exists():
            instances = models.SalesOrderDetails.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.SalesOrderMaster.objects.filter(CompanyID=instance).exists():
            instances = models.SalesOrderMaster.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.SalesReturnMaster.objects.filter(CompanyID=instance).exists():
            instances = models.SalesReturnMaster.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
        ...
        if models.StockPosting_Log.objects.filter(CompanyID=instance).exists():
            instances = models.StockPosting_Log.objects.filter(
                CompanyID=instance)
            for i in instances:
                i.delete()
