...
from api.v8.banks.serializers import BankSerializer, BankRestSerializer
from api.v8.brands.serializers import ListSerializer
from api.v8.banks.functions import generate_serializer_errors
from rest_framework import status
from api.v8.banks.functions import get_auto_Bankid, get_BankCode
from api.v8.accountLedgers.functions import get_auto_id, get_auto_LedgerPostid
import datetime
    ...
    if instances:
        for instance in instances:
            ...
            if AccountLedger.objects.filter(BranchID=BranchID, LedgerName=LedgerName, CompanyID=CompanyID).exists():
                ...
                LedgerID = account_ledger.LedgerID
                if not LedgerPosting.objects.filter(LedgerID=LedgerID, BranchID=BranchID, CompanyID=CompanyID).exclude(VoucherType="LOB").exists():
                    # if not LPInstances:
                    instance.delete()
