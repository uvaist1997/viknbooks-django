...
class BankRestSerializer(serializers.ModelSerializer):
    ...
    def get_as_on_date(self, instances):
        ...
        as_on_date = ""
        if AccountLedger.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerName=LedgerName).exists():
            as_on_date = AccountLedger.objects.get(CompanyID=CompanyID,BranchID=BranchID,LedgerName=LedgerName).as_on_date
        return as_on_date
