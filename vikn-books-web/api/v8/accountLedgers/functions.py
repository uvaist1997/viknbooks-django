...
def get_auto_id(model, BranchID, CompanyID):
    ...
    if model.objects.filter(BranchID=BranchID).exists():
        max_value = model.objects.filter(
            BranchID=BranchID, CompanyID=CompanyID).aggregate(Max('LedgerID'))
...
def get_auto_LedgerPostid(model, BranchID, CompanyID):
    ...
    if model.objects.filter(BranchID=BranchID, CompanyID=CompanyID).exists():
        max_value = model.objects.filter(
            BranchID=BranchID, CompanyID=CompanyID).aggregate(Max('LedgerPostingID'))
...
def get_auto_idfor_party(model, BranchID, CompanyID):
    ...
    if model.objects.filter(BranchID=BranchID, CompanyID=CompanyID).exists():
        max_value = model.objects.filter(
            BranchID=BranchID, CompanyID=CompanyID).aggregate(Max('PartyID'))
        if max_value:
    ...
    if action == 'create':
        if table.AccountLedger.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
            ...
            if not Amount:
                Amount = 0
            if CrOrDr == "Dr":
                updated_balance = converted_float(instance_balance) + converted_float(Amount)
            else:
                updated_balance = converted_float(instance_balance) - converted_float(Amount)
        ...
        if table.LedgerPosting.objects.filter(CompanyID=CompanyID, BranchID=BranchID, VoucherMasterID=VoucherMasterID, VoucherType=VoucherType).exists():
            ledger_ins = table.LedgerPosting.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, VoucherMasterID=VoucherMasterID, VoucherType=VoucherType)
            for l in ledger_ins:
                print(l.LedgerID, "<<<<<<<<<<<.>>>>>>>>>>>>")
                LedgerID = l.LedgerID
                print(LedgerID)
                ...
                print(balance)
                updated_balance = converted_float(instance_balance) - converted_float(balance)
                instance.Balance = updated_balance
                instance.save()
...
def get_shippingAddress(CompanyID, BranchID, LedgerID):
    ...
    if table.UserAdrress.objects.filter(CompanyID=CompanyID, Branch__BranchID=BranchID, Party__LedgerID=LedgerID, Type__in=types).exists():
        address_instances = table.UserAdrress.objects.filter(
            CompanyID=CompanyID, Branch__BranchID=BranchID, Party__LedgerID=LedgerID, Type__in=types)
        ...
        if table.Parties.objects.filter(CompanyID=CompanyID, BranchID=BranchID, LedgerID=LedgerID).exists():
            ...
            if Country_Shipping:
                country_Shipping = table.Country.objects.get(
                    id=Country_Shipping)
