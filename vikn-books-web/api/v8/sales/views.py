...
from brands.models import Activity_Log,UserAdrress, ExpenseMaster, PrintSettings, State, VoucherNoTable, QrCode, SalesMaster, SalesMaster_Log, SalesDetails, SalesDetails_Log, StockPosting, LedgerPosting,\
...
from api.v8.sales.serializers import GST_CDNR_Serializer, GST_B2B_Serializer, SalesEstimateForOrderSerializer, SalesMasterSerializer, SalesMasterRestSerializer, SalesMasterRest1Serializer, SalesDetailsSerializer,\
...
from api.v8.salesOrders.serializers import SalesOrderDetailsRestSerializer
from api.v8.purchases.serializers import PurchaseMasterRestSerializer, PurchasePrintSerializer
from api.v8.salesReturns.serializers import SalesReturnMasterRestSerializer, SalesReturnMasterReportSerializer
from api.v8.purchaseReturns.serializers import PurchaseReturnMasterRestSerializer, PurchaseReturnPrintSerializer
from api.v8.sales.functions import export_to_excel_sales, export_to_excel_sales_taxgroup, export_to_excel_salesReturn, export_to_excel_taxSummary, sales_excel_data, sales_gst_excel_data, export_to_excel_sales_gst, sales_taxgroup_excel_data, salesReturn_excel_data, set_LoyaltyCalculation, get_Program, edit_LoyaltyCalculation, generate_serializer_errors, get_month, get_stock_value, b2b_suppliers_excel, b2cl_excel, b2cs_excel, cdnr_excel, cdnur_excel, hsn_excel, default_b2cl_excel,\
    b2ba_excel, b2cla_excel, b2csa_excel, cdnra_excel, cdnura_excel, exp_excel, expa_excel, at_excel, ata_excel, atadj_excel, atadja_excel, exemp_excel, docs_excel, master_excel, taxSummary_InwardSupplies, taxSummary_OutwardSupplies, taxSummary_excel_data, vat_taxSummary_InwardSupplies, vat_taxSummary_OutwardSupplies, vat_zeroRated_taxSummary_InwardSupplies, vat_zeroRated_taxSummary_OutwardSupplies, zeroRated_taxSummary_InwardSupplies, zeroRated_taxSummary_OutwardSupplies

from rest_framework import status
from api.v8.sales.functions import get_auto_id, get_auto_idMaster, get_auto_stockPostid, get_date_list, gstr1_data, SetBatchInSales
from api.v8.accountLedgers.serializers import AccountLedgerListSerializer
...
from main.functions import get_company, activity_log, generateVoucherNoFunction
from api.v8.stockPostings.serializers import StockPostingSerializer, StockPostingRestSerializer, ExcessStockMasterSerializer, ShortageStockMasterSerializer, DamageStockMasterSerializer, UsedStockMasterSerializer, UsedStockDetailSerializer
from api.v8.companySettings.serializers import CompanySettingsRestSerializer, QRCompanySettingsRestSerializer
from api.v8.settings.serializers import QRPrintSettingsSerializer
...
from api.v8.purchaseOrders.serializers import PurchaseOrderPrintSerializer
from api.v8.sales.functions import get_Genrate_VoucherNo, taxSummary_OutwardSupplies
from django.db.models import Q, Prefetch, Sum
from api.v8.loyaltyProgram.functions import get_point_auto_id
...
from api.v8.sales.tasks import taskAPIfor_gstr1_report
from main.functions import update_voucher_table, extract_alphabet_from_string
...
def create_sale(request):
    try:
        with transaction.atomic():
            ...
            try:
                ...
                if table.VoucherNoGenerateTable.objects.filter(CompanyID=CompanyID, VoucherType=VoucherType, LastInvoiceNo__isnull=False).exists():
                    PreFix = table.VoucherNoGenerateTable.objects.filter(CompanyID=CompanyID, VoucherType=VoucherType).exclude(LastInvoiceNo__isnull=True).first().PreFix
                else:
                    PreFix = "SI"
                        ...
                        if ProductID:
                            ...
                            if product_is_Service == False:
                                ...
                                if check_EnableProductBatchWise == True or check_EnableProductBatchWise == "True":
                                    setbach = SetBatchInSales(CompanyID, BranchID, Batch_salesPrice, Qty_batch, BatchCode, check_AllowUpdateBatchPriceInSales,
                                                              PriceListID, "SI")
        ...
        activity_id = activity_log(request._request, CompanyID, 'Error', CreatedUserID, 'Sales',
        ...
        with transaction.atomic():
            ...
            if SalesDetails.objects.filter(CompanyID=CompanyID, BranchID=BranchID, SalesMasterID=SalesMasterID).exists():
                ...
                for i in sale_ins:
                    ...
                    instance_Qty = float(
                        instance_MultiFactor) * float(instance_qty_sum)
                    if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode).exists():
                        ...
                        batch_ins.StockOut = float(
                            StockOut) - float(instance_Qty)
                        batch_ins.save()
                    ...
                    if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=SalesMasterID, VoucherDetailID=i.SalesDetailsID, ProductID=i.ProductID, BranchID=BranchID, VoucherType="SI").exists():
                        stock_inst = StockPosting.objects.filter(
                            CompanyID=CompanyID, VoucherMasterID=SalesMasterID, VoucherDetailID=i.SalesDetailsID, ProductID=i.ProductID, BranchID=BranchID, VoucherType="SI").first()
                        stock_inst.QtyOut = float(
                            stock_inst.QtyOut) - float(instance_Qty)
                        stock_inst.save()
                        update_stock(CompanyID, BranchID, i.ProductID)
            ...
            # customer with partial cash /bank and customer with cash/bank
                ...
                update_ledger = UpdateLedgerBalance(
                    CompanyID, BranchID, BankID, SalesMasterID, VoucherType, BankAmount, "Cr", "create")
            # customer with partial cash /bank and customer with cash/bank
                    ...
                    if PriceList.objects.filter(CompanyID=CompanyID, ProductID=ProductID_Deleted, DefaultUnit=True).exists():
                        ...
                        if not deleted_pk == '' or not deleted_pk == 0:
                            if SalesDetails.objects.filter(CompanyID=CompanyID, pk=deleted_pk).exists():
                                ...
                                qty_batch = float(
                                    deleted_detail.FreeQty) + float(deleted_detail.Qty)
                                Qty_batch = float(
                                    MultiFactor) * float(qty_batch)
                                ...
                                if StockPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=SalesMasterID_Deleted, VoucherDetailID=SalesDetailsID_Deleted, ProductID=ProductID_Deleted, BranchID=BranchID, VoucherType="SI").exists():
                                    stock_instances_delete = StockPosting.objects.filter(
                                        CompanyID=CompanyID, VoucherMasterID=SalesMasterID_Deleted, VoucherDetailID=SalesDetailsID_Deleted, ProductID=ProductID_Deleted, BranchID=BranchID, VoucherType="SI")
                        ...
                        if product_is_Service == False:
                            if StockPosting.objects.filter(CompanyID=CompanyID, WareHouseID=WarehouseID, VoucherMasterID=SalesMasterID, VoucherDetailID=SalesDetailsID, BranchID=BranchID, VoucherType="SI", ProductID=ProductID).exists():
                                stock_instance = StockPosting.objects.filter(
                                    CompanyID=CompanyID, WareHouseID=WarehouseID, VoucherMasterID=SalesMasterID, VoucherDetailID=SalesDetailsID, BranchID=BranchID, VoucherType="SI", ProductID=ProductID).first()
    ...
    if serialized1.is_valid():
        ...
        if UserID:
            ...
            he = SalesMaster.objects.filter(
                CompanyID=CompanyID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate).exists()
            print(he, UserID)
    ...
    if serialized1.is_valid():
        ...
        if sales_data:
            count_divided_sales = math.ceil(
                float(count_sales) / items_per_page)
            count_divided_sales_return = math.ceil(
                float(count_sales_return) / items_per_page)
            # New Design function Start
            # sales_jsnDatas = convertOrderdDict(sales_data)
            sales_jsnDatas = sales_data
            ...
            response_data = {
                ...
                # "Total_netAmt_sale": round(Total_netAmt_sale, PriceRounding),
                # "Total_nsouetAmt_saleRetn": round(Total_netAmt_saleRetn, PriceRounding),
                # "Total_tax_sale": round(Total_tax_sale, PriceRounding),
                # "Total_tax_saleRetn": round(Total_tax_saleRetn, PriceRounding),
                # "Total_billDiscount_sale": round(Total_billDiscount_sale, PriceRounding),
                # "Total_billDiscount_saleRetn": round(Total_billDiscount_saleRetn, PriceRounding),
...
def sales_return_report(request):
    ...
    if serialized1.is_valid():
        ...
        if SalesReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, VoucherDate__gte=FromDate, VoucherDate__lte=ToDate).exists():
            ...
            serialized_salesReturn = SalesReturnMasterReportSerializer(sale_return_sort_pagination, many=True, context={"CompanyID": CompanyID, "PriceRounding": PriceRounding,
                                                                                                                        "FromDate": FromDate, "ToDate": ToDate})
            salesReturn_data = serialized_salesReturn.data
...
def sales_integrated_report(request):
    ...
    if serialized1.is_valid():
        ...
        tot_dic = {
            ...
            "BankSales": TotalBankSales,
            "NetSales": TotalNetSales,
            "CashSalesReturn": TotalCashSalesReturn,
            "DebitSalesRetrn": TotalDebitSalesRetrn,
            "BankSalesReturn": TotalBankSalesReturn,
            "NetSalesReturn": TotalNetSalesReturn,
...
def generate_printReport(request):
    from api.v8.sales.serializers import SalesPrintSerializer
    from api.v8.salesReturns.serializers import SalesReturnMasterPrintSerializer
    data = request.data
    CompanyID = data['CompanyID']
    CompanyID = get_company(CompanyID)
    CreatedUserID = data['CreatedUserID']
    ...
    if VoucherType == "SI":
        if SalesMaster.objects.filter(CompanyID=CompanyID, id=unq_id).exists():
            ...
            serialized = SalesPrintSerializer(instance, context={"CompanyID": CompanyID,"request": request,
                                                                 "PriceRounding": PriceRounding})
                ...
                if count_divided_datas == page_number:
                    ...
                    for i in total_jsnDatas:
                        ...
                        for t in check_array_for_totalStock:
                            ...
                            tot_CurrentStock += total_stock
                            tot_CurrentBaseStock += CurrentBaseStock
                ...
                if not SalesMasterID in unq_master_ids:
                    ...
                    for s in sales_gsts:
                        if not s in unq_sales_gsts:
                            ...
                            Total_invoice_value_b2b_suppliers += float(
                                GrandTotal)
                            Total_taxable_value_b2b_suppliers += float(
                                taxable_value)
                            Total_cess_value_b2b_suppliers += float(
                                CESS_amount)
        ...
        if SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, shipping_tax_amount__gt=0, Date__gte=FromDate, Date__lte=ToDate, TaxType__in=tax_types).exists():
            ...
            for i in jsnDatas:
                ...
                if not SalesMasterID in unq_master_ids:
                    ...
                    total_value = float(
                        s.shipping_tax_amount) + float(s.ShippingCharge)
                ...
                if not SalesReturnMasterID in unq_master_ids:
                    ...
                    for s in sales_gsts:
                        if not s in unq_sales_gsts and float(GrandTotal) >= 250000:
                            ...
                            Total_invoice_value_b2cl += float(GrandTotal)
                            Total_taxable_value_b2cl += float(
                                taxable_value) * -1
                            Total_cess_value_b2cl += float(CESS_amount)
...
def scanned_invoice(request):
    ...
    if VoucherType == "SI":
        if SalesMaster.objects.filter(id=Unq_id).exists():
            ...
            LedgerID = instance.LedgerID
            if Parties.objects.filter(CompanyID=CompanyID, LedgerID=LedgerID, BranchID=BranchID).exists():
                CRNo = Parties.objects.filter(
                    CompanyID=CompanyID, LedgerID=LedgerID, BranchID=BranchID).first().CRNo
                parties_VATNumber = Parties.objects.filter(
                    CompanyID=CompanyID, LedgerID=LedgerID, BranchID=BranchID).first().VATNumber
            sales_details = SalesDetails.objects.filter(
                CompanyID=CompanyID, SalesMasterID=SalesMasterID, BranchID=BranchID)

            for s in sales_details:
                ProductID = s.ProductID
                ProductName = Product.objects.filter(
                    CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID).first().ProductName
                ProductNameArabic = Product.objects.filter(
                    CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID).first().Description
                details.append({
                    "ProductName": ProductName,
        ...
        if SalesReturnMaster.objects.filter(id=Unq_id).exists():
            ...
            sales_return_details = SalesReturnDetails.objects.filter(
                CompanyID=CompanyID, SalesReturnMasterID=SalesReturnMasterID, BranchID=BranchID)
        ...
        if PurchaseReturnMaster.objects.filter(id=Unq_id).exists():
            ...
            purchase_return_details = PurchaseReturnDetails.objects.filter(
                CompanyID=CompanyID, PurchaseReturnMasterID=PurchaseReturnMasterID, BranchID=BranchID)
    ...
    if serialized1.is_valid():
        ...
        for tx in tax_types:
            if SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate, TaxType=tx).exists():
                ...
                sales_details = SalesDetails.objects.filter(
                    CompanyID=CompanyID, BranchID=BranchID, SalesMasterID__in=sales_ids).exclude(IGSTPerc=0)
...
def search_sales_list(request):
    ...
    if serialized1.is_valid():
        ...
        if SalesMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            ...
            serialized = SalesMasterRest1Serializer(instances, many=True, context={
...
def sales_report_excel(request):
    ...
    title = str(get_company(CompanyID).CompanyName) + \
        str(",")+str(FromDate)+str(" TO ")+str(ToDate)
    if Type == '1':
        # decide file name
        ...
        response['Content-Disposition'] = 'attachment; filename="Sales Return Report.xls"'
