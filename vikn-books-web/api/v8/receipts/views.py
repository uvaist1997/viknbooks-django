from brands.models import PaymentMaster,ReceiptMaster, ReceiptMaster_Log, ReceiptDetails, ReceiptDetails_Log,\
...
from api.v8.receipts.serializers import ReceiptPrintSerializer,ReceiptMasterSerializer, ReceiptMasterRestSerializer, ReceiptDetailsSerializer, ReceiptDetailsRestSerializer
from api.v8.payments.serializers import ListSerializer,PaymentPrintSerializer
from api.v8.receipts.functions import export_to_excel_receiptReport, generate_serializer_errors, get_voucher_type, receiptReport_excel_data
from rest_framework import status
from api.v8.receipts.functions import get_auto_id, get_auto_idMaster
            ...
            if is_ReceiptsOK:
                ...
                for receiptDetaillog in receiptDetailslogs:
                    ...
                    if float(Discount) > 0:
                        ...
                        update_ledger = UpdateLedgerBalance(
                            CompanyID, BranchID, 72, ReceiptMasterID, VoucherType, Discount, "Dr", "create")
...
def print_receipt(request):
    ...
    if voucher_type == "CR" or voucher_type == "BR":
        if ReceiptMaster.objects.filter(pk=voucher_id).exists():
            ...
            serialized = ReceiptPrintSerializer(instance, context={"CompanyID": CompanyID, "PriceRounding": PriceRounding})
            data = serialized.data
        ...
        if PaymentMaster.objects.filter(pk=voucher_id).exists():
            ...
            serialized = PaymentPrintSerializer(instance, context={"CompanyID": CompanyID, "PriceRounding": PriceRounding})
            data = serialized.data
    if data:
        company_instance = CompanySettings.objects.get(pk=CompanyID.id)
        company_serialized = CompanySettingsRestSerializer(
