...
from api.v8.salesReturns.serializers import SalesReturnMasterSerializer, SalesReturnMasterRestSerializer,\
...
from api.v8.salesReturns.functions import generate_serializer_errors, sales_return_point
from rest_framework import status
from api.v8.salesReturns.functions import get_auto_id, get_auto_idMaster
from api.v8.sales.functions import get_auto_stockPostid, get_actual_point
...
from api.v8.sales.functions import get_Genrate_VoucherNo,SetBatchInSales
    ...
    try:
        with transaction.atomic():
            ...
            if is_SalesReturnOK:
                ...
                invoice_date = sales_return_instance.CreatedDate.strftime("%d-%b-%y %I:%M:%S %p")
                fatoora_obj = Fatoora(
...
def delete_salesReturnMaster(request, pk):
    ...
    instances = None
    if selecte_ids:
        if SalesReturnMaster.objects.filter(CompanyID=CompanyID, pk__in=selecte_ids).exists():
        ...
        if SalesReturnMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID).exists():
            if length < 3:
                ...
                instances = instances.filter((Q(VoucherNo__icontains=product_name)) | (
                    Q(VoucherDate__icontains=product_name))| (
                    Q(CustomerName__icontains=product_name)))
            serialized = SalesReturnMaster1RestSerializer(instances, many=True, context={
