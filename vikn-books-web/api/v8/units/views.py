from brands.models import Unit, Unit_Log, Product, PriceList,UQCTable
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from api.v8.units.serializers import UnitSerializer, UnitRestSerializer, ListSerializer
from api.v8.units.functions import generate_serializer_errors, get_auto_id
...
def delete_selected(request):
    ...
    for s in units:
        if s.UnitID == "1":
            ...
            if not PriceList.objects.filter(BranchID=BranchID, UnitID=s.UnitID, CompanyID=CompanyID).exists():
                BranchID = s.BranchID
                UnitID = s.UnitID
                UnitName = s.UnitName
                Notes = s.Notes
