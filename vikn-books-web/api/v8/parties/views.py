from brands.models import Branch,UserAdrress,Parties, Parties_Log, AccountLedger, AccountLedger_Log, LedgerPosting, LedgerPosting_Log
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes
from rest_framework.response import Response
from rest_framework.permissions import AllowAny, IsAuthenticated
from api.v8.parties.serializers import PartiesSerializer, PartiesRestSerializer, PartyListSerializer,UserAdrressSerializer
# from api.v8.accountLedgers.serializers import AccountLedgerSerializer
# from api.v8.brands.serializers import ListSerializer
from api.v8.parties.functions import generate_serializer_errors
from rest_framework import status
from api.v8.parties.functions import get_auto_id, get_auto_idLedger, get_PartyCode
from api.v8.accountLedgers.functions import get_auto_LedgerPostid,get_shippingAddress
from main.functions import get_company, activity_log
from api.v8.general.functions import isValidMasterCardNo
from api.v8.parties.tasks import import_party_task
from api.v8.parties.utils import in_memory_file_to_temp
...
from api.v8.accountLedgers.functions import get_LedgerCode, get_auto_id as generated_ledgerID
...
def create_party(request):
    try:
            ...
            if party_serialized.is_valid():
                ...
                if is_Gstn_validate:
                    ...
                    if not is_nameExist:
                        ...
                        if State_Shipping:
                            State_Shipping = model.State.objects.get(id=State_Shipping)
                        if Country_Shipping:
                            Country_Shipping = model.Country.objects.get(id=Country_Shipping)
        ...
        activity_log(request._request, CompanyID, 'Error', CreatedUserID, party_type,
        ...
        with transaction.atomic():
            ...
            if LedgerPosting.objects.filter(CompanyID=CompanyID, VoucherMasterID=LedgerID, BranchID=BranchID, VoucherType="LOB").exists():
                ledgerPostInstances = LedgerPosting.objects.filter(
                    CompanyID=CompanyID, VoucherMasterID=LedgerID, BranchID=BranchID, VoucherType="LOB")
                ...
                if is_Gstn_validate:
                    ...
                    if party_ok:
                        ...
                        if instanceCrOrDr == "Cr":
                            instanceOpeningBalance = float(instanceOpeningBalance) * -1
                        elif instanceCrOrDr == "Dr":
                            instanceOpeningBalance = instanceOpeningBalance

                        latest_balance = float(instanceBalance) - float(instanceOpeningBalance)
                    ...
                    if UserAdrress.objects.filter(CompanyID=CompanyID,Branch=branch,Party=party_instance,Type="BillingAddress").exists():
                        userAddress_bill = UserAdrress.objects.filter(CompanyID=CompanyID,Branch=branch,Party=party_instance,Type="BillingAddress").first()
                    ...
                    if UserAdrress.objects.filter(CompanyID=CompanyID,Branch=branch,Party=party_instance,Type="ShippingAddress").exists():
                        userAddress_ship = UserAdrress.objects.filter(CompanyID=CompanyID,Branch=branch,Party=party_instance,Type="ShippingAddress").first()
                    ...
                    if BankDetails:
                        ...
                        for b in BankDetails:
                            if b['detailID'] == 1:
                                ...
                                bank_detail_instance = model.PartyBankDetails.objects.get(id=b['id'])
...
def search_party_list(request):
    ...
    if serialized1.is_valid():
        BranchID = serialized1.data['BranchID']
        if Parties.objects.filter(CompanyID=CompanyID, BranchID=BranchID,PartyType=PartyType).exists():
            if length < 3:
                instances = Parties.objects.filter(
                    CompanyID=CompanyID, BranchID=BranchID,PartyType=PartyType)
