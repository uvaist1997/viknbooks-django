...
from brands.models import VoucherNoTable, SalesOrderMaster, SerialNumbers, SalesOrderMaster_Log, SalesOrderDetails, SalesOrderDetails_Log, GeneralSettings
from rest_framework.renderers import JSONRenderer
from rest_framework.decorators import api_view, permission_classes, renderer_classes
from rest_framework.response import Response
from rest_framework.permissions import IsAuthenticated, IsAuthenticated
from api.v8.salesOrders.serializers import SalesOrderMasterSerializer, SalesOrderMasterRestSerializer, SalesOrderDetailsSerializer, SalesOrderDetailsRestSerializer, SalesOrderMaster1RestSerializer, SalesOrderReportSerializer
from api.v8.brands.serializers import ListSerializer
from api.v8.salesOrders.functions import generate_serializer_errors
from rest_framework import status
from api.v8.salesOrders.functions import get_auto_id, get_auto_idMaster
import datetime
...
def edit_salesOrder(request, pk):
    try:
        with transaction.atomic():
            ...
            if SerialNumbers.objects.filter(CompanyID=CompanyID, SalesMasterID=SalesOrderMasterID, BranchID=BranchID, VoucherType="SO").exists():
                SerialNumbersInstances = SerialNumbers.objects.filter(
                    CompanyID=CompanyID, SalesMasterID=SalesOrderMasterID, BranchID=BranchID, VoucherType="SO")
            ...
            for salesOrderDetail in salesOrderDetails:
                ...
                if ProductID:
                    ...
                    # ======
                    print(
                        SerialNos, "PPPPPPPPPPPPPPPP*************************PPPPPPPPPPPPPPPPPPP")
...
def report_salesOrder(request):
    ...
    print(test)
    if SalesOrderMaster.objects.filter(CompanyID=CompanyID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate, IsInvoiced__in=ReportTypes).exists():
        instance = SalesOrderMaster.objects.filter(
            CompanyID=CompanyID, BranchID=BranchID, Date__gte=FromDate, Date__lte=ToDate, IsInvoiced__in=ReportTypes)

        serialized = SalesOrderReportSerializer(instance, many=True, context={"CompanyID": CompanyID,
                                                                              "PriceRounding": PriceRounding, })
