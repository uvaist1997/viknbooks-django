...
from brands.models import Batch,LedgerPosting,Parties,QrCode,State,Country,PurchaseReturnMaster, PurchaseReturnDetails, AccountLedger, Product, PriceList, Warehouse, Unit, TaxCategory
...
class PurchaseReturn1MasterRestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'PurchaseReturnMasterID', 'VoucherNo', 'VoucherDate', 'RefferenceBillNo',
                  ...
                  'LedgerID', 'LedgerName', 'TotalTax', 'GrandTotal','TotalGrossAmt_Rounded')
...
class PurchaseReturnMasterRestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PurchaseReturnMaster
        fields = ('id','Attention','PrintAddress','PrintCustomerName','Tax_no','CRNo','Mobile','City','State','Country','PostalCode','GrandTotal_print','NetTotal_print','GrossAmt_print','TotalDiscount_print','TotalTax_print','TotalTaxableAmount_print','VATAmount_print','GSTNumber','qr_image','TotalDiscount_print', 'TotalGrossAmt_print','PurchaseReturnMasterID', 'BranchID', 'Action', 'VoucherNo', 'VoucherDate', 'RefferenceBillNo',
                  ...
                  'ProductList','Balance','SGST_final_list','IGST_final_list')
    ...
    def get_SGST_final_list(self, instances):
        ...
        SGST_final_list = func.GST_finalList_fun(instances,purchase_details,"SGST",PriceRounding,"PurchaseReturn")
    ...
    def get_IGST_final_list(self, instances):
        ...
        purchase_details = PurchaseReturnDetails.objects.filter(
            CompanyID=CompanyID, PurchaseReturnMasterID=PurchaseReturnMasterID, BranchID=BranchID)
        IGST_final_list = func.GST_finalList_fun(instances,purchase_details,"IGST",PriceRounding,"PurchaseReturn")
    ...
    def get_ProductList(self, instances):
        ...
        produc_instances = Product.objects.filter(CompanyID=CompanyID,BranchID=BranchID,ProductID__in=product_ids)
        ProductList = []
    ...
    def get_Balance(self, instances):
        ...
        BranchID = instances.BranchID
        Balance = 0
        if LedgerPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=LedgerID).exists():
            ledgers = LedgerPosting.objects.filter(CompanyID=CompanyID,BranchID=BranchID,LedgerID=LedgerID)
    ...
    def get_qr_image(self, instances):
        ...
        serialized = QRCodeSerializer(qr_code,  context={"CompanyID": CompanyID,"request": request })
...
class PurchaseReturnDetailsRestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PurchaseReturnDetails
        fields = ('id','TaxableAmount_print','DiscountAmount_print','NetAmount_print','VATAmount_print','VATPerc_print', 'NetAmount_print','unitPrice_print','PurchaseReturnDetailsID', 'BranchID', 'Action', 'PurchaseReturnMasterID', 'DeliveryDetailsID', 'OrderDetailsID', 'ProductID',
                  ...
                  'PurchasePrice','GST_Inclusive','Vat_Inclusive','ProductTaxName','ActualProductTaxName','ActualProductTaxID',"product_description",
                  'BatchList','ManufactureDatePrint','ExpiryDatePrint')
    ...
    def get_ManufactureDatePrint(self, sales_details):
        ...
        if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode).exists():
            batch_details = Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode).first()
    ...
    def get_product_description(self, purchaseReturn_details):
        ...
        if Product.objects.filter(CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID).exists():
            Description = Product.objects.get(CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID).Description
...
class PurchaseReturnPrintSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('PurchaseReturnMasterID', 'BranchID', 'Action', 'VoucherNo', 'VoucherDate', 'RefferenceBillNo',
                  ...
                  'LedgerID', 'LedgerName', 'PriceCategoryID', 'EmployeeID', 'PurchaseAccount', 'PurchaseAccountName', 'DeliveryMasterID', 'OrderMasterID', 'CustomerName',
                  'Address1', 'Address2', 'Address3', 'Notes', 'FinacialYearID', 'TotalGrossAmt', 'TotalTax','TaxNo',
                  'NetTotal', 'AdditionalCost', 'GrandTotal', 'RoundOff', 'WarehouseID', 'IsActive', 'CreatedDate', 'UpdatedDate', 'CreatedUserID',
                  'TaxID', 'TaxType', 'VATAmount', 'SGSTAmount', 'CGSTAmount', 'IGSTAmount', 'TAX1Amount', 'TAX2Amount', 'TAX3Amount',
