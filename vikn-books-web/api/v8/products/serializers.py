...
from api.v8.priceLists.serializers import PriceListRestSerializer, SinglePriceListRestSerializer
from api.v8.workOrder.serializers import Batch_ListSerializer, UnitListSerializer
...
class ProductSearchSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ('id','Description', 'ProductID','ProductCode','Barcode','ProductName','Stock','HSNCode','PurchasePrice','SalesPrice'
            ,'is_inclusive','BatchCode')
...
class ProductSearchInvoiceSerializer(serializers.ModelSerializer):
    ...
    def get_DefaultSalesPrice(self, instances):
        ...
        return str(get_DefaultSalesPrice)
    ...
    def get_DefaultPurchasePrice(self, instances):
        ...
        return str(DefaultPurchasePrice)
...
class BarCodeSearchSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'ProductID', 'BranchID', 'DefaultUnitName', 'DefaultUnitID', 'DefaultSalesPrice','ProductCode','HSNCode',
                  ...
                  'GST_SalesTax', 'Tax3_SalesTax', 'Tax2_SalesTax', 'Tax1_SalesTax', 'SalesTax', 'Stock','VatID',
...
class BatchCodeSearchSerializer(serializers.ModelSerializer):
    ...
    def get_PriceListID(self, instances):
        ...
        if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode).exists():
            PriceListID = Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode).first().PriceListID
        else:
            PriceListID = PriceList.objects.get(
                CompanyID=CompanyID, ProductID=ProductID, BranchID=BranchID, DefaultUnit=True).PriceListID
        return PriceListID
    ...
    def get_Stock(self, instances):
        ...
        if Batch.objects.filter(CompanyID=CompanyID, BranchID=BranchID, BatchCode=BatchCode).exists():
            ...
            StockIn = float(batch_StockIn) / float(batch_MultiFactor)
            StockOut = float(batch_StockOut) / float(batch_MultiFactor)
            Stock = float(StockIn) - \
                float(StockOut)
    ...
    def get_SalesPrice(self,instances):
        ...
        SalesPrice = 0
        if Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exists():
            SalesPrice = Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).first().SalesPrice
    ...
    def get_PurchasePrice(self,instances):
        ...
        PurchasePrice = 0
        if Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exists():
            PurchasePrice = Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).first().PurchasePrice
    ...
    def get_BarCode(self,instances):
        ...
        if Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exists():
            PriceListID = Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).first().PriceListID
            BarCode = PriceList.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PriceListID=PriceListID).first().Barcode
    ...
    def get_MRP(self,instances):
        ...
        if Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exists():
            PriceListID = Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).first().PriceListID
            MRP = PriceList.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PriceListID=PriceListID).first().MRP
    ...
    def get_SalesPrice1(self,instances):
        ...
        if Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exists():
            PriceListID = Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).first().PriceListID
            SalesPrice1 = PriceList.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PriceListID=PriceListID).first().SalesPrice1
    ...
    def get_SalesPrice2(self,instances):
        ...
        if Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exists():
            PriceListID = Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).first().PriceListID
            SalesPrice2 = PriceList.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PriceListID=PriceListID).first().SalesPrice2
    ...
    def get_SalesPrice3(self,instances):
        ...
        if Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).exists():
            PriceListID = Batch.objects.filter(CompanyID=CompanyID,BranchID=BranchID,BatchCode=BatchCode).first().PriceListID
            SalesPrice3 = PriceList.objects.filter(CompanyID=CompanyID,BranchID=BranchID,PriceListID=PriceListID).first().SalesPrice3
